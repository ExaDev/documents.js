...
- **pptx speaker notes do not survive `pptxToPdf`/`pdfToPptx` in either direction.** PDF has no concept of hidden presenter notes — a PDF page is exactly what gets printed or projected — so `convertPresentationToLayout` never lays `ContentSlide.notes` out onto the page, and `reconstructPresentation` has no notes text to recover on the way back. Confirmed by round-tripping a real Keynote-authored pptx with speaker notes through `pptxToPdf` then `pdfToPptx`: `PptxSlide.notes` reads the original text correctly from the source file, but is empty on the reconstructed one. `openPptx`/`readPptxContent` and `PptxSlide.notes` itself are unaffected — this gap is specific to the PDF round trip.
