...
export function odfFormulaBytes(mathMlInner: string, options: { readonly starMath?: string } = {}): Uint8Array<ArrayBuffer> {
  const annotation = options.starMath === undefined ? '' : `<math:annotation encoding="StarMath 5.0">${options.starMath}</math:annotation>`;
...
export function odfFormulaPackage(mathMlInner: string, options?: { readonly starMath?: string }): Package {
