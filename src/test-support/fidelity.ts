...
export function assertPartsUnchangedExcept(before: Package, after: Package, touchedPaths: readonly string[]): void {
  const touched = new Set(touchedPaths);
  const allPaths = new Set([...Object.keys(before.parts), ...Object.keys(after.parts)]);
  for (const path of allPaths) {
    if (touched.has(path)) {
      continue;
    }
    if (!partsEqual(before.parts[path], after.parts[path])) {
      throw new Error(`part '${path}' changed unexpectedly (it is not in the touched-paths list: [${[...touched].join(', ')}])`);
    }
  }
}
