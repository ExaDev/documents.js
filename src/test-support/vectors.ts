import type { ContentEmbeddedObjectBlock, ContentVector, PageSize } from 'document-schema.js';
...
export const VECTOR_FIXTURE: readonly ContentVector[] = [
  { kind: 'rect', frame: { xPt: 10, yPt: 20, widthPt: 40, heightPt: 30 }, fill: RED, paintOrder: 0 },
  { kind: 'ellipse', frame: { xPt: 60, yPt: 20, widthPt: 40, heightPt: 30 }, stroke: { color: BLACK, widthPt: 1.5 }, paintOrder: 1 },
  { kind: 'line', from: { xPt: 200, yPt: 140 }, to: { xPt: 10, yPt: 100 }, stroke: { color: BLACK, widthPt: 2 }, paintOrder: 2 },
  {
    kind: 'path',
    ...
    subpaths: [
      {
        ...
        closed: true,
        segments: [
          { kind: 'cubic', control1: { xPt: 20, yPt: 0 }, control2: { xPt: 40, yPt: 60 }, to: { xPt: 60, yPt: 60 } },
  ...
  { kind: 'rect', frame: { xPt: 300, yPt: 300, widthPt: 80, heightPt: 40 }, rotationDeg: 30, fill: BLUE, paintOrder: 4 },
];
...
export function withoutRotation(vectors: readonly ContentVector[]): ContentVector[] {
  return vectors.map((vector) => (vector.kind === 'line' ? vector : { ...vector, rotationDeg: undefined }));
}
...
export function rotationsOf(vectors: readonly ContentVector[]): (number | undefined)[] {
  return vectors.map((vector) => (vector.kind === 'line' ? undefined : vector.rotationDeg));
}
