...
export interface DrawingXfrm {
  ...
  // Clockwise, per ECMA-376's own a:xfrm/@rot convention -- the opposite sense from src/pdf/matrix.ts's counter-clockwise rotationMatrix(), so a caller building a LayoutItem must negate this, not pass it straight through.
...
function readThemeSlotColor(colorEl: XmlElement): LayoutColor | undefined {
...
function readClrScheme(clrSchemeEl: XmlElement): Map<string, LayoutColor> {
  const map = new Map<string, LayoutColor>();
...
export interface DrawingTheme {
  readonly colorScheme: ReadonlyMap<string, LayoutColor>;
...
export function readTheme(themeRoot: XmlElement): DrawingTheme {
  ...
  const colorScheme = clrSchemeEl === undefined ? new Map<string, LayoutColor>() : readClrScheme(clrSchemeEl);
...
export function readSchemeColor(schemeClrEl: XmlElement, colorMap: ReadonlyMap<string, string>, theme: DrawingTheme): LayoutColor | undefined {
...
export function readSrgbColor(srgbClrEl: XmlElement): LayoutColor | undefined {
...
export function readSolidFillColor(solidFillEl: XmlElement | undefined, colorMap: ReadonlyMap<string, string>, theme: DrawingTheme): LayoutColor | undefined {
