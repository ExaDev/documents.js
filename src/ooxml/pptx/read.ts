...
//
// A vector-only p:sp -- one readPptx itself always reads as an ordinary, empty ContentShape regardless of content -- is carried through too, as a real 'drawing'-kind embedded object collapsing the run of shape slots it occupied (see ./vector.ts's own collapseVectorShapeRuns). readPptx has no vector-geometry handling at all, so this is a second, independent pass over each slide's own raw p:sld, mirroring how src/ooxml/docx/vector.ts and src/odf/vector/detect.ts recover the same geometry ooxml.js's/odf.js's own readers do not.
export function readPptxContent(pkg: Package): ContentDocument {
  ...
  const slides = pptxDoc.slides.map((slide, slideIndex) => {
    ...
    return spTree === undefined ? slide : collapseVectorShapeRuns(slide, slideIndex, spTree.children);
  });
