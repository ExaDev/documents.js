...
function formulaOnlyEquation(shape: XmlElement): XmlElement | undefined {
  ...
  for (const child of paragraph!.children) {
    ...
    if (child.type !== 'element' || child.tag === 'a:pPr' || child === mathPara) {
      continue;
    }
    return undefined;
  }
...
function collectFormulaShapes(children: readonly XmlNode[], state: WalkState, out: FoundFormulaShape[]): void {
...
export function spliceSlideFormulas(slide: ContentSlide, slideIndex: number, spTreeChildren: readonly XmlNode[], onMathDiagnostic?: OmmlDiagnosticSink): ContentSlide {
  ...
  for (const { shapeIndex, equation } of found) {
    ...
    shapes[shapeIndex] = { ...shape, blocks: [buildFormulaBlock({ mathml }, shape.frame, sourcePath)] };
  }
  return { ...slide, shapes };
}
