...
import { attr, childrenWithTag, elementsWithTag, resolveRelationships, rootElement } from 'ooxml.js';
import type { LayoutColor } from '../../model/color';
import { drawingMlFontSizeToPt } from '../../model/units';
import type { DrawingTheme, DrawingXfrm } from '../drawingml';
import { EMPTY_THEME, readColorMap, readSolidFillColor, readTheme, readXfrm, resolveThemeFontReference } from '../drawingml';

// The placeholder -> layout -> master -> theme inheritance cascade: most real shapes (a title placeholder, chiefly) carry no a:xfrm of their own at all, and skipping this means every such shape renders at (0,0) with zero size, and every themed run renders black. Resolution follows each part's own relationships, never a filename convention.
