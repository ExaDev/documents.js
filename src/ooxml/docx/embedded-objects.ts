import type { ContentBlock, ContentSection, ContentTable, PageSize } from 'document-schema.js';
...
import { collectBodyParagraphs, collectBodyTables, equationFrame, PARAGRAPH_NON_CONTENT_TAGS } from './formula';
...
function paragraphEmbeddings(block: Extract<ContentBlock, { kind: 'paragraph' }>, paragraph: XmlElement, blockIndex: number, onMathDiagnostic?: OmmlDiagnosticSink): ParagraphEmbeddings {
  ...
  if (vectors.length > 0) {
    ...
    placements.push({ index: insertAt, build: (sourcePath) => ({ ...buildDrawingBlock({ widthPt: 0, heightPt: 0 }, vectorValues), sourcePath }) });
  }

  return { placements, consume: isEmbeddedObjectOnlyParagraph(paragraph, equations, vectors) };
}
