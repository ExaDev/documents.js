...
import { renderContentDocumentToMarkdown, MarkdownRenderDiagnosticCodes } from './render';
...
function slide(blocks: readonly ContentBlock[], notes = ''): ContentSlide {
  return { size: { widthPt: 960, heightPt: 540 }, shapes: [shape(blocks)], notes };
}
...
function drawPage(shapes: readonly ContentShape[], vectors: readonly ContentVector[] = []): ContentDrawPage {
  return { size: { widthPt: 400, heightPt: 300 }, shapes: [...shapes], vectors: [...vectors] };
}
...
function cell(row: number, column: number, displayText: string, overrides: Partial<ContentSheetCell> = {}): ContentSheetCell {
  return { row, column, value: { kind: 'string', value: displayText }, displayText, ...overrides };
}

function sheet(cells: readonly ContentSheetCell[], overrides: Partial<ContentSheet> = {}): ContentSheet {
  return { name: 'Sheet1', cells: [...cells], columns: [], rows: [], images: [], printSettings: PRINT_SETTINGS, ...overrides };
}
...
function collect(): { diagnostics: MarkdownRenderDiagnostic[]; onDiagnostic: (diagnostic: MarkdownRenderDiagnostic) => void } {
...
describe('renderContentDocumentToMarkdown', () => {
  ...
  it('renders each slide under its own level-2 heading in slide order, keeping shape text in array order, and drops only non-empty speaker notes', () => {
    const document = presentationDoc([slide([paragraph([{ text: 'Quarterly review' }])], 'Remember the demo.'), slide([paragraph([{ text: 'Roadmap' }])])]);
    const { diagnostics, onDiagnostic } = collect();
    ...
    const document = spreadsheetDoc([
      sheet(
        [
          ...
          cell(0, 1, 'Cost'),
          cell(1, 0, 'Total display text ignored in favour of runs', { runs: [{ text: 'Coffee', bold: true }] }),
          cell(1, 1, '350'),
          cell(1, 2, 'Tax'),
        ],
        { name: 'Budget', columns: [{ index: 0, widthPt: 60 }, { index: 1, widthPt: 60 }, { index: 2, widthPt: 60 }] },
      ),
    ]);
    const { diagnostics, onDiagnostic } = collect();
    const text = renderContentDocumentToMarkdown(document, { onDiagnostic });
    expect(text).toBe('## Budget\n\n| Item | Cost |  |\n| :---: | --- | --- |\n| **Coffee** | 350 | Tax |');
    expect(diagnostics.map((diagnostic) => diagnostic.code)).toEqual([MarkdownRenderDiagnosticCodes.SPREADSHEET_SHEET_AS_TABLE]);
  ...
  it('excludes hidden rows and columns from the table entirely, reporting them once, and never emits the hidden cells\' text', () => {
    const document = spreadsheetDoc([
      sheet([cell(0, 0, 'A'), cell(0, 1, 'HiddenC'), cell(0, 2, 'B'), cell(1, 0, 'HiddenR'), cell(2, 0, 'C'), cell(2, 2, 'D')], {
        columns: [{ index: 0, widthPt: 50 }, { index: 1, widthPt: 9999, hidden: true }, { index: 2, widthPt: 50 }],
        rows: [{ index: 0, heightPt: 10 }, { index: 1, heightPt: 10, hidden: true }, { index: 2, heightPt: 10 }],
      }),
    ]);
    const { diagnostics, onDiagnostic } = collect();
    const text = renderContentDocumentToMarkdown(document, { onDiagnostic });
  ...
  it('reports anchored images and embedded objects on a sheet as dropped, while the cells still render', () => {
    const document = spreadsheetDoc([
      sheet([cell(0, 0, 'A')], {
        images: [{ kind: 'image', format: 'png', base64: 'aGk=', widthPt: 10, heightPt: 10, anchorRow: 0, anchorColumn: 1, offsetXPt: 0, offsetYPt: 0 }],
    ...
    expect(diagnostics[1]?.message).toContain('1 anchored image(s) and 1 embedded object(s)');
  });
  ...
  it('renders a sheet with no cells at all as an explicit empty placeholder rather than an empty section, and reports no sheet-as-table diagnostic for it', () => {
    const { diagnostics, onDiagnostic } = collect();
    const text = renderContentDocumentToMarkdown(spreadsheetDoc([sheet([])]), { onDiagnostic });
  ...
  it('renders each drawing page under its own level-2 heading, keeping text-carrying shapes in shape order, and reports dropped vector primitives once per page that carries them', () => {
    const rect: ContentVector = { kind: 'rect', frame: { xPt: 10, yPt: 20, widthPt: 30, heightPt: 40 }, fill: RED };
    const document = drawingDoc([drawPage([shape([paragraph([{ text: 'A label' }])])], [rect]), drawPage([])]);
    const { diagnostics, onDiagnostic } = collect();
    const text = renderContentDocumentToMarkdown(document, { onDiagnostic });
  ...
  it('renders a standalone formula document as its own StarMath stand-in text when the source carried one', () => {
    ...
    expect(diagnostics.map((diagnostic) => diagnostic.code)).toEqual([MarkdownRenderDiagnosticCodes.FORMULA_AS_PLACEHOLDER]);
    expect(diagnostics[0]?.severity).toBe('warning');
  });

  it('renders a formula carrying no StarMath annotation as the literal marker stand-in, never as nothing', () => {
    const { diagnostics, onDiagnostic } = collect();
    const text = renderContentDocumentToMarkdown(formulaDocument(), { onDiagnostic });
  ...
  it('makes no diagnostic callback at all when the caller supplies no onDiagnostic -- the default sink is a genuine no-op, not a throw', () => {
    const text = renderContentDocumentToMarkdown(presentationDoc([slide([paragraph([{ text: 'Solo' }])], 'notes dropped silently here')]));
    expect(text).toBe('## Slide 1\n\nSolo');
  });
  ...
  it('passes its options through to the underlying markdown writer on the degrade path, not just the wordprocessing passthrough: CRLF line endings reach the emitted text', () => {
    const text = renderContentDocumentToMarkdown(spreadsheetDoc([sheet([cell(0, 0, 'A'), cell(0, 1, 'B')])]), { lineEnding: 'crlf' });
  ...
  it('passes front matter emission through too: a degraded document with a metadata title gains a front matter block when asked', () => {
    const text = renderContentDocumentToMarkdown(presentationDoc([slide([paragraph([{ text: 'Solo' }])])], { title: 'Deck' }), { frontMatter: true });
  ...
  it('forwards the abort signal into the underlying writer on the degrade path', () => {
    ...
    controller.abort();
    expect(() => renderContentDocumentToMarkdown(presentationDoc([slide([paragraph([{ text: 'Solo' }])])]), { signal: controller.signal })).toThrow();
  });
});
