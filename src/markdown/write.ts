...
// A construct marker (document-schema.js 4.2.0's constructStart/constructEnd -- a docx SDT, an ODF field, a tracked-change span, a bookmark, a hyperlink region, a division) has no CommonMark/GFM spelling of its own, and unlike an embedded formula it has no plain-text stand-in either: a division's column count and a tracked-change's author carry nothing a reader could usefully render as text. markdown-codec's own writer (renderTopLevelBlock in that package's emit/emit.ts) has no arm for either marker kind and falls through to `undefined`, which its caller then dereferences with `.length`, crashing with an undebuggable TypeError three stack frames past this package's own code. buildMarkdownText refuses loudly and by name instead, before a marker ever reaches writeMarkdown -- the same "no silent approximation" contract this package's other markdown gaps keep (see MarkdownDiagnosticCodes), just with no degraded rendering to fall back to.
...
// ContentDocument (the wordprocessing variant) -> markdown text. A thin adapter over markdown-codec's own writeMarkdown, lives beside read.ts rather than under src/edit/markdown/ -- src/edit/markdown/editor.ts's MarkdownEditor calls this function directly as its own toMarkdownText, rather than this module importing anything back from src/edit/. markdown does now have a live-view editor (MarkdownEditor), but unlike docx/pptx/odt/odp/ods/odg it holds a mutable in-memory ContentDocument rather than a real XmlElement tree inside a decoded Package -- markdown has no such tree at all -- so this function, not a method on some MarkdownBody, is still the whole write path.
//
...
export function buildMarkdownText(document: ContentDocument, options?: WriteMarkdownOptions): string {
  ...
  return writeMarkdown({ ...document, sections }, options);
}
