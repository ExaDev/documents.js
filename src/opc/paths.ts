...
export function buildRelativeTarget(fromPartPath: string, toPartPath: string): string {
  ...
  let common = 0;
  while (common < fromDirs.length && common < toDirs.length && fromDirs[common] === toDirs[common]) {
    common++;
  }
