...
import { attr, childrenWithTag, resolveRelationships, rootElement } from 'ooxml.js';
...
describe('addRootRelationship', () => {
  it('writes the relationship into "_rels/.rels", not "/_rels/.rels"', () => {
    ...
    const id = addRootRelationship(pkg, { type: CORE_PROPERTIES_TYPE, target: 'docProps/core.xml' });
    expect(id).toBe('rId1');
    expect(Object.keys(pkg.parts)).toEqual(['_rels/.rels']);

    const rels = rootElement(pkg.parts['_rels/.rels']);
    expect(rels).toBeDefined();
    const [relationship] = rels === undefined ? [] : childrenWithTag(rels, 'Relationship');
    expect(relationship).toBeDefined();
    expect(relationship === undefined ? undefined : attr(relationship, 'Id')).toBe('rId1');
    expect(relationship === undefined ? undefined : attr(relationship, 'Type')).toBe(CORE_PROPERTIES_TYPE);
    expect(relationship === undefined ? undefined : attr(relationship, 'Target')).toBe('docProps/core.xml');
  });
  ...
  it('shares the same "_rels/.rels" part a scaffold\'s own hardcoded root relationship already wrote, rather than creating an orphaned duplicate', () => {
              ...
              children: [
                {
                  ...
                  attributes: [
                    { name: 'Id', value: 'rId1' },
                    { name: 'Type', value: 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument' },
    ...
    const rels = rootElement(pkg.parts['_rels/.rels']);
    const relationships = rels === undefined ? [] : childrenWithTag(rels, 'Relationship');
    expect(relationships).toHaveLength(2);
    expect(relationships.map((relationship) => attr(relationship, 'Id'))).toEqual(['rId1', 'rId2']);
  });
});
