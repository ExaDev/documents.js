...
//
// formulaPlaceholderText below is what remains of the old mechanism, and it has a genuine, permanent job rather than being a leftover: a consumer that cannot render MathML at all still needs something readable to show, and every one of them writes it as ordinary text. Three real ones exist -- buildDocxPackage and buildOdtPackage (neither writes OOXML's own OMML math markup, nor an embedded ODF formula sub-package), src/markdown/write.ts (CommonMark/GFM has no math construct, and markdown-codec's own writer emits nothing at all for an embedded-object block), and this package's own wordprocessing layout engine, for a block whose formula carries no MathML nodes to typeset.
