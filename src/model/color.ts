...
function applyShadeOrTint(color: LayoutColor, kind: 'shade' | 'tint', value: number): LayoutColor {
...
function rgbToHsl(color: LayoutColor): Hsl {
  const { r, g, b } = color;
...
function hslToRgb(hsl: Hsl): LayoutColor {
  const { h, s, l } = hsl;
  if (s === 0) {
    return { r: l, g: l, b: l };
  }
...
function applyLumModOrOff(color: LayoutColor, kind: 'lumMod' | 'lumOff', value: number): LayoutColor {
...
export function applyColorTransforms(base: LayoutColor, transforms: readonly ColorTransform[]): LayoutColor {
  let color = base;
