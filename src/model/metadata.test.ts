...
describe('resolveMetadataTimestamps', () => {
  ...
  it('never consults the clock when both createdIso and modifiedIso are already present', () => {
    const metadata = { createdIso: '2020-01-01T00:00:00.000Z', modifiedIso: '2021-01-01T00:00:00.000Z' };
    const throwingClock = { now: () => { throw new Error('clock should not be consulted'); } };
  ...
  it('fills in only the missing field when exactly one of the two is already present, never overwriting the one that is', () => {
    const withCreatedOnly = resolveMetadataTimestamps({ createdIso: '2020-01-01T00:00:00.000Z' }, fixedClock(NOW));
    ...
    const withModifiedOnly = resolveMetadataTimestamps({ modifiedIso: '2020-01-01T00:00:00.000Z' }, fixedClock(NOW));
  ...
  it('preserves every other metadata field untouched while filling in timestamps', () => {
    const result = resolveMetadataTimestamps({ title: 'My Doc', author: 'A. Writer' }, fixedClock(NOW));
