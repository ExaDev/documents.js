...
import { caladeaBoldBytes, caladeaItalicBytes, caladeaRegularBytes, embeddedFontOdtPackage } from '../test-support/fonts';
...
function odtWithFontFaceDecls(decls: string, fontParts: Record<string, Uint8Array<ArrayBuffer>>) {
...
describe('extractOdfEmbeddedFonts', () => {
  it('recovers every embedded face with its family and the exact font bytes', () => {
    ...
    expect(fonts.map(({ family, bold, italic }) => ({ family, bold, italic }))).toEqual([
    ...
    expect(fonts.map((font) => postScriptNameOf(font.bytes))).toEqual(['Caladea-Regular', 'Caladea-Bold']);
  });
  ...
  it('reads italic from the font file own OS/2 fsSelection bits', () => {
    ...
    expect(fonts.map(({ bold, italic }) => ({ bold, italic }))).toEqual([{ bold: false, italic: true }]);
  });
  ...
  it.each([
  ...
  ])('resolves loext:font-weight=%s loext:font-style=%s to bold=%s italic=%s', (weight, style, bold, italic) => {
    const fonts = extractOdfEmbeddedFonts(
      odtWithFontFaceDecls(
    ...
    expect(fonts.map((font) => ({ bold: font.bold, italic: font.italic }))).toEqual([{ bold, italic }]);
  });
  ...
  it('falls back to style:name when the face declares no svg:font-family', () => {
    const fonts = extractOdfEmbeddedFonts(
      odtWithFontFaceDecls('<style:font-face style:name="Caladea"><svg:font-face-src><svg:font-face-uri xlink:href="Fonts/x.ttf"/></svg:font-face-src></style:font-face>', {
        'Fonts/x.ttf': caladeaRegularBytes(),
      }),
    );
  ...
  it('ignores a font face that declares no embedded source', () => {
    expect(extractOdfEmbeddedFonts(odtWithFontFaceDecls('<style:font-face style:name="Caladea" svg:font-family="Caladea"/>', {}))).toEqual([]);
  });
  ...
  it('throws when a svg:font-face-uri references a part the package does not contain', () => {
    expect(() =>
      extractOdfEmbeddedFonts(
        odtWithFontFaceDecls('<style:font-face style:name="Caladea" svg:font-family="Caladea"><svg:font-face-src><svg:font-face-uri xlink:href="Fonts/missing.ttf"/></svg:font-face-src></style:font-face>', {}),
      ),
  ...
  it('throws when a svg:font-face-uri carries no xlink:href', () => {
    expect(() =>
      extractOdfEmbeddedFonts(odtWithFontFaceDecls('<style:font-face style:name="Caladea" svg:font-family="Caladea"><svg:font-face-src><svg:font-face-uri/></svg:font-face-src></style:font-face>', {})),
    ).toThrow(OdfEmbeddedFontError);
  });
  ...
  it('throws when an embedded part is not a recognisable font', () => {
    expect(() =>
      extractOdfEmbeddedFonts(
        odtWithFontFaceDecls('<style:font-face style:name="Caladea" svg:font-family="Caladea"><svg:font-face-src><svg:font-face-uri xlink:href="Fonts/x.ttf"/></svg:font-face-src></style:font-face>', {
