...
import { caladeaBoldBytes, caladeaItalicBytes, caladeaRegularBytes, embeddedFontDocxPackage, embeddedFontPptxPackage } from '../test-support/fonts';
...
describe('extractOoxmlEmbeddedFonts (docx)', () => {
  ...
  it('throws when an embed element names a relationship id its .rels part does not declare', () => {
    ...
    rels.nodes = [];
    expect(() => extractOoxmlEmbeddedFonts(pkg, 'docx')).toThrow(OoxmlEmbeddedFontError);
  });
  ...
  it('follows the relationship graph rather than assuming word/fontTable.xml', () => {
    ...
    expect(extractOoxmlEmbeddedFonts(pkg, 'docx')).toEqual([{ family: 'Caladea', bold: false, italic: true, bytes: caladeaItalicBytes() }]);
  });
});
...
describe('extractOoxmlEmbeddedFonts (pptx)', () => {
  ...
  it('recovers unobfuscated faces declared in p:embeddedFontLst', () => {
    ...
    expect(fonts.map((font) => postScriptNameOf(font.bytes))).toEqual(['Caladea-Regular', 'Caladea-Italic']);
  });
  ...
  it('throws when a p:embeddedFont declares no p:font typeface', () => {
    ...
    expect(() => extractOoxmlEmbeddedFonts(pkg, 'pptx')).toThrow(OoxmlEmbeddedFontError);
  });
