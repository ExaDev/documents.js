...
import { caladeaRegularBytes, obfuscateFontBytes, SPEC_FONT_KEY_BYTES, SPEC_FONT_KEY_GUID } from '../test-support/fonts';
import { deobfuscateEmbeddedFont, deriveFontKey, FontDeobfuscationError, looksLikeSfnt } from './obfuscation';
...
describe('deriveFontKey', () => {
  ...
  it('reproduces the ECMA-376 Part 4 2.8.1 worked example exactly', () => {
    expect([...deriveFontKey(SPEC_FONT_KEY_GUID)]).toEqual([...SPEC_FONT_KEY_BYTES]);
  });

  it('accepts the same GUID without braces', () => {
    expect([...deriveFontKey('001B70DC-AA60-4AD5-90EC-18A0948E1EAE')]).toEqual([...SPEC_FONT_KEY_BYTES]);
  });
  ...
  it('does not produce the GUID hex digits in written order', () => {
    const forwardOrder = [0x00, 0x1b, 0x70, 0xdc, 0xaa, 0x60, 0x4a, 0xd5, 0x90, 0xec, 0x18, 0xa0, 0x94, 0x8e, 0x1e, 0xae];
    expect([...deriveFontKey(SPEC_FONT_KEY_GUID)]).not.toEqual(forwardOrder);
  });

  it('throws for a GUID with the wrong number of hex digits', () => {
    expect(() => deriveFontKey('{001B70DC-AA60-4AD5-90EC-18A0948E1E}')).toThrow(FontDeobfuscationError);
  });

  it('throws for a GUID containing a non-hexadecimal pair', () => {
    expect(() => deriveFontKey('{001B70DC-AA60-4AD5-90EC-18A0948E1EAZ}')).toThrow(FontDeobfuscationError);
  });
});

describe('looksLikeSfnt', () => {
  it.each(Object.entries(SFNT_SIGNATURES))('accepts the %s signature', (_name, signature) => {
  ...
  it('rejects an obfuscated font', () => {
    expect(looksLikeSfnt(obfuscateFontBytes(caladeaRegularBytes(), SPEC_FONT_KEY_BYTES))).toBe(false);
  });
...
describe('deobfuscateEmbeddedFont', () => {
  ...
  it('recovers a real font from bytes obfuscated with the specification worked-example key', () => {
    ...
    const recovered = deobfuscateEmbeddedFont(obfuscateFontBytes(original, SPEC_FONT_KEY_BYTES), SPEC_FONT_KEY_GUID);
  ...
  it('returns already-clear bytes unchanged even when a font key is given', () => {
    const original = caladeaRegularBytes();
    expect(deobfuscateEmbeddedFont(original, SPEC_FONT_KEY_GUID)).toBe(original);
  });

  it('does not mutate the input bytes', () => {
    const obfuscated = obfuscateFontBytes(caladeaRegularBytes(), SPEC_FONT_KEY_BYTES);
  ...
  it('throws for obfuscated bytes with no font key to undo them with', () => {
    expect(() => deobfuscateEmbeddedFont(obfuscateFontBytes(caladeaRegularBytes(), SPEC_FONT_KEY_BYTES), undefined)).toThrow(FontDeobfuscationError);
  });
  ...
  it('throws when deobfuscating with the wrong key', () => {
    const obfuscated = obfuscateFontBytes(caladeaRegularBytes(), SPEC_FONT_KEY_BYTES);
    expect(() => deobfuscateEmbeddedFont(obfuscated, '{FFFFFFFF-FFFF-FFFF-FFFF-FFFFFFFFFFFF}')).toThrow(FontDeobfuscationError);
  });

  it('throws for a part shorter than the 32-byte obfuscated prefix', () => {
    expect(() => deobfuscateEmbeddedFont(new Uint8Array(16), SPEC_FONT_KEY_GUID)).toThrow(FontDeobfuscationError);
  });
});
