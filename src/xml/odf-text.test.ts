...
describe('encodeOdfText', () => {
  ...
  it('encodes a run of exactly 2 spaces as text:s', () => {
    expect(encodeOdfText('  ')).toEqual([{ type: 'element', tag: 'text:s', attributes: [{ name: 'text:c', value: '2' }], children: [] }]);
  });

  it('encodes a run of 3 spaces as text:s', () => {
    expect(encodeOdfText('   ')).toEqual([{ type: 'element', tag: 'text:s', attributes: [{ name: 'text:c', value: '3' }], children: [] }]);
  });

  it('encodes a run of 10 spaces as text:s', () => {
    expect(encodeOdfText(' '.repeat(10))).toEqual([{ type: 'element', tag: 'text:s', attributes: [{ name: 'text:c', value: '10' }], children: [] }]);
  });

  it('encodes a tab as text:tab', () => {
    expect(encodeOdfText('\t')).toEqual([{ type: 'element', tag: 'text:tab', attributes: [], children: [] }]);
  });

  it('encodes a newline as text:line-break', () => {
    expect(encodeOdfText('\n')).toEqual([{ type: 'element', tag: 'text:line-break', attributes: [], children: [] }]);
  });
  ...
  it('encodes a mixed sequence, coalescing adjacent literal runs into as few text nodes as practical', () => {
    ...
    expect(nodes).toEqual([
      { type: 'text', value: 'a' },
      { type: 'element', tag: 'text:s', attributes: [{ name: 'text:c', value: '2' }], children: [] },
      { type: 'text', value: 'b' },
  ...
  it('XML-encodes literal text content', () => {
    expect(encodeOdfText('Tom & Jerry')).toEqual([{ type: 'text', value: 'Tom &amp; Jerry' }]);
  });
});
...
describe('decodeOdfText', () => {
  ...
  it('decodes a pre-existing text:s with no text:c attribute as a single space, per the ODF default', () => {
    expect(decodeOdfText([{ type: 'element', tag: 'text:s', attributes: [], children: [] }])).toBe(' ');
  });

  it('recurses into a nested text:span', () => {
    const nodes: XmlNode[] = [
      { type: 'text', value: 'a' },
      { type: 'element', tag: 'text:span', attributes: [{ name: 'text:style-name', value: 'T1' }], children: [{ type: 'text', value: 'b' }] },
      { type: 'text', value: 'c' },
    ];
