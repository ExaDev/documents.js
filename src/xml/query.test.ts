...
import { findChildElement, findChildElements, findDescendantElement, walkElements } from './query';
...
describe('xml/query', () => {
  ...
  it('walkElements descends into children and each cursor carries its true containing array', () => {
    ...
    expect(cursors[1]?.container).toBe(paragraph.type === 'element' ? paragraph.children : []);
  });
