...
function firstDrawPageChildren(pkg: Package): XmlElement['children'] {
  ...
  const page = presentation === undefined ? undefined : findChildElement(presentation.children, 'draw:page');
  if (page === undefined) {
...
describe('OdpSlide.shapes / tables', () => {
  it('shapes() excludes a table frame (a draw:frame whose direct content is table:table) -- only the text box and picture come back', () => {
    ...
    slide.addTextBox({ frame: { xPt: 0, yPt: 0, widthPt: 100, heightPt: 50 }, text: 'Title' });
    slide.addImage({ frame: { xPt: 0, yPt: 60, widthPt: 50, heightPt: 50 }, format: 'png', bytes: PNG_BYTES });
    slide.addTable({ frame: { xPt: 0, yPt: 120, widthPt: 200, heightPt: 80 }, table: { rows: 2, columns: 2 } });
  ...
  it('tables() returns exactly the one added table, a fully working OdpTableShape', () => {
    ...
    const slide = editor.addSlide();
    slide.addTextBox({ frame: { xPt: 0, yPt: 0, widthPt: 100, heightPt: 50 }, text: 'Title' });
    slide.addTable({ frame: { xPt: 0, yPt: 60, widthPt: 200, heightPt: 80 }, table: { rows: 2, columns: 2 } });

    const tables = slide.tables();
    expect(tables).toHaveLength(1);
    expect(tables[0]?.table.rows()).toHaveLength(2);
    expect(tables[0]?.table.rows()[0]?.cells()).toHaveLength(2);
    expect(tables[0]?.shape.frame).toEqual({ xPt: 0, yPt: 60, widthPt: 200, heightPt: 80 });
  ...
  it('tables() reflects true document position, not addTable call order -- swapping the two table frames in the raw XML tree reorders tables()', () => {
    ...
    slide.addTable({ frame: { xPt: 0, yPt: 0, widthPt: 100, heightPt: 40 }, table: { rows: 1, columns: 3 } });
    slide.addTextBox({ frame: { xPt: 0, yPt: 50, widthPt: 100, heightPt: 40 }, text: 'Interleaved' });
    slide.addTable({ frame: { xPt: 0, yPt: 100, widthPt: 100, heightPt: 40 }, table: { rows: 1, columns: 1 } });

    expect(slide.shapes()).toHaveLength(1);
    expect(slide.shapes()[0]?.text).toBe('Interleaved');
    expect(slide.tables().map((t) => t.table.rows()[0]?.cells().length)).toEqual([3, 1]);
    ...
    pageChildren.forEach((c, i) => {
      if (c.type === 'element' && c.tag === 'draw:frame' && findChildElement(c.children, 'table:table') !== undefined) {
    ...
    if (firstFrame === undefined || secondFrame === undefined) {
      throw new Error('expected both table draw:frame indices to resolve to a real element');
    }
    pageChildren[firstIndex] = secondFrame;
    pageChildren[secondIndex] = firstFrame;

    expect(slide.tables().map((t) => t.table.rows()[0]?.cells().length)).toEqual([1, 3]);
  });

  it('a table survives a full toBytes() -> decodePackage round trip and is still found by tables()', () => {
    const editor = createOdp();
    const slide = editor.addSlide();
    slide.addTable({ frame: { xPt: 0, yPt: 0, widthPt: 100, heightPt: 40 }, table: { rows: 1, columns: 1 } });
