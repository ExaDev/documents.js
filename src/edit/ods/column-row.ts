...
export function ensureColumnDefaultWidth(pkg: Package, tableElement: XmlElement, index: number): void {
  ensureColumnCoverage(tableElement, index + 1);
  const columnElement = replaceRun(tableElement.children, isElementWithTag(COLUMN_TAG), index, COLUMN_REPEAT_ATTR, () => el(COLUMN_TAG), HEADER_COLUMNS_TAG);
...
export function ensureRowDefaultHeight(pkg: Package, tableElement: XmlElement, index: number): void {
  const rowElement = replaceRun(tableElement.children, isElementWithTag(ROW_TAG), index, ROW_REPEAT_ATTR, () => el(ROW_TAG), HEADER_ROWS_TAG);
...
export function writeColumnHidden(pkg: Package, tableElement: XmlElement, index: number, hidden: boolean): void {
...
export function writeRowHidden(pkg: Package, tableElement: XmlElement, index: number, hidden: boolean): void {
