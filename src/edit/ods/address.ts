...
export function collectRunMembers(children: XmlNode[], isMember: (node: XmlNode) => node is XmlElement, headerWrapperTag: string | undefined): RunMember[] {
  ...
  children.forEach((node, outerPosition) => {
    ...
    if (headerWrapperTag !== undefined && node.tag === headerWrapperTag) {
      node.children.forEach((inner, innerPosition) => {
        if (isMember(inner)) {
          members.push({ node: inner, container: node.children, containerPosition: innerPosition, outerPosition });
        }
      });
      return;
    }
    if (isMember(node)) {
      members.push({ node, container: children, containerPosition: outerPosition, outerPosition });
    }
  });
  return members;
}
...
export function replaceRun(
  ...
  const insertAt = lastMember === undefined ? children.length : lastMember.outerPosition + 1;
...
export function ensureColumnCoverage(tableElement: XmlElement, minCount: number): void {
  ...
  if (lastColumn === undefined) {
    const firstRowOrHeaderPosition = tableElement.children.findIndex(
      (node) => isElementWithTag(ROW_TAG)(node) || isElementWithTag(HEADER_ROWS_TAG)(node),
    );
...
export function resolveCellNode(tableElement: XmlElement, row: number, column: number): XmlElement {
  ...
  const rowElement = replaceRun(tableElement.children, isElementWithTag(ROW_TAG), row, ROW_REPEAT_ATTR, () => el(ROW_TAG), HEADER_ROWS_TAG);
