...
import { COLUMN_REPEAT_ATTR, COLUMN_TAG, COVERED_CELL_TAG, ROW_REPEAT_ATTR, ROW_TAG, ensureColumnCoverage, isElementWithTag, replaceRun, resolveCellNode } from './address';
...
describe('replaceRun: appending beyond every existing run\'s coverage', () => {
  it('targetIndex 0 on an empty array creates exactly one element, no gap placeholder', () => {
    ...
    const target = replaceRun(children, isElementWithTag(ROW_TAG), 0, ROW_REPEAT_ATTR, () => el(ROW_TAG));
  ...
  it('a far-out targetIndex on an empty array creates exactly TWO elements -- a gap placeholder and the target -- never one per skipped position', () => {
    const children: XmlNode[] = [];
    const target = replaceRun(children, isElementWithTag(ROW_TAG), 500, ROW_REPEAT_ATTR, () => el(ROW_TAG));
  ...
  it('appending after existing real content only pads the NEW gap, not the whole range from zero', () => {
    ...
    const target = replaceRun(children, isElementWithTag(ROW_TAG), 10, ROW_REPEAT_ATTR, () => el(ROW_TAG));
...
describe('replaceRun: individuating within an existing repeated run', () => {
  it('a run whose own repeat count is already 1 is returned unchanged -- no split, no mutation', () => {
    const row = el(ROW_TAG, {}, [el('table:table-cell', { 'office:value-type': 'string' })]);
  ...
  it('splitting the LAST position of a repeated run produces [before, target] -- no empty "after" element', () => {
    ...
    run.attributes.push({ name: ROW_REPEAT_ATTR, value: '10' });
    const children: XmlNode[] = [run];
    const target = replaceRun(children, isElementWithTag(ROW_TAG), 9, ROW_REPEAT_ATTR, () => el(ROW_TAG));
    expect(children).toHaveLength(2);
  ...
  it('splitting the MIDDLE of a huge repeated run (a real LibreOffice-scale count) produces exactly three elements, in O(1) regardless of the run size', () => {
    ...
    const target = replaceRun(children, isElementWithTag(ROW_TAG), 500000, ROW_REPEAT_ATTR, () => el(ROW_TAG));
  ...
  it('splitting a run that carries genuine repeated CONTENT preserves that content identically in every resulting part', () => {
    const cell = el('table:table-cell', { 'office:value-type': 'string', 'office:string-value': 'Q1' });
    const run = el(ROW_TAG, {}, [cell]);
    run.attributes.push({ name: ROW_REPEAT_ATTR, value: '4' });
    const children: XmlNode[] = [run];
    const target = replaceRun(children, isElementWithTag(ROW_TAG), 2, ROW_REPEAT_ATTR, () => el(ROW_TAG));
    expect(children).toHaveLength(3);
    for (const node of children) {
      ...
      if (innerCell?.type !== 'element') {
        throw new Error('expected each split part to carry its own clone of the repeated cell');
      }
      expect(innerCell.attributes.find((a) => a.name === 'office:string-value')?.value).toBe('Q1');
    }
    ...
    if (target.children[0]?.type === 'element') {
      target.children[0].attributes.push({ name: 'office:value-type', value: 'string' });
    }
    const firstNode = children[0];
    if (firstNode?.type !== 'element' || firstNode.children[0]?.type !== 'element') {
      throw new Error('expected the first split part to carry its own element cell');
    }
    expect(firstNode.children[0].attributes.filter((a) => a.name === 'office:value-type')).toHaveLength(1);
  });
});
...
describe('ensureColumnCoverage', () => {
  it('adds exactly one new table:table-column run covering the shortfall when no columns exist yet', () => {
    ...
    expect(table.children.indexOf(columns[0]!)).toBeLessThan(table.children.findIndex(isElementWithTag(ROW_TAG)));
  });
...
describe('resolveCellNode', () => {
  it('resolving a far-out cell on a fresh table materializes a bounded number of elements, never one per skipped row/column', () => {
    ...
    const cellsInTargetRow = targetRow.children.filter((c): c is XmlElement => c.type === 'element' && c.tag === 'table:table-cell');
  ...
  it('also grows the table\'s own declared column coverage to at least column + 1', () => {
    ...
    const columns = table.children.filter(isElementWithTag(COLUMN_TAG));
    const covered = columns.reduce((sum, c) => sum + Number(repeatAttr(c, COLUMN_REPEAT_ATTR) ?? '1'), 0);
  ...
  it('a position covered by a merge resolves to the table:covered-table-cell marker, not a fresh table:table-cell', () => {
    ...
    anchor.attributes.push({ name: 'table:number-columns-spanned', value: '2' });
