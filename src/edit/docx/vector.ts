...
import { buildVectorShapeProperties, vectorPlacementBox, vectorShapeName } from '../drawingml/vector';
...
function position(tag: 'wp:positionH' | 'wp:positionV', offsetPt: number): XmlElement {
...
export function buildAnchoredVectorDrawing(vector: ContentVector, drawingId: number, relativeHeight: number): XmlElement {
  ...
  const anchor = el('wp:anchor', { ...ANCHOR_ATTRS, relativeHeight: String(relativeHeight) }, [
  ...
  return el('w:drawing', { 'xmlns:wp': DRAWING_NS.wp, 'xmlns:a': DRAWING_NS.a, 'xmlns:wps': DRAWING_NS.wps }, [anchor]);
}
