...
export function readCurrentStyleProperties(pkg: Package, element: XmlElement, family: StyleFamily): StyleProperties {
...
export function applyStyleChange(pkg: Package, element: XmlElement, family: StyleFamily, change: Partial<StyleProperties>): void {
  const current = readCurrentStyleProperties(pkg, element, family);
  const merged: StyleProperties = { ...current, ...change };
  const name = StyleRegistry.forPart(pkg, CONTENT_PART_PATH).intern({ family, properties: merged });
