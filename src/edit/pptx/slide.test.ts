...
function findSpTreeChildren(part: Part | undefined, slidePartPath: string): XmlElement['children'] {
  const slideRoot = rootElement(part);
  const cSld = slideRoot?.children.find((c): c is XmlElement => c.type === 'element' && c.tag === 'p:cSld');
  const spTree = cSld?.children.find((c): c is XmlElement => c.type === 'element' && c.tag === 'p:spTree');
  if (spTree === undefined) {
...
describe('PptxSlide.shapes / tables', () => {
  ...
  it('tables() reflects true document position, not addTable call order -- swapping the two graphic frames in the raw XML tree reorders tables()', () => {
    ...
    expect(slide.tables().map((t) => t.rows()[0]?.cells().length)).toEqual([3, 1]);
    ...
    const spTreeChildren = findSpTreeChildren(editor.toPackage().parts[slidePartPath], slidePartPath);
    ...
    if (firstFrame === undefined || secondFrame === undefined) {
      throw new Error('expected both p:graphicFrame indices to resolve to a real element');
    }
    ...
    expect(slide.tables().map((t) => t.rows()[0]?.cells().length)).toEqual([1, 3]);
  });
