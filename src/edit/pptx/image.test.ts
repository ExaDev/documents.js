...
describe('PptxSlide.addImage media wiring', () => {
  it('adds the media part, content-type default, and a relationship from the slide to it', () => {
    ...
    slide.addImage({ frame: { xPt: 0, yPt: 0, widthPt: 100, heightPt: 50 }, format: 'png', bytes: PNG_BYTES });
    ...
    const hasDefault = contentTypesRoot?.children.some(
      (c) => c.type === 'element' && c.tag === 'Default' && attr(c, 'Extension') === 'png',
    );
    expect(hasDefault).toBe(true);

    const rels = resolveRelationships(pkg, 'ppt/slides/slide1.xml');
    const imageRel = [...rels.values()].find((r) => r.target === 'ppt/media/image1.png');
  ...
  it('numbers successive images across slides without colliding', () => {
    ...
    slide1.addImage({ frame: { xPt: 0, yPt: 0, widthPt: 10, heightPt: 10 }, format: 'png', bytes: PNG_BYTES });
    slide2.addImage({ frame: { xPt: 0, yPt: 0, widthPt: 10, heightPt: 10 }, format: 'png', bytes: PNG_BYTES });
    const pkg = editor.toPackage();
    expect(pkg.parts['ppt/media/image1.png']).toBeDefined();
