...
describe('PdfTextItem', () => {
  it('getters read back exactly what appendText was given', () => {
    ...
    const item = page.appendText({ xPt: 10, yPt: 20, text: 'Hello', font: DEFAULT_LAYOUT_FONT, sizePt: 12, color: BLACK });
  ...
  it('setters mutate the live item in place', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    const item = page.appendText({ xPt: 0, yPt: 0, text: 'Original', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });
  ...
  it('rejects a nonpositive sizePt before ever touching the live item', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    const item = page.appendText({ xPt: 0, yPt: 0, text: 'X', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });
    expect(() => {
      item.sizePt = 0;
    }).toThrow(/positive/);
    expect(() => {
  ...
  it('appendText itself rejects a nonpositive sizePt', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    expect(() => page.appendText({ xPt: 0, yPt: 0, text: 'X', font: DEFAULT_LAYOUT_FONT, sizePt: 0, color: BLACK })).toThrow(/positive/);
...
describe('PdfRectItem / PdfEllipseItem', () => {
  it('rect accepts a zero-width/height box (nonnegative), ellipse does not (positive)', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    expect(() => page.appendRect({ xPt: 0, yPt: 0, widthPt: 0, heightPt: 0 })).not.toThrow();
    expect(() => page.appendEllipse({ xPt: 0, yPt: 0, widthPt: 0, heightPt: 10 })).toThrow(/positive/);
    expect(() => page.appendEllipse({ xPt: 0, yPt: 0, widthPt: 10, heightPt: 0 })).toThrow(/positive/);
  });

  it('rect rejects a negative width/height', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    expect(() => page.appendRect({ xPt: 0, yPt: 0, widthPt: -1, heightPt: 10 })).toThrow(/nonnegative/);
  });

  it('fill/stroke round trip, and stroke.widthPt must be positive', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    const rect = page.appendRect({ xPt: 0, yPt: 0, widthPt: 10, heightPt: 10, fill: RED, stroke: { color: BLUE, widthPt: 2 } });
  ...
  it('setting widthPt/heightPt validates per-kind (rect nonnegative, ellipse positive)', () => {
    ...
    const ellipse = page.appendEllipse({ xPt: 0, yPt: 0, widthPt: 10, heightPt: 10 });
    expect(() => {
      ellipse.widthPt = 0;
    }).toThrow(/positive/);
  });
...
describe('PdfLineItem', () => {
  it('reads/writes every field, and rejects a nonpositive widthPt', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    const line = page.appendLine({ x1Pt: 0, y1Pt: 0, x2Pt: 10, y2Pt: 10, color: BLACK, widthPt: 1, style: 'dashed' });
    ...
    expect(() => page.appendLine({ x1Pt: 0, y1Pt: 0, x2Pt: 1, y2Pt: 1, color: BLACK, widthPt: 0 })).toThrow(/positive/);
    expect(() => {
      line.widthPt = -1;
    }).toThrow(/positive/);
  });
...
describe('PdfPathItem', () => {
  it('subpaths is a whole-array-replace setter', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    const original = [{ startXPt: 0, startYPt: 0, closed: false, segments: [{ kind: 'line' as const, xPt: 10, yPt: 10 }] }];
    const path = page.appendPath({ subpaths: original, fill: RED, fillRule: 'evenodd' });
    expect(path.subpaths).toEqual(original);
    expect(path.fillRule).toBe('evenodd');

    const replacement = [{ startXPt: 5, startYPt: 5, closed: true, segments: [{ kind: 'line' as const, xPt: 20, yPt: 20 }] }];
...
describe('PdfImageItem', () => {
  it('appendImage registers the bytes in the shared registry and setImage repoints imageId without touching position/size', () => {
    ...
    const image = page.appendImage({ xPt: 5, yPt: 5, widthPt: 30, heightPt: 30, bytes: firstBytes, format: 'png' });
    const originalImageId = image.imageId;
    expect(editor.toLayoutDocument().images[originalImageId]).toBeDefined();

    const secondBytes = encodePng({ width: 2, height: 2, channels: 3, data: new Uint8Array([0, 255, 0, 0, 255, 0, 0, 255, 0, 0, 255, 0]) });
  ...
  it('appendImage rejects a nonpositive widthPt/heightPt', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    expect(() => page.appendImage({ xPt: 0, yPt: 0, widthPt: 0, heightPt: 10, bytes: tinyPngBytes(), format: 'png' })).toThrow(/positive/);
  });
});
...
describe('PdfLinkItem', () => {
  it('reads/writes uri and geometry', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    const link = page.appendLink({ uri: 'https://example.com', xPt: 0, yPt: 0, widthPt: 50, heightPt: 20 });
