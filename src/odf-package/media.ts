...
//
// Manifest sync is delegated whole to syncManifest (this file's own manifest.ts re-export of odf.js's build+write pair) rather than hand-appending a single manifest:file-entry: buildManifest already resolves a binary part's media type by sniffing its actual bytes (odf.js's own resolvePartMediaType falls back to sniffImageFormat for an unrecognised extension), so re-deriving the whole manifest from the package's current parts is both correct and the only way to add this one entry without duplicating that sniffing logic here. This requires the package to already carry a "mimetype" part (via setDocumentMediaType or an equivalent) -- buildManifest has no way to guess a document's own ODF variant from its parts alone, and throws a clear error if neither a mimetype part nor a documentMediaType override is available.
export function addImageMedia(pkg: Package, imageBytes: Uint8Array<ArrayBuffer>, format: 'png' | 'jpeg'): AddedOdfMedia {
