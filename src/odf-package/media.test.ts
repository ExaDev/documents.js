...
const PNG_BYTES: Uint8Array<ArrayBuffer> = new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 1, 2, 3, 4]);
...
describe('addImageMedia', () => {
  it('adds the binary part under Pictures/ and a matching manifest entry, verified by round-tripping through encodePackage/decodePackage', () => {
    ...
    expect(manifest.entries.find((entry) => entry.fullPath === 'Pictures/image1.png')).toEqual({
  ...
  it('numbers each format own images independently', () => {
    ...
    expect(manifest.entries.find((entry) => entry.fullPath === 'Pictures/image1.jpeg')?.mediaType).toBe('image/jpeg');
  });
  ...
  it('keeps every previously added image listed in the manifest after a later image is added', () => {
    ...
    expect(paths).toEqual(['/', 'Pictures/image1.jpeg', 'Pictures/image1.png', 'content.xml'].sort());
  });

  it('throws when the package has no known document media type yet', () => {
    const pkg: Package = { parts: {} };
    expect(() => addImageMedia(pkg, PNG_BYTES, 'png')).toThrow(/documentMediaType/);
  });
});
