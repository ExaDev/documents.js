...
function token(tag: string, text: string, attrs: Record<string, string> = {}): MathMlElement {
...
function firstByTag(root: XmlElement, tag: string): XmlElement {
  ...
  if (found === undefined) {
    throw new Error(`expected the OMML tree to contain a ${tag} element, got: ${buildXml([root])}`);
  }
  return found;
}
...
function mathText(element: XmlElement): string[] {
  return elementsWithTag([element], 'm:t').map((t) => t.children.map((child) => (child.type === 'text' ? child.value : '')).join(''));
}
...
describe('buildOfficeMath: token elements', () => {
  ...
  it('writes a multi-character mi upright, matching MathML3 3.2.3\'s own function-name convention', () => {
    expect(attr(firstByTag(oMath([token('mi', 'sin')]), 'm:sty'), 'm:val')).toBe('p');
  });

  it('writes mn and mo upright', () => {
    expect(attr(firstByTag(oMath([token('mn', '42')]), 'm:sty'), 'm:val')).toBe('p');
    expect(attr(firstByTag(oMath([token('mo', '+')]), 'm:sty'), 'm:val')).toBe('p');
  });
  ...
  it('maps every mathvariant onto OMML\'s own m:scr script and m:sty style axes', () => {
    ...
    for (const [variant, scr, sty] of cases) {
      ...
      expect(childrenWithTag(rPr, 'm:scr').map((element) => attr(element, 'm:val'))).toEqual(scr === undefined ? [] : [scr]);
  ...
  it('inherits an mstyle\'s own mathvariant into descendant tokens', () => {
    const root = oMath([mel('mstyle', { mathvariant: 'double-struck' }, [token('mi', 'N')])]);
...
describe('buildOfficeMath: mfrac -> m:f', () => {
  it('writes num and den in OMML\'s own order', () => {
    const root = oMath([mel('mfrac', {}, [token('mi', 'a'), token('mi', 'b')])]);
  ...
  it('writes a zero linethickness as m:fPr/m:type="noBar", OMML\'s own barless fraction shape', () => {
    const fraction = firstByTag(oMath([mel('mfrac', { linethickness: '0' }, [token('mn', '1'), token('mn', '2')])]), 'm:f');
  ...
  it('leaves an ordinary fraction with no m:fPr at all', () => {
    expect(childTags(firstByTag(oMath([mel('mfrac', { linethickness: '1pt' }, [token('mn', '1'), token('mn', '2')])]), 'm:f'))).toEqual(['m:num', 'm:den']);
  });
});

describe('buildOfficeMath: msqrt/mroot -> m:rad', () => {
  it('writes a square root with a hidden, present-but-empty degree', () => {
    const radical = firstByTag(oMath([mel('msqrt', {}, [token('mi', 'x')])]), 'm:rad');
  ...
  it('treats msqrt\'s own children as an implicit row rather than a single radicand', () => {
    const radical = firstByTag(oMath([mel('msqrt', {}, [token('mi', 'a'), token('mo', '+'), token('mi', 'b')])]), 'm:rad');
  ...
  it('swaps mroot\'s own (radicand, index) order into OMML\'s own (degree, base) order, with no degHide', () => {
    const radical = firstByTag(oMath([mel('mroot', {}, [token('mi', 'x'), token('mn', '3')])]), 'm:rad');
...
describe('buildOfficeMath: scripts', () => {
  it('writes msub as m:sSub, msup as m:sSup, and msubsup as m:sSubSup in OMML\'s own child order', () => {
    expect(childTags(firstByTag(oMath([mel('msub', {}, [token('mi', 'x'), token('mn', '1')])]), 'm:sSub'))).toEqual(['m:e', 'm:sub']);
    expect(childTags(firstByTag(oMath([mel('msup', {}, [token('mi', 'x'), token('mn', '2')])]), 'm:sSup'))).toEqual(['m:e', 'm:sup']);

    const subSup = firstByTag(oMath([mel('msubsup', {}, [token('mi', 'y'), token('mn', '1'), token('mn', '2')])]), 'm:sSubSup');
...
describe('buildOfficeMath: limits', () => {
  it('writes munder as m:limLow and mover as m:limUpp', () => {
    expect(childTags(firstByTag(oMath([mel('munder', {}, [token('mo', '∑'), token('mi', 'i')])]), 'm:limLow'))).toEqual(['m:e', 'm:lim']);
    expect(childTags(firstByTag(oMath([mel('mover', {}, [token('mo', '∑'), token('mi', 'n')])]), 'm:limUpp'))).toEqual(['m:e', 'm:lim']);
  });

  it('nests munderover as an m:limUpp over an m:limLow, OMML having no single both-limits element', () => {
    const root = oMath([mel('munderover', {}, [token('mo', '∑'), token('mi', 'i'), token('mi', 'n')])]);
  ...
  it('takes a movablelimits operator\'s own limits as ordinary scripts outside display style, matching the layout engine\'s own \\nolimits behaviour', () => {
    const root = oMath([mel('mstyle', { displaystyle: 'false' }, [mel('munderover', {}, [token('mo', '∑'), token('mi', 'i'), token('mi', 'n')])])]);
    expect(elementsWithTag([root], 'm:limLow')).toHaveLength(0);
    expect(childTags(firstByTag(root, 'm:sSubSup'))).toEqual(['m:e', 'm:sub', 'm:sup']);
  });

  it('still stacks a NON-movablelimits base outside display style', () => {
    const root = oMath([mel('mstyle', { displaystyle: 'false' }, [mel('mover', {}, [token('mi', 'x'), token('mo', '¯')])])]);
...
describe('buildOfficeMath: mtable -> m:m', () => {
  const matrix = mel('mtable', { columnalign: 'left right' }, [
    mel('mtr', {}, [mel('mtd', {}, [token('mn', '1')]), mel('mtd', {}, [token('mn', '2')])]),
    mel('mtr', {}, [mel('mtd', {}, [token('mn', '3')]), mel('mtd', {}, [token('mn', '4')])]),
  ]);
  ...
  it('writes one m:mr per row and one m:e per cell', () => {
    ...
    expect(rows.map(childTags)).toEqual([['m:e', 'm:e'], ['m:e', 'm:e']]);
  ...
  it('carries columnalign through as one m:mc per column, each with its own count and justification', () => {
    ...
    expect(columns.map((column) => attr(firstByTag(column, 'm:count'), 'm:val'))).toEqual(['1', '1']);
    expect(columns.map((column) => attr(firstByTag(column, 'm:mcJc'), 'm:val'))).toEqual(['left', 'right']);
  });

  it('repeats columnalign\'s own last entry across the remaining columns, per MathML\'s own rule, and defaults to centre', () => {
    const wide = mel('mtable', { columnalign: 'left' }, [mel('mtr', {}, [mel('mtd', {}, [token('mn', '1')]), mel('mtd', {}, [token('mn', '2')])])]);
    const aligns = childrenWithTag(firstByTag(oMath([wide]), 'm:mcs'), 'm:mc').map((column) => attr(firstByTag(column, 'm:mcJc'), 'm:val'));
    expect(aligns).toEqual(['left', 'left']);

    const plain = mel('mtable', {}, [mel('mtr', {}, [mel('mtd', {}, [token('mn', '1')])])]);
...
describe('buildOfficeMath: rows, mstyle, and semantics flatten', () => {
  it('flattens an mrow straight into the containing slot, OMML having no row element of its own', () => {
    const numerator = firstByTag(oMath([mel('mfrac', {}, [mel('mrow', {}, [token('mi', 'a'), token('mo', '+'), token('mi', 'b')]), token('mi', 'c')])]), 'm:num');
  ...
  it('renders semantics\' own first non-annotation child and skips every annotation', () => {
    const root = oMath([mel('semantics', {}, [mel('mfrac', {}, [token('mi', 'a'), token('mi', 'b')]), mel('annotation', { encoding: 'StarMath 5.0' }, [mtxt('{a} over {b}')])])]);
...
describe('buildOfficeMath: degradation', () => {
  it('degrades a construct with no OMML equivalent to a literal-text run carrying its own content, and reports it', () => {
    const { element, diagnostics } = buildOfficeMath([mel('mmultiscripts', {}, [token('mi', 'F'), token('mn', '1'), token('mn', '2')])]);
    if (element === undefined) {
      throw new Error('expected the degraded construct to still produce OMML content');
    }
    expect(mathText(element)).toEqual(['F12']);
    expect(diagnostics).toEqual([{ kind: 'unsupported-element', detail: 'mmultiscripts' }]);
  });

  it('degrades only the unsupported construct, leaving the rest of the formula real OMML', () => {
    const { element, diagnostics } = buildOfficeMath([mel('mrow', {}, [mel('mfrac', {}, [token('mi', 'a'), token('mi', 'b')]), mel('maction', { actiontype: 'toggle' }, [token('mi', 'z')])])]);
    if (element === undefined) {
      throw new Error('expected OMML content');
    }
    expect(elementsWithTag([element], 'm:f')).toHaveLength(1);
    expect(diagnostics.map((diagnostic) => diagnostic.detail)).toEqual(['maction']);
  });

  it('approximates mspace as a single literal space, since OMML has no width-parameterised spacer at all', () => {
    const { element, diagnostics } = buildOfficeMath([token('mi', 'a'), mel('mspace', { width: '1em' }), token('mi', 'b')]);
    if (element === undefined) {
      throw new Error('expected OMML content');
    }
    expect(mathText(element)).toEqual(['a', ' ', 'b']);
    expect(diagnostics).toEqual([{ kind: 'approximated-element', detail: 'mspace' }]);
  });

  it('writes nothing at all, and reports nothing, for a zero-width mspace', () => {
    const { element, diagnostics } = buildOfficeMath([mel('mspace', { width: '0em' })]);
...
describe('buildOfficeMathParagraph', () => {
  it('wraps the equation in m:oMathPara, OMML\'s own display-equation container', () => {
    const { element } = buildOfficeMathParagraph([mel('mfrac', {}, [token('mi', 'a'), token('mi', 'b')])]);
    if (element === undefined) {
  ...
  it('declares the OMML namespace exactly once, on the fragment\'s own root, so it stays valid inside any host docx', () => {
    ...
    expect(attr(element, 'xmlns:m')).toBe('http://schemas.openxmlformats.org/officeDocument/2006/math');
    const nested: XmlNode[] = element.children;
    expect(elementsWithTag(nested, 'm:oMath').map((child) => attr(child, 'xmlns:m'))).toEqual([undefined]);
  });

  it('XML-encodes a construct\'s own literal text rather than corrupting the serialized part', () => {
    const { element } = buildOfficeMathParagraph([token('mo', '<'), token('mo', '&')]);
    if (element === undefined) {
      throw new Error('expected an m:oMathPara element');
    }
