...
export type CellTypeRule = 'boolean-literal' | 'iso-date' | 'named-month-date' | 'plain-number' | 'grouped-number' | 'percentage' | 'currency';

export type CellTypeDeclineReason = 'ambiguous-boolean-word' | 'ambiguous-date-order' | 'ambiguous-grouping-separator' | 'leading-zero-digits' | 'precision-loss';

export type CellTypeInferenceResult = { readonly outcome: 'retyped'; readonly value: ContentCellValue; readonly rule: CellTypeRule } | { readonly outcome: 'declined'; readonly reason: CellTypeDeclineReason };
...
export type CellTypeInference = { readonly sheetIndex: number; readonly row: number; readonly column: number; readonly displayText: string } & CellTypeInferenceResult;
...
const MONTH_NAMES: ReadonlyMap<string, number> = new Map(
  ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october', 'november', 'december'].flatMap((name, index) => [
...
const AMBIGUOUS_BOOLEAN_WORDS: ReadonlySet<string> = new Set(['yes', 'no', 'y', 'n', 'on', 'off']);
...
function parseNumericLiteral(text: string): NumericLiteral | { readonly declined: CellTypeDeclineReason } | undefined {
  ...
  if (grouped?.groups !== undefined) {
    ...
    return { canonicalDecimal: `${sign === '-' ? '-' : ''}${lead}${groups!.replaceAll(',', '')}${fraction === undefined ? '' : `.${fraction}`}`, grouped: true };
  }
  ...
  return { canonicalDecimal: `${sign === '-' ? '-' : ''}${integer}${fraction === undefined ? '' : `.${fraction}`}`, grouped: false };
}

function isDecline(value: NumericLiteral | { readonly declined: CellTypeDeclineReason } | undefined): value is { readonly declined: CellTypeDeclineReason } {
...
function inferCurrency(text: string): CellTypeInferenceResult | undefined {
  ...
  return { outcome: 'retyped', value: { kind: 'currency', value: sign === '-' ? -magnitude : magnitude, currency: CURRENCY_SYMBOLS.get(symbol!) }, rule: 'currency' };
}
...
function inferPercentage(text: string): CellTypeInferenceResult | undefined {
  ...
  return { outcome: 'retyped', value: { kind: 'percentage', value: magnitude / PERCENT_HUNDREDTHS }, rule: 'percentage' };
}
...
function inferDate(text: string): CellTypeInferenceResult | undefined {
  ...
  if (iso?.groups !== undefined) {
    ...
    return isRealCalendarDate(year, month, day) ? { outcome: 'retyped', value: { kind: 'date', value: isoDateValue(year, month, day) }, rule: 'iso-date' } : undefined;
  }
  ...
  if (named?.groups !== undefined) {
    ...
    return isRealCalendarDate(year, month, day) ? { outcome: 'retyped', value: { kind: 'date', value: isoDateValue(year, month, day) }, rule: 'named-month-date' } : undefined;
  }
  return NUMERIC_SEPARATED_DATE_PATTERN.test(text) ? { outcome: 'declined', reason: 'ambiguous-date-order' } : undefined;
}
...
function inferBoolean(text: string): CellTypeInferenceResult | undefined {
  ...
  if (literal !== undefined) {
    return { outcome: 'retyped', value: { kind: 'boolean', value: literal }, rule: 'boolean-literal' };
  }
  return AMBIGUOUS_BOOLEAN_WORDS.has(lowered) ? { outcome: 'declined', reason: 'ambiguous-boolean-word' } : undefined;
}
...
function inferNumber(text: string): CellTypeInferenceResult | undefined {
  ...
  return { outcome: 'retyped', value: { kind: 'number', value }, rule: literal.grouped ? 'grouped-number' : 'plain-number' };
}
...
export function inferCellValue(displayText: string): CellTypeInferenceResult | undefined {
