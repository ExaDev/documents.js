...
import { embeddedFontDocxPackage, embeddedFontOdtPackage, embeddedFontPptxPackage } from '../test-support/fonts';
import { extractSourceFontsForFormat, UnsupportedFontSourceFormatError } from './document-fonts';
...
describe('extractSourceFontsForFormat', () => {
  it('docx: dispatches through ooxml.js decodePackage to the "docx" discriminant', () => {
    ...
    const viaPackage = extractSourceFonts({ kind: 'docx', package: embeddedFontDocxPackage() });
  ...
  it('pptx: dispatches through ooxml.js decodePackage to the "pptx" discriminant', () => {
    ...
    const viaPackage = extractSourceFonts({ kind: 'pptx', package: embeddedFontPptxPackage() });
    expect(viaFormat).toEqual(viaPackage);
    expect(viaFormat.length).toBeGreaterThan(0);
  });
  ...
  it('odt: dispatches through odf.js decodePackage to the "odf" discriminant', () => {
    ...
    const viaPackage = extractSourceFonts({ kind: 'odf', package: embeddedFontOdtPackage() });
    expect(viaFormat).toEqual(viaPackage);
    expect(viaFormat.length).toBeGreaterThan(0);
  });
  ...
  it('throws UnsupportedFontSourceFormatError, naming the six supported formats, for a format that carries no source-embedded fonts', () => {
    for (const format of ['pdf', 'xlsx', 'markdown', 'odf'] as const) {
      expect(() => extractSourceFontsForFormat(format, new Uint8Array())).toThrow(UnsupportedFontSourceFormatError);
      expect(() => extractSourceFontsForFormat(format, new Uint8Array())).toThrow(/docx, pptx, odt, odp, ods, odg/);
    }
  });
  ...
  it('carries the rejected format on the error itself, not only in its message', () => {
    ...
    if (!(caught instanceof UnsupportedFontSourceFormatError)) {
      throw new Error('expected extractSourceFontsForFormat to throw UnsupportedFontSourceFormatError for xlsx');
    }
