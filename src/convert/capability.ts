...
// Deliberately 4 members, not document-schema.js's own 5 -- ContentDocument gained a genuine 'formula' kind in 2.0.0 (see src/model/formula.ts's own top-of-file note), but `odf`'s own capability entry below still models `variant: undefined` rather than `'formula'`: nothing in this package yet actually routes readOdfFormulaContent/odfToPdf through a real `{kind:'formula', ...}` ContentDocument (they still produce/consume the local EmbeddedFormula/StandaloneFormulaContent side-channel shape instead), so adding 'formula' here would describe a capability that doesn't exist yet rather than the one that does. Extend this union only once that real wiring lands.
export type ContentVariant = 'wordprocessing' | 'presentation' | 'spreadsheet' | 'drawing';

export interface FormatCapability {
  readonly format: DocumentFormat;
  // The ContentDocument variant this format reads into / builds from, when it participates in that shared pivot at all. `pdf` is the LayoutDocument pivot itself, not a ContentDocument variant; `odf` (a standalone ODF formula document) reads into its own EmbeddedFormula/StandaloneFormulaContent shape, not a ContentDocument at all yet (see this module's own ContentVariant note above and convert.ts's own odfToPdf comment) -- both leave this undefined.
