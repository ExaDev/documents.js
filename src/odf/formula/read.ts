...
// Reads a standalone .odf package's own formula content as a real 'formula'-kind ContentDocument -- a thin adapter over odf.js's own readOdfFormulaDocument, exactly the pattern this package's other src/odf/{odt,odp,ods,odg}/read.ts modules already follow (readOdtContent over readOdt, readOdpContent over readOdp, ...), and now genuinely the same SHAPE as them too rather than a side-channel value of its own: document-schema.js 2.0.0's ContentDocument union has a 'formula' variant carrying exactly `{ mathml, starMath }`, so a standalone formula document IS a ContentDocument, and odf.js already builds that envelope itself. The source document's own metadata comes through with it, the same way every other X-to-PDF conversion in this package carries its source metadata into writePdf's own Info dict.
...
export function readOdfEmbeddedFormula(outerPkg: Package, subPackagePath: string): ContentFormula | undefined {
  ...
  try {
    ...
    // readOdfFormulaDocument's declared return type is the full ContentDocument union even though it always produces the formula variant -- narrowed rather than asserted, matching every other readXContent guard in this package.
