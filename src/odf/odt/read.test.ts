...
describe('readOdtContent: a real inline image', () => {
  it('produces [paragraph, image] as two adjacent blocks, with the paragraph never consumed', () => {
    ...
    expect(blocks[0]).toMatchObject({ kind: 'paragraph', runs: [{ text: 'Some text' }] });
    expect(blocks[1]).toMatchObject({ kind: 'image', format: 'png', widthPt: 100, heightPt: 50 });
    expect(blocks[2]).toMatchObject({ kind: 'paragraph', runs: [{ text: 'After' }] });
  });
});
