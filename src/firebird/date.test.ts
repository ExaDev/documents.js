...
import { decodeFirebirdDate, decodeFirebirdTime, formatFirebirdDate, formatFirebirdTime, formatFirebirdTimestamp } from './date';
...
describe('decodeFirebirdDate', () => {
  ...
  it('decodes a genuine value cross-checked against this package\'s own real fixture (2020-01-15)', () => {
    ...
    const days = Math.round((Date.UTC(2020, 0, 15) - Date.UTC(1858, 10, 17)) / 86400000);
  ...
  it('round-trips through formatFirebirdDate as an ISO-shaped string', () => {
    const days = Math.round((Date.UTC(2024, 11, 31) - Date.UTC(1858, 10, 17)) / 86400000);
    expect(formatFirebirdDate(days)).toBe('2024-12-31');
  });

  it('handles a leap-year boundary correctly (29 February 2024)', () => {
    const days = Math.round((Date.UTC(2024, 1, 29) - Date.UTC(1858, 10, 17)) / 86400000);
...
describe('decodeFirebirdTime', () => {
  it('decodes 0 as midnight', () => {
    expect(decodeFirebirdTime(0)).toEqual({ hours: 0, minutes: 0, seconds: 0, fractions: 0 });
  });

  it('decodes 13:45:30.5000 correctly (ISC_TIME_SECONDS_PRECISION = 10000 ticks/second)', () => {
    const ticks = ((13 * 3600 + 45 * 60 + 30) * 10000) + 5000;
    expect(decodeFirebirdTime(ticks)).toEqual({ hours: 13, minutes: 45, seconds: 30, fractions: 5000 });
  });
...
describe('formatFirebirdTimestamp', () => {
  it('combines date and time into one space-separated string', () => {
    ...
    expect(formatFirebirdTimestamp(days, ticks)).toBe('2020-01-15 08:00:00.000');
  });
});
