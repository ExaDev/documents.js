// A local, structurally-compatible mirror of odf.js's own XmlNode/XmlElement (src/model/node.ts there), deliberately NOT imported from odf.js: src/mathml/ is a pure layout module with zero ODF (or PDF) knowledge, matching this package's existing src/layout/ isolation rule, and readOdfFormula's own return shape (odf.js's OdfFormulaDocument.mathml: XmlNode[]) is the actual raw tree this module is designed to consume. TypeScript's structural typing means odf.js's real XmlNode[] value type-checks directly against MathMlNode[] with no cast anywhere -- the same "mirror the shape, don't import the package" pattern src/interop.test.ts already verifies holds between ooxml.js and odf.js.
...
// Real MathML producers (confirmed against LibreOffice's own content.xml output) write element tags with a "math:" namespace prefix when math is not the document's default namespace (<math:mfrac>, <math:mrow>, ...), and bare, unprefixed tags when it is (<mfrac>, <mrow>, ...) -- odf.js's own readOdfFormula already handles exactly this ambiguity for the root element (MATH_ROOT_TAGS = ['math', 'math:math']). This module applies the same tolerance uniformly to every element, not just the root: strip a single leading "prefix:" segment before comparing against a canonical MathML tag name, so this layout engine works unmodified regardless of which form a given producer chose.
