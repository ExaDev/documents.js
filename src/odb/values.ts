...
export function cellComparisonKey(value: ContentCellValue): CellComparisonKey | undefined {
  switch (value.kind) {
    case 'number':
    case 'percentage':
    case 'currency':
...
export function compareCellKeys(left: CellComparisonKey, right: CellComparisonKey, fail: CellValueFailure): number {
...
export function compareCellValues(left: ContentCellValue, right: ContentCellValue, fail: CellValueFailure): number {
  const leftKey = cellComparisonKey(left);
  const rightKey = cellComparisonKey(right);
  if (leftKey === undefined || rightKey === undefined) {
    throw fail('a NULL value reached a comparison that had already ruled NULL out');
  }
...
export function cellValuesEqual(left: ContentCellValue, right: ContentCellValue): boolean {
  const leftKey = cellComparisonKey(left);
  const rightKey = cellComparisonKey(right);
  if (leftKey === undefined || rightKey === undefined) {
...
function numericOf(value: ContentCellValue, aggregate: CellAggregateFunction, fail: CellValueFailure): number {
  const key = cellComparisonKey(value);
  if (key?.valueClass !== 'numeric') {
    throw fail(`${aggregate} requires numeric values, but found a ${value.kind} value`);
  }
  return key.numeric;
}
...
export function aggregateCellValues(aggregate: CellAggregateFunction, values: readonly ContentCellValue[], fail: CellValueFailure): ContentCellValue {
