...
// 2cm margins on an A4 page -- the same odf.js/readOdt fallback src/convert/convert.ts's own INLINE_SECTION_MARGIN_PT documents, reused here for the identical reason: a table built from nothing has no real page-layout to read a margin from either.
...
function tableToSheet(table: HsqldbTable): ContentSheet {
  const cells: ContentSheetCell[] = table.columns.map((column, columnIndex) => ({
  ...
  table.rows.forEach((row, rowIndex) => {
    row.forEach((value, columnIndex) => {
      cells.push({ row: rowIndex + 1, column: columnIndex, value, displayText: displayTextFor(value) });
    });
  });

  const columns: ContentSheetColumn[] = table.columns.map((_, index) => ({ index, widthPt: COLUMN_WIDTH_PT }));
  const rows: ContentSheetRow[] = Array.from({ length: table.rows.length + 1 }, (_unused, index) => ({ index, heightPt: ROW_HEIGHT_PT }));

  return { name: table.tableName, cells, columns, rows, images: [], printSettings: DEFAULT_PRINT_SETTINGS };
}
