...
function salesReport() {
  ...
  if (report === undefined) {
    throw new Error('fixture regression: form-and-report.odb no longer declares any report');
  }
  return report;
}
...
function reportRows(): SqlResultSet {
  ...
  const query = readOdbInventory(pkg).queries.find((candidate) => candidate.name === commandName);
  if (query?.command === undefined) {
    throw new Error(`fixture regression: the report's own rpt:command "${String(commandName)}" names no saved query with a db:command`);
  }
...
function everySalesRow(): SqlResultSet {
  return evaluateSelect(parseSelect('SELECT REGION, QUARTER, CUSTOMER, AMOUNT FROM SALES ORDER BY REGION ASC, QUARTER ASC, AMOUNT DESC'), readOdbTables(formAndReportOdbPackage()));
}

function numberAt(band: RptBandInstance | undefined, index: number): number | undefined {
  const value = band?.values[index];
  return value?.kind === 'number' ? value.value : undefined;
}

function stringAt(band: RptBandInstance | undefined, index: number): string | undefined {
  const value = band?.values[index];
  return value?.kind === 'string' ? value.value : undefined;
}

// Every band of one kind (and, for a group band, one level) in print order.
function bandsOfKind(bands: readonly RptBandInstance[], kind: RptBandInstance['kind'], groupLevel?: number): readonly RptBandInstance[] {
  return bands.filter((band) => band.kind === kind && (groupLevel === undefined || band.groupLevel === groupLevel));
}
...
describe('the real SalesByRegion report own formulas, read straight out of form-and-report.odb', () => {
  it('declares the formulas this engine is built for, rather than this test restating them', () => {
    ...
    expect(report.functions).toEqual([{ name: 'LEFT_QUARTER', formula: EXPECTED_FUNCTION_FORMULA }]);

    const definition = rptDefinitionFromReport(report);
    expect(definition.groups.map((group) => group.groupExpression)).toEqual([EXPECTED_OUTER_GROUP_EXPRESSION, EXPECTED_INNER_GROUP_EXPRESSION]);
    ...
    expect(definition.groups[1]?.footer?.formulas).toEqual([undefined, EXPECTED_FOOTER_TOTAL_FORMULA]);
    expect(definition.groups[0]?.footer?.formulas).toEqual([undefined, EXPECTED_FOOTER_TOTAL_FORMULA]);
    expect(definition.reportFooter?.formulas).toEqual([undefined, EXPECTED_FOOTER_TOTAL_FORMULA]);

    // The detail band and both group headers are plain field: bindings -- no computation, just a column value passing through.
    expect(definition.detail?.formulas).toEqual(['field:[CUSTOMER]', 'field:[AMOUNT]']);
  ...
  it('runs over the four rows its own rpt:command query returns', () => {
    ...
    expect(rows.rows.map((row) => (row[2]?.kind === 'string' ? row[2].value : undefined))).toEqual(['Acme Ltd', 'Bolt Supplies', 'Crown Foods', 'Everest Tools']);
  });

  it('emits every band in print order, with the group breaks falling at exactly the right row transitions', () => {
    const { bands } = runRptReport(rptDefinitionFromReport(salesReport()), reportRows());

    expect(bands.map((band) => `${band.kind}${band.groupLevel === undefined ? '' : `:${String(band.groupLevel)}`}@${band.rowIndex === undefined ? '-' : String(band.rowIndex)}`)).toEqual([
      'report-header@-',
  ...
  it('computes LEFT_QUARTER through the named rpt:function, and each group header field', () => {
    ...
    expect(bandsOfKind(bands, 'group-header', 0).map((band) => stringAt(band, 0))).toEqual(['North', 'South']);
    ...
    expect(bandsOfKind(bands, 'group-header', 1).map((band) => stringAt(band, 0))).toEqual(['Q1', 'Q2', 'Q2']);
  });
  ...
  it('totals exactly the right AMOUNT values in each of the three real SUM scopes', () => {
    ...
    expect(bandsOfKind(bands, 'group-footer', 1).map((band) => numberAt(band, 1))).toEqual([1540.5, 2750.25, 1810]);

    // Outer ("Region total:").
    expect(bandsOfKind(bands, 'group-footer', 0).map((band) => numberAt(band, 1))).toEqual([4290.75, 1810]);

    // Report ("Grand total:") -- every row the query returned.
    expect(bandsOfKind(bands, 'report-footer').map((band) => numberAt(band, 1))).toEqual([6100.75]);
  ...
  it('passes each detail row own CUSTOMER and AMOUNT straight through its field: bindings', () => {
    ...
    expect(detail.map((band) => stringAt(band, 0))).toEqual(['Acme Ltd', 'Bolt Supplies', 'Crown Foods', 'Everest Tools']);
    expect(detail.map((band) => numberAt(band, 1))).toEqual([1200.5, 340, 2750.25, 1810]);
  });
});
...
describe('the same real report own formulas over all six real SALES rows', () => {
  ...
  it('breaks the inner group at the South-to-West transition, where the quarter itself does not change', () => {
    const { bands } = runRptReport(rptDefinitionFromReport(salesReport()), everySalesRow());

    expect(bandsOfKind(bands, 'group-header', 0).map((band) => stringAt(band, 0))).toEqual(['North', 'South', 'West']);
    expect(bandsOfKind(bands, 'group-header', 1).map((band) => `${String(band.rowIndex)}:${String(stringAt(band, 0))}`)).toEqual(['0:Q1', '2:Q2', '3:Q1', '4:Q2', '5:Q2']);
  });
  ...
  it('totals each scope over exactly its own rows', () => {
    ...
    expect(bandsOfKind(bands, 'group-footer', 1).map((band) => numberAt(band, 1))).toEqual([1540.5, 2750.25, 95.75, 1810, 60]);
    expect(bandsOfKind(bands, 'group-footer', 0).map((band) => numberAt(band, 1))).toEqual([4290.75, 1905.75, 60]);
    expect(bandsOfKind(bands, 'report-footer').map((band) => numberAt(band, 1))).toEqual([6256.5]);
  });
  ...
  it('reaches the same per-region totals the SQL engine own GROUP BY does', () => {
    const grouped = evaluateSelect(parseSelect('SELECT REGION, SUM(AMOUNT) FROM SALES GROUP BY REGION ORDER BY REGION ASC'), readOdbTables(formAndReportOdbPackage()));
    const viaSql = grouped.rows.map((row) => (row[1]?.kind === 'number' ? row[1].value : undefined));

    const { bands } = runRptReport(rptDefinitionFromReport(salesReport()), everySalesRow());
    expect(bandsOfKind(bands, 'group-footer', 0).map((band) => numberAt(band, 1))).toEqual(viaSql);
  });
  ...
  it('groups South and West Q2 together when the quarter is the only group, proving HASCHANGED itself carries no nesting rule', () => {
    ...
    if (inner === undefined) {
      throw new Error('fixture regression: the report no longer declares a second, inner group');
    }
    const { bands } = runRptReport({ functions: [{ name: 'LEFT_QUARTER', formula: EXPECTED_FUNCTION_FORMULA }], reportHeader: undefined, groups: [inner], detail: undefined, reportFooter: undefined }, everySalesRow());

    expect(bandsOfKind(bands, 'group-footer', 0).map((band) => numberAt(band, 1))).toEqual([1540.5, 2750.25, 95.75, 1870]);
...
describe('rpt:LEFT over real fixture data long enough to actually truncate', () => {
  ...
  it('takes the first n characters of a real column value', () => {
    ...
    const { bands } = runRptReport(
      {
        functions: [{ name: 'CUSTOMER_PREFIX', formula: 'rpt:LEFT([CUSTOMER];4)' }],
    ...
    expect(detail.map((band) => stringAt(band, 0))).toEqual(['Acme Ltd', 'Bolt Supplies', 'Crown Foods', 'Delta Print', 'Everest Tools', 'Foxglove Design']);
    expect(detail.map((band) => stringAt(band, 1))).toEqual(['Acme', 'Bolt', 'Crow', 'Delt', 'Ever', 'Foxg']);
  });

  it('resolves the report own LEFT_QUARTER by name to the two-character quarter prefix', () => {
    const { bands } = runRptReport(
      {
        functions: [{ name: 'LEFT_QUARTER', formula: EXPECTED_FUNCTION_FORMULA }],
        reportHeader: undefined,
        groups: [],
    ...
    expect(bands.filter((band) => band.kind === 'detail').map((band) => stringAt(band, 0))).toEqual(['Q1', 'Q1', 'Q2', 'Q1', 'Q2', 'Q2']);
  });
});

describe('the report own data reaching a formula', () => {
  it('carries a field: value through unchanged, kind included', () => {
    const { bands } = runRptReport({ functions: [], reportHeader: undefined, groups: [], detail: { formulas: ['field:[AMOUNT]'] }, reportFooter: undefined }, reportRows());
