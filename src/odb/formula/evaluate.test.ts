...
import { RptFormulaEvaluationError, RptFormulaUnsupportedError } from './errors';
import type { RptBandDefinition, RptBandInstance, RptGroupDefinition, RptReportDefinition } from './evaluate';
...
function resultSet(columns: readonly string[], rows: readonly (readonly ContentCellValue[])[]): SqlResultSet {
  return { columns, rows };
}
...
function group(groupExpression: string, options: Partial<Omit<RptGroupDefinition, 'groupExpression'>> = {}): RptGroupDefinition {
  return { groupExpression, functions: options.functions ?? [], header: options.header, footer: options.footer };
}

function report(definition: Partial<RptReportDefinition>): RptReportDefinition {
  return { functions: definition.functions ?? [], reportHeader: definition.reportHeader, groups: definition.groups ?? [], detail: definition.detail, reportFooter: definition.reportFooter };
}

function numberAt(instance: RptBandInstance, index: number): number | undefined {
  const value = instance.values[index];
  return value?.kind === 'number' ? value.value : undefined;
}

function totalsOf(bands: readonly RptBandInstance[], kind: RptBandInstance['kind'], groupLevel?: number): readonly (number | undefined)[] {
  return bands.filter((instance) => instance.kind === kind && (groupLevel === undefined || instance.groupLevel === groupLevel)).map((instance) => numberAt(instance, 0));
}
...
describe('group scoping', () => {
  ...
  it('closes group footers innermost first and opens group headers outermost first', () => {
    const { bands } = runRptReport(
      report({
        groups: [group('rpt:HASCHANGED("A")', { header: band(), footer: band() }), group('rpt:HASCHANGED("B")', { header: band(), footer: band() })],
        detail: band(),
      }),
      resultSet(
        ['A', 'B'],
    ...
    expect(bands.map((instance) => `${instance.kind}:${String(instance.groupLevel ?? '-')}`)).toEqual([
      'group-header:0',
      'group-header:1',
      'detail:-',
      'group-footer:1',
  ...
  it('gives a group HEADER the true total for the group it is opening, not a running total of its first row', () => {
    const { bands } = runRptReport(
      report({ groups: [group('rpt:HASCHANGED("A")', { header: band('rpt:SUM([V])'), footer: band('rpt:SUM([V])') })] }),
      resultSet(
        ['A', 'V'],
        [
          [text('x'), amount(1)],
  ...
  it('scopes a detail-band aggregate to the innermost group, and to the whole report when there are none', () => {
    ...
    const grouped = runRptReport(report({ groups: [group('rpt:HASCHANGED("A")')], detail: band('rpt:SUM([V])') }), rows);
    expect(totalsOf(grouped.bands, 'detail')).toEqual([3, 3, 4]);

    const ungrouped = runRptReport(report({ detail: band('rpt:SUM([V])') }), rows);
  ...
  it('scopes a named rpt:function aggregate to where it was DECLARED, not to the band that referenced it', () => {
    ...
    const definition = report({
      ...
      groups: [group('rpt:HASCHANGED("A")', { functions: [{ name: 'GROUP_TOTAL', formula: 'rpt:SUM([V])' }], footer: band('field:[GROUP_TOTAL]', 'field:[REPORT_TOTAL]') })],
      detail: band('field:[GROUP_TOTAL]', 'field:[REPORT_TOTAL]'),
    });

    const { bands } = runRptReport(definition, rows);
    const footers = bands.filter((instance) => instance.kind === 'group-footer');
    ...
    expect(bands.filter((instance) => instance.kind === 'detail').map((instance) => numberAt(instance, 0))).toEqual([3, 3, 4]);
  });
});
...
describe('rpt:HASCHANGED', () => {
  it('is true on the first row and whenever the value differs from the immediately preceding one', () => {
    const { bands } = runRptReport(
      ...
      resultSet(['A'], [[text('x')], [text('x')], [text('y')], [text('y')], [text('x')]]),
    );
  ...
  it('treats two NULLs as unchanged and a NULL against a value as changed', () => {
    const { bands } = runRptReport(report({ detail: band('rpt:HASCHANGED([A])') }), resultSet(['A'], [[NULL_VALUE], [NULL_VALUE], [text('x')], [NULL_VALUE]]));
  ...
  it('compares the value of a named function, not the function name', () => {
    const { bands } = runRptReport(
      report({ functions: [{ name: 'PREFIX', formula: 'rpt:LEFT([A];1)' }], detail: band('rpt:HASCHANGED("PREFIX")') }),
...
describe('rpt:LEFT', () => {
  it('takes a prefix, passes a shorter value through whole, and propagates NULL', () => {
    const { bands } = runRptReport(report({ detail: band('rpt:LEFT([A];3)') }), resultSet(['A'], [[text('abcdef')], [text('ab')], [text('')], [NULL_VALUE]]));

    expect(bands.map((instance) => instance.values[0])).toEqual([{ kind: 'string', value: 'abc' }, { kind: 'string', value: 'ab' }, { kind: 'string', value: '' }, NULL_VALUE]);
  });

  it('counts characters rather than UTF-16 code units, so it never splits a surrogate pair', () => {
    const { bands } = runRptReport(report({ detail: band('rpt:LEFT([A];2)') }), resultSet(['A'], [[text('\u{1F600}\u{1F601}\u{1F602}')]]));

    expect(bands[0]?.values[0]).toEqual({ kind: 'string', value: '\u{1F600}\u{1F601}' });
  });

  it('refuses a non-text value rather than inventing a number format for it', () => {
    expect(() => runRptReport(report({ detail: band('rpt:LEFT([A];2)') }), resultSet(['A'], [[amount(1234)]]))).toThrow(/rpt:LEFT requires a text value, but found a number value/);
  });
});

describe('aggregates', () => {
  const ROWS = resultSet(['V'], [[amount(1)], [amount(2)], [amount(6)], [NULL_VALUE]]);

  it('implements the same five the SQL engine does, skipping NULLs', () => {
    const { bands } = runRptReport(report({ reportFooter: band('rpt:SUM([V])', 'rpt:COUNT([V])', 'rpt:AVG([V])', 'rpt:MIN([V])', 'rpt:MAX([V])') }), ROWS);

    expect(bands[0]?.values).toEqual([amount(9), amount(3), amount(3), amount(1), amount(6)]);
  });

  it('returns NULL from every aggregate but COUNT when a scope holds no non-NULL value', () => {
    const { bands } = runRptReport(report({ reportFooter: band('rpt:SUM([V])', 'rpt:COUNT([V])', 'rpt:AVG([V])', 'rpt:MIN([V])', 'rpt:MAX([V])') }), resultSet(['V'], [[NULL_VALUE], [NULL_VALUE]]));

    expect(bands[0]?.values).toEqual([NULL_VALUE, amount(0), NULL_VALUE, NULL_VALUE, NULL_VALUE]);
  });

  it('refuses to total non-numeric values rather than coercing them', () => {
    expect(() => runRptReport(report({ reportFooter: band('rpt:SUM([A])') }), resultSet(['A'], [[text('12')]]))).toThrow(/SUM requires numeric values, but found a string value/);
  });

  it('still emits the report header and footer over an empty result set', () => {
    const { bands } = runRptReport(report({ reportHeader: band(undefined), groups: [group('rpt:HASCHANGED("A")', { footer: band('rpt:SUM([V])') })], detail: band('field:[V]'), reportFooter: band('rpt:SUM([V])', 'rpt:COUNT([V])') }), resultSet(['A', 'V'], []));

    expect(bands.map((instance) => instance.kind)).toEqual(['report-header', 'report-footer']);
...
describe('the failure policy', () => {
  ...
  it('refuses a reference naming neither a column nor a declared function', () => {
    expect(() => runRptReport(report({ detail: band('field:[MISSING]') }), ROWS)).toThrow(/names neither a declared rpt:function nor a column/);
    expect(() => runRptReport(report({ reportFooter: band('rpt:SUM([MISSING])') }), ROWS)).toThrow(/names neither a declared rpt:function nor a column/);
  });
  ...
  it('refuses an unresolvable rpt:HASCHANGED reference on the very first row, without waiting for a second row to compare against', () => {
    expect(() => runRptReport(report({ detail: band('rpt:HASCHANGED([MISSING])') }), ROWS)).toThrow(/names neither a declared rpt:function nor a column/);
    expect(() => runRptReport(report({ groups: [group('rpt:HASCHANGED("MISSING")')] }), ROWS)).toThrow(/names neither a declared rpt:function nor a column/);
  });

  it('refuses a reference that matches both a declared function and a column, rather than letting one shadow the other', () => {
    expect(() => runRptReport(report({ functions: [{ name: 'A', formula: 'rpt:LEFT([A];1)' }], detail: band('field:[A]') }), ROWS)).toThrow(/reference "A" is ambiguous/);
  });

  it('refuses a function name declared twice', () => {
    const definition = report({
      functions: [{ name: 'F', formula: 'field:[A]' }],
      groups: [group('rpt:HASCHANGED("A")', { functions: [{ name: 'F', formula: 'field:[V]' }] })],
    });
    expect(() => runRptReport(definition, ROWS)).toThrow(/declared more than once/);
  });

  it('refuses a self-referential function rather than recursing until the stack runs out', () => {
    expect(() => runRptReport(report({ functions: [{ name: 'F', formula: 'field:[F]' }], detail: band('field:[F]') }), ROWS)).toThrow(/refers to itself/);
    expect(() =>
      runRptReport(
        report({
          functions: [
  ...
  it('refuses a group expression that depends on an aggregate, before reading a single row', () => {
    expect(() => runRptReport(report({ groups: [group('rpt:SUM([V])')] }), ROWS)).toThrow(/a group expression cannot depend on the aggregate/);
    expect(() => runRptReport(report({ functions: [{ name: 'T', formula: 'rpt:SUM([V])' }], groups: [group('rpt:HASCHANGED("T")')] }), ROWS)).toThrow(/a group expression cannot depend on the aggregate/);
  });

  it('catches a circular group expression even when the result set is empty', () => {
    expect(() => runRptReport(report({ groups: [group('rpt:SUM([V])')] }), resultSet(['A', 'V'], []))).toThrow(RptFormulaEvaluationError);
  });

  it('refuses a group expression that does not evaluate to a boolean break test', () => {
    expect(() => runRptReport(report({ groups: [group('field:[A]')] }), ROWS)).toThrow(/must evaluate to a boolean break test/);
  });

  it('refuses a per-row formula in the report header or footer, which belong to no row', () => {
    expect(() => runRptReport(report({ reportHeader: band('field:[A]') }), ROWS)).toThrow(/needs a data row/);
    expect(() => runRptReport(report({ reportFooter: band('rpt:LEFT([A];1)') }), ROWS)).toThrow(/needs a data row/);
    expect(() => runRptReport(report({ reportFooter: band('rpt:HASCHANGED([A])') }), ROWS)).toThrow(/needs a data row/);
  });

  it('refuses an unsupported function anywhere in the report before running it', () => {
    expect(() => runRptReport(report({ detail: band('rpt:PAGENUMBER()') }), ROWS)).toThrow(RptFormulaUnsupportedError);
    expect(() => runRptReport(report({ functions: [{ name: 'F', formula: 'rpt:TODAY()' }] }), ROWS)).toThrow(RptFormulaUnsupportedError);
    expect(() => runRptReport(report({ groups: [group('rpt:WHENEVER([A])')] }), ROWS)).toThrow(RptFormulaUnsupportedError);
  });

  it('leaves an element with no formula of its own as an undefined value rather than inventing one', () => {
    const { bands } = runRptReport(report({ detail: band(undefined, 'field:[A]', undefined) }), ROWS);
