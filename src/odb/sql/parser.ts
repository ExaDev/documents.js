...
const AGGREGATE_FUNCTIONS: ReadonlySet<string> = new Set<SqlAggregateFunction>(['COUNT', 'SUM', 'AVG', 'MIN', 'MAX']);
...
export type SqlOperand = { readonly kind: 'column'; readonly column: SqlColumnRef } | { readonly kind: 'literal'; readonly literal: SqlLiteral };

export type SqlPredicate =
  | { readonly kind: 'comparison'; readonly operator: SqlComparisonOperator; readonly left: SqlOperand; readonly right: SqlOperand }
  | { readonly kind: 'isNull'; readonly operand: SqlOperand; readonly negated: boolean }
  | { readonly kind: 'like'; readonly operand: SqlOperand; readonly pattern: string; readonly negated: boolean }
  | { readonly kind: 'in'; readonly operand: SqlOperand; readonly values: readonly SqlLiteral[]; readonly negated: boolean }
  | { readonly kind: 'between'; readonly operand: SqlOperand; readonly lower: SqlOperand; readonly upper: SqlOperand; readonly negated: boolean }
  | { readonly kind: 'not'; readonly predicate: SqlPredicate }
  | { readonly kind: 'and'; readonly left: SqlPredicate; readonly right: SqlPredicate }
  | { readonly kind: 'or'; readonly left: SqlPredicate; readonly right: SqlPredicate };

export type SqlAggregateArgument = { readonly kind: 'star' } | { readonly kind: 'column'; readonly column: SqlColumnRef };
...
export type SqlSelectItem =
  ...
  | { readonly kind: 'aggregate'; readonly aggregate: SqlAggregateFunction; readonly argument: SqlAggregateArgument; readonly outputName: string };
...
function rejectOutOfScopeConstructs(tokens: readonly SqlToken[], sql: string): void {
  tokens.forEach((token, index) => {
    ...
    if (token.kind === 'identifier') {
      ...
      if (next?.kind === 'punctuation' && next.punctuation === '(') {
        throw new HsqldbSqlUnsupportedError(`a scalar function (${token.text})`, sql);
      }
    }
  });
}
...
class SqlParser {
  ...
  private peek(): SqlToken {
    ...
    if (token === undefined) {
      throw new HsqldbSqlParseError('unexpected end of statement', this.sql, this.sql.length);
    }
    return token;
  }
  ...
  private parseColumnRef(): SqlColumnRef {
    ...
    if (this.atPunctuation('.')) {
      ...
      return { qualifier: first, column: second, text: this.sql.slice(start, this.previousEnd()) };
    }
    return { qualifier: undefined, column: first, text: this.sql.slice(start, this.previousEnd()) };
  }
  ...
  private parseAggregate(aggregate: SqlAggregateFunction): SqlSelectItem {
    ...
    if (this.atPunctuation('*')) {
      if (aggregate !== 'COUNT') {
        this.fail(`a column name -- ${aggregate}(*) is not valid SQL, only COUNT(*) is`);
      }
      this.advance();
      this.takePunctuation(')');
      return { kind: 'aggregate', aggregate, argument: { kind: 'star' }, outputName: 'COUNT(*)' };
    }
    const column = this.parseColumnRef();
    this.takePunctuation(')');
    return { kind: 'aggregate', aggregate, argument: { kind: 'column', column }, outputName: `${aggregate}(${column.column.name})` };
  }
  ...
  private parseSelectList(): readonly SqlSelectItem[] {
    if (this.atPunctuation('*')) {
      this.advance();
      if (this.atPunctuation(',')) {
        throw new HsqldbSqlUnsupportedError('a select list mixing "*" with named columns', this.sql);
      }
      return [{ kind: 'star' }];
    }
  ...
  private parseOperand(): SqlOperand {
    ...
    if (token.kind === 'punctuation' && token.punctuation === '(') {
      throw new HsqldbSqlUnsupportedError('a parenthesised value expression', this.sql);
    }
  ...
  private parseInList(operand: SqlOperand, negated: boolean): SqlPredicate {
    ...
    for (;;) {
      if (this.peek().kind === 'identifier') {
        throw new HsqldbSqlUnsupportedError('a column reference inside an IN list', this.sql);
      }
  ...
  private parseLike(operand: SqlOperand, negated: boolean): SqlPredicate {
    ...
    if (token.kind !== 'string') {
      if (token.kind === 'identifier') {
        throw new HsqldbSqlUnsupportedError('a non-literal LIKE pattern', this.sql);
      }
  ...
  private parsePostfix(operand: SqlOperand): SqlPredicate {
    ...
    if (token.kind === 'operator') {
      this.advance();
      return { kind: 'comparison', operator: token.operator, left: operand, right: this.parseOperand() };
    }
    if (token.kind === 'punctuation' && (token.punctuation === '*' || token.punctuation === '-')) {
      throw new HsqldbSqlUnsupportedError('an arithmetic expression', this.sql);
    }
    if (token.kind === 'keyword') {
      if (token.keyword === 'IS') {
  ...
  private parseGroupByColumn(): SqlColumnRef {
    const token = this.peek();
    if (token.kind === 'keyword' && isAggregateFunction(token.keyword)) {
      this.fail('a column name -- an aggregate function is not valid in GROUP BY');
    }
  ...
  private parseOrderByTerm(): SqlOrderByTerm {
    const token = this.peek();
    if (token.kind === 'keyword' && isAggregateFunction(token.keyword)) {
      throw new HsqldbSqlUnsupportedError('an aggregate function in ORDER BY', this.sql);
    }
    if (token.kind === 'number') {
      throw new HsqldbSqlUnsupportedError('an ordinal column reference in ORDER BY (ORDER BY 1)', this.sql);
    }
    const column = this.parseColumnRef();
  ...
  parseStatement(): SqlSelectStatement {
    ...
    if (this.atPunctuation('.')) {
      ...
      throw new HsqldbSqlUnsupportedError('a schema-qualified table name', this.sql);
    }
    if (this.atPunctuation(',')) {
