...
const EXPECTED_SAVED_QUERY = 'SELECT "SALES"."REGION", "SALES"."QUARTER", "SALES"."CUSTOMER", "SALES"."AMOUNT" FROM "SALES" WHERE "SALES"."AMOUNT" >= 100 ORDER BY "SALES"."REGION" ASC, "SALES"."QUARTER" ASC, "SALES"."AMOUNT" DESC';

function savedQuery(): string {
  const query = readOdbInventory(formAndReportOdbPackage()).queries.find((candidate) => candidate.name === 'HighValueSales');
  if (query?.command === undefined) {
    throw new Error('fixture regression: form-and-report.odb no longer declares a HighValueSales query with a db:command');
  }
...
describe('src/odb/sql against the real form-and-report.odb saved query', () => {
  ...
  it('reads the real six-row SALES table the query runs against', () => {
    ...
    expect(tables[0]?.columns.map((column) => column.name)).toEqual(['AMOUNT', 'ID', 'REGION', 'QUARTER', 'CUSTOMER']);
  ...
  it('executes it to exactly the four rows that pass AMOUNT >= 100, in REGION/QUARTER ascending then AMOUNT descending order', () => {
    const result = evaluateSelect(parseSelect(savedQuery()), readOdbTables(formAndReportOdbPackage()));
  ...
  it('drops exactly the two rows below the AMOUNT threshold, and nothing else', () => {
    const tables = readOdbTables(formAndReportOdbPackage());
    const everyCustomer = evaluateSelect(parseSelect('SELECT CUSTOMER FROM SALES'), tables).rows.flatMap((row) => (row[0]?.kind === 'string' ? [row[0].value] : []));
    const passingCustomers = evaluateSelect(parseSelect(savedQuery()), tables).rows.flatMap((row) => (row[2]?.kind === 'string' ? [row[2].value] : []));

    expect(everyCustomer).toEqual(['Acme Ltd', 'Bolt Supplies', 'Crown Foods', 'Delta Print', 'Everest Tools', 'Foxglove Design']);
    expect(everyCustomer.filter((customer) => !passingCustomers.includes(customer))).toEqual(['Delta Print', 'Foxglove Design']);
  });

  it('aggregates the same real table by region, the query shape the fixture own Report Builder report renders', () => {
    const result = evaluateSelect(parseSelect('SELECT REGION, COUNT(*), SUM(AMOUNT), MIN(AMOUNT), MAX(AMOUNT) FROM SALES GROUP BY REGION ORDER BY REGION ASC'), readOdbTables(formAndReportOdbPackage()));

    expect(result.columns).toEqual(['REGION', 'COUNT(*)', 'SUM(AMOUNT)', 'MIN(AMOUNT)', 'MAX(AMOUNT)']);
    expect(result.rows).toEqual([
      [{ kind: 'string', value: 'North' }, { kind: 'number', value: 3 }, { kind: 'number', value: 4290.75 }, { kind: 'number', value: 340 }, { kind: 'number', value: 2750.25 }],
      [{ kind: 'string', value: 'South' }, { kind: 'number', value: 2 }, { kind: 'number', value: 1905.75 }, { kind: 'number', value: 95.75 }, { kind: 'number', value: 1810 }],
      [{ kind: 'string', value: 'West' }, { kind: 'number', value: 1 }, { kind: 'number', value: 60 }, { kind: 'number', value: 60 }, { kind: 'number', value: 60 }],
    ]);
  });
});
