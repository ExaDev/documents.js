...
/**
 * release-workspace.config.json holds two independent lists of conventional-commit types, and nothing in the file itself forces them to agree.
 *
...
const CONFIG_FILE = "release-workspace.config.json";
...
const config = requireRecord(
  JSON.parse(readFileSync(new URL(CONFIG_FILE, import.meta.url), "utf8")),
  "the top level",
);
...
describe("release-workspace.config.json", () => {
