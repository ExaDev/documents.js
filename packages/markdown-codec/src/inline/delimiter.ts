...
export type EmphasisWrapperFactory = (
...
export function processEmphasis(
  ...
  createWrapper: EmphasisWrapperFactory,
): void {
  ...
  while (closer !== undefined) {
    ...
    const floor = openersFloor.has(signature)
    ...
    closer.count -= used;
    openerNode.literal = openerNode.literal.slice(
      0,
      openerNode.literal.length - used,
    );
    closerNode.literal = closerNode.literal.slice(
      0,
      closerNode.literal.length - used,
    );
