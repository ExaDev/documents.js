...
const NO_REFERENCES: LinkReferenceMap = new Map<
  string,
...
function parse(
  source: string,
  references: LinkReferenceMap = NO_REFERENCES,
): MarkdownInlineNode[] {
...
describe("text, escapes, and character references", () => {
  ...
  it("keeps a character reference as its own node, recording both the source spelling and the decoded value", () => {
    expect(parse("&amp;")).toEqual([
      { type: "entity", raw: "&amp;", value: "&" },
    ]);
    expect(parse("&#x41;")).toEqual([
      { type: "entity", raw: "&#x41;", value: "A" },
    ]);
  });
...
describe("inline math (ExaDev/markdown-codec#53)", () => {
  it("recognises \\( \\) as inline math, literal excluding the delimiters", () => {
    expect(parse("\\(E = mc^2\\)")).toEqual([
      { type: "mathInline", literal: "E = mc^2" },
    ]);
  });

  it("closes at the first \\), no nested-\\(-awareness", () => {
    expect(parse("\\(f(x)\\)")).toEqual([
      { type: "mathInline", literal: "f(x)" },
    ]);
  });

  it('treats an unmatched \\( as today\'s ordinary backslash escape of "("', () => {
    expect(parse("\\(no closer here")).toEqual([
      { type: "text", value: "(no closer here" },
    ]);
  });
...
describe("emphasis, strong emphasis, and the flanking rules", () => {
  ...
  it("allows intraword emphasis with `*` but not with `_`", () => {
    ...
    expect(parse("foo_bar_baz")).toEqual([
...
describe("links and images", () => {
  ...
  it("flattens an image description to plain text rather than keeping inline children", () => {
    expect(parse("![a *b* `c`](/url)")).toEqual([
      { type: "image", destination: "/url", alt: "a b c" },
    ]);
  });
    ...
    expect(parse("[outer ![inner](/i)](/o)")).toEqual([
      {
        ...
        children: [
          { type: "text", value: "outer " },
          { type: "image", destination: "/i", alt: "inner" },
        ],
      },
    ]);
  });
...
describe("autolinks and raw HTML", () => {
  it("parses a URI autolink verbatim", () => {
    expect(parse("<https://a.example/b?c=d&e>")).toEqual([
      {
        type: "autolink",
        destination: "https://a.example/b?c=d&e",
        email: false,
      },
    ]);
  });
  ...
  it("flags an email autolink", () => {
    expect(parse("<foo@bar.example.com>")).toEqual([
      { type: "autolink", destination: "foo@bar.example.com", email: true },
    ]);
  });

  it("keeps a raw inline HTML tag as its literal source text", () => {
    expect(parse('<span class="x">')).toEqual([
...
describe("GFM extended autolinks", () => {
  it("links a www-prefixed run, prepending the scheme to the destination only", () => {
    expect(
      parseInlines("see www.example.com now", NO_REFERENCES, NO_FOOTNOTES),
    ).toEqual([
      { type: "text", value: "see " },
      {
        type: "link",
  ...
  it("links a bare http(s) run", () => {
    expect(
      parseInlines("https://example.com/a", NO_REFERENCES, NO_FOOTNOTES),
    ).toEqual([
      {
        type: "link",
  ...
  it("links a bare email address through a mailto: destination", () => {
    expect(
      parseInlines("mail me@example.com", NO_REFERENCES, NO_FOOTNOTES),
    ).toEqual([
      { type: "text", value: "mail " },
      {
        type: "link",
        destination: "mailto:me@example.com",
        children: [{ type: "text", value: "me@example.com" }],
      },
    ]);
  });

  it("rejects a domain with no dot", () => {
    expect(parseInlines("www.example", NO_REFERENCES, NO_FOOTNOTES)).toEqual([
      { type: "text", value: "www.example" },
    ]);
  });

  it("links a bare ftp run, the third scheme GFM recognises alongside http and https", () => {
    expect(
      parseInlines("ftp://foo.bar.baz", NO_REFERENCES, NO_FOOTNOTES),
    ).toEqual([
      {
        type: "link",
        destination: "ftp://foo.bar.baz",
  ...
  it("drops a trailing dot from an email address but rejects one ending in a hyphen or underscore outright", () => {
    expect(parseInlines("a.b-c_d@a.b.", NO_REFERENCES, NO_FOOTNOTES)).toEqual([
      {
        type: "link",
        destination: "mailto:a.b-c_d@a.b",
        children: [{ type: "text", value: "a.b-c_d@a.b" }],
      },
      { type: "text", value: "." },
    ]);
    expect(parseInlines("a.b-c_d@a.b-", NO_REFERENCES, NO_FOOTNOTES)).toEqual([
      { type: "text", value: "a.b-c_d@a.b-" },
    ]);
    expect(parseInlines("a.b-c_d@a.b_", NO_REFERENCES, NO_FOOTNOTES)).toEqual([
      { type: "text", value: "a.b-c_d@a.b_" },
    ]);
  });

  it("never creates an autolink inside an existing link or code span", () => {
    expect(
      parseInlines("[www.example.com](/u)", NO_REFERENCES, NO_FOOTNOTES),
    ).toEqual([
      {
        type: "link",
        destination: "/u",
    ...
    expect(
      parseInlines("`www.example.com`", NO_REFERENCES, NO_FOOTNOTES),
    ).toEqual([{ type: "codeSpan", literal: "www.example.com" }]);
  });
