...
//
// Pandoc's inline note form (`^[note text here]`) is deliberately NOT recognised here: it has no GitHub analogue, and the ContentDocument mapping this package lowers onto (an anchor construct wrapping the note's own block extent, src/lower/lower.ts) has nowhere to put a note whose body sits inline in the middle of a paragraph -- a reference site lowers onto a run-level point anchor extent (src/lower/inline.ts's own footnoteReference case), but that extent marks a site; it does not carry a body.
