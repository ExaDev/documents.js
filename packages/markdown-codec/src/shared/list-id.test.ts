...
describe("mintListNumId / parseListNumId", () => {
  it("mints a monotonically increasing, never-reused id per call", () => {
    ...
    expect(
      mintListNumId(state, { type: "bullet", task: false, loose: false }),
    ).toBe("md1:bullet");
    expect(
      mintListNumId(state, { type: "bullet", task: false, loose: false }),
    ).toBe("md2:bullet");
  ...
  it("appends +task and +loose suffixes independently", () => {
    const state = createNumIdMintState();
    expect(
      mintListNumId(state, { type: "bullet", task: true, loose: false }),
    ).toBe("md1:bullet+task");
    expect(
      mintListNumId(state, { type: "bullet", task: false, loose: true }),
    ).toBe("md2:bullet+loose");
    expect(
      mintListNumId(state, {
        type: "ordered",
        start: 5,
    ...
    ).toBe("md3:ordered@5+task+loose");
  });

  it("parses every minted shape back losslessly", () => {
    expect(parseListNumId("md1:bullet")).toEqual({
      type: "bullet",
      start: undefined,
      task: false,
      loose: false,
    });
    expect(parseListNumId("md2:ordered@3")).toEqual({
      type: "ordered",
      start: 3,
      task: false,
      loose: false,
    });
    expect(parseListNumId("md3:bullet+task")).toEqual({
      type: "bullet",
      start: undefined,
      task: true,
      loose: false,
    });
    expect(parseListNumId("md3:ordered@5+task+loose")).toEqual({
      type: "ordered",
      start: 5,
      task: true,
      loose: true,
