...
export type MarkdownImageResolver = (
  destination: string,
  context: MarkdownImageResolveContext,
) => MarkdownResolvedImageBytes | undefined;
...
): ResolvedMarkdownImage | undefined {
  const bytes =
    decodeDataUriImage(destination) ?? resolver?.(destination, context)?.bytes;
  if (bytes === undefined) {
    return undefined;
  }
