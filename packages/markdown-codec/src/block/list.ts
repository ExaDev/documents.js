...
function matchMarker(
  ...
  return {
    ...
    data: { type: "ordered", delimiter, start, markerOffset: indent },
  };
}
...
export function parseListMarker(
  ...
  if (
    containerIsParagraph &&
    !NON_SPACE_PATTERN.test(rest.slice(match.length))
  ) {
    return undefined;
  }
...
function endsWithBlankLine(block: BlockNode): boolean {
  ...
  while (current !== undefined) {
    ...
    if (
      !current.lastLineChecked &&
      (current.kind === "list" || current.kind === "listItem")
    ) {
