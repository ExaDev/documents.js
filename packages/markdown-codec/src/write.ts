...
//
// writeMarkdown checks the package's own `kind` itself, ahead of flattening, rather than letting a non-'wordprocessing' package reach emitMarkdown's own check by way of flattenTree preserving `kind`: flattenTree has kind-specific validation of its own (a 'formula' package's single-ContentFormula-node constraint, a 'spreadsheet' sheet group's own "no style ref" constraint) that can throw a bare Error before ever reaching that check, for a package this function was never going to accept anyway. Checking first means every non-'wordprocessing' package reaches the identical, correctly-typed MarkdownUnsupportedDocumentKindError, regardless of what else is wrong with it.
//
// flattenTree's own envelope (document-schema.js's dist/flatten.js) carries forward only `metadata` and, when present, `symbolTable` -- a DocumentTree's `definitions`/`layers`/`attachments`/`destinations`/`pages` tables have no flat-ContentDocument home to land in and are silently absent from its return value. This matters here specifically because the tree form exists for interop with packages ooxml.js/odf.js/documents.js produce, which DO populate those tables (a docx footnote/comment table, page geometry) -- reportDroppedPackageTables below turns that structural gap into a MarkdownDiagnosticCodes.PACKAGE_TABLE_DROPPED diagnostic per non-empty table, matching this package's own "every mapping gap reports through the sink as a stable code" contract, rather than leaving it silent the way a bare pass-through to flattenTree would. The one table with its own markdown spelling is exempt from that report: this package's own link-tenant entries in `definitions` (src/read.ts's splice) render as `[label]: destination "title"` lines ahead of flattening instead of being dropped, so only a foreign-tenant definitions table reports.
//
