...
export const MarkdownDiagnosticCodes = {
  ...
  HEADING_LINE_BREAK_UNSAFE_FOR_SETEXT:
    "md/heading-line-break-unsafe-for-setext",
  HEADING_STYLE_OVERRIDDEN_FOR_LINE_BREAK:
    "md/heading-style-overridden-for-line-break",
...
// Thrown by writeMarkdown/writeMarkdownContent when a ContentTable breaks the grid rule (ContentTableCell in document-schema.js): rows of differing lengths, content or a span on a position a merged region covers, or a region running past the grid or into another. The throw tier rather than a degrade, for the same reason as MarkdownUnbalancedConstructMarkersError: a pipe row or an HTML <tr> states anchors only, so a covered position's content has no cell to go in, and any repair would be a guess at which of two regions the producer meant. Detected via document-schema.js's own findTableGridFault, the one shared definition of the rule, and carried whole so a caller can locate the fault without re-running the check.
export class MarkdownTableGridFaultError extends MarkdownWriteError {
