...
// The index, in text's own line split, of the first line that is NOT blank -- or -1 when every line is (including the single-line, wholly-blank case). Shared by embedsUnsafeBreakForSetext (is any line from that point on still blank?) and (a later change) renderParagraphBody's own setext-promotion diagnostic.
