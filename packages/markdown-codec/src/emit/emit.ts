...
function embedsUnsafeBreakForSetext(text: string): boolean {
  // Index 0 is the run's own leading line, exempt for the reason the doc comment above explains; every OTHER line -- trailing or interior -- is checked against CommonMark's own blank-line definition rather than a literal-empty-string check.
  return text
    .split("\n")
    .slice(1)
