...
// Whether `line` -- a line of a would-be setext heading's own text AFTER its first -- would itself be read as one of the six paragraph-interrupting constructs CommonMark's setext grammar names (spec 0.31.2, "Setext headings", the clause immediately quoted on unsafeSetextBreakReason above), rather than being lazily absorbed as more of the paragraph the underline is about to convert. Reuses the read side's own matchers rather than re-deriving six grammars locally -- src/block/block.ts's ATX_MARKER_PATTERN/CODE_FENCE_PATTERN/THEMATIC_BREAK_PATTERN, src/block/list.ts's parseListMarker, src/html/html.ts's matchHtmlBlockStart -- against a throwaway LineCursor for this one line, so the write side's promotion refusal and the read side's actual reparse can never drift apart. Deliberately excludes indented code (4+ columns) and HTML start condition 7 (a lone open/closing tag on its own line): CommonMark's own paragraph-interruption exceptions bar both from interrupting an open paragraph, so a line matching only one of those is correctly absorbed as continuation text rather than fracturing the heading -- confirmed directly against the reference implementation ("Foo\n    bar\n---" and "Foo\n<a>\n---" both still parse as one intact setext heading, indented/tag line included verbatim). `matchHtmlBlockStart`'s own `interruptsParagraph` parameter is exactly this distinction (it excludes only condition 7), which is why `true` is passed rather than checking every one of the seven start conditions unconditionally.
function nonFirstLineInterruptsSetextParagraph(line: string): boolean {
  ...
  if (
    ...
    matchHtmlBlockStart(rest, true) !== undefined
  ) {
    return true;
  }
...
function unsafeSetextBreakReason(text: string): UnsafeSetextBreakReason {
  ...
  for (const line of text.split(LINE_ENDING_PATTERN)) {
    ...
    if (nonFirstLineInterruptsSetextParagraph(line)) {
  ...
  const hazard =
    reason === "leading-indentation"
      ...
      : reason === "interrupting-line"
        ? `a line after the heading's own first would itself be interpretable as a code fence, ATX heading, block quote, thematic break, list item, or HTML block -- setext's own grammar (spec 0.31.2) requires every one of the heading's lines to instead read as ordinary paragraph text, so promoting would read that line back as the start of a fresh, unrelated block rather than more of the heading`
