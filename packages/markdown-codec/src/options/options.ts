...
export interface WriteMarkdownStyleOptions {
  // A level 1/2 heading whose own content embeds a hard or soft break is rendered as setext regardless of this setting (MarkdownDiagnosticCodes.HEADING_STYLE_OVERRIDDEN_FOR_LINE_BREAK when the EFFECTIVE style being overridden is 'atx' -- this option's own default just as much as an explicit caller choice) -- ATX is a single physical line with no way to hold an embedded newline, while setext's own grammar spans one or more lines, PROVIDED the break's own placement leaves setext's underline immediately after the heading's real text with no blank line in between; a break placed such that promotion would corrupt the reparse (most commonly a trailing break, from an ordinary Word heading ending in a manual line break or page break) collapses to a single space instead, exactly like the level 3-6 case, reporting MarkdownDiagnosticCodes.HEADING_LINE_BREAK_UNSAFE_FOR_SETEXT rather than promoting. A level 3-6 heading has no setext fallback at all and always collapses the break to a single space (MarkdownDiagnosticCodes.HEADING_LINE_BREAK_COLLAPSED). Requesting 'setext' explicitly can be downgraded back to ATX the same way even for a heading with NO embedded break at all, when the heading's own rendered text is unsafe on its own terms -- its first line opens with 4 or more columns of space/tab indentation (setext's own grammar caps that at 3), or the text is entirely blank with nothing for the underline to attach to -- again reporting MarkdownDiagnosticCodes.HEADING_LINE_BREAK_UNSAFE_FOR_SETEXT rather than silently honouring the request into a corrupt reparse.
