...
function isCommonMarkExampleArray(
  value: unknown,
): value is CommonMarkExample[] {
...
function isHtmlEntity(value: unknown): value is HtmlEntity {
  ...
  return (
    Array.isArray(value.codepoints) && typeof value.characters === "string"
  );
}

function isHtmlEntityTable(
  value: unknown,
): value is Record<string, HtmlEntity> {
