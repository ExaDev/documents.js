...
- **`md/heading-style-overridden-for-line-break`** — the setext promotion above overriding an explicit `headingStyle: 'atx'`; the caller's configured style was not honoured for that one heading because ATX cannot hold the embedded break at all.
