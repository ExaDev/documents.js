...
// CONTENT_DEFS's own MathMlAttribute/MathMlElement/MathMlNode entries are generated (content-json-schema-defs.ts's own MATHML_JSON_DEFS), not hand-authored, so the comparison below is a generation-determinism check for these three ids, not the independent drift check every other id gets -- see this file's own top comment for the full reasoning and the empirical proof.
