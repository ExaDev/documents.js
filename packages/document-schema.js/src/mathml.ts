...
//
// One narrow exception, introduced by ExaDev/documents.js#937's z.lazy() rewrite below: before that change, MathMlNodeSchema was a z.custom() predicate wrapping isMathMlNode, and z.custom() always returns its input verbatim on a successful parse -- an object carrying an extra, unrecognised key survived .parse() untouched. MathMlNodeSchema is now a real z.discriminatedUnion() of plain z.object() schemas, which strip (rather than reject or preserve) unrecognised keys by default, so the identical input now has that key silently dropped by .parse(). This is a real, if narrow, behavioural change, and it is fine specifically here, not by the general "content must never silently drop" rule above: odf.js's own XmlElement/XmlText/etc. mirror (src/model/node.ts) that this shape is transcribed from carries exactly these fields and no others, so no producer in this family has ever populated an extra key to begin with, and the published JSON Schema fragment for these three types (content-json-schema-defs.ts's MathMlAttribute/MathMlElement/MathMlNode, now generated rather than hand-authored) already declared `additionalProperties: false` even under the old z.custom() encoding -- a schema-conformant document could never legitimately carry an extra key on a MathML node in the first place.
