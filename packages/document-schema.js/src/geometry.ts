...
// A single positioned placement of a content node on one rendered page -- PDF user-space points (origin bottom-left, y increasing upward), plus the page it belongs to. pageIndex is 0-based, matching DocumentTreeSchema's own `pages` array index (src/package.ts): `pages[frame.pageIndex]` names the page a given frame renders onto and that page's own dimensions. A content node carries an ARRAY of these (see FusedNode in src/content.ts), not a single optional one, because pagination or line-wrapping can render one semantic node -- a paragraph whose runs wrap across a page boundary is the common case -- into more than one place without splitting or duplicating the node itself. This is the fusion primitive that replaced DocumentTree's original approach of correlating a wholly separate layout tree's own positioned items back to their originating content node purely by matching sourcePath strings.
