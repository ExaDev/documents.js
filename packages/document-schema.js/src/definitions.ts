...
// The styles tenant of the definitions facility: string id -> resolved entry. Ids are minted by the producer's factoring pass (s1, s2, ... in deterministic order -- minting determinism is documents.js's law iii), and a tree node's `style` ref names a key in exactly this record.
