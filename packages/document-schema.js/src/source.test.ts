...
describe("SourceResidueSchema", () => {
  it("validates a residue value as exactly { format, xml }", () => {
    expect(
      SourceResidueSchema.safeParse({ format: "docx", xml: "<w:docPartObj/>" })
        .success,
    ).toBe(true);
  });
  ...
  it("requires both halves", () => {
    expect(SourceResidueSchema.safeParse({ format: "docx" }).success).toBe(
      false,
    );
    expect(SourceResidueSchema.safeParse({ xml: "<w:x/>" }).success).toBe(
      false,
    );
  });
  ...
  it("treats the xml as opaque text -- any string validates, and only a string does", () => {
    expect(
      SourceResidueSchema.safeParse({
        format: "markdown",
        xml: '<div class="raw html">&amp;</div>',
      }).success,
    ).toBe(true);
    expect(
      SourceResidueSchema.safeParse({ format: "markdown", xml: "" }).success,
    ).toBe(true);
    expect(
      SourceResidueSchema.safeParse({ format: "pdf", xml: 42 }).success,
    ).toBe(false);
  });
});
