...
import type { SectionConstructGroupNode, SectionGroupNode, ShapeConstructGroupNode, ShapeGroupNode, SlideGroupNode } from './package-node';
...
function refsOf(pkg: DocumentTree): { readonly ref: string; readonly nodeKind: string }[] {
...
describe('factorStyles minting', () => {
  ...
  it('carries a package\'s definitions table through re-factoring untouched', () => {
    ...
    const withDefinitions: DocumentTree = { ...minted, definitions: { tenantNote: { kind: 'tenant-note' } } };
  ...
  it('carries a package\'s source residue table through re-factoring untouched, and never factors residue into a styles entry', () => {
    ...
    const withResidue: DocumentTree = { ...minted, source: { 'word/settings.xml': { format: 'docx', xml: '<w:settings/>' } } };
  ...
  it('factors a paragraph tuple nested inside a shape-flow construct group onto the construct group\'s own ref (document-schema.js 4.1.0)', () => {
    ...
    const pkg: DocumentTree = { kind: 'presentation', metadata: {}, children: [slideGroup] };
    const minted = mint(pkg);
...
function findGroupByText(pkg: DocumentTree, text: string): { node: unknown; style?: string } | undefined {
