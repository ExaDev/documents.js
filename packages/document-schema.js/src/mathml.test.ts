...
import { type MathMlNode, MathMlElementSchema, MathMlNodeSchema, isMathMlNode } from './mathml';
...
const everyVariant: MathMlNode[] = [
  ...
  { type: 'pi', target: 'xml-stylesheet', content: 'href="a.xsl"' },
  {
    ...
    children: [{ type: 'element', tag: 'mi', attributes: [], children: [{ type: 'text', value: 'x' }] }],
  },
];
...
describe('isMathMlNode', () => {
        ...
        children: [
          {
            ...
            tag: 'mfrac',
            attributes: [],
            children: [{ type: 'element', tag: 'msqrt', attributes: [], children: [{ type: 'text', value: 2 }] }],
          },
        ],
      }),
    ).toBe(false);
  ...
  it('rejects a node whose payload is missing or wrongly typed for its own variant', () => {
    ...
    expect(isMathMlNode({ type: 'declaration', attributes: [{ name: 'version' }] })).toBe(false);
    expect(isMathMlNode({ type: 'element', tag: 'mi', attributes: [] })).toBe(false);
...
describe("structural compatibility with odf.js's own XmlNode", () => {
  it('accepts an odf.js-shaped node tree as a MathMlNode with no cast, and vice versa', () => {
    const fromOdf: OdfXmlNode = {
      ...
      children: [
        { type: 'element', tag: 'mi', attributes: [], children: [{ type: 'text', value: 'x' }] },
