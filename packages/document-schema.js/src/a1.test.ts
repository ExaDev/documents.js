...
describe("parseRangeReference / rangeReference", () => {
  it("parses a genuine two-cell range, normalising start/end regardless of corner order", () => {
    expect(parseRangeReference("A1:B2")).toEqual({
      startRow: 0,
      startColumn: 0,
      endRow: 1,
      endColumn: 1,
    });
    expect(parseRangeReference("B2:A1")).toEqual({
      startRow: 0,
      startColumn: 0,
      endRow: 1,
      endColumn: 1,
  ...
  it("treats a single cell reference (no colon) as a one-cell range", () => {
    expect(parseRangeReference("C3")).toEqual({
      startRow: 2,
      startColumn: 2,
      endRow: 2,
      endColumn: 2,
  ...
  it("round-trips through rangeReference", () => {
    expect(
      rangeReference({ startRow: 0, startColumn: 0, endRow: 19, endColumn: 8 }),
    ).toBe("A1:I20");
  });
});
