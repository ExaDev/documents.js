...
describe("ExactRationalSchema", () => {
  it("accepts canonical integer-string halves, including the integer-as-over-one and exact-negative cases", () => {
    expect(
      ExactRationalSchema.safeParse({ numerator: "0", denominator: "1" })
        .success,
    ).toBe(true);
    expect(
      ExactRationalSchema.safeParse({ numerator: "-381", denominator: "1250" })
        .success,
    ).toBe(true);
    expect(
      ExactRationalSchema.safeParse({
        numerator: "196133",
        denominator: "20000",
      }).success,
    ).toBe(true);
  ...
  it("rejects every non-canonical spelling: float text, leading zeros, negative zero, and zero or negative denominators", () => {
    for (const numerator of ["1.5", "007", "-0", "1e3", ""]) {
      expect(
        ExactRationalSchema.safeParse({ numerator, denominator: "2" }).success,
      ).toBe(false);
    }
    for (const denominator of ["0", "-2", "00", "2.5", ""]) {
      expect(
        ExactRationalSchema.safeParse({ numerator: "1", denominator }).success,
      ).toBe(false);
    }
  });
});
...
describe("DimensionVectorSchema", () => {
  it("accepts a sparse vector over any subset of the SI bases, exponents negative included", () => {
    ...
    expect(
      DimensionVectorSchema.safeParse({ length: 1, time: -1 }).success,
    ).toBe(true);
    expect(
      DimensionVectorSchema.safeParse({ length: 1, mass: 1, time: -2 }).success,
    ).toBe(true);
  ...
  it("rejects dimensions outside the seven SI bases and non-integer exponents", () => {
    expect(DimensionVectorSchema.safeParse({ information: 1 }).success).toBe(
      false,
    );
    expect(DimensionVectorSchema.safeParse({ length: 1.5 }).success).toBe(
      false,
    );
    expect(DimensionVectorSchema.safeParse({ length: "1" }).success).toBe(
      false,
    );
  });
});
...
describe("SymbolTableSchema", () => {
      ...
      contexts: [
        {
          ...
          bases: [
            ...
            {
              unit: "si:volt",
              value: { numerator: "11000", denominator: "1" },
            },
          ],
        },
      ],
  ...
  it("requires symbols and units outright, with contexts the only optional array", () => {
    expect(
      SymbolTableSchema.safeParse({ symbols: [], units: [] }).success,
    ).toBe(true);
  ...
  it("rejects a symbol entry missing the (glyph, scope, id) key it is looked up by", () => {
    expect(
      MathSymbolEntrySchema.safeParse({ glyph: "U", id: "symbols:voltage" })
        .success,
    ).toBe(false);
    expect(
      MathSymbolEntrySchema.safeParse({
        glyph: "U",
        scope: "document",
        id: "symbols:voltage",
      }).success,
...
describe("MathPresentationSchema and MathProvenanceSchema", () => {
  ...
  it("requires the provenance source and edit trail, with the page reference optional", () => {
    expect(
      MathProvenanceSchema.safeParse({
        source: "odf:content.xml#Object1",
        editTrail: [],
      }).success,
    ).toBe(true);
    expect(
      MathProvenanceSchema.safeParse({
        ...
        pageRef: "page 4",
        editTrail: ["value corrected against the ledger"],
      }).success,
    ).toBe(true);
    expect(
      MathProvenanceSchema.safeParse({ source: "odf:content.xml#Object1" })
        .success,
    ).toBe(false);
  });
});
...
describe("MathUncertaintySchema", () => {
  it("accepts an exact magnitude with optional unit override and coverage factor", () => {
    expect(
      MathUncertaintySchema.safeParse({
        magnitude: { numerator: "1", denominator: "4" },
      }).success,
    ).toBe(true);
    expect(
      MathUncertaintySchema.safeParse({
  ...
  it("rejects a non-positive coverage factor and a float-text magnitude", () => {
    ...
    expect(
      MathUncertaintySchema.safeParse({
        magnitude: { numerator: "0.25", denominator: "1" },
      }).success,
    ).toBe(false);
  });
});
...
describe("the MathExpression grammar", () => {
  it("validates each non-recursive variant through its own named schema", () => {
    ...
    expect(
      MathSymSchema.safeParse({ kind: "sym", id: "symbols:voltage" }).success,
    ).toBe(true);
    expect(
      MathUnparsedSchema.safeParse({
        kind: "unparsed",
        latex: "\\oint_C \\mathbf{B} \\cdot d\\mathbf{l}",
      }).success,
    ).toBe(true);
  });
...
describe("QuantitySchema", () => {
  ...
  it("rejects a non-numeric magnitude, a missing dimension, and a non-quantity discriminant", () => {
    ...
    expect(
      QuantitySchema.safeParse({ kind: "quantity", magnitude: 1 }).success,
    ).toBe(false);
    expect(
      QuantitySchema.safeParse({
        kind: "nonsense",
...
describe("FormulaBindingsSchema", () => {
  ...
  it("rejects a value that is neither a Quantity nor an Interval", () => {
    expect(
      FormulaBindingsSchema.safeParse({ m: { kind: "nonsense", magnitude: 1 } })
        .success,
    ).toBe(false);
  });
});
