...
function entryOf(
  ...
  if (styles === undefined) {
    throw new Error(
      "flattenTree: a group carries a style ref but the package has no styles table",
    );
  }
...
export function flattenTree(pkg: DocumentTree): ContentDocument {
  ...
  switch (pkg.kind) {
    ...
    case "presentation":
      return {
        ...
        slides: pkg.children.map((group): ContentSlide => {
          ...
          return {
            ...untag(group.node),
            shapes: group.children.map((shape) =>
              flattenShape(styles, chain, shape),
            ),
          };
        }),
      };
    ...
    case "formula": {
      ...
      if (
        pkg.children.length !== 1 ||
        first === undefined ||
        !ContentFormulaSchema.safeParse(first).success
      ) {
        throw new Error(
          "flattenTree: a formula package takes exactly one ContentFormula node",
        );
      }
...
function flattenSectionChildren(
  ...
  for (const child of children) {
    if (isHeadingGroup(child)) {
      ...
      blocks.push(
        resolveAnchor(styles, own, child.node),
        ...flattenSectionChildren(styles, own, child.children),
      );
    } else if (isListGroup(child)) {
      const own = chainWithRef(chain, child);
      blocks.push(
        resolveAnchor(styles, own, child.node),
        ...flattenListChildren(styles, own, child.children),
      );
    } else if (isConstructGroup(child)) {
      const own = chainWithRef(chain, child);
      blocks.push(
        { kind: "constructStart", descriptor: child.node },
        ...flattenSectionChildren(styles, own, child.children),
        { kind: "constructEnd" },
      );
...
function isHeadingGroup(child: SectionChild): child is HeadingGroupNode {
  return (
    ...
    child.node.headingLevel !== undefined
  );
}
...
function resolveAnchor(
  styles: StylesTable | undefined,
  chain: readonly string[],
  anchor: HeadingGroupNode["node"] | ListGroupNode["node"],
): ContentParagraph {
  const entry = entryOf(styles, chain);
