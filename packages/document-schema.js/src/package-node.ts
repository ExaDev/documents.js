...
export const SectionDescriptorSchema = ContentSectionSchema.omit({
  blocks: true,
})
...
export const SheetDescriptorSchema = ContentSheetSchema.omit({
  images: true,
...
export const DrawPageDescriptorSchema = ContentDrawPageSchema.omit({
  shapes: true,
  vectors: true,
})
...
export const ShapeDescriptorSchema = ContentShapeSchema.omit({
  blocks: true,
}).strict();
...
//
// This exclusion is also a type-level break beyond the exhaustive-switch case ContentBlock's own union addition already causes on its own (src/content.ts): SectionChild, ShapeChild, ListChild, and TreeLeaf below all resolve their block-leaf member to TreeBlockLeaf rather than ContentBlock, so a value already typed ContentBlock (or ContentBlock[]) no longer assigns into any of them -- e.g. `const children: SectionChild[] = blocks` where blocks: ContentBlock[] stops compiling on upgrade. The narrowing is required correctness, not a side effect to work around: an assignment that fails here is exactly the marker-at-a-leaf case the exclusion exists to catch at compile time, so the fix on the calling side is to prove the array holds no marker (narrow to TreeBlockLeaf[], or accept it directly from wherever it was produced), never to cast past it.
//
...
export type TreeBlockLeaf = Exclude<
  ContentBlock,
...
export type SectionChild =
  HeadingGroupNode | ListGroupNode | SectionConstructGroupNode | TreeBlockLeaf;
...
export type ShapeChild =
  ListGroupNode | ShapeConstructGroupNode | TreeBlockLeaf;
...
export type ListChild = ListGroupNode | ShapeConstructGroupNode | TreeBlockLeaf;
...
function isGroupWrapper(
  ...
  if (
    !Object.keys(value).every(
      (key) => key === "node" || key === "style" || key === "children",
    )
  )
    return false;
...
// The block-leaf guard behind TreeBlockLeaf above, and the reason that exclusion is enforced at runtime rather than left to the types: these predicates are the untrusted-input boundary, so a marker smuggled into a tree by a producer that flattened one encoding into the other has to fail here, where documentFromJson can report it, rather than parse and quietly leave a package no round trip can reproduce.
export function isTreeBlockLeaf(value: unknown): value is TreeBlockLeaf {
  if (isContentConstructStart(value) || isContentConstructEnd(value))
    return false;
  return isContentBlock(value);
}

export const TreeBlockLeafSchema = z.custom<TreeBlockLeaf>(isTreeBlockLeaf);
...
function isSectionChild(value: unknown): value is SectionChild {
  return (
    ...
    isLeafChild(TreeBlockLeafSchema, value)
  );
}
...
function isSheetChild(value: unknown): value is SheetChild {
  return (
    isLeafChild(ContentSheetImageSchema, value) ||
    isLeafChild(ContentEmbeddedObjectSchema, value)
  );
}
...
export function isSlideGroupNode(value: unknown): value is SlideGroupNode {
  return (
    isRecord(value) &&
    SlideDescriptorSchema.safeParse(value.node).success &&
    isGroupWrapper(value, isShapeGroupNode)
  );
}
...
export function isSheetGroupNode(value: unknown): value is SheetGroupNode {
  return (
    isRecord(value) &&
    SheetDescriptorSchema.safeParse(value.node).success &&
    isGroupWrapper(value, isSheetChild)
  );
}

export function isDrawPageGroupNode(
  value: unknown,
): value is DrawPageGroupNode {
  return (
    isRecord(value) &&
...
export function isShapeGroupNode(value: unknown): value is ShapeGroupNode {
  return (
    isRecord(value) &&
    ShapeDescriptorSchema.safeParse(value.node).success &&
    isGroupWrapper(value, isShapeChild)
  );
}
...
export function isHeadingGroupNode(value: unknown): value is HeadingGroupNode {
  return (
    isRecord(value) &&
    HeadingParagraphSchema.safeParse(value.node).success &&
    isGroupWrapper(value, isSectionChild)
  );
}
...
export function isListGroupNode(value: unknown): value is ListGroupNode {
  return (
    isRecord(value) &&
    ListParagraphSchema.safeParse(value.node).success &&
    isGroupWrapper(value, isListChild)
  );
}
...
export function isSectionConstructGroupNode(
  value: unknown,
): value is SectionConstructGroupNode {
  return (
    isRecord(value) &&
    ConstructDescriptorSchema.safeParse(value.node).success &&
    isGroupWrapper(value, isSectionChild)
  );
}

export function isShapeConstructGroupNode(
  value: unknown,
): value is ShapeConstructGroupNode {
  return (
    isRecord(value) &&
    ConstructDescriptorSchema.safeParse(value.node).success &&
    isGroupWrapper(value, isShapeChild)
...
export function isTreeLeaf(value: unknown): value is TreeLeaf {
  return packageLeafUnion.safeParse(value).success;
}

export function isTreeGroup(value: unknown): value is TreeGroup {
  return (
...
export const SectionGroupSchema =
  z.custom<SectionGroupNode>(isSectionGroupNode);
...
export const DrawPageGroupSchema =
  z.custom<DrawPageGroupNode>(isDrawPageGroupNode);
export const ShapeGroupSchema = z.custom<ShapeGroupNode>(isShapeGroupNode);
export const HeadingGroupSchema =
  z.custom<HeadingGroupNode>(isHeadingGroupNode);
export const ListGroupSchema = z.custom<ListGroupNode>(isListGroupNode);
export const SectionConstructGroupSchema = z.custom<SectionConstructGroupNode>(
  isSectionConstructGroupNode,
);
export const ShapeConstructGroupSchema = z.custom<ShapeConstructGroupNode>(
  isShapeConstructGroupNode,
);
export const TreeGroupSchema = z.custom<TreeGroup>(isTreeGroup);
export const TreeLeafSchema = z.custom<TreeLeaf>(isTreeLeaf);
export const TreeNodeSchema = z.custom<TreeNode>(isTreeNode);
