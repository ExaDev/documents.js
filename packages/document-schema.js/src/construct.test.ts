...
describe("field carries an instruction, with the cached scalar result beside it", () => {
  it("accepts a docx complex field and a pptx a:fld (ooxml.js#65: w:instrText, a:fld slidenum)", () => {
    expect(
      FieldDescriptorSchema.safeParse({
        kind: "field",
  ...
  it("rejects a field with no instruction, and one carrying an invented harmonised type", () => {
    expect(
      FieldDescriptorSchema.safeParse({ kind: "field", cachedResult: "12" })
        .success,
    ).toBe(false);
    expect(
      FieldDescriptorSchema.safeParse({
...
describe("anchor names an extent or a reference site, and points at its definition when it has one", () => {
  it("accepts a docx bookmark and an ODF text:bookmark (ooxml.js#65, odf.js#59)", () => {
    expect(
      AnchorDescriptorSchema.safeParse({
        kind: "anchor",
        anchorType: "bookmark",
        name: "_Toc12345",
      }).success,
...
describe("link carries the target vocabulary a flat run field cannot express", () => {
  ...
  it("accepts an internal target -- docx @w:anchor, a pptx slide jump, a PDF GoTo//Dest (the one new vocabulary #24 asks for)", () => {
    expect(
      LinkDescriptorSchema.safeParse({
        kind: "link",
        target: { kind: "internal", anchor: "_Toc12345" },
      }).success,
    ).toBe(true);
  });
...
describe("division is the ODF text:section shape, decided first-class rather than degraded", () => {
  ...
  it("rejects a fractional or zero column count, and a linked link with no href", () => {
    expect(
      DivisionDescriptorSchema.safeParse({ kind: "division", columnCount: 1.5 })
        .success,
    ).toBe(false);
    expect(
      DivisionDescriptorSchema.safeParse({ kind: "division", columnCount: 0 })
        .success,
    ).toBe(false);
    expect(
      DivisionDescriptorSchema.safeParse({
        kind: "division",
        linked: { sectionName: "Body" },
      }).success,
    ).toBe(false);
  });
  ...
  it("rejects text:filter-name smuggled onto the linked DivisionSource, a DivisionSource smuggled onto source, and a bad-format residue on source", () => {
    ...
    expect(
      DivisionDescriptorSchema.safeParse({
        kind: "division",
        linked: { href: "../one.odt", filterName: "writer8" },
      }).success,
    ).toBe(false);
    expect(
      DivisionDescriptorSchema.safeParse({
        kind: "division",
        source: { href: "../one.odt", sectionName: "Body" },
      }).success,
    ).toBe(false);
    expect(
      DivisionDescriptorSchema.safeParse({
        ...
        source: {
          format: "not-a-format",
          xml: "<text:filter-name>writer8</text:filter-name>",
        },
      }).success,
    ).toBe(false);
  });
...
describe("the construct descriptor union", () => {
  it("admits exactly the six kinds and nothing else", () => {
    ...
    for (const descriptor of descriptors) {
      expect(ConstructDescriptorSchema.safeParse(descriptor).success).toBe(
        true,
      );
    }
    expect(
