...
//
// BALANCE IS NECESSARY BUT NOT SUFFICIENT: a marker's extent can still cross a heading-group or list-group scope boundary. Headings and list items carry no delimiter of their own in the flat form -- a heading paragraph's scope runs until the next paragraph in the same block list whose headingLevel is shallower than or equal to its own, and a list item's scope runs until nesting shallows back out, exactly the nesting decompose infers when it builds HeadingGroupNode/ListGroupNode (src/package-node.ts) from a flat list. A constructStart opened while a list scope is open is resolved deterministically (ExaDev/document-schema.js#1022): decompose closes the list scope first, exactly as a plain paragraph would, and hoists the construct group out to the enclosing heading scope -- so a construct at the tail of a list, with nothing after it that would have continued that list, no longer strands as a phantom child of the last item it happens to follow, and a list genuinely still open past the construct's own extent simply reopens its own nesting once the region closes. Heading scope has no equivalent resolution: a constructStart/constructEnd pair whose extent contains a paragraph that would close a heading scope already open when the pair started still leaves decompose no single correct tree to build, and findConstructMarkerImbalance below cannot see it either way -- balance is a property of the marker pair alone, not of what sits between the two markers, and detecting a heading-scope crossing means walking the same heading nesting decompose already builds while constructing the tree, so decompose (src/decompose.ts) remains the only place that could enforce it and does not yet. A producer emitting a marker pair (ooxml.js, odf.js, markdown-codec, pdf-codec) must never open one inside a heading scope that some other block inside the extent then closes.
//
