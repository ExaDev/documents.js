...
function documentPackage(): DocumentTree {
  ...
  return {
    kind: "wordprocessing",
    metadata: { title: "schema-io fixture" },
    children: [section],
  };
}
...
describe("schemaUriFor", () => {
  it("builds a release-pinned jsdelivr URL, one per kind", () => {
    expect(schemaUriFor("DocumentTree")).toBe(
      uriForVersion(packageVersion, "document-tree"),
    );
    expect(schemaUriFor("ContentDocument")).toBe(
      uriForVersion(packageVersion, "content-document"),
    );
  });
});
...
describe("documentSchemaKindOf", () => {
  it("recognizes both live kinds", () => {
    expect(
      documentSchemaKindOf(documentTreeWithSchema(documentPackage())),
    ).toBe("DocumentTree");
    expect(
      documentSchemaKindOf(contentDocumentWithSchema(wordprocessingDocument())),
    ).toBe("ContentDocument");
  });
...
describe("documentFromJson dispatches on the $schema URI", () => {
  it("round-trips each live kind stamped with the installed release URI", () => {
    const pkg = documentPackage();
    expect(documentFromJson(documentTreeWithSchema(pkg))).toEqual({
      kind: "DocumentTree",
      value: pkg,
    });

    const content = wordprocessingDocument();
    expect(documentFromJson(contentDocumentWithSchema(content))).toEqual({
  ...
  it("accepts a URI from another release of the SAME major -- patch and minor releases validate a major's dumps", () => {
    ...
    const tagged = {
      ...
      $schema: uriForVersion(`${installedMajor}.9.9`, "document-tree"),
    };
    expect(documentFromJson(tagged)).toEqual({
      kind: "DocumentTree",
      value: pkg,
    });
  });
  ...
  it("throws UnrecognizedDocumentSchemaError, carrying the offending value, for unrecognized input", () => {
    ...
    ).toThrow(UnrecognizedDocumentSchemaError);
    try {
      documentFromJson({
        $schema: "https://example.com/not-a-real-schema.json",
      });
    ...
    expect(() => documentFromJson(null)).toThrow(
      UnrecognizedDocumentSchemaError,
    );
  });
