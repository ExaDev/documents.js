...
export function factorStyles(pkg: DocumentTree): DocumentTree {
  ...
  const carried = {
    ...(pkg.definitions !== undefined ? { definitions: pkg.definitions } : {}),
    ...(pkg.source !== undefined ? { source: pkg.source } : {}),
  };
  if (pkg.definitions === undefined && pkg.source === undefined)
    return reassembled;
...
): readonly ParagraphKey[] {
  return PARAGRAPH_STYLE_KEYS.filter(
    (key) =>
      !frozen.has(key) &&
      extent.every((paragraph) => paragraph[key] !== undefined),
  );
}

function commonRunKeys(
  extent: readonly ContentParagraph[],
...
function bestParagraphCandidate(
  extent: readonly ContentParagraph[],
  keys: readonly ParagraphKey[],
  factored: ReadonlySet<ContentParagraph>,
  ...
  for (const paragraph of extent) {
    ...
    if (existing === undefined)
      groups.set(key, { tuple, keys, positions: [paragraph] });
    else existing.positions.push(paragraph);
  }
  return bestGroup(groups);
}

function bestRunCandidate(
  extent: readonly ContentParagraph[],
  keys: readonly RunKey[],
  factored: ReadonlySet<ContentRun>,
  ...
  for (const paragraph of extent) {
    for (const run of paragraph.runs) {
      ...
      const existing = groups.get(key);
      if (existing === undefined)
        groups.set(key, { tuple, keys, positions: [run] });
...
function bestGroup<
  T extends { readonly tuple: unknown; readonly positions: unknown[] },
>(groups: Map<string, T>): T | undefined {
  let best: T | undefined;
  for (const group of groups.values()) {
    if (group.positions.length < 2) continue;
    if (best === undefined || group.positions.length > best.positions.length)
      best = group;
  }
  return best;
}
...
function plan(
  ...
  if (paragraphCandidate !== undefined || runCandidate !== undefined) {
    const content: StyleEntry = {
      ...(paragraphCandidate !== undefined
        ? { paragraph: paragraphCandidate.tuple }
        : {}),
    ...
    case "wordprocessing":
      return {
        ...
        children: pkg.children.map((group) =>
          rebuildSectionGroup(group, [], state),
        ),
      };
    case "presentation":
      return {
...
function paragraphStripsOf(
  chain: ChainStrips,
  paragraph: ContentParagraph,
): readonly ParagraphKey[] | undefined {
...
function runStripsOf(
  chain: ChainStrips,
  run: ContentRun,
): readonly RunKey[] | undefined {
...
function rebuildSlideGroup(
  ...
  const children = group.children.map((shape) =>
...
function rebuildParagraph(
  ...
  for (const run of base.runs) {
    ...
    const rebuilt =
      runStrips === undefined ? run : stripRunKeys(run, runStrips);
    changed ||= rebuilt !== run;
    runs.push(rebuilt);
  }
