...
**Landed for real in [ExaDev/documents.js#937](https://github.com/ExaDev/documents.js/issues/937).** `src/mathml.ts` now defines `MathMlNodeSchema` exactly this way, and `content-json-schema-defs.ts` computes its `$defs.MathMlAttribute`/`MathMlElement`/`MathMlNode` fragments from a small local registry (`z.toJSONSchema` with a `#/$defs/<id>` uri callback, the same cross-reference mechanism `content-json-schema-defs.test.ts`'s own live comparison already used) rather than transcribing them by hand — confirmed to reproduce the identical nested-`$ref` shape the hand-authored fragment used to carry. `ContentFormulaSchema` itself is unaffected: its `mathml` field now reaches a real schema, but its `content` field still reaches the opaque `MathExpressionSchema`, so the whole fragment stays hand-transcribed regardless. `ContentBlockSchema`/`ContentEmbeddedObjectSchema` are harder (mutual recursion across table/cell/row, plus the full `ContentDocument` cycle) and remain out of scope, unspiked.
