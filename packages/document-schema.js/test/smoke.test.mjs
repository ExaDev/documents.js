...
describe('smoke: generated JSON Schema files', () => {
  ...
  it("document-tree.schema.json is the tree form: five kind arms, no formatVersion or content field, per-kind children refs, and styles/definitions tables at the root", () => {
    ...
    for (const variant of documentTree.oneOf) {
      ...
      // The three construct tables reuse the generic definitions entry rather than minting parallel entry shapes.
      for (const table of ['layers', 'attachments', 'destinations']) {
        expect(variant.properties[table].additionalProperties.$ref).toBe('#/$defs/DefinitionEntry');
      }
    }
  ...
  it('document-tree.schema.json publishes the construct vocabulary: six descriptor kinds and the two group wrappers that carry them', () => {
    ...
    // Division carries both halves of #743: its external-chapter link on `linked` and its own residue on `source`, one name per fact.
    expect(documentTree.$defs.DivisionDescriptor.properties.linked).toEqual({ $ref: '#/$defs/DivisionSource' });
    expect(documentTree.$defs.DivisionDescriptor.properties.source).toEqual({ $ref: '#/$defs/SourceResidue' });
  });
