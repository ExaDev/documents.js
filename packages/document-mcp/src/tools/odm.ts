...
import { base64ToBytes, OdmUnresolvedSectionError, odmToPdf } from 'documents.js';
import { z } from 'zod';
...
const OdmMasterSourceSchema = z.union([
  z.object({ path: z.string().describe('Filesystem path to the .odm master document to convert.') }),
  z.object({ bytesBase64: z.string().describe('Base64-encoded .odm master document bytes.') }),
]);

async function resolveOdmMasterBytes(source: z.infer<typeof OdmMasterSourceSchema>): Promise<Uint8Array<ArrayBuffer>> {
  if ('path' in source) {
...
export function registerOdmTools(server: McpServer): void {
  server.registerTool(
    ...
    async ({ source, chapters, chaptersDir, output }) => {
      ...
      const resolveSubDocument = (href: string): Uint8Array<ArrayBuffer> | undefined => {
