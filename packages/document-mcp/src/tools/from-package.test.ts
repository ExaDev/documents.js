...
describe("from_package", () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(
      packagePath,
      JSON.stringify(documentTreeWithSchema(capturedPackage)),
    );
    ...
    await writeFile(
      sheetPackagePath,
      JSON.stringify(documentTreeWithSchema(capturedSheetPackage)),
    );
  });
  ...
  it("rebuilds a real pdf from the dumped DocumentTree, returned inline", async () => {
    const result = await pair.client.callTool({
      name: "from_package",
      arguments: { source: { path: packagePath }, targetFormat: "pdf" },
    });

    expect(result.isError).toBeFalsy();
    if (
      !isRecord(result.structuredContent) ||
      typeof result.structuredContent.bytesBase64 !== "string"
    ) {
  ...
  it("still rejects xlsx when the dumped package is the wrong ContentDocument kind", async () => {
    const result = await pair.client.callTool({
      name: "from_package",
      arguments: { source: { path: packagePath }, targetFormat: "xlsx" },
    });

    expect(result.isError).toBe(true);
    const [block] = result.content;
    expect(block?.type === "text" ? block.text : undefined).toContain(
      'expected a ContentDocument of kind "spreadsheet", got "wordprocessing"',
    );
  });
  ...
  it("rejects a source with no recognised $schema", async () => {
    ...
    const result = await pair.client.callTool({
      name: "from_package",
      arguments: { source: { path: plainPath }, targetFormat: "docx" },
    });

    expect(result.isError).toBe(true);
    const [block] = result.content;
    expect(block?.type === "text" ? block.text : undefined).toContain(
      "no recognised $schema",
    );
  });
  ...
  it("rejects a pre-schema-4 package dump (formatVersion 1, layout half) through the rename tombstone", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        $schema:
          "https://cdn.jsdelivr.net/npm/document-schema.js@2.7.17/schemas/document-package.schema.json",
        formatVersion: 1,
        content: {
          kind: "wordprocessing",
          formatVersion: 2,
    ...
    const result = await pair.client.callTool({
      name: "from_package",
      arguments: { source: { path: legacyPath }, targetFormat: "docx" },
    });
  ...
  it("also rejects the fused formatVersion 2 shape (schema 3.x flat content+pages) through the same rename tombstone", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        $schema:
          "https://cdn.jsdelivr.net/npm/document-schema.js@3.0.0/schemas/document-package.schema.json",
        formatVersion: 2,
        content: {
          kind: "wordprocessing",
          formatVersion: 2,
  ...
  it("rejects a layout-document dump with the demotion error naming where the schema moved", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        $schema:
          "https://cdn.jsdelivr.net/npm/document-schema.js@3.0.0/schemas/layout-document.schema.json",
        formatVersion: 1,
        pages: [],
      }),
    );
    ...
    const result = await pair.client.callTool({
      name: "from_package",
      arguments: { source: { path: layoutPath }, targetFormat: "docx" },
    });
  ...
  it("rejects source text that is not valid JSON", async () => {
    ...
    const result = await pair.client.callTool({
      name: "from_package",
      arguments: { source: { path: brokenPath }, targetFormat: "docx" },
    });
