...
import { odbReportNotSpecifiedResult, OdbRenderReportOutputSchema } from './odb-render-report';
...
async function connect(): Promise<ConnectedPair> {
  ...
  const client = new Client({ name: 'odb-render-report-test-client', version: '0.0.0' });
...
const EXPECTED_REPORT_STRINGS = ['Sales by region', 'North', 'South', 'Region total:', '4290.75', '1810', 'Grand total:', '6100.75'];
...
describe('odb_render_report', () => {
  ...
  beforeAll(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'document-mcp-odb-render-report-'));
  });
  ...
  it('renders the fixture\'s single report to docx via a filesystem path source, with no report name given (auto-selected)', async () => {
    const result = await pair.client.callTool({
      ...
      arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, targetFormat: 'docx' },
    });

    expect(result.isError).toBeFalsy();
    const structured = OdbRenderReportOutputSchema.parse(result.structuredContent);
  ...
  it('renders the fixture\'s single report to pdf via inline base64 bytes, with the report named explicitly, writing to outputPath', async () => {
    ...
    const result = await pair.client.callTool({
      name: 'odb_render_report',
      arguments: {
        source: { bytesBase64: bytesToBase64(loadFormAndReportOdbBytes()), format: 'pdf' },
  ...
  it('renders the fixture\'s single report to odt', async () => {
    const result = await pair.client.callTool({
      name: 'odb_render_report',
      arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, targetFormat: 'odt' },
    });
  ...
  it('returns an isError result naming the offending name when an unrecognised report is requested', async () => {
    const result = await pair.client.callTool({
      name: 'odb_render_report',
      arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, report: 'NoSuchReport', targetFormat: 'docx' },
    });

    expect(result.isError).toBe(true);
    const [block] = result.content;
    expect(block?.type).toBe('text');
    expect(block?.type === 'text' ? block.text : undefined).toContain('NoSuchReport');
    expect(block?.type === 'text' ? block.text : undefined).toContain(FORM_AND_REPORT_REPORT_NAME);
  });
});
...
describe('odbReportNotSpecifiedResult', () => {
  it('reports isError: true, the error message verbatim, and availableReports in structuredContent', () => {
    const error = new OdbReportNotSpecifiedError(['SalesByRegion', 'SalesByCustomer']);

    const result = odbReportNotSpecifiedResult(error);

    expect(result.isError).toBe(true);
    expect(result.content).toStrictEqual([{ type: 'text', text: error.message }]);
    expect(result.content[0]?.type === 'text' ? result.content[0].text : undefined).toContain('SalesByRegion');
    expect(result.content[0]?.type === 'text' ? result.content[0].text : undefined).toContain('SalesByCustomer');
    if (!isRecord(result.structuredContent)) {
      throw new Error('expected structuredContent to be a record');
    }
    expect(result.structuredContent.availableReports).toStrictEqual(['SalesByRegion', 'SalesByCustomer']);
  });
  ...
  it('names no available reports when the .odb declares none at all', () => {
    ...
    expect(result.isError).toBe(true);
    expect(result.content[0]?.type === 'text' ? result.content[0].text : undefined).toBe('readOdbReportContent: this .odb declares no reports at all');
    if (!isRecord(result.structuredContent)) {
      throw new Error('expected structuredContent to be a record');
    }
