...
describe('outline_document', () => {
  ...
  it('outlines a multi-page drawing, one group per page -- the odg regression: buildSvgText refuses more than one page, so the internal probe target routes odg through pdf, not svg', async () => {
    ...
    const result = await pair.client.callTool({ name: 'outline_document', arguments: { source: { bytesBase64: bytesToBase64(editor.toBytes()), format: 'odg' } } });
...
function buildFormatFixtures(): Record<Exclude<DocumentFormat, 'markdown' | 'ods' | 'odg'>, { readonly bytes: Uint8Array<ArrayBuffer>; readonly kind: string }> {
  const docxBytes = (() => {
  ...
  return {
    csv: { bytes: new TextEncoder().encode('Name,Age\nAlice,30\n'), kind: 'spreadsheet' },
...
describe('outline_document across every source format', () => {
  ...
  it('the fixture table plus the deep-asserted markdown/ods/odg cases above cover every DocumentFormat this port exposes', () => {
    expect(new Set([...Object.keys(fixtures), 'markdown', 'ods', 'odg'])).toEqual(new Set(DOCUMENT_FORMATS));
  });

  it.each(Object.entries(fixtures))('outlines a %s source without error, reporting the source\'s own content kind', async (format, fixture) => {
    const result = await pair.client.callTool({ name: 'outline_document', arguments: { source: { bytesBase64: bytesToBase64(fixture.bytes), format } } });

    expect(result.isError).toBeFalsy();
    expect(result.structuredContent).toMatchObject({ sourceFormat: format, kind: fixture.kind });
  });
});
