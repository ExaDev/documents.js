...
async function resolveOdbBytes(
...
export function registerOdbTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async ({ source }) => {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify(forms) }],
  ...
  server.registerTool(
    ...
    async ({ source, sql, query }) => {
      ...
      const resolvedSql =
        spec.kind === "literal"
          ? spec.sql
          : resolveSavedQuerySql(pkg, spec.name);
      const result = evaluateSelect(
        parseSelect(resolvedSql),
        readOdbTables(pkg),
      );
      return {
