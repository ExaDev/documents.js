...
describe("odm_to_pdf", () => {
  ...
  it("resolves chapters from chaptersDir, matched by each href's own basename, and writes the result to outputPath", async () => {
    ...
    await writeFile(
      join(chaptersDir, "chapter1.odt"),
      chapterOdtBytes("Dir Chapter One", "Body one from chaptersDir."),
    );
    await writeFile(
      join(chaptersDir, "chapter2.odt"),
      chapterOdtBytes("Dir Chapter Two", "Body two from chaptersDir."),
    );
  ...
  it("prefers an explicit chapters override over chaptersDir for the same href", async () => {
    ...
    await writeFile(
      join(chaptersDir, "chapter1.odt"),
      chapterOdtBytes("From Directory", "Should be shadowed."),
    );
        ...
        chapters: [
          {
            ...
            source: {
              bytesBase64: bytesToBase64(
                chapterOdtBytes("From Override", "Should win."),
              ),
              format: "odt",
            },
          },
  ...
  it("returns an isError result naming every unresolved href when neither chapters nor chaptersDir resolves them", async () => {
    ...
    const result = await pair.client.callTool({
      name: "odm_to_pdf",
      arguments: { source: { bytesBase64: bytesToBase64(master) } },
    });

    expect(result.isError).toBe(true);
    expect(result.structuredContent).toStrictEqual({
      hrefs: ["../missing-a.odt", "../missing-b.odt"],
    });
