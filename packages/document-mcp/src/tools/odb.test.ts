...
import { FORM_AND_REPORT_ODB_PATH, loadFormAndReportOdbBytes } from '../test-support/odb-fixture';
...
const TableSummarySchema = z.object({ tableName: z.string(), columns: z.array(z.object({ name: z.string() })), rows: z.array(z.unknown()) });
const OdbFormSummarySchema = z.object({ name: z.string(), href: z.string(), forms: z.array(z.object({ name: z.string(), command: z.string(), commandType: z.string() })) });
const OdbReportSummarySchema = z.object({ name: z.string(), href: z.string(), command: z.string(), commandType: z.string(), caption: z.string() });
...
function xmlNodesContainText(nodes: readonly XmlNode[], needle: string): boolean {
  return nodes.some((node) => {
    if (node.type === 'text') {
      return node.value.includes(needle);
    }
    if (node.type === 'element') {
      return node.attributes.some((attribute) => attribute.value.includes(needle)) || xmlNodesContainText(node.children, needle);
    }
    return false;
  });
}
...
function packageContainsText(pkg: Package, needle: string): boolean {
  return Object.values(pkg.parts).some((part) => part.kind === 'xml' && xmlNodesContainText(part.nodes, needle));
}
...
describe('odb tools', () => {
  ...
  describe('odb_tables', () => {
    it("lists the fixture's real SALES table via a filesystem path source", async () => {
      const result = await pair.client.callTool({ name: 'odb_tables', arguments: { source: { path: FORM_AND_REPORT_ODB_PATH } } });

      expect(result.isError).toBeFalsy();
      const tables = arrayStructuredContentSchema(TableSummarySchema).parse(result.structuredContent);
      ...
      expect(table.columns.map((column) => column.name)).toEqual(['AMOUNT', 'ID', 'REGION', 'QUARTER', 'CUSTOMER']);
      ...
      const rawTables = arrayStructuredContentSchema(z.unknown()).parse(result.structuredContent);
      expect(block?.type === 'text' ? JSON.parse(block.text) : undefined).toStrictEqual(rawTables);
    });

    it('reads via inline base64 bytes too', async () => {
      const result = await pair.client.callTool({
        name: 'odb_tables',
        arguments: { source: { bytesBase64: bytesToBase64(loadFormAndReportOdbBytes()), format: 'pdf' } },
      });
  ...
  describe('odb_forms', () => {
    it("lists the fixture's real SalesForm, bound to SALES", async () => {
      const result = await pair.client.callTool({ name: 'odb_forms', arguments: { source: { path: FORM_AND_REPORT_ODB_PATH } } });

      expect(result.isError).toBeFalsy();
      const forms = arrayStructuredContentSchema(OdbFormSummarySchema).parse(result.structuredContent);
      ...
      expect(form.forms[0]).toMatchObject({ name: 'SalesForm', command: 'SALES', commandType: 'table' });
    });
  });

  describe('odb_reports', () => {
    it("lists the fixture's real SalesByRegion report, bound to the HighValueSales saved query", async () => {
      const result = await pair.client.callTool({ name: 'odb_reports', arguments: { source: { path: FORM_AND_REPORT_ODB_PATH } } });

      expect(result.isError).toBeFalsy();
      const reports = arrayStructuredContentSchema(OdbReportSummarySchema).parse(result.structuredContent);
      expect(reports).toHaveLength(1);
      expect(reports[0]).toMatchObject({ name: 'SalesByRegion', href: 'reports/Obj11', command: 'HighValueSales', commandType: 'query', caption: 'Sales by region' });
    });
  });

  describe('odb_query', () => {
    it("runs the fixture's own saved HighValueSales query by name, to the real four filtered/sorted rows", async () => {
      const result = await pair.client.callTool({ name: 'odb_query', arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, query: 'HighValueSales' } });
    ...
    it('runs a literal SQL SELECT with GROUP BY over the same real table', async () => {
      const result = await pair.client.callTool({
        name: 'odb_query',
        arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, sql: 'SELECT REGION, COUNT(*), SUM(AMOUNT) FROM SALES GROUP BY REGION ORDER BY REGION ASC' },
      });
      ...
      expect(result.structuredContent).toStrictEqual({
        ...
        rows: [
          [{ kind: 'string', value: 'North' }, { kind: 'number', value: 3 }, { kind: 'number', value: 4290.75 }],
          [{ kind: 'string', value: 'South' }, { kind: 'number', value: 2 }, { kind: 'number', value: 1905.75 }],
          [{ kind: 'string', value: 'West' }, { kind: 'number', value: 1 }, { kind: 'number', value: 60 }],
        ],
      });
    });

    it('reports an isError result naming the offending saved-query name when it does not resolve', async () => {
      const result = await pair.client.callTool({ name: 'odb_query', arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, query: 'NoSuchQuery' } });

      expect(result.isError).toBe(true);
      const [block] = result.content;
      expect(block?.type === 'text' ? block.text : undefined).toContain('NoSuchQuery');
      expect(block?.type === 'text' ? block.text : undefined).toContain('HighValueSales');
    });

    it('reports an isError result when both sql and query are given', async () => {
      const result = await pair.client.callTool({
        name: 'odb_query',
        arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, sql: 'SELECT * FROM SALES', query: 'HighValueSales' },
      });

      expect(result.isError).toBe(true);
      const [block] = result.content;
      expect(block?.type === 'text' ? block.text : undefined).toContain('not both');
    });

    it('reports an isError result when neither sql nor query is given', async () => {
      const result = await pair.client.callTool({ name: 'odb_query', arguments: { source: { path: FORM_AND_REPORT_ODB_PATH } } });

      expect(result.isError).toBe(true);
      const [block] = result.content;
      expect(block?.type === 'text' ? block.text : undefined).toContain('Provide either');
    });

    it('reports an isError result naming the unsupported construct for a SQL feature outside the closed grammar', async () => {
      const result = await pair.client.callTool({
        name: 'odb_query',
        arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, sql: 'SELECT * FROM SALES JOIN OTHER ON SALES.ID = OTHER.ID' },
      });
  ...
  describe('odb_to_csv', () => {
    it('exports the real SALES table as CSV, with no table name needed (exactly one table)', async () => {
      const result = await pair.client.callTool({ name: 'odb_to_csv', arguments: { source: { path: FORM_AND_REPORT_ODB_PATH } } });
      ...
      const csvText = Buffer.from(structured.bytesBase64, 'base64').toString('utf-8');
    ...
    it('writes to outputPath when given, and honours an explicit table name', async () => {
      ...
      const result = await pair.client.callTool({
        name: 'odb_to_csv',
        arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, table: 'SALES', output: { outputPath } },
      });
    ...
    it('reports an isError result naming the offending table when an unrecognised table is requested', async () => {
      const result = await pair.client.callTool({ name: 'odb_to_csv', arguments: { source: { path: FORM_AND_REPORT_ODB_PATH }, table: 'NO_SUCH_TABLE' } });

      expect(result.isError).toBe(true);
      const [block] = result.content;
      expect(block?.type === 'text' ? block.text : undefined).toContain('NO_SUCH_TABLE');
      expect(block?.type === 'text' ? block.text : undefined).toContain('SALES');
    });
  });

  describe('odb_to_xlsx', () => {
    it('exports one workbook carrying the real SALES table as its own sheet', async () => {
      const result = await pair.client.callTool({ name: 'odb_to_xlsx', arguments: { source: { path: FORM_AND_REPORT_ODB_PATH } } });
      ...
      const xlsxBytes = new Uint8Array(Buffer.from(structured.bytesBase64, 'base64'));
