...
const FontFileInputSchema = z.union([
  z.object({ path: z.string().describe('Filesystem path to the font file (.ttf/.otf) to read.') }),
  z.object({ bytesBase64: z.string().describe('Base64-encoded font file bytes.') }),
]);
...
async function resolveFontFileInput(input: z.infer<typeof FontFileInputSchema>): Promise<ResolvedFontFileInput> {
  ...
  return { bytes: base64ToBytes(input.bytesBase64), source: 'inline font bytes' };
}
  ...
  server.registerTool(
    'fonts',
    {
      ...
      inputSchema: z.object({
        source: DocumentInputSchema.describe('The docx/pptx/odt/odp/ods/odg document to extract source-embedded font faces from.'),
      }),
    },
    async ({ source }) => {
      try {
        ...
        const summaries: FontFaceSummary[] = faces.map((face) => ({ family: face.family, bold: face.bold, italic: face.italic, byteLength: face.bytes.length }));
