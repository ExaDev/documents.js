...
describe('convert_document', () => {
  ...
  it('converts a document read from a filesystem path to inline base64 bytes', async () => {
    const result = await pair.client.callTool({ name: 'convert_document', arguments: { source: { path: fixtureDocxPath }, targetFormat: 'pdf' } });
    ...
    if (!('bytesBase64' in structured.output)) {
      throw new Error('expected an inline bytesBase64 output when no outputPath was given');
    }
  ...
  it('converts inline base64 document bytes to a file written at outputPath', async () => {
    ...
    const result = await pair.client.callTool({
      ...
      arguments: { source: { bytesBase64: bytesToBase64(buildFixtureDocx()), format: 'docx' }, targetFormat: 'pdf', output: { outputPath } },
    });

    expect(result.isError).toBeFalsy();
    const structured = ConvertDocumentOutputSchema.parse(result.structuredContent);
    if (!('path' in structured.output)) {
      throw new Error('expected a written path output when outputPath was given');
    }
  ...
  it('reports a structured font substitution when asked, for a family this package vendors a metric-compatible substitute for', async () => {
    ...
    const editor = createDocx();
    const run = editor.body.appendParagraph().appendRun({ text: 'Styled text.' });
    run.fontFamily = 'Calibri';

    const result = await pair.client.callTool({
      name: 'convert_document',
      arguments: { source: { bytesBase64: bytesToBase64(editor.toBytes()), format: 'docx' }, targetFormat: 'pdf', onSubstitutionDiagnostics: true },
    });

    expect(result.isError).toBeFalsy();
    const structured = ConvertDocumentOutputSchema.parse(result.structuredContent);
    expect(structured.fontSubstitutions).toBeDefined();
    expect(structured.fontSubstitutions?.some((substitution) => substitution.requestedFamily === 'Calibri' && substitution.reason === 'vendored-substitute')).toBe(true);
  ...
  it('omits fontSubstitutions entirely when onSubstitutionDiagnostics is not set, even though the diagnostic still fires', async () => {
    ...
    const result = await pair.client.callTool({
      name: 'convert_document',
      arguments: { source: { bytesBase64: bytesToBase64(editor.toBytes()), format: 'docx' }, targetFormat: 'pdf' },
    });
...
describe('list_document_conversions', () => {
  ...
  it('lists every source/target pair the local DocumentConverter port supports', async () => {
    ...
    expect(result.isError).toBeFalsy();
    const structured = ListDocumentConversionsOutputSchema.parse(result.structuredContent);
    expect(structured.conversions.length).toBeGreaterThan(0);
    expect(structured.conversions).toContainEqual({ source: 'docx', target: 'pdf' });
    expect(structured.conversions).toContainEqual({ source: 'pdf', target: 'docx' });
  });
});
