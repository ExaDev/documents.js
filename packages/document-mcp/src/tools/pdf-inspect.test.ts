...
async function connect(): Promise<ConnectedPair> {
  ...
  const client = new Client({ name: 'pdf-inspect-test-client', version: '0.0.0' });
...
describe('pdf_inspect', () => {
  ...
  it('summarises page count, per-page item-kind histogram, metadata, and image formats -- via a filesystem path', async () => {
    const result = await pair.client.callTool({ name: 'pdf_inspect', arguments: { source: { path: pdfPath } } });

    expect(result.isError).toBeFalsy();
    const summary = result.structuredContent;
    if (!isRecord(summary)) {
      throw new Error('expected pdf_inspect to return a structured summary object');
    }
    ...
    if (!isRecord(page1Kinds) || !isRecord(page2Kinds)) {
      throw new Error('expected each page summary to carry an itemKinds histogram');
    }
    ...
    if (!isRecord(imagesByFormat)) {
      throw new Error('expected pdf_inspect summary.imagesByFormat to be an object');
    }
  ...
  it('returns the full parsed LayoutDocument via inline base64 bytes when full: true', async () => {
    ...
    const result = await pair.client.callTool({
      name: 'pdf_inspect',
      arguments: { source: { bytesBase64: bytesToBase64(pdfBytes), format: 'pdf' }, full: true },
    });
  ...
  it('rejects a non-PDF source', async () => {
    const result = await pair.client.callTool({
      name: 'pdf_inspect',
      arguments: { source: { bytesBase64: bytesToBase64(new Uint8Array([1, 2, 3])), format: 'docx' } },
    });

    expect(result.isError).toBe(true);
    const [block] = result.content;
    expect(block?.type === 'text' ? block.text : undefined).toContain('requires a PDF document');
  });
});
