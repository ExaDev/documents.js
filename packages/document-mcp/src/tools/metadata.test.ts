...
describe("metadata_read / metadata_write", () => {
  ...
  it("reads a freshly created docx's own metadata via inline base64 bytes", async () => {
    ...
    const result = await pair.client.callTool({
      name: "metadata_read",
      arguments: {
        source: { bytesBase64: bytesToBase64(bytes), format: "docx" },
      },
    });
    ...
    if (!isRecord(metadata)) {
      throw new Error(
        "expected metadata_read to return a structured LayoutMetadata object",
      );
    }
  ...
  it("writes title/author/subject/keywords to a file on disk, then reads them back from that same file", async () => {
    ...
    if (!isRecord(writeOutput)) {
      throw new Error(
        "expected metadata_write to return a structured WrittenDocumentOutput object",
      );
    }
    expect(writeOutput.path).toBe(patchedPath);
    expect(typeof writeOutput.byteLength).toBe("number");

    const readResult = await pair.client.callTool({
      name: "metadata_read",
      arguments: { source: { path: patchedPath } },
    });
  ...
  it("patches an xlsx file in place now that documents.js wires a real xlsx content codec, leaving its cells untouched", async () => {
    ...
    const result = await pair.client.callTool({
      name: "metadata_write",
      arguments: {
        source: { bytesBase64: bytesToBase64(xlsxBytes), format: "xlsx" },
        targetFormat: "xlsx",
        setTitle: "New xlsx title",
      },
    });
    ...
    const odsBackBytes = xlsxToOds(
      base64ToBytes(result.structuredContent.bytesBase64),
    );
