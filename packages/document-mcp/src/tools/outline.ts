...
import { createLocalDocumentConverter, type DocumentFormat } from 'documents.js';
import { z } from 'zod';
...
function toOutlineJson(children: readonly OutlineChild[]): OutlineChildJson[] {
  return children.map((child) =>
    isOutlineNode(child) ? { text: child.text, level: child.level, children: toOutlineJson(child.children) } : { kind: leafKind(child), text: outlineLeafText(child) },
  );
}
...
export function registerOutlineTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async ({ source }, ctx) => {
      ...
      const result = await converter.convert({ source: { format, bytes }, targetFormat }, { signal });
      ...
      if (pkg === undefined) {
        throw new Error(`the ${format}-to-${targetFormat} conversion reported no DocumentPackage, so there is no tree to outline`);
      }

      const structuredContent = { sourceFormat: format, kind: pkg.kind, outline: toOutlineJson(buildOutline(pkg)) };
