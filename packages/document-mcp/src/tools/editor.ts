...
// Wraps a thrown error into an isError CallToolResult, carrying the thrown message verbatim -- matches metadata.ts's own toErrorResult, duplicated here rather than shared since each tools/*.ts file is a self-contained registration unit in this codebase (see server.ts) with no shared-helpers module of its own.
...
// Every field docx's and odt's own RunInit/ParagraphInit accept (identical shapes in both -- see edit/docx/run.ts and edit/odt/run.ts), plus markdown's own divergent extras (hyperlink, code) validated as unsupported outside markdown below rather than silently dropped. There is deliberately no separate pptx/odp/ods/odg paragraph-append tool here: this first editor surface covers only the wordprocessing paragraph/run family (docx/odt/markdown), the same "styledWordprocessingDocument vs wordprocessingDocument" split the document-cli TUI's own reducer.ts already draws between the docx/odt-only fields (underline/fontFamily/sizePt/color/headingLevel/alignment) and the markdown-compatible ones (text/bold/italic/strike) -- slide/sheet/drawing editing is real remaining scope, tracked separately.
  ...
  server.registerTool(
    "document_create",
    {
      ...
      inputSchema: z.object({
        format: WritableFormatSchema.describe(
          "The format of document to create.",
        ),
    ...
    "document_append_paragraphs",
    {
      ...
      inputSchema: z.object({
        source: DocumentInputSchema.describe(
          "The document to append paragraphs to.",
        ),
        targetFormat: z
    ...
    async ({ source, targetFormat, paragraphs, output }) => {
      try {
        ...
        if (targetFormat === "markdown") {
          ...
          const editor =
            targetFormat === "docx" ? openDocx(bytes) : openOdt(bytes);
