...
describe('fonts', () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(odtPath, buildOdtWithEmbeddedFont({ family: FIXTURE_FONT_FAMILY, fontBytes }));
  });
  ...
  it("lists the one embedded face the odt fixture declares, via a filesystem path", async () => {
    const result = await pair.client.callTool({ name: 'fonts', arguments: { source: { path: odtPath } } });

    expect(result.isError).toBeFalsy();
    expect(result.structuredContent).toStrictEqual({ faces: [{ family: FIXTURE_FONT_FAMILY, bold: false, italic: false, byteLength: fontBytes.length }] });
  ...
  it('lists the one embedded face the odt fixture declares, via inline base64 bytes', async () => {
    const odtBytes = buildOdtWithEmbeddedFont({ family: FIXTURE_FONT_FAMILY, fontBytes });

    const result = await pair.client.callTool({ name: 'fonts', arguments: { source: { bytesBase64: bytesToBase64(odtBytes), format: 'odt' } } });
  ...
  it('returns an isError result naming the rejected format for a format with no source-embedded-font concept', async () => {
    const result = await pair.client.callTool({ name: 'fonts', arguments: { source: { bytesBase64: bytesToBase64(new Uint8Array([0])), format: 'xlsx' } } });
...
describe('describe_font_file', () => {
  ...
  beforeAll(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'document-mcp-describe-font-file-'));
    fontBytes = vendoredFontBytes();
  ...
  it('reads a real vendored font file via a filesystem path', async () => {
    const result = await pair.client.callTool({ name: 'describe_font_file', arguments: { source: { path: fontPath } } });

    expect(result.isError).toBeFalsy();
    expect(result.structuredContent).toStrictEqual({ family: 'Caladea', bold: false, italic: false });
  });

  it('reads a real vendored font file via inline base64 bytes', async () => {
    const result = await pair.client.callTool({ name: 'describe_font_file', arguments: { source: { bytesBase64: bytesToBase64(fontBytes) } } });
  ...
  it('returns an isError result for bytes that are not a recognised sfnt font at all', async () => {
    ...
    const result = await pair.client.callTool({ name: 'describe_font_file', arguments: { source: { bytesBase64: bytesToBase64(garbage) } } });
  ...
  it('returns an isError result naming a TrueType Collection as its own actionable case, not a bare parse failure', async () => {
    ...
    const result = await pair.client.callTool({ name: 'describe_font_file', arguments: { source: { bytesBase64: bytesToBase64(ttc) } } });
