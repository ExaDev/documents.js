...
export const DocumentInputSchema = z.union([
  z.object({
    path: z.string().describe('Filesystem path to the document to read. The document format is inferred from the file extension.'),
  }),
  z.object({
 ...
 */
export async function resolveDocumentInput(input: DocumentInput, options?: { readonly signal?: AbortSignal }): Promise<ResolvedDocumentInput> {
  if ('path' in input) {
