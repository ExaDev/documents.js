...
| `compute_formula`           | Reads every formula a document embeds (docx/odt/markdown paragraphs and pptx/odp slide shapes; structurally, the walk also covers a drawing page's own shape flow and a table cell's own blocks, though no format's writer populates a formula in either position today; a spreadsheet's own cell-anchored formula objects; a standalone `.odf` formula document; and a formula nested inside another embedded object at any depth — e.g. a formula embedded in a drawing embedded in a spreadsheet) and evaluates each through `document-compute.js`'s units-typed `evaluate()` — an agent's way to check whether a document's stated arithmetic actually checks out. A formula referencing a symbol needs that symbol's value supplied via `bindings`, keyed by the document's own symbol-table id; each formula reports its own outcome independently (`evaluated`, `no-content`, or `error`), so one formula needing more bindings never blocks the others. |
