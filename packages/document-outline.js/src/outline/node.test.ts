...
describe("isOutlineNode guard", () => {
  it("accepts builder output for every package kind", () => {
    const packages = [
      ...
      presentationPackage([
        slideGroup([
          shapeGroup([
            listGroup("A", 0, [imageBlock("a picture")]),
            listGroup("B", 1, []),
          ]),
        ]),
      ]),
      spreadsheetPackage([
        sheetGroup({
          name: "Revenue",
          images: [sheetImage("a chart")],
          embeddedObjects: [embeddedObject()],
        }),
      ]),
      drawingPackage([
        drawPageGroup([shapeGroup([paragraph("text box")]), vectorLine()]),
      ]),
      formulaPackage("x^2"),
    ];
  ...
  it("rejects near-misses", () => {
    ...
    expect(isOutlineNode({ text: "x", level: "1", children: [] })).toBe(false);
    expect(isOutlineNode({ text: "x", level: Number.NaN, children: [] })).toBe(
      false,
    );
    expect(
      isOutlineNode({
    ...
    expect(
      isOutlineNode({ text: "x", level: 1, children: [{ nonsense: true }] }),
    ).toBe(false);
...
describe("OutlineNodeSchema", () => {
  ...
  it("rejects a near-miss through safeParse", () => {
    expect(OutlineNodeSchema.safeParse({ text: "x", level: 1 }).success).toBe(
      false,
    );
    expect(
      OutlineNodeSchema.safeParse({
        text: "x",
        level: 1,
        children: [{ nonsense: true }],
      }).success,
    ).toBe(false);
  });
});
