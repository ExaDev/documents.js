...
describe('flattenOutline', () => {
  it('flattens to leaves in document order, skipping group nodes', () => {
    ...
    const pkg = wordprocessingPackage([
      sectionGroup([
        before,
        headingGroup('Chapter', 1, [listGroup('A', 0, [listGroup('B', 1, [nestedTable])]), headingGroup('Section', 2, [after])]),
      ]),
    ]);
    ...
    expect(flattenOutline(buildOutline(pkg))).toEqual([before, nestedTable, after]);
  });

  it('returns an empty array for an outline of empty groups', () => {
    const pkg = wordprocessingPackage([sectionGroup([headingGroup('Chapter', 1, [headingGroup('Section', 2, [])])])]);
...
describe('leafContentHash', () => {
  it('hashes independently constructed identical content identically regardless of key order', () => {
    ...
    const second: ContentParagraph = { runs: [{ text: 'same text' }], kind: 'paragraph' };
  ...
  it('differs when the content differs', () => {
    ...
    const styled: ContentParagraph = { kind: 'paragraph', runs: [{ text: 'same text', bold: true }] };
  ...
  it('treats an explicitly undefined optional field as absent', () => {
    const first = paragraph('same text');
    const second: ContentParagraph = { kind: 'paragraph', runs: [{ text: 'same text' }], styleId: undefined };
    expect(leafContentHash(first)).toBe(leafContentHash(second));
  });
  ...
  it('names the document, not the factoring: a resolved leaf hashes equal to its unfactored twin', () => {
    ...
    const unfactored = wordprocessingPackage([
      sectionGroup([
        {
          node: { kind: 'paragraph', runs: [{ text: 'Chapter' }], headingLevel: 1, indentLeftPt: 24 },
          children: [paragraph('body', { indentLeftPt: 24 })],
        },
      ]),
    ]);
    const resolvedLeaf = flattenOutline(buildOutline(effectivePackage(factored)))[0];
