...
// text/level are readonly because they are fixed at construction; children is a mutable array because the builder pushes into it while assembling a scope (and consumers receive the same arrays it grew). Deep immutability is not claimed -- as with document-schema's own inferred types, the type describes the shape, and mutation discipline is the caller's.
