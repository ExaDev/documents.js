...
A named `siblingId` not found among the parent's existing edges of the requested kind is refused loudly — there is no position to compute otherwise. When bisection between two neighbours has no room left (most commonly `start` against a first child, which — like every first child `projectDocumentGraph` itself ever mints — already sits at the order-key scheme's own floor), `insertEdge` catches the resulting `OrderKeyBudgetExhaustedError` itself and rebalances: every one of that sibling group's edges is re-minted with a fresh, evenly spaced `renumberedOrderKeys` set that includes the new edge in the requested position, so the caller never has to handle the exhaustion by hand.
