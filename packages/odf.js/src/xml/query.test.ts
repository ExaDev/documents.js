...
describe("rootElement", () => {
  it("returns the first element node, skipping a leading declaration", () => {
    ...
    expect(rootElement([{ type: "declaration", attributes: [] }, root])).toBe(
      root,
    );
  });
  ...
  it("returns undefined when there is no element node at all", () => {
    expect(
      rootElement([{ type: "declaration", attributes: [] }]),
    ).toBeUndefined();
...
describe("findChildElement", () => {
  ...
  it("returns undefined when no direct child matches", () => {
    expect(
      findChildElement([el("office:font-face-decls")], "office:meta"),
    ).toBeUndefined();
  });
...
describe("attrValue", () => {
  it("returns the value of a present attribute", () => {
    expect(
      attrValue(el("style:style", { "style:name": "P1" }), "style:name"),
    ).toBe("P1");
  });
