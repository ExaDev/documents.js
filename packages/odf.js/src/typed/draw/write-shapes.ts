...
//
// A paintOrder ODF cannot spell -- negative, fractional (ContentShapeSchema declares a plain z.number() deliberately, "to allow fractional insertion between two existing values later"), or too large to round-trip through JavaScript's own shortest-round-trip String() without switching to exponent notation (Number.isSafeInteger's own 2^53 bound, well under xsd:nonNegativeInteger's own unbounded range but the largest this codec's String(zIndex) call below can spell without repeating the exact "e" defect formatOdfLength's own expandExponential was written to close, see that function's own note) -- is NOT approximated by rounding it to a neighbouring integer: that would silently reorder a shape past a sibling, changing what the document renders as. This function returns undefined for that case, and the CALLER resolves it to the shape's own document-encounter index instead (the identical `odfZIndexOf(paintOrder) ?? documentIndex` canonicalDrawShape below already computes for the canonical form) -- writeDrawFrame and writeDrawVector both write that RESOLVED value unconditionally, never omitting draw:z-index, so a page whose items only partly state an ODF-spellable paintOrder still states an explicit, unambiguous stacking order for every one of them rather than leaving some to a document-order fallback a real consumer applies inconsistently against the ones that do carry the attribute (confirmed against real LibreOffice output: an item with no draw:z-index at all is appended after every item that has one, regardless of its own array position).
...
//
// PAINT ORDER is always present on the way back, never optional: the readers' own walkers stamp every frame they read (typed/draw/shapes.ts's paintOrderKey), so this canonical form states the same value. A paintOrder ODF can spell (a non-negative safe integer -- see odfZIndexOf above, which this reads the answer off rather than re-deriving) is written as draw:z-index and comes back exactly; anything else -- absent, negative, or fractional -- resolves to the shape's own DOCUMENT-ENCOUNTER index, which the CALLER supplies as `documentIndex` because only the caller knows what its own page emits and in what order (odp emits one draw:frame per shape and nothing else, so a shape's index in its slide's own shapes array is its encounter index; odg emits its page's vectors after its shapes, so a shape's encounter index is still its array index but a vector's is offset past every shape -- see typed/odg/write.ts). writeDrawFrame writes that resolved value as draw:z-index UNCONDITIONALLY (never omitting the attribute) so this canonical form and the written file can never disagree about a page whose items only partly state an ODF-spellable paintOrder -- see writeDrawFrame's own note and odfZIndexOf's above for why an omitted attribute on some items but not others would read back wrong on a real consumer.
//
...
export function writeDrawShapes(
  ...
  return shapes.map((shape, index) =>
    writeDrawFrame(shape, listState, state, index),
  );
}
