    ...
    nodes: [
      el("office:document-styles", {}, [
        el("office:automatic-styles", {}, [
          el("style:page-layout", { "style:name": "PM0" }, [
...
function buildFixturePackage(): Package {
  ...
  const customRect = el(
    "draw:custom-shape",
    {
      "draw:name": "CustomRect1",
      "draw:style-name": "grCustom",
      "svg:width": "5cm",
      "svg:height": "3cm",
      "svg:x": "1cm",
      "svg:y": "8cm",
  ...
  const customEllipse = el(
    "draw:custom-shape",
    {
      "draw:name": "CustomEllipse1",
      "draw:style-name": "grCustom",
      "svg:width": "5cm",
      "svg:height": "3cm",
      "svg:x": "13cm",
  ...
  const frame = el(
    ...
    [el("draw:text-box", {}, [el("text:p", {}, [txt("Page two text")])])],
  );
  const page2 = el(
    "draw:page",
    { "draw:name": "page2", "draw:master-page-name": "Default" },
    [frame],
  );
      ...
      el("office:document-content", {}, [
        el("office:automatic-styles", {}, [
          ...
          graphicStyle("grCurve", {
            ...
            "svg:stroke-width": "0.02cm",
          }),
          graphicStyle("grPolygon", {
...
describe("readOdgContent", () => {
  ...
  it("reads the closed curve's real svg:d geometry correctly end to end (viewBox-scaled cubic segment)", () => {
    const { pages } = readOdgContent(buildFixturePackage());
    const path = pages[0]?.vectors.find(
      (v) => v.kind === "path" && v.subpaths[0]?.closed === true,
    );
    if (path?.kind !== "path") {
  ...
  it("reads the polygon's draw:points geometry as a closed straight-line path", () => {
    ...
    expect(polygon.subpaths[0]?.segments.every((s) => s.kind === "line")).toBe(
      true,
    );
  });
  ...
  it("reads a second page's draw:frame text content via the SAME shared shape-walking logic odp uses", () => {
    ...
    expect(pages[1]?.shapes[0]?.blocks[0]).toMatchObject({
      kind: "paragraph",
      runs: [{ text: "Page two text" }],
    });
  });
...
describe("readOdg: the package-native reader over the same fixture", () => {
  ...
  it("carries each page's size on its group node, its shapes as groups, and its vector primitives as the leaves after them", () => {
    ...
    expect(firstPage.children.slice(firstContentPage.shapes.length)).toEqual(
      firstContentPage.vectors,
    );
    ...
    if (shapeGroup === undefined || !("children" in shapeGroup)) {
      throw new Error(
        "expected the second page's shape to decompose into its own group node, not a bare leaf",
      );
    }
...
describe("readOdgContent: residue rows", () => {
    ...
    return {
      parts: {
        "content.xml": {
          ...
          nodes: [
            el("office:document-content", {}, [
              el("office:body", {}, [el("office:drawing", {}, [page])]),
            ]),
          ],
        },
        "styles.xml": stylesXml(),
  ...
  it("quarantines a non-content XML part at the package tier keyed by its part path, spliced onto readOdg's root", () => {
    ...
    expect(readOdg(pkg).source?.["settings.xml"]?.xml).toContain(
