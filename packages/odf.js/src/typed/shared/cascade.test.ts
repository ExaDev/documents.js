...
describe("resolveStyle: no styleName", () => {
  ...
  it("resolves to just the family default-style when styleName is undefined -- a bare, unstyled element is ordinary valid ODF, not a diagnostic", () => {
    ...
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([defaultStyle]) },
    };
    expect(resolveStyle(undefined, "paragraph", pkg)).toEqual({
  ...
  it("does not apply a default-style from a DIFFERENT family", () => {
    const defaultStyle = el("style:default-style", { "style:family": "text" }, [
      textProps({ "fo:font-weight": "bold" }),
    ]);
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([defaultStyle]) },
...
describe("resolveStyle: single-layer resolution (no parent chain)", () => {
  ...
  it("merges default-style and the referenced style field-by-field, referenced style winning on overlap", () => {
    const defaultStyle = el("style:default-style", { "style:family": "text" }, [
      textProps({ "fo:font-weight": "bold", "fo:font-size": "12pt" }),
    ]);
    const t1 = styleStyle("T1", "text", {}, [
      textProps({ "fo:font-size": "18pt" }),
    ]);
    const pkg: Package = {
      parts: {
    ...
    expect(resolveStyle("T1", "text", pkg)).toEqual({
      properties: { bold: true, sizePt: 18 },
      diagnostics: [],
    });
  });
});
...
describe("resolveStyle: parent chain, root-first application", () => {
  it("applies a two-level chain (content.xml automatic style -> styles.xml named style) root-first, target last", () => {
    ...
    const standard = styleStyle("Standard", "paragraph", {}, [
      textProps({ "fo:font-weight": "normal" }),
    ]);
    const textBody = styleStyle(
      "Text_20_body",
      "paragraph",
    ...
    const p1 = styleStyle(
      ...
      [paragraphProps({ "fo:text-align": "right" })],
    );
    const pkg: Package = {
      parts: {
        "content.xml": contentPackage([p1]),
  ...
  it("lets the most specific ancestor in the chain override an earlier, less specific one on the same field", () => {
    const grandparent = styleStyle("GP", "text", {}, [
      textProps({ "fo:font-size": "10pt" }),
    ]);
    const parent = styleStyle(
      "P",
      "text",
      { "style:parent-style-name": "GP" },
    ...
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([grandparent, parent, child]) },
    };
    expect(resolveStyle("C", "text", pkg)).toEqual({
      properties: { sizePt: 12 },
      diagnostics: [],
    });
  });
  ...
  it('applies no fourth "direct formatting" layer -- the referenced style\'s own properties are the final word, exactly as parsed', () => {
    ...
    expect(resolveStyle("C", "paragraph", pkg).properties.alignment).toBe(
      "right",
    );
  });
});
...
describe("resolveStyle: cycle guard", () => {
  it("detects a direct two-style cycle (A -> B -> A), breaks it, and reports a warning diagnostic instead of throwing or hanging", () => {
    const a = styleStyle("A", "paragraph", { "style:parent-style-name": "B" }, [
      paragraphProps({ "fo:text-align": "left" }),
    ]);
    const b = styleStyle("B", "paragraph", { "style:parent-style-name": "A" }, [
      paragraphProps({ "fo:text-align": "right" }),
    ]);
  ...
  it("detects a self-referential style (A -> A) as a cycle of length one", () => {
    const a = styleStyle("A", "text", { "style:parent-style-name": "A" }, [
      textProps({ "fo:font-weight": "bold" }),
    ]);
  ...
  it("does not falsely flag a long but genuinely acyclic chain", () => {
    const root = styleStyle("L0", "text", {}, [
      textProps({ "fo:font-weight": "normal" }),
    ]);
    const l1 = styleStyle(
      "L1",
      "text",
      { "style:parent-style-name": "L0" },
      [],
    );
    const l2 = styleStyle(
      "L2",
      "text",
      { "style:parent-style-name": "L1" },
      [],
    );
    const l3 = styleStyle("L3", "text", { "style:parent-style-name": "L2" }, [
      textProps({ "fo:font-style": "italic" }),
    ]);
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([root, l1, l2, l3]) },
    };
...
describe("resolveStyle: missing style", () => {
  ...
  it("reports a warning and stops when an ANCESTOR further up an otherwise-valid chain is missing, but still applies what was found before that point", () => {
    const child = styleStyle(
      "C",
      "paragraph",
      { "style:parent-style-name": "GhostParent" },
      [paragraphProps({ "fo:text-align": "center" })],
    );
...
describe("resolveStyle: cross-part resolution", () => {
  it("resolves a parent-style-name reference that crosses from content.xml into styles.xml", () => {
    const namedParent = styleStyle("Heading", "paragraph", {}, [
      textProps({ "fo:font-weight": "bold" }),
    ]);
    const automaticChild = styleStyle(
      "P5",
      "paragraph",
    ...
    expect(resolveStyle("P5", "paragraph", pkg)).toEqual({
      properties: { bold: true, alignment: "center" },
      diagnostics: [],
    });
  });
  ...
  it("does not let a same-name, different-family style collide -- family scoping is respected during lookup", () => {
    ...
    const tableNamed = styleStyle("Shared", "table-cell", {}, [
      textProps({ "fo:font-weight": "bold" }),
    ]);
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([paragraphNamed, tableNamed]) },
    };
    expect(resolveStyle("Shared", "paragraph", pkg)).toEqual({
...
describe("resolveStyle: missing parts", () => {
  it("returns an empty result with no diagnostics when neither content.xml nor styles.xml exist and styleName is undefined", () => {
    expect(resolveStyle(undefined, "graphic", { parts: {} })).toEqual({
      properties: {},
      diagnostics: [],
    });
  });
  ...
  it("ignores a non-XML content.xml/styles.xml part rather than throwing", () => {
    const pkg: Package = {
      parts: {
        "content.xml": { kind: "binary", base64: "" },
        "styles.xml": { kind: "binary", base64: "" },
      },
    };
    expect(resolveStyle(undefined, "paragraph", pkg)).toEqual({
      properties: {},
  ...
  it("ignores an XML part with no root element at all (e.g. an empty node list)", () => {
    const pkg: Package = {
      parts: { "styles.xml": { kind: "xml", nodes: [] } },
    };
    expect(resolveStyle(undefined, "paragraph", pkg)).toEqual({
      properties: {},
      diagnostics: [],
...
describe("resolveStyle: malformed style entries during collection", () => {
  it("ignores a style:style missing style:name, missing style:family, or carrying an unrecognised family, without throwing", () => {
    const noName = el("style:style", { "style:family": "paragraph" }, [
      paragraphProps({ "fo:text-align": "left" }),
    ]);
    const noFamily = el("style:style", { "style:name": "NoFamily" }, [
      paragraphProps({ "fo:text-align": "left" }),
    ]);
    const badFamily = styleStyle("BadFamily", "not-a-real-family", {}, [
      paragraphProps({ "fo:text-align": "left" }),
    ]);
    const pkg: Package = {
    ...
    expect(
      resolveStyle("BadFamily", "paragraph", pkg).diagnostics[0]?.message,
    ).toMatch(/not found/i);
  });

  it("ignores a style:default-style missing style:family or carrying an unrecognised family, without throwing", () => {
    const noFamily = el("style:default-style", {}, [
      paragraphProps({ "fo:text-align": "left" }),
    ]);
    const badFamily = el(
      "style:default-style",
      { "style:family": "not-a-real-family" },
      [paragraphProps({ "fo:text-align": "left" })],
    );
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([noFamily, badFamily]) },
    };
    expect(resolveStyle(undefined, "paragraph", pkg)).toEqual({
      properties: {},
      diagnostics: [],
  ...
  it("skips a non-element child (e.g. formatting whitespace between style:style elements) while collecting styles", () => {
    ...
    const pkg: Package = {
      parts: { "content.xml": contentPackage([txt("\n  "), p1]) },
    };
    expect(resolveStyle("P1", "paragraph", pkg)).toEqual({
      properties: { alignment: "center" },
      diagnostics: [],
  ...
  it("ignores an element child that is neither style:style nor style:default-style while collecting styles (e.g. text:notes-configuration, a real office:styles sibling)", () => {
    ...
    const notesConfig = el("text:notes-configuration", {
      "text:note-class": "footnote",
    });
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([p1, notesConfig]) },
    };
    expect(resolveStyle("P1", "paragraph", pkg)).toEqual({
      properties: { alignment: "center" },
      diagnostics: [],
  ...
  it("keeps the FIRST style:default-style for a family when a (malformed) document has more than one, ignoring the rest", () => {
    const first = el("style:default-style", { "style:family": "paragraph" }, [
      paragraphProps({ "fo:text-align": "left" }),
    ]);
    const second = el("style:default-style", { "style:family": "paragraph" }, [
      paragraphProps({ "fo:text-align": "right" }),
    ]);
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([first, second]) },
    };
    expect(resolveStyle(undefined, "paragraph", pkg)).toEqual({
      properties: { alignment: "left" },
      diagnostics: [],
...
describe("resolveStyleElementChain", () => {
  it("returns the same root-first element order resolveStyle folds through, for a caller that needs a property vocabulary StyleProperties does not model", () => {
    const grandparent = styleStyle("GP", "graphic", {}, [
      el("style:graphic-properties", { "fo:padding-left": "0.25cm" }),
    ]);
    const parent = styleStyle(
      "P",
      "graphic",
      { "style:parent-style-name": "GP" },
      [el("style:graphic-properties", { "fo:padding-top": "0.125cm" })],
    );
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([grandparent, parent]) },
    };
    ...
    expect(
      result.elements.map(
        (element) =>
          element.attributes.find((a) => a.name === "style:name")?.value,
      ),
    ).toEqual(["GP", "P"]);
  });
  ...
  it("returns just the family default-style element when styleName is undefined", () => {
    ...
    expect(
      resolveStyleElementChain(undefined, "graphic", pkg).elements,
  ...
  it("reports the same cyclic-chain diagnostic resolveStyle itself reports, since resolveStyle is now a thin wrapper over this", () => {
    ...
    const pkg: Package = { parts: { "styles.xml": stylesPackage([a, b]) } };
    expect(
      resolveStyleElementChain("A", "graphic", pkg).diagnostics[0]?.message,
    ).toMatch(/cyclic/i);
  });
});
...
describe("findStyleElement", () => {
  it("finds a style:style by (name, family) without walking any parent chain", () => {
    const target = styleStyle("ce1", "table-cell", {}, [
      el("style:table-cell-properties", { "fo:background-color": "#ff0000" }),
    ]);
  ...
  it("finds a style defined in styles.xml, not just content.xml", () => {
    const target = styleStyle("co1", "table-column", {}, [
      el("style:table-column-properties", { "style:column-width": "5cm" }),
    ]);
    const pkg: Package = {
      parts: { "styles.xml": stylesPackage([], [target]) },
    };
...
describe("resolveStyleElementChain: real-world consumption pattern", () => {
  it("lets a caller fold an arbitrary properties child across the whole chain, root-first, later entries overriding earlier ones", () => {
    const standard = styleStyle("standard", "graphic", {}, [
      el("style:graphic-properties", {
        "fo:padding-left": "0.25cm",
        "fo:padding-top": "0.125cm",
      }),
    ]);
    const gr1 = styleStyle(
      "gr1",
      "graphic",
      { "style:parent-style-name": "standard" },
      [el("style:graphic-properties", { "fo:padding-top": "0.5cm" })],
    );
    const pkg: Package = {
      parts: {
        "styles.xml": stylesPackage([standard]),
        "content.xml": contentPackage([gr1]),
      },
    };
    ...
    for (const element of elements) {
      ...
      paddingLeft =
        props?.attributes.find((a) => a.name === "fo:padding-left")?.value ??
        paddingLeft;
      paddingTop =
        props?.attributes.find((a) => a.name === "fo:padding-top")?.value ??
        paddingTop;
    }
