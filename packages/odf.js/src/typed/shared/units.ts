...
// The same fixed-point decimal spelling formatOdfLength below produces, without a unit suffix — for the ODF values that are a bare NUMBER rather than a length. There are three in this package's own write path: two in a vector primitive's geometry (typed/shared/path.ts) — svg:viewBox's four user-space numbers, and svg:d's own path coordinates — and a third in typed/draw/write-shapes.ts's frameGeometryAttrs, whose draw:transform="rotate(<radians>)" angle is a bare radians value with no unit suffix either. svg:viewBox has no exponent form at all (parseOdfViewBox's own pattern encodes the four-number grammar it accepts), so the identical silent-invalidity hazard expandExponential above exists to close for lengths applies verbatim there; svg:d's own number grammar and draw:transform's rotate() both admit an exponent syntactically, but writing one spelling for every number this package emits closes the hazard once rather than per attribute, and keeps every one of them spelled the same way as the lengths they are combined with.
...
const GRAD_PER_FULL_TURN = 400;
const DEGREES_PER_HALF_TURN = 180;
const DEGREES_PER_GRAD = DEGREES_PER_FULL_TURN / GRAD_PER_FULL_TURN;
