...
export function getOdfSpaceCount(element: XmlElement): number {
  ...
  if (!Number.isInteger(parsed) || parsed < 0 || String(parsed) !== raw) {
    throw new Error(
      `getOdfSpaceCount: text:s has a malformed text:c attribute: "${raw}"`,
    );
  }
  return parsed;
}
...
export function segmentOdfText(
  ...
  protectTrailing: boolean,
): OdfTextSegment[] {
  const segments: OdfTextSegment[] = [];
  let plain = "";
  ...
  while (index < text.length) {
    ...
    const collapses =
      spaces.length > 1 ||
      (index === 0 && protectLeading) ||
      (end === text.length && protectTrailing);
    if (collapses) {
      flushPlain();
      segments.push({ kind: "space", text: spaces });
    } else {
      plain += spaces;
    }
    index = end;
...
export function buildOdfInlineNodes(
  ...
  return segments.map((segment) => {
    switch (segment.kind) {
      case "text":
        return txt(encodeXmlText(segment.text));
      case "space":
        return segment.text.length === 1
          ? el("text:s")
          : el("text:s", { "text:c": String(segment.text.length) });
      case "tab":
        return el("text:tab");
      case "lineBreak":
