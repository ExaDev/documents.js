...
function findMasterPageElement(
  pkg: Package,
  masterPageName: string | undefined,
  ...
  const masterStyles =
    root === undefined
      ? undefined
      : findChildElement(root.children, "office:master-styles");
  if (masterStyles === undefined) {
    return undefined;
  }
  return childrenWithTag(masterStyles, "style:master-page").find(
    (element) => attrValue(element, "style:name") === masterPageName,
  );
}
...
export function resolvePageLayoutProperties(
  pkg: Package,
  masterPageName: string | undefined,
): XmlElement | undefined {
  ...
  const pageLayoutName =
    masterPage === undefined
      ? undefined
      : attrValue(masterPage, "style:page-layout-name");
  const pageLayout = findPageLayoutElement(pkg, pageLayoutName);
  return pageLayout === undefined
    ? undefined
    : childrenWithTag(pageLayout, "style:page-layout-properties")[0];
}
...
export function resolveDrawPageSize(
  page: XmlElement,
  pkg: Package,
): PageSize | undefined {
...
export function readOdfMasterPageElements(pkg: Package): XmlElement[] {
  const stylesPart = pkg.parts[STYLES_PART];
  const stylesRoot =
    stylesPart?.kind === "xml" ? rootElement(stylesPart.nodes) : undefined;
  const masterStyles =
    stylesRoot === undefined
      ? undefined
      : findChildElement(stylesRoot.children, "office:master-styles");
  return masterStyles === undefined
    ? []
    : childrenWithTag(masterStyles, "style:master-page");
}
