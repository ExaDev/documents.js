...
describe("readOdm: scope boundaries and error paths (synthetic packages)", () => {
  ...
  it("skips a top-level text:section with no text:section-source child -- ODF's generic, non-master-document section (e.g. multi-column layout), not a chapter reference", () => {
    const plainSection = el(
      ...
      [
        el("text:p", {}, [
          txt("ordinary in-document content, not a linked chapter"),
        ]),
      ],
    );
    const pkg: Package = {
      parts: {
        "content.xml": {
          ...
          nodes: [
            el("office:document-content", {}, [
              el("office:body", {}, [el("office:text", {}, [plainSection])]),
            ]),
          ],
        },
      },
  ...
  it("skips a text:section-source-bearing section missing its own required text:name, without failing the whole document", () => {
    const unnamed = el("text:section", { "text:style-name": "Sect1" }, [
      el("text:section-source", { "xlink:href": "chapter.odt" }),
    ]);
    const named = el(
      "text:section",
      { "text:name": "Chapter1", "text:style-name": "Sect1" },
    ...
    const pkg: Package = {
      parts: {
        "content.xml": {
          ...
          nodes: [
            el("office:document-content", {}, [
              el("office:body", {}, [el("office:text", {}, [unnamed, named])]),
            ]),
          ],
        },
      },
  ...
  it("skips a section whose text:section-source is missing its own required xlink:href", () => {
    const noHref = el(
      "text:section",
      { "text:name": "Chapter1", "text:style-name": "Sect1" },
      [el("text:section-source", { "text:filter-name": "writer8" })],
    );
    const pkg: Package = {
      parts: {
        "content.xml": {
          ...
          nodes: [
            el("office:document-content", {}, [
              el("office:body", {}, [el("office:text", {}, [noHref])]),
            ]),
          ],
        },
      },
    ...
    const pkg: Package = {
      parts: {
        "content.xml": {
          ...
          nodes: [
            el("office:document-content", {}, [
              el("office:body", {}, [el("office:text", {}, [section])]),
            ]),
          ],
        },
      },
  ...
  it("preserves document order across more than two chapters, with a skipped (non-chapter) section in between not disturbing the order of the valid ones around it", () => {
    ...
    const interlude = el(
      "text:section",
      { "text:name": "Interlude", "text:style-name": "Sect1" },
      [el("text:p", {}, [txt("not a linked chapter")])],
    );
    const chapter2 = el(
      "text:section",
      { "text:name": "Chapter2", "text:style-name": "Sect1" },
      [
        el("text:section-source", {
          "xlink:href": "chapter2.odt",
          "text:filter-name": "writer8",
        }),
      ],
    );
    const chapter3 = el(
      "text:section",
      { "text:name": "Chapter3", "text:style-name": "Sect1" },
      [
        el("text:section-source", {
          "xlink:href": "chapter3.odt",
          "text:filter-name": "writer8",
        }),
      ],
    );
    ...
    expect(readOdm(pkg).sections.map((section) => section.name)).toEqual([
      "Chapter1",
      "Chapter2",
      "Chapter3",
    ]);
