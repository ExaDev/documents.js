...
function buildFixturePackage(): Package {
  ...
  const titleFrame = el(
    "draw:frame",
    {
      ...
      "svg:width": "200pt",
      "svg:height": "60pt",
      "draw:transform": "rotate(0.5235987755982988) translate(50pt 50pt)",
    },
    [el("draw:text-box", {}, [el("text:p", {}, [txt("Slide One Title")])])],
  );
  const groupShapeA = el(
    "draw:frame",
    {
      "draw:name": "A",
      "svg:x": "50pt",
    ...
    [el("draw:text-box", {}, [el("text:p", {}, [txt("A")])])],
  );
  const groupShapeB = el(
    "draw:frame",
    {
      "draw:name": "B",
      "svg:x": "150pt",
    ...
    [el("draw:text-box", {}, [el("text:p", {}, [txt("B")])])],
  );
  ...
  const notes = el("presentation:notes", {}, [
    el(
      ...
      [
        el("draw:text-box", {}, [
          el("text:p", {}, [txt("First line of notes.")]),
          el("text:p", {}, [txt("Second line.")]),
        ]),
      ],
    ),
  ]);
  const slide1 = el(
    "draw:page",
    { "draw:name": "Slide1", "draw:master-page-name": "Default" },
    [titleFrame, group, notes],
  );
  ...
  const table = el("table:table", {}, [
    ...
    el("table:table-column"),
    el("table:table-row", {}, [
      el("table:table-cell", { "table:number-columns-spanned": "2" }, [
        el("text:p", {}, [txt("Spanned Header")]),
      ]),
      el("table:covered-table-cell"),
    ]),
    el("table:table-row", {}, [
      el("table:table-cell", {}, [el("text:p", {}, [txt("R2C1")])]),
      el("table:table-cell", {}, [el("text:p", {}, [txt("R2C2")])]),
    ]),
  ]);
  const tableFrame = el(
    "draw:frame",
  ...
  const slide2 = el(
    "draw:page",
    { "draw:name": "Slide2", "draw:master-page-name": "Default" },
    [imageFrame, tableFrame],
  );
  ...
  const contentXml: Package["parts"][string] = {
    ...
    nodes: [
      el("office:document-content", {}, [
        el("office:body", {}, [
          el("office:presentation", {}, [slide1, slide2]),
        ]),
      ]),
    ],
  };
  ...
  const metaXml: Package["parts"][string] = {
    ...
    nodes: [
      el("office:document-meta", {}, [
        el("office:meta", {}, [el("dc:title", {}, [txt("My Presentation")])]),
      ]),
    ],
  };
...
function buildListFixturePackage(): Package {
  const styledList = el("text:list", { "text:style-name": "L1" }, [
    ...
    el("text:list-item", {}, [
      el("text:p", {}, [txt("Beta")]),
      el("text:list", {}, [
        el("text:list-item", {}, [el("text:p", {}, [txt("Beta.1")])]),
        el("text:list-item", {}, [el("text:p", {}, [txt("Beta.2")])]),
      ]),
    ]),
    el("text:list-item", {}, [el("text:p", {}, [txt("Gamma")])]),
  ]);
  const siblingList = el("text:list", {}, [
    el("text:list-item", {}, [el("text:p", {}, [txt("Delta")])]),
  ]);
  const bodyFrame = el(
    "draw:frame",
    {
  ...
  const asideFrame = el(
    ...
    [
      el("draw:text-box", {}, [
        el("text:list", {}, [
          el("text:list-item", {}, [el("text:p", {}, [txt("Epsilon")])]),
        ]),
      ]),
    ],
  );
  const slide = el(
    "draw:page",
    { "draw:name": "ListSlide", "draw:master-page-name": "Default" },
    [bodyFrame, asideFrame],
  );
    ...
    parts: {
      "content.xml": {
        ...
        nodes: [
          el("office:document-content", {}, [
            el("office:automatic-styles", {}, [
              el("text:list-style", { "style:name": "L1" }, [
...
function paragraphsWithText(
  ...
  const paragraphs = shape.blocks.filter(
...
describe("readOdpContent: text:list content inside slide text frames", () => {
  it("reads a nested text:list as one numId across both depths, with level read off the actual text:list-in-text:list-item nesting and document order preserved across listed and unlisted paragraphs", () => {
    const paragraphs = paragraphsWithText(
      ...
      "Body",
      ["Intro", "Alpha", "Beta", "Beta.1", "Beta.2", "Gamma", "Delta"],
    );
  ...
  it("leaves list undefined on paragraphs outside any text:list, including one sharing a text box with a list", () => {
    const { slides } = readOdpContent(buildListFixturePackage());
    expect(
      paragraphsWithText(slides, "Body", [
        "Intro",
        "Alpha",
        "Beta",
        "Beta.1",
    ...
    expect(
      readOdpContent(buildFixturePackage()).slides[0]?.shapes.find(
        (s) => s.name === "Title",
      )?.blocks[0],
  ...
  it("resolves the ordered-vs-bullet kind prefix from the referenced text:list-style, and leaves an unstyled list unprefixed -- the same shared numId convention the odt reader mints", () => {
    ...
    expect(paragraphs[1]?.list).toEqual({
      numId: "bullet:list1",
      level: 0,
    } satisfies ContentListMembership);
    expect(paragraphs[6]?.list).toEqual({
      numId: "list2",
      level: 0,
    } satisfies ContentListMembership);
  });
...
describe("readOdpContent", () => {
  ...
  it("reads a rotated shape's real text content and its pixel-verified geometry (see transform.test.ts for the render-based derivation)", () => {
    ...
    expect(title?.blocks[0]).toMatchObject({
      kind: "paragraph",
  ...
  it("reads an image shape's referenced media part on the slide with no notes", () => {
    const { slides } = readOdpContent(buildFixturePackage());
    const imageShape = slides[1]?.shapes.find(
      (s) => s.blocks[0]?.kind === "image",
    );
    expect(imageShape?.blocks[0]).toMatchObject({
      kind: "image",
      format: "png",
      widthPt: 60,
      heightPt: 60,
  ...
  it("reads a table shape with a spanned header row and a covered cell", () => {
    const { slides } = readOdpContent(buildFixturePackage());
    const tableShape = slides[1]?.shapes.find(
      (s) => s.blocks[0]?.kind === "table",
    );
    ...
    expect(
      table.rows[1]?.cells.map((c) =>
        c.blocks[0]?.kind === "paragraph"
          ? c.blocks[0].runs[0]?.text
          : undefined,
      ),
    ).toEqual(["R2C1", "R2C2"]);
  });
...
describe("readOdpContent: residue rows", () => {
  function slidePackage(
    ...
    automaticStyles?: ReturnType<typeof el>,
  ): Package {
    const contentChildren =
      automaticStyles === undefined ? [] : [automaticStyles];
    return {
      parts: {
        "content.xml": {
          kind: "xml",
          nodes: [
            el("office:document-content", {}, [
              ...contentChildren,
              el("office:body", {}, [el("office:presentation", {}, [page])]),
            ]),
          ],
        },
        "styles.xml": stylesXml(),
  ...
  it("quarantines a slide's transition attributes from its drawing-page style (both the legacy presentation: and the ODF 1.2 smil: spellings), its sound, and its animation trees in the slide's own residue", () => {
    ...
    const page = el(
      ...
      [
        ...
        el("anim:par", { "pres:node-type": "timing-root" }, [
          el("anim:animate", { "smil:attributeName": "x" }),
        ]),
        el(
          "draw:frame",
          {
          ...
          [el("draw:text-box", {}, [el("text:p", {}, [txt("Real content")])])],
        ),
      ],
    );
  ...
  it("quarantines the REAL transitions.odp fixture's Impress-written smil transitions on their own slides", () => {
    ...
    for (const slide of slides) {
      expect(slide.source?.xml).not.toContain(
  ...
  it("quarantines a non-content XML part at the package tier keyed by its part path, spliced onto readOdp's root", () => {
    ...
    expect(readOdp(pkg).source?.["settings.xml"]?.xml).toContain(
