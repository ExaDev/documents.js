...
import {
  ...
  collectOdfUnmappedShapeResidue,
  odfAttributeElement,
  odfResidue,
...
function readSlideNotes(page: XmlElement): string {
  ...
  return elementsWithTag(notes.children, "text:p")
    .map(decodeOdfText)
    .join("\n");
}
  ...
  for (const partPath of ["content.xml", "styles.xml"] as const) {
    ...
    for (const containerTag of [
      ...
      for (const style of childrenWithTag(container, "style:style")) {
        if (
          attrValue(style, "style:family") === "drawing-page" &&
...
const ODP_PRESENTATION_EXTRA_TAGS: ReadonlySet<string> = new Set([
  "presentation:sound",
...
function readSlideResidue(
  ...
  const drawingPageProperties = findDrawingPageProperties(
    pkg,
    attrValue(page, "draw:style-name"),
  );
  if (
    drawingPageProperties !== undefined &&
    ODP_TRANSITION_ATTRIBUTES.some(
      (name) => attrValue(drawingPageProperties, name) !== undefined,
    )
  ) {
    elements.push(
      odfAttributeElement(drawingPageProperties, ...ODP_TRANSITION_ATTRIBUTES),
    );
  }
  for (const child of page.children) {
    if (
      child.type === "element" &&
      ODP_PRESENTATION_EXTRA_TAGS.has(child.tag)
    ) {
      elements.push(child);
    }
  }
