...
describe("readOdbReport: form-and-report.odb (real LibreOffice Report Builder output)", () => {
  ...
  it("reads the page header band's two column labels, in document order", () => {
    expect(report.pageHeader?.elements.map((element) => element.text)).toEqual([
      "Customer",
      "Amount",
    ]);
  });
    ...
    expect(report.detail?.elements).toEqual([
      {
        ...
        formula: "field:[CUSTOMER]",
        dataField: "CUSTOMER",
      },
      {
        tag: "rpt:formatted-text",
        name: "Formatted field",
        formula: "field:[AMOUNT]",
        dataField: "AMOUNT",
      },
    ]);
  });
  ...
  it("reads the report-level rpt:function LibreOffice minted for the prefix-character grouping, which the inner group's expression references by name", () => {
    ...
    expect(report.groups[0]?.groups[0]?.groupExpression).toBe(
  ...
  it("reads each group footer's SUM as a computed expression -- a formula with no dataField, unlike a plain bound field", () => {
    for (const footer of [
      ...
      const sum = footer?.elements.find(
        (element) => element.tag === "rpt:formatted-text",
      );
  ...
  it("reads each footer's label alongside its SUM, in document order", () => {
    expect(
      report.groups[0]?.groups[0]?.footer?.elements.map(
        (element) => element.text ?? element.formula,
      ),
    ).toEqual(["Quarter total:", "rpt:SUM([AMOUNT])"]);
    expect(
      report.groups[0]?.footer?.elements.map(
        (element) => element.text ?? element.formula,
      ),
    ).toEqual(["Region total:", "rpt:SUM([AMOUNT])"]);
    expect(
      report.reportFooter?.elements.map(
        (element) => element.text ?? element.formula,
      ),
    ).toEqual(["Grand total:", "rpt:SUM([AMOUNT])"]);
  });

  it('reads each group header\'s single bound field, unwrapping the "field:[COLUMN]" formula into a real column name', () => {
    expect(report.groups[0]?.header?.elements).toEqual([
      {
        tag: "rpt:formatted-text",
        name: "Formatted field",
        formula: "field:[REGION]",
        dataField: "REGION",
      },
    ]);
    expect(report.groups[0]?.groups[0]?.header?.elements).toEqual([
      {
        tag: "rpt:formatted-text",
        name: "Formatted field",
        formula: "field:[QUARTER]",
        dataField: "QUARTER",
      },
    ]);
  });
...
describe("readOdbReport: synthetic report shapes", () => {
      ...
      parts: {
        ...
        "reports/Obj1/content.xml": {
          ...
          nodes: [
            el("office:document-content", {}, [
              el("office:body", {}, [
                el("office:report", reportAttrs, reportChildren),
              ]),
            ]),
          ],
        },
      ...
      el("rpt:detail", {}, [
        el("table:table", { "table:name": "Detail" }, [
          el("table:table-row", {}, [
            el("table:table-cell", {}, [
              el("text:p", {}, [
                el("rpt:formatted-text", { "rpt:formula": "field:[X]" }, [
  ...
  it("reads a group-scoped rpt:function separately from the report-level ones", () => {
    const pkg = reportPackage([
      el("rpt:function", {
        "rpt:name": "ReportTotal",
        "rpt:formula": "rpt:SUM([AMOUNT])",
      }),
      el("rpt:group", { "rpt:sort-expression": "REGION" }, [
        el("rpt:function", {
          "rpt:name": "GroupTotal",
          "rpt:formula": "rpt:SUM([QTY])",
        }),
      ]),
    ]);
    const report = readOdbReport(pkg, "R");
    expect(report.functions).toEqual([
      { name: "ReportTotal", formula: "rpt:SUM([AMOUNT])" },
    ]);
    expect(report.groups[0]?.functions).toEqual([
      { name: "GroupTotal", formula: "rpt:SUM([QTY])" },
    ]);
  });

  it("skips an rpt:function missing either of its mandatory attributes rather than returning it half-populated", () => {
    const pkg = reportPackage([
      el("rpt:function", { "rpt:name": "NoFormula" }),
      el("rpt:function", { "rpt:formula": "rpt:SUM([A])" }),
    ]);
  ...
  it("never treats a bare rpt: element with no rpt:report-element child as a control", () => {
    const pkg = reportPackage([
      el("rpt:detail", {}, [
        el("table:table", { "table:name": "Detail" }, [
          el("table:table-cell", {}, [
            el("rpt:conditional-print-expression", {
              "rpt:formula": "rpt:[X]>1",
            }),
          ]),
        ]),
  ...
  it("leaves a formula this reader does not recognise as a bound field standing verbatim, never partially parsed", () => {
    const pkg = reportPackage([
      el("rpt:detail", {}, [
        el("table:table", {}, [
          el("rpt:formatted-text", { "rpt:formula": "field:[]" }, [
            el("rpt:report-element", {}),
          ]),
          el("rpt:formatted-text", { "rpt:formula": "rpt:NOW()" }, [
            el("rpt:report-element", {}),
          ]),
        ]),
      ]),
...
describe("readOdbReport: error paths", () => {
  ...
  it("throws when the .odb declares no report by that name", () => {
    expect(() =>
      readOdbReport({ parts: { "content.xml": baseContent } }, "Missing"),
    ).toThrow(/no report named "Missing"/);
  });

  it("throws when the declared sub-document is absent from the package", () => {
    expect(() =>
      readOdbReport({ parts: { "content.xml": baseContent } }, "R"),
