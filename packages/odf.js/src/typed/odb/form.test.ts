  ...
  it("reads every bound control with its real element tag, UNO control implementation, and form:data-field binding", () => {
    expect(form.forms[0]?.controls).toEqual([
      ...
      },
      {
        ...
        controls: [],
        name: "numAmount",
        controlImplementation: "ooo:com.sun.star.form.component.NumericField",
        dataField: "AMOUNT",
        id: "control5",
      },
    ]);
...
describe("readOdbForm: synthetic control-tree shapes", () => {
      ...
      parts: {
        ...
        "forms/Obj1/content.xml": {
          ...
          nodes: [
            el("office:document-content", {}, [
              el("office:body", {}, [
                el("office:text", {}, [el("office:forms", {}, formsChildren)]),
              ]),
            ]),
          ],
        },
  ...
  it("reads several top-level form:form siblings, in document order", () => {
    const pkg = formPackage([
      el("form:form", { "form:name": "One" }),
      el("form:form", { "form:name": "Two" }),
    ]);
    expect(
      readOdbForm(pkg, "F").forms.map((definition) => definition.name),
    ).toEqual(["One", "Two"]);
  });
  ...
  it("reads a form:filter and form:order when the form declares them", () => {
    const pkg = formPackage([
      el("form:form", {
        "form:name": "Filtered",
        "form:filter": '"AMOUNT" > 100',
        "form:order": '"REGION" ASC',
        "form:datasource": "SalesDb",
      }),
    ]);
        ...
        "forms/Obj1/content.xml": {
          ...
          nodes: [
            el("office:document-content", {}, [
              el("office:body", {}, [
                el("office:text", {}, [
                  el("text:p", {}, [txt("plain text, no controls")]),
                ]),
              ]),
            ]),
          ],
...
describe("readOdbForm: error paths", () => {
  ...
  it("throws when the .odb declares no form by that name", () => {
    expect(() =>
      readOdbForm({ parts: { "content.xml": baseContent } }, "Missing"),
    ).toThrow(/no form named "Missing"/);
  });

  it("throws when the declared sub-document is absent from the package", () => {
    expect(() =>
      readOdbForm({ parts: { "content.xml": baseContent } }, "F"),
    ).toThrow(/forms\/Obj1\/content\.xml/);
  });
  ...
  it("throws when the sub-document's content.xml is a binary part rather than XML", () => {
    const pkg: Package = {
      parts: {
        ...
        "forms/Obj1/content.xml": { kind: "binary", base64: "" },
      },
    };
