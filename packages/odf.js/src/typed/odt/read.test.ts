...
describe("readOdtContent: kitchen-sink.odt (real LibreOffice output)", () => {
  ...
  it('maps a level-2 heading onto styleId "Heading2" and headingLevel 2', () => {
    const sectionHeading = blocks.find(
      (b) =>
        b.kind === "paragraph" && b.runs[0]?.text === "Section One Point One",
    );
  ...
  it("reads plain multi-paragraph body text in document order", () => {
    ...
    expect(first.runs.map((r) => r.text).join("")).toContain(
      "first paragraph of chapter one",
    );
    expect(second.runs.map((r) => r.text).join("")).toContain(
      "second paragraph, immediately following",
    );
  });
  ...
  it("preserves whitespace through decodeOdfText-equivalent handling: a text:s run of 3 spaces, a text:tab, and a text:line-break", () => {
    const whitespaceParagraph = blocks.find(
      (b) =>
        b.kind === "paragraph" && b.runs.some((r) => r.text.includes("Word1")),
    );
  ...
  it("reads a 2-level nested bulleted list as one numId, level 0 for top-level items and level 1 for nested items", () => {
    const bulletA = asParagraph(
      blocks.find(
        (b) => b.kind === "paragraph" && b.runs[0]?.text === "Bullet A",
      ),
    );
    const bulletB = asParagraph(
      blocks.find(
        (b) => b.kind === "paragraph" && b.runs[0]?.text === "Bullet B",
      ),
    );
    const nested1 = asParagraph(
      blocks.find(
        (b) =>
          b.kind === "paragraph" && b.runs[0]?.text === "Bullet B nested 1",
      ),
    );
    const nested2 = asParagraph(
      blocks.find(
        (b) =>
          b.kind === "paragraph" && b.runs[0]?.text === "Bullet B nested 2",
      ),
    );
    const bulletC = asParagraph(
      blocks.find(
        (b) => b.kind === "paragraph" && b.runs[0]?.text === "Bullet C",
      ),
    );
  ...
  it("reads a 2-level nested numbered list as a DIFFERENT numId from the bulleted list", () => {
    ...
    const numA = asParagraph(
      blocks.find((b) => b.kind === "paragraph" && b.runs[0]?.text === "Num A"),
    );
    const numB = asParagraph(
      blocks.find((b) => b.kind === "paragraph" && b.runs[0]?.text === "Num B"),
    );
    const numNested1 = asParagraph(
      blocks.find(
        (b) => b.kind === "paragraph" && b.runs[0]?.text === "Num B nested 1",
      ),
    );
    const numC = asParagraph(
      blocks.find((b) => b.kind === "paragraph" && b.runs[0]?.text === "Num C"),
    );
  ...
  it("does not carry list membership onto the heading immediately following the numbered list", () => {
    const tableSection = asParagraph(
      blocks.find(
        (b) => b.kind === "paragraph" && b.runs[0]?.text === "Table Section",
      ),
    );
  ...
  it("reads the table's own data rows in document order", () => {
    const table = asTable(blocks.find((b) => b.kind === "table"));
    const row2Texts = table.rows[1]?.cells.map(
      (cell) => asParagraph(cell.blocks[0]).runs[0]?.text,
    );
    const row3Texts = table.rows[2]?.cells.map(
      (cell) => asParagraph(cell.blocks[0]).runs[0]?.text,
    );
  ...
  it("reads the final chapter heading and paragraph after the table, in document order", () => {
    const chapterTwoIndex = blocks.findIndex(
      (b) => b.kind === "paragraph" && b.runs[0]?.text === "Chapter Two",
    );
    ...
    expect(closing.runs.map((r) => r.text).join("")).toContain(
      "second chapter's opening paragraph",
    );
  });
});
...
describe("readOdtContent: master pages after the first, and header/footer content (synthetic packages built to the OASIS grammar)", () => {
  ...
  function masterPage(
    ...
    return el(
      "style:master-page",
      { "style:name": name, "style:page-layout-name": pageLayoutName },
      children,
    );
  }
  ...
  function pageLayout(
    ...
    return el("style:page-layout", { "style:name": name }, [
          ...
          nodes: [
            el("office:document-styles", {}, [
              el("office:automatic-styles", {}, [
                pageLayout("PM1", "210mm", "297mm"),
                pageLayout("PM2", "297mm", "210mm"),
              ]),
              el("office:master-styles", {}, [
                masterPage("Standard", "PM1"),
                masterPage("Landscape", "PM2", landscapeChildren),
              ]),
            ]),
          ],
        },
  ...
  it("splits a second section at a paragraph whose style switches master page, with that master page's geometry and breakType nextPage", () => {
    ...
    expect(sections[0]?.pageSize).toEqual({
      widthPt: knownLength("210mm"),
      heightPt: knownLength("297mm"),
    });
    expect(sections[1]?.pageSize).toEqual({
      widthPt: knownLength("297mm"),
      heightPt: knownLength("210mm"),
    });
  ...
  it("drops a construct extent that spans the master-page switch, keeping each section's own markers balanced", () => {
    ...
    const pkg = packageWith(
      [
        el("text:section", { "text:name": "SpansSwitch" }, [
          el("text:p", {}, [txt("before")]),
          el("text:p", { "text:style-name": "LandscapePara" }, [txt("after")]),
        ]),
      ],
      [switchStyle],
    );
    ...
    for (const section of sections) {
      ...
      expect(
        section.blocks.filter((block) => block.kind === "constructEnd"),
      ).toHaveLength(0);
    }
  });
  ...
  it("reads a master page's style:header and style:footer as block content keyed to that master page", () => {
    const pkg = packageWith(
      ...
      [
        ...
        el("style:footer", {}, [
          el("text:p", {}, [el("text:page-number", {}, [txt("2")])]),
        ]),
      ],
    );
    const { headerFooterParts } = readOdtContent(pkg);
    expect(headerFooterParts).toHaveLength(2);
    expect(headerFooterParts?.[0]).toMatchObject({
      masterPage: "Landscape",
      kind: "header",
      variant: "default",
    });
    expect(headerFooterParts?.[0]?.blocks).toHaveLength(1);
    expect(headerFooterParts?.[1]).toMatchObject({
      masterPage: "Landscape",
      kind: "footer",
      variant: "default",
    });
    ...
    expect(footerParagraph).toMatchObject({
      kind: "paragraph",
      runs: [{ text: "2" }],
    });
  });
  ...
  it('reads the left and first header variants under their own variant names, and skips a style:display="false" header', () => {
    ...
    const { headerFooterParts } = readOdtContent(pkg);
    expect(
      headerFooterParts?.map((part) => `${part.kind}:${part.variant}`),
    ).toEqual(["header:left", "header:first", "footer:first"]);
  });
});
      ...
      parts: {
        "content.xml": {
          ...
          nodes: [
            el("office:document-content", {}, [
              el("office:body", {}, [
                el("office:text", {}, [el("text:p", {}, [txt("hello")])]),
              ]),
            ]),
          ],
        },
...
describe("readOdt: the package-native reader over the same real fixtures", () => {
  ...
  it("groups this fixture's headings into real heading groups carrying their following blocks, rather than a flat block list", () => {
    const documentPackage = wordprocessingPackage(
      readOdt(loadFixture("kitchen-sink.odt")),
    );
    ...
    for (const child of section.children) {
      if (
        !("node" in child) ||
        !("kind" in child.node) ||
        child.node.kind !== "paragraph"
      ) {
        throw new Error(
          "expected every top-level section child to be a heading group",
        );
      }
  ...
  it("assembles minimal.odt into a package that round-trips identically", () => {
    ...
    const content = readOdtContent(pkg);
    assertPackageRoundTrip(readOdt(pkg), {
      kind: "wordprocessing",
      metadata: content.metadata,
      sections: content.sections,
    });
  ...
  it("throws from the package-native reader exactly as the content reader does, on a package with no content.xml", () => {
    ...
    expect(() => readOdt(pkg)).toThrow(
      "readOdtContent: package has no content.xml part",
    );
  });
});
