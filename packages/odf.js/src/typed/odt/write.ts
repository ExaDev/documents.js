...
import { canonicalImage, canonicalMetadata } from "../shared/canonicalise";
// The block plan lives in write-plan.ts; writeOdtContent and normaliseOdtContent both consume it.
import { planDocument, type PlannedSection } from "./write-plan";
import {
  buildOdfListStyle,
  listKindOf,
  writeOdfList,
...
function assertWritableParagraph(
  ...
  paragraphChangeIds: ReadonlyMap<ProvenanceDescriptor, string> | undefined,
): void {
  for (const extent of paragraph.constructs ?? []) {
    if (
      odfRunConstructWriteKind(extent, definitions, paragraphChangeIds) ===
      undefined
    ) {
      const anchorType =
...
function isOdfSplicedBlockDescriptor(descriptor: ConstructDescriptor): boolean {
  if (
    ...
    (descriptor.anchorType === "bookmark" ||
...
export function assertWritableBlock(
  block: ContentBlock,
  blockDefinitions: Readonly<Record<string, DefinitionEntry>> | undefined,
  blockChangeIds: ReadonlyMap<ProvenanceDescriptor, string> | undefined,
): asserts block is
  | ContentParagraph
  | ContentTable
  | ContentImageBlock
...
export function normaliseOdtContent(
  ...
  const planned = planDocument(document.sections, undefined, undefined);
  return {
    kind: "wordprocessing",
    ...
    if (block.kind === "constructStart") {
      ...
      const changeId =
        block.descriptor.kind === "provenance"
          ...
          : undefined;
      // A block-scope comment range splices an office:annotation onto the extent's first paragraph and its office:annotation-end onto the last, named by the same office:name the reader's annotation-half walk keys back through. The annotation element itself needs its body, which the definitions table holds when writeOdt threaded it; with no table in reach the splice builders stay unset and the pair was already refused by name upstream.
      const commentDefinition =
        block.descriptor.kind === "anchor" &&
        block.descriptor.anchorType === "comment" &&
        block.descriptor.definition !== undefined
          ? definitions?.[block.descriptor.definition]
          : undefined;
      const commentDescriptor =
        commentDefinition !== undefined &&
        block.descriptor.kind === "anchor" &&
        block.descriptor.anchorType === "comment"
          ? block.descriptor
          : undefined;
        ...
        spliceEnd:
          bookmark !== undefined
            ...
            : changeId !== undefined
              ...
              : commentDescriptor !== undefined
                ? () => writeOdfAnnotationEndHalf(commentDescriptor.name)
                : undefined,
        startSpliced: false,
...
export function writeOdtContent(
  ...
  // ProvenanceDescriptor is not discriminated on its change field, so a plain property check narrows nothing: a type guard states which changes ODF can spell. moveFrom/moveTo have no region element at all (a move is a deletion plus an insertion), so descriptors carrying them never mint an id and their extents stay refused by name upstream.
  function isOdfWritableProvenance(
    descriptor: RunConstructExtent["descriptor"],
  ): descriptor is ProvenanceDescriptor & {
  ...
  // Tracked-change descriptors mint document-wide region ids (chg1, chg2, ... in first-encounter order, the same deterministic minting convention every other id in this writer uses) and one text:tracked-changes container emitted ahead of the body's first content, exactly where the reader's collector finds it. Descriptors key the map by object identity: the same descriptor object reaches every paragraph writer through the plan, so identity is both correct and allocation-free.
  ...
  const planned = planDocument(document.sections, definitions, changeIds);
  ...
  if (changeRegions.length > 0) {
    // The region container precedes the body's content: ODF permits it anywhere in the text body, and first position is where every real producer puts it — emitted only when at least one region exists, so an unchanged document carries no empty container.
    textElement.children.push(writeOdfTrackedChanges(changeRegions));
  }
