...
//
// This reader is deliberately thin: paragraph/run reading (readOdfParagraph), the heading-identity step over a text:h (readParagraphOrHeading), and table reading (readOdfTable) already live in typed/shared/ -- built for reuse across odt/ods/odp/odg, not odt-specific -- so this module's own job is the odt-SPECIFIC structure those shared readers have no opinion on: walking office:text's actual block sequence (paragraphs interleaved with lists and tables, in document order) and resolving the document's own page geometry from its first master page. readOdfParagraph is tag-agnostic (it never inspects which tag its own caller found it at) and reads text:h exactly as it reads text:p, so this reader calls straight through to it for both, then overrides ONLY the resulting heading identity (styleId plus headingLevel) for a heading -- see readParagraphOrHeading, imported from typed/shared/paragraph.ts.
//
// SCOPE -- the fidelity construct rows (ExaDev/documents.js#719) are read: fields as run-level constructs with their cached text (the *-ref cross-reference displays among them, per typed/shared/text.ts's field tag set), bookmarks and tracked changes at both run and block scope, reference-marks as bookmark anchors at both scopes (a text:reference-mark is a point anchor, a reference-mark-start/-end pair is a range anchor in its own pairing family, since ODF keeps reference-mark names and bookmark names in separate namespaces), notes and annotations as anchors with definitions-table bodies, text:section as a division, the index wrappers as index content controls with their cached bodies, office:forms as content controls, and anchored draw:frames lifted from text flow (images, text boxes, and embedded formula/drawing/presentation/chart objects). Master pages are read as a whole-page inventory, not just the first: a paragraph style's style:master-page-name switch splits a new ContentSection with that master page's geometry (see splitOdtSections below), and every master page's style:header/style:footer variants read as real block flow keyed to the master page (OdtDocument.headerFooterParts + the positional sectionMasterPages that name which master page each section came from). Explicit page breaks (fo:break-before/fo:break-after) are NO LONGER a gap either: styles/properties.ts's StyleProperties models them, so the shared cascade this reader relies on surfaces them onto every paragraph as pageBreakBefore/pageBreakAfter. A nested text:list or table:table inside a table cell reads the same way it would at body level -- readOdfTable's own cell reading (typed/shared/table.ts) recurses into the identical shared list and table walkers this reader uses, threaded through the same document-wide listIdState so a list minted inside a cell gets an identity as unique as one minted anywhere else in the body. List marker GLYPHS (the exact bullet character or number format string) remain unread -- only the ordered-vs-bullet KIND is resolved (see typed/shared/list.ts's resolveOdfListKind), since that is what downstream consumers need to render <ol> vs <ul>.
//
...
function readBlocks(
  ...
  for (const node of nodes) {
    ...
    if (node.tag === "text:p" || node.tag === "text:h") {
      ...
      blocks.push(readOdfTable(node, pkg, state.listIdState));
