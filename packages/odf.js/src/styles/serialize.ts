...
export function buildStylePropertyElements(
  ...
  if (paragraphAttributes.length > 0) {
    elements.push(
      el("style:paragraph-properties", attributesToRecord(paragraphAttributes)),
    );
  }
  ...
  if (textAttributes.length > 0) {
    elements.push(
      el("style:text-properties", attributesToRecord(textAttributes)),
    );
  }
...
export function canonicalPropertiesString(properties: StyleProperties): string {
  const attributes = [
    ...paragraphPropertiesToAttributes(properties),
    ...textPropertiesToAttributes(properties),
  ];
  return attributes
