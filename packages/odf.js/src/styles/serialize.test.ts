...
describe("buildStylePropertyElements", () => {
  it("emits style:paragraph-properties before style:text-properties, matching real ODF producer output and the schema element sequence", () => {
    const elements = buildStylePropertyElements({
      alignment: "center",
      bold: true,
    });
  ...
  it("omits a properties element entirely when that half of the bag is unset, never emitting an empty element", () => {
    expect(
      buildStylePropertyElements({ bold: true }).map((e) => e.tag),
    ).toEqual(["style:text-properties"]);
    expect(
      buildStylePropertyElements({ alignment: "left" }).map((e) => e.tag),
    ).toEqual(["style:paragraph-properties"]);
    expect(buildStylePropertyElements({})).toEqual([]);
  });

  it("serializes to the exact real-world XML shape", () => {
    const xml = buildXml(
      buildStylePropertyElements({ bold: true, color: { r: 1, g: 0, b: 0 } }),
    );
    expect(xml).toBe(
      '<style:text-properties fo:font-weight="bold" fo:color="#ff0000"></style:text-properties>',
    );
  });
});
...
describe("canonicalPropertiesString", () => {
  ...
  it("differs when any single field differs", () => {
    ...
    expect(canonicalPropertiesString(base)).not.toBe(
      canonicalPropertiesString({ bold: false, sizePt: 12 }),
    );
    expect(canonicalPropertiesString(base)).not.toBe(
      canonicalPropertiesString({ bold: true, sizePt: 14 }),
    );
    expect(canonicalPropertiesString(base)).not.toBe(
      canonicalPropertiesString({ bold: true }),
    );
  });
  ...
  it("treats a field explicitly set to undefined the same as that field being entirely absent, matching StyleProperties' own optional-field semantics", () => {
    expect(canonicalPropertiesString({})).toBe(
      canonicalPropertiesString({ bold: undefined }),
    );
  });
