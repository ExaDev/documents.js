...
- **OpenOffice.org 1.x writing** (`writeSxw`/`writeSxwContent`, `writeSxc`/`writeSxcContent`, `writeSxi`/`writeSxiContent`, and `writeSxd`/`writeSxdContent`, plus `transformToOoo1Package`, the read-side transform's own inverse) — `.sxw`, built on `writeOdt`/`writeOdtContent`; `.sxc`, built on `writeOds`/`writeOdsContent`; `.sxi`, built on `writeOdp`/`writeOdpContent`; `.sxd`, built on `writeOdg`/`writeOdgContent`. Every format this package reads on the OpenOffice.org 1.x side it now writes as well, through one wrapper pair each rather than a second writer.
...
| Format          | Reader    | Flat sibling     | Writer     | Flat sibling      | Package kind     |
| --------------- | --------- | ---------------- | ---------- | ----------------- | ---------------- |
| `.sxw` / `.stw` | `readSxw` | `readSxwContent` | `writeSxw` | `writeSxwContent` | `wordprocessing` |
| `.sxc` / `.stc` | `readSxc` | `readSxcContent` | `writeSxc` | `writeSxcContent` | `spreadsheet`    |
| `.sxi` / `.sti` | `readSxi` | `readSxiContent` | `writeSxi` | `writeSxiContent` | `presentation`   |
| `.sxd` / `.std` | `readSxd` | `readSxdContent` | `writeSxd` | `writeSxdContent` | `drawing`        |
