...
> A hand-written, dependency-minimal codec for the OpenDocument Format (ODF — OASIS/ISO 26300): `.odt`/`.ods`/`.odp`/`.odg`/`.odf`/`.odb`/`.odm` and their template variants, built on [Zod 4](https://zod.dev) codecs — plus full read **and write** support for the pre-OASIS OpenOffice.org 1.x / StarOffice 6-7 documents ODF was based on (`.sxw`/`.sxc`/`.sxi`/`.sxd`). `.odt`, `.ods`, `.odp`, `.odg`, `.odf`, and `.odm` all read and write; `.odb` writes its own front-end definitions (`writeOdb`), with the embedded engine's opaque storage deliberately never authored.
...
- **What each reader actually covers** — wordprocessing (`readOdt`), presentation (`readOdp`), drawing (`readOdg`: vector primitives in `draw:z-index`-aware paint order), spreadsheet (`readOds`: every `office:value-type`, cell/page-anchored images, embedded sub-documents including charts, cell comments — a `table:table-cell`'s own `office:annotation`, its text/author/timestamp read (and, uniquely among this reader's own gaps, written back too — ExaDev/documents.js#949) into `ContentSheetCell.comment` — data-validation rules (ExaDev/documents.js#925): a sheet's own `table:content-validation-name` references resolved against the document-wide `table:content-validations` definitions, into `ContentSheet.dataValidations`; and conditional-formatting rules (ExaDev/documents.js#1075): each `calcext:conditional-format`'s own condition/colour-scale/data-bar/icon-set/date-is children into `ContentSheet.conditionalFormats`, a rule this reader cannot close-form model (an unrecognised condition value or cfvo entry type, or a target-range-address parsing to no range) quarantining individually rather than the whole `calcext:conditional-formats` element), each expressed in `document-schema.js`'s own `ContentSection`/`ContentSlide`/`ContentDrawPage`/`ContentSheet` vocabulary — plus the fidelity construct vocabulary in `readOdt` (fields including the cross-reference displays, bookmarks and reference-marks, tracked changes, notes, annotations, divisions, index wrappers, forms, and anchored frames; see [Fidelity constructs](#fidelity-constructs)).
- **`readOdfFormulaMathMl`** — resolves a standalone/embedded `.odf` formula's bare-MathML `content.xml` into raw MathML nodes plus a StarMath annotation, with no pivot shaping at all. **`readOdfFormulaContent`** wraps that into a real `'formula'`-kind `ContentDocument`, and **`readOdfFormula`** into a `DocumentTree`. The writer ladder mirrors it exactly — **`writeOdfFormulaMathMl`** takes the raw document, **`writeOdfFormulaContent`** the flat `ContentDocument`, **`writeOdfFormula`** the `DocumentTree` — writing the package shape a real producer writes: a bare `<math>` root carrying the MathML namespace as the part's default namespace (declaring `xmlns:math` instead when the content itself uses prefixed tags), a stored `mimetype`, `meta.xml`, and a synced manifest, with no `office:document-content` wrapper at all. The MathML nodes are written verbatim, StarMath annotation included — it lives inside the nodes, not alongside them.
- **`readOdm`** — resolves a `.odm` master document into an ordered list of chapter references (`{ name, href, filterName? }`); chapters are genuinely external `.odt` files by ODF design, never cached. **`writeOdm`** is its inverse: one top-level `text:section` per chapter carrying a self-closing `text:section-source` with the href (and filter name) verbatim and nothing else, matching the real-producer shape the reader's own verification transcribed — cached chapter content is deliberately not written, for the same reason the reader deliberately does not read it.
...
Paragraphs, headings, runs (character formatting, hyperlinks), whitespace, lists, tables (including a list or a further nested table inside a table cell — a cell's own content model recurses through the identical shared readers/writers `readOdfTable`/`writeOdfTable` use for a document's top-level tables), images, explicit page breaks, per-section page geometry, and `meta.xml` all round-trip: `flattenTree(readOdt(writeOdt(document)))` reproduces `document` up to the same normalisation `normaliseOdtContent` states explicitly (its own doc comment — a section-less input, for instance, has no page geometry to invent, so it refuses rather than fabricating one).

Of the fidelity constructs `readOdt` reads, a **field** and a **bookmark** anchor (point or ranged, entirely within one paragraph) now write back too ([ExaDev/documents.js#969](https://github.com/ExaDev/documents.js/issues/969)): a field reconstructs its own element verbatim from its cached instruction and re-wraps whatever run range covers its cached text; a bookmark writes as `text:bookmark` (a point) or a `text:bookmark-start`/`text:bookmark-end` pair (a range) — both via `ContentParagraph.constructs`, spliced into the run-writing pipeline by `typed/shared/paragraph.ts`'s `writeOdfParagraphChildren`, which never lets a construct boundary be swallowed by run merging or span/hyperlink grouping. A **division** (`text:section`, including its name, protected flag, column-count style, and external-chapter link) and an **index wrapper** (`text:table-of-content` and its six siblings, recovering which of the seven to write from the wrapper's own quarantined `*-source` residue — the one deliberate exception to "residue is never re-emitted", since here it names structural identity rather than content) also write back, from a block-scope `constructStart`/`constructEnd` pair around the blocks they wrap.

Still refused **by name**: a **note** (footnote/endnote) or **annotation** (comment) anchor and a **tracked-change** provenance wrapper — each needs the definitions-table body a bare `ContentDocument` write has no access to, which `writeOdt`'s own `DocumentTree` entry point does carry, the natural next place to wire this up; a bookmark/tracked-change/comment range that spans **several blocks** rather than sitting inside one paragraph, which ODF spells as a marker on the first/last paragraph _inside_ the range rather than a wrapping element, a materially different write path from the division/index case above; **office:forms** controls, since the reader's own point-pair encoding flattens a form's real parent/child nesting into a sequence with no reliable way back to the original tree; and **embedded objects** ([ExaDev/documents.js#972](https://github.com/ExaDev/documents.js/issues/972)'s own write-side sub-document infrastructure, tracked separately). Every one of these still throws naming exactly what it carries, rather than silently dropping it, since writing a document that silently lost semantic content would be worse than not writing it at all. The quarantined residue channel splits in two: the package-level table (non-content parts such as `settings.xml`, and document-scope vendor-extension elements) is restored verbatim by `writeOdt` itself, since a whole non-content part is never touched or interpreted by anything this writer does either way; a construct's own residue, and the body-walk quarantine buckets a paragraph or block can carry, stay dropped — re-emitting one of those into content the writer is regenerating from a possibly-edited document would be actively wrong, since there is no structural position left to safely reinsert it at — a known, tracked restorable-fidelity gap rather than a silent one.
...
Data-validation rules and conditional-formatting rules round-trip: rules intern into one document-wide `table:content-validations` definition per distinct written content (named `val1`, `val2`, …, referenced from every in-range cell through `table:content-validation-name` — the only carrier ODF offers, so a rule extending past the content-bearing grid materialises the cells it needs), and conditional formats group back into `calcext:conditional-format` wrappers per distinct range list with `calcext:condition`/`calcext:color-scale`/`calcext:data-bar`/`calcext:icon-set`/`calcext:date-is` children, a rule's style minted as a named table-cell style. Two narrow refusals remain on the conditional-format side, each by name: the `containsBlanks`/`notContainsBlanks` members (no spelling in `calcext:condition`'s own vocabulary) and a rule carrying `priority`, `stopIfTrue`, an `aboveAverage` `stdDev`, a reversed icon set, or a `num` cfvo threshold (facts the calcext extension has no attribute for). Embedded objects are refused **by name** for every sheet — no embedded-sub-document package writer exists anywhere in this package's typed layer. A cell's own `numberFormatCode` is not written as a `number:*` data-style/`style:data-style-name` reference for the same reason as before: `readOdsContent` does not populate that field for any cell today, so there is no genuine inverse to write against. Sheet-level residue and a rule's own quarantined `source` residue are dropped, the same deliberate exception `writeOdt` makes for a paragraph's own residue; the package-level table (non-content parts, vendor-extension elements, `table:calculation-settings`) restores, for the same reason it does there.
...
- **A gradient/bitmap/hatch fill resolves its real definition, not a flat approximation alone** (ExaDev/documents.js#954): `draw:fill="gradient"`/`"bitmap"`/`"hatch"` each name a separate `<draw:gradient>`/`<draw:fill-image>`/`<draw:hatch>` style-part element (a real, standalone `office:styles` child, not nested inside the referencing style), resolved by name and carried losslessly on `ContentVector.fillPattern` alongside the existing flat `fill` swatch — a gradient's own `draw:start-color` or a hatch's own `draw:color` when the style states no direct `draw:fill-color` of its own, still undefined for a bitmap fill (no single colour to average without decoding pixels). `draw:opacity`/`svg:stroke-opacity` read into `fillOpacity`/`ContentStroke.opacity`, and a `"dash"`-mode stroke's own named `<draw:stroke-dash>` definition resolves its real `dots1`/`dots2`/`distance` run-length pattern onto `ContentStroke.dashPattern`, alongside the existing `style: 'dashed'`.
- **`draw:custom-shape` recognises nine presets, not three**: `'rectangle'`/`'ellipse'` map to their plain variants; `'round-rectangle'` builds a genuinely rounded-corner path when the shape's own `draw:handle`/`draw:modifiers` resolve a corner radius, falling back to the plain rect approximation only when they don't; `'diamond'`/`'isosceles-triangle'`/`'right-triangle'`/`'pentagon'`/`'hexagon'`/`'octagon'` build a fixed polygon inscribed in the shape's own frame. Every other preset (including `'parallelogram'`/`'trapezoid'`, whose real geometry needs the same kind of adjustment handle round-rectangle reads but this reader does not yet evaluate for them) still salvages as a text-only shape when it carries real text, with its whole `draw:enhanced-geometry` quarantined in residue — evaluating the general `draw:enhanced-path` formula language itself remains out of scope.
