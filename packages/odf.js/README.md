...
`readOdt` reads ODF's fidelity constructs into `document-schema.js`'s harmonised construct vocabulary — the semantic channel of the family's two-channel fidelity model (the quarantined residue channel is the other half). No real-producer fixture in this repository exercises these rows yet: every fixture below the unit suites is programmatic, built to the OASIS element grammar, and real-producer verification is outstanding.
...
- **Anchored frames** — a `draw:frame` in text flow lifts to blocks after its paragraph: images, text-box content, and embedded objects (formula, drawing, presentation, chart; a spreadsheet degrades to its preview image — reading it would need the odt→ods reader edge this package deliberately refuses).
