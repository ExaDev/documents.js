import { SI_BASE_DIMENSIONS, type DimensionVector, type SiBaseDimension } from 'document-schema.js';
...
export function dimensionExponent(dimension: DimensionVector, base: SiBaseDimension): number {
...
function combine(a: DimensionVector, b: DimensionVector, op: (x: number, y: number) => number): DimensionVector {
...
export function scaleDimension(dimension: DimensionVector, k: number): DimensionVector {
  const result: DimensionVector = {};
  for (const base of SI_BASE_DIMENSIONS) {
    ...
    if (!Number.isInteger(scaled)) {
      throw new RangeError(`dimensions.ts: scaling '${base}' by ${k} does not land on an integer exponent`);
    }
    result[base] = scaled;
  }
  return result;
...
export function dimensionToString(dimension: DimensionVector): string {
  const parts = SI_BASE_DIMENSIONS.filter((base) => dimensionExponent(dimension, base) !== 0).map(
    (base) => `${base}^${dimensionExponent(dimension, base)}`,
  );
