import { type DimensionVector, type Quantity } from "document-schema.js";
import {
  dimensionsEqual,
  divideDimensions,
  isDimensionless,
...
export function divideQuantities(a: Quantity, b: Quantity): Quantity {
  if (b.magnitude === 0) {
    throw new DivisionByZeroError(
      "math:divide",
      `divisor magnitude is exactly zero (dividend magnitude ${a.magnitude})`,
    );
  }
  return quantity(
    a.magnitude / b.magnitude,
...
export function powQuantity(base: Quantity, exponent: Quantity): Quantity {
  ...
  return quantity(
    Math.pow(base.magnitude, exponent.magnitude),
    scaleDimension(base.dimension, exponent.magnitude),
  );
}
...
export function sqrtQuantity(a: Quantity): Quantity {
  if (a.magnitude < 0) {
    throw new NumericDomainError(
      "math:sqrt",
      `magnitude must be non-negative, got ${a.magnitude}`,
    );
  }
...
function requireDimensionless(a: Quantity, operator: string): void {
  if (!isDimensionless(a.dimension)) {
    throw new IncompatibleDimensionsError(
      ...
      a.dimension,
      {},
      "trigonometric functions take a dimensionless (radian) argument",
    );
  }
}
