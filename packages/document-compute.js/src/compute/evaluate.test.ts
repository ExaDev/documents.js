...
import type { FormulaBindings, MathExpression, MathMatrix, MathUnparsed, SymbolTable } from 'document-schema.js';
...
function qty(value: { numerator: string; denominator: string }, unit: string): MathExpression {
...
describe('evaluate: qty', () => {
  it('resolves a unit-registry id against the supplied SymbolTable and converts to SI-coherent magnitude', () => {
    const result = evaluate(qty({ numerator: '10', denominator: '1' }, 'imperial:foot'), {}, context);
  ...
  it('applies an affine unit\'s offset as well as its factor', () => {
    const result = evaluate(qty({ numerator: '0', denominator: '1' }, 'demo:celsius'), {}, context);
  ...
  it('throws UnknownUnitError for a unit id the symbol table does not carry', () => {
    expect(() => evaluate(qty({ numerator: '1', denominator: '1' }, 'nonexistent:unit'), {}, context)).toThrow(UnknownUnitError);
  });
});
...
describe('evaluate: app -- arithmetic over bound symbols', () => {
  ...
  it('adds', () => {
    expect(evaluate(app('math:add', [num('2'), num('3')]), {})).toEqual(quantity(5, {}));
  });

  it('subtracts', () => {
    expect(evaluate(app('math:subtract', [num('5'), num('3')]), {})).toEqual(quantity(2, {}));
  });

  it('multiplies bound symbols, combining dimensions (F = m * a)', () => {
    expect(evaluate(app('math:multiply', [sym('m'), sym('a')]), bindings)).toEqual(
  ...
  it('divides bound symbols, combining dimensions (speed = distance / time)', () => {
    ...
    expect(evaluate(app('math:divide', [sym('distance'), sym('time')]), speedBindings)).toEqual(
      quantity(5, { length: 1, time: -1 }),
    );
  });

  it('raises via math:pow', () => {
    expect(evaluate(app('math:pow', [num('2'), num('3')]), {})).toEqual(quantity(8, {}));
  });

  it('throws IncompatibleDimensionsError adding across incompatible dimensions', () => {
    const mismatched = app('math:add', [qty({ numerator: '1', denominator: '1' }, 'si:metre'), qty({ numerator: '1', denominator: '1' }, 'si:second')]);
    expect(() => evaluate(mismatched, {}, context)).toThrow(IncompatibleDimensionsError);
  });

  it('throws DivisionByZeroError dividing by an exact zero', () => {
    expect(() => evaluate(app('math:divide', [num('5'), num('0')]), {})).toThrow(DivisionByZeroError);
  });

  it('throws UnsupportedExpressionError for an unknown operator id', () => {
    expect(() => evaluate(app('math:frobnicate', [num('1')]), {})).toThrow(UnsupportedExpressionError);
  });
});
...
describe('evaluate: intervals, reusing the same evaluator', () => {
  it('mixes a bound Interval with a plain Quantity literal via point-interval promotion', () => {
    ...
    const result = evaluate(app('math:multiply', [sym('cosPhi'), num('2')]), bindings);
  ...
  it('propagates IncompatibleDimensionsError through interval arithmetic just as it does for Quantity', () => {
    ...
    expect(() => evaluate(app('math:add', [sym('a'), sym('b')]), bindings)).toThrow(IncompatibleDimensionsError);
  });
});

describe('evaluate: sum / prod binders', () => {
  it('sums i from 1 to 3', () => {
    const expression: MathExpression = { kind: 'sum', binder: 'i', lower: num('1'), upper: num('3'), body: sym('i') };
    expect(evaluate(expression, {})).toEqual(quantity(6, {}));
  });

  it('multiplies i from 1 to 4 (4!)', () => {
    const expression: MathExpression = { kind: 'prod', binder: 'i', lower: num('1'), upper: num('4'), body: sym('i') };
...
describe('evaluate: out-of-scope node kinds', () => {
  ...
  it('throws UnsupportedExpressionError for an unparsed node', () => {
    const expression: MathUnparsed = { kind: 'unparsed', latex: '\\frobnicate{x}' };
    expect(() => evaluate(expression, {})).toThrow(UnsupportedExpressionError);
  });
});
