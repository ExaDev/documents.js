...
describe("CoverageMask", () => {
  it("clears every subsample count on reset", () => {
    const maskWidth = 2;
    const maskHeight = 2;
    const mask = new CoverageMask(maskWidth, maskHeight);
    mask.fillPolygons(
      [
        [
          { x: 0, y: 0 },
  ...
  it("does not paint a pixel a degenerate segment's own forward and reverse crossing coincide on exactly", () => {
    ...
    const maskWidth = 5;
    const mask = new CoverageMask(maskWidth, maskWidth);
    mask.fillPolygons(
      [
        [
          { x: 0, y: 0 },
    ...
    const pixelRow = 2;
    const pixelColumn = 3;
    expect(mask.countAt(pixelRow * maskWidth + pixelColumn)).toBe(0);
  });
  ...
  it("fully covers a pixel whose polygon vertex sits exactly on a subsample scanline", () => {
    ...
    expect(mask.countAt(0)).toBe(COVERAGE_DENOMINATOR);
  });
  ...
  it("does not bleed coverage into the next row down when a span overruns the canvas's right edge", () => {
    ...
    const firstColumn = 0;
    const rowJustPastRect = 2;
    const rowAfterThat = 3;
    expect(mask.countAt(rowJustPastRect * maskWidth + firstColumn)).toBe(0);
    expect(mask.countAt(rowAfterThat * maskWidth + firstColumn)).toBe(0);
  });
  ...
  it("cuts a hole under evenodd where nonzero keeps a same-wound overlap solid", () => {
    ...
    const centreIndex = centreRow * maskWidth + centreColumn;
    const nonzero = new CoverageMask(maskWidth, maskWidth);
    nonzero.fillPolygons([outer, inner], "nonzero");
    expect(nonzero.countAt(centreIndex)).toBeGreaterThan(0);
    const evenodd = new CoverageMask(maskWidth, maskWidth);
  ...
  it("cuts a hole under nonzero when the inner ring is wound the opposite way to the outer one", () => {
    ...
    const outerRingRow = 1;
    expect(
      mask.countAt(outerRingRow * maskWidth + centreColumn),
    ).toBeGreaterThan(0);
  });
});
