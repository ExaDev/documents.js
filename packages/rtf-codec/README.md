...
| Form fields                                              | A `\field` whose `\*\fldinst` names FORMTEXT/FORMCHECKBOX/FORMDROPDOWN, plus whatever `\*\formfield` data it carries (`\*\ffname`, a checkbox's `\ffres`/`\ffdefres`, a dropdown's `\*\ffl` entries), as a `contentControl` construct (plainText/checkbox/dropDown) |
...
A `contentControl` construct mints a real `\*\formfield` only for the three controlTypes RTF's own vocabulary actually spells (plainText/checkbox/dropDown); any other controlType (richText, comboBox, date, and the rest) degrades through the same construct-gap diagnostic every other unrepresentable construct uses.
