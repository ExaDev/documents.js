...
| Form fields                                              | A `\field` whose `\*\fldinst` names FORMTEXT/FORMCHECKBOX/FORMDROPDOWN, plus whatever `\*\formfield` data it carries (`\*\ffname` as its tag, `\*\ffhelptext` as its alias, `\ffprot` as its lock, a checkbox's own `\ffres`/`\ffdefres` as its checked state, a dropdown's own `\ffres`/`\ffdefres` as its selected entry alongside its `\*\ffl` option list), as a `contentControl` construct (plainText/checkbox/dropDown) |
...
A `contentControl` construct mints a real `\*\formfield` only for the three controlTypes RTF's own vocabulary actually spells (plainText/checkbox/dropDown); any other controlType (richText, comboBox, date, and the rest) degrades through the same construct-gap diagnostic every other unrepresentable construct uses. A dropDown always mints `\ffhaslistbox`, whether or not it carries any options, since [MS-DOC] 2.9.78 FFDataBits.fHasListBox must be set for a list-type field regardless of how many entries the list holds. `\ffres`/`\ffdefres` are minted alongside it only when the field's own recorded selection genuinely names one of `options`, and omitted together in both remaining cases — no selection was ever recorded, or the recorded `value` names none of `options` — rather than guess an index that would silently point at the wrong entry. RTF itself tolerates that omission cleanly (a real fixture with neither control word present decodes as an unset value), so this is not a gap this codec papers over with a fabricated default: [MS-DOC] 2.9.78 FFData.wDef "MUST exist if and only if" the field is a checkbox or dropdown is a real MS-DOC production rule, and one this codec deliberately does not always satisfy, matching what real-world producers already do. The unmatched-value case is reported through the same diagnostic sink every other unrepresentable construct in this writer uses, since it is real, signalable data loss rather than an absence. Options beyond [MS-DOC] 2.9.78 FFData.hsttbDropList's own 25-entry limit are truncated, also with a diagnostic — not an arbitrary cutoff, since FFDataBits' own `iRes` field reserves index 25 as its "undefined selection" sentinel, so a 26th real entry would collide with it.
