...
A `contentControl` construct mints a real `\*\formfield` only for the three controlTypes RTF's own vocabulary actually spells (plainText/checkbox/dropDown); any other controlType (richText, comboBox, date, and the rest) degrades through the same construct-gap diagnostic every other unrepresentable construct uses. A dropDown always mints `\ffhaslistbox`, whether or not it carries any options, since [MS-DOC] 2.9.78 FFDataBits.fHasListBox must be set for a list-type field regardless of how many entries the list holds; `\ffdefres` is minted alongside it only when a valid index exists to write — the field's own recorded selection when `value` names one of `options`, or entry 0 (RTF's own spec-forced first-entry default) when no selection was recorded at all — and omitted entirely both when there are no options to index and when `value` names none of them, rather than mint a `\ffdefres` that would silently point at the wrong entry.
