...
// "\outlinelevelN ... a value from 0 to 8 ... In the default case, no outline level is specified (same as body text)."
...
export function applyParagraphControlWord(
  ...
  switch (name) {
    ...
    case "ltrpar":
      ...
      break;
    case "outlinelevel":
      // "\outlinelevelN ... a value from 0 to 8 ... In the default case, no outline level is specified (same as body text)." A value above this is a producer's own spelling of body text, so it clears the level rather than becoming a tenth heading depth.
      state.para.outlineLevel =
        param === undefined || param > MAX_OUTLINE_LEVEL ? undefined : param;
      break;
    case "li":
    case "lin":
