...
// Whatever `{\*\formfield ...}` handed the reader beyond the field's own instruction: the bookmark-style name from `{\*\ffname ...}`, the human-readable label from `{\*\ffhelptext ...}` (gated by `ownHelp`, see below), a dropdown's own `{\*\ffl ...}` entries, the result indices `\ffres`/`\ffdefres` carry, and the `\ffprot` protection bit. RTF 1.5's own Form Fields table defines the result indices purely in list-field terms, but they serialise the binary FFDataBits structure [MS-DOC] 2.9.79 defines, whose iRes field carries a real, spec-defined meaning per iType — a checkbox's checked state (0/1/25-undefined) for iTypeChck, a zero-based \ffl index for iTypeDrop (25 again for an undefined selection); see formFieldContentControl below for how each iType's own reading is decided. Optional end to end — `\*\formfield` itself is optional per the grammar, so a bare FORMTEXT/FORMCHECKBOX/FORMDROPDOWN instruction with no `\*\formfield` group still names a control type on its own. A plainText field's own default text (`{\*\ffdeftext ...}`, FFData.xstzTextDef) is deliberately NOT one of these members: nothing here ever promotes it onto a contentControl (see formFieldContentControl's own top comment for why), so read.ts skips its content whole rather than collecting a value with no consumer.
export interface RtfFormFieldData {
  readonly name: string;
  readonly helpText: string;
  // [MS-DOC] 2.9.79 FFDataBits.fOwnHelp, verbatim: "A bit that specifies whether the form field has custom help text in FFData.xstzHelpText. If fOwnHelp is 0, FFData.xstzHelpText contains an empty or auto-generated string." Read from `\ffownhelp` via read.ts's own `\b`/`\i`-style toggleValue, not formFieldValueBit: RTF 1.9.1's own control-word-type table classifies `\ffownhelpN` as a Value control word rather than a Toggle word, whose own literal default would read a bare (unparameterised) `\ffownhelp` as false — but LibreOffice's real RTF exporter emits exactly that bare form (with no numeric parameter) whenever the control model exposes a HelpText property at all — sw/source/filter/ww8/rtfattributeoutput.cxx gates every FFOWNHELP emission on `xPropSetInfo->hasPropertyByName("HelpText")`, a property-existence check rather than a literal unconditional emission — immediately before a `{\*\ffhelptext ...}` destination that carries the control's genuine HelpText property, so the literal Value-word default would silently discard real author-set text from that producer on every read. read.ts's own comment on applyFormFieldControlWord's "ffownhelp" case has the full citation and reasoning; an explicit `\ffownhelp0` still reads as false, and the control word never appearing in the group at all defaults identically to false too, since FormFieldState's own initial value already starts there — only the bare form's own default changed. formFieldContentControl below gates promoting `helpText` to the descriptor's `alias` on this flag, so a genuinely auto-generated (or wholly absent) xstzHelpText string never surfaces as an author-set alias.
...
// Anchored on the instruction's own leading token, not merely present anywhere in the string: a field instruction's grammar is "<field-type> [switches]" (see this module's top-of-file comment and read.ts's own \fldinst-destination comment), so the type keyword only ever legitimately appears first. An unanchored \b-only match would also fire on the identical word sitting inside a switch argument of an unrelated field — e.g. HYPERLINK "http://example.com/FORMTEXT" — wrapping it in a fabricated form field it never named. Mirrors HYPERLINK_TARGET (read.ts) anchoring past HYPERLINK's own name to require a following quoted target rather than matching the bare word anywhere.
...
// A form field's instruction plus whatever `\*\formfield` data the reader collected, folded into the one construct document-schema.js gives a content control — undefined for an ordinary field (HYPERLINK, PAGE, and the rest) whose instruction names none of RTF's three form-field keywords, so the reader's existing hyperlink-only handling for those is untouched. The field's actual DISPLAYED text is not duplicated onto `value` here: it rides the ordinary runs the extent already wraps (the `\fldrslt` content). `ContentControlDescriptor.value` is defined as the control's CURRENT scalar value ("a control whose value IS its rendered content carries that content in `children` and leaves this absent" — document-schema.js's own field comment), and for a text form field the current value IS the wrapped-run text this function has no access to, never the field's default/reset text. What a plainText field's own `\ffdeftext` carries — FFData.xstzTextDef — is a genuinely different fact from that current value, so it is deliberately NOT promoted to `value` below, matching the checkbox/dropDown branches immediately below it, which likewise let a genuinely current fact (`\ffres`) win over a reset default (`\ffdefres`) rather than reporting the default as though it were current. RtfFormFieldData carries no `defaultText` member at all for exactly this reason: nothing here ever reads `\ffdeftext` back onto anything the descriptor exposes, so read-parse.ts skips its content whole rather than collect a value with no consumer (SILENT_SKIP_DESTINATIONS in read-state.ts). This codec's own writer mints `\ffdeftext` from `value` when building RTF from a descriptor a real producer (documents.js's own PDF AcroForm-to-contentControl reconstruction, for one) supplied, but that is a write-only use of `value` in the opposite direction, unrelated to reading `\ffdeftext` back in.
export function formFieldContentControl(
  ...
  // controlType === "plainText" deliberately has no branch here promoting a default/reset text (\ffdeftext, FFData.xstzTextDef) onto `value` — see this function's own top comment for why: `value` names the CURRENT value, `\ffdeftext` names the DEFAULT/reset text, and the field's real current content already rides the wrapped runs the extent covers. RtfFormFieldData carries no field for it at all (read.ts skips \ffdeftext's content whole), precisely so there is nothing here to reintroduce this way by mistake — promoting it into `value` was tried and reverted once already, reporting the default placeholder text as if it were the real field content and discarding the fact that genuinely current text sits in the runs.
