...
// The highest code point rtfBytesFromLatin1 accepts: a byte-preserving string has exactly one code unit per byte, so any code unit above this proves the string was decoded through a multi-byte encoding instead.
const MAX_LATIN1_CODE_POINT = 0xff;
const HEX_RADIX = 16;
// "U+XXXX" — a code point's own hex digits, zero-padded to this width in the diagnostic message below.
...
export function rtfBytesFromLatin1(source: string): Uint8Array {
  ...
  for (let index = 0; index !== source.length; index += 1) {
    ...
    if (code > MAX_LATIN1_CODE_POINT) {
      throw new RtfParseError(
        "rtf/not-byte-preserving-string",
        `character at index ${String(index)} is U+${code.toString(HEX_RADIX).toUpperCase().padStart(CODE_POINT_HEX_DIGITS, "0")}, above U+00FF: this string was decoded through a multi-byte encoding and no longer holds the file's bytes. Read the .rtf file as bytes and pass the Uint8Array directly.`,
      );
    }
    out[index] = code;
  }
