...
// \ffownhelpN and \ffprotN are classified as "Value" control words, not "Toggle" words like \b/\i, in RTF 1.9.1's own Appendix B ("Index of RTF Control Words") -- but Appendix B's own "Value"/"Toggle" definitions ("Value: This control word requires a parameter" / "Toggle: ... turn on the property [or] turn it off") state no default for an omitted parameter themselves; that default lives in a genuinely separate part of the spec, "Conventions of an RTF Reader"'s own "Change Formatting Property" entry, which states it in full: "If a parameter is needed and not specified, then a default value is used... If the control word does not specify a default, then RTF readers should assume a default of 0 except for the toggle control words (like \b), which have a default of 1." Appendix B settles WHICH of those two defaults applies here (Value, not Toggle, so 0); Change Formatting Property is where that 0-vs-1 rule itself comes from. RTF's own Form Fields table states the identical 0-default fact for this specific pair without ever describing a bare-word meaning of its own: "\ffownhelpN: 1 if there is associated help text, 0 otherwise" and "\ffprotN: 1 if this field is protected, 0 otherwise" name only an explicit 0/1 parameter -- unlike \b, whose own bare-word meaning is not stated in its own alphabetical table entry (which reads only "\b* Bold.", the trailing asterisk marking it as one of the toggle words the "Control Word" section's own preamble already described) but in that preamble prose itself, two sections earlier: "For example, \b turns on bold and \b0 turns off bold." A bare \ffownhelp or \ffprot therefore reads as 0/false here, not true.
