...
// The content of a `\*\formfield` group, emitted in a fixed, deterministic order this writer itself chooses -- RTF's own "Form Fields" table (RTF 1.5/1.6, and by inheritance 1.9.1, which changed no control words this codec reads or writes here) is a flat control-word table with no grammar production of its own at all, unlike the Fields/Bookmarks/Index Entries/Table of Contents sections immediately alongside it in the same spec, each of which DOES carry a real `<production>` built from the spec's own Formal Syntax operators (`?`/`&`/`*`/`|`) -- e.g. Fields' own `<fieldmod> \flddirty? & \fldedit? & \fldlock? & \fldpriv?`, whose `&` operator itself means "in any order" per the spec's own Formal Syntax section, the opposite of a fixed sequence. `formparams`/`formstrings` are this codec's own descriptive shorthand for two natural groupings of the Form Fields table -- the numeric flag/index control words (`\fftype`, `\ffownhelp`, `\ffprot`, `\ffhaslistbox`, `\ffdefres`/`\ffres`) versus the destination strings (`\ffname`, `\ffdeftext`, `\ffhelptext`, `\ffl`) -- not RTF nonterminals the spec names, and no ordering constraint between or within them is spec-mandated: a real producer is free to interleave the two (LibreOffice's own exporter does, emitting a `{\*\ffname ...}` destination before its `\ffownhelp` flag). The order used here is fftype, ffownhelp, ffprot, ffhaslistbox, ffdefres, ffres, then ffname, ffdeftext, ffhelptext, then the ffl entries -- chosen for this writer's own internal consistency and determinism, not extracted from any spec production. Built as separate fragments -- `ffOwnHelpFragment` and `ffProtFragment` computed first, ahead of the controlType-specific block, precisely because this writer's own chosen order places both of them before every field that block decides; `controlTypeParams` for that block itself (a checkbox/dropDown's ffdefres+ffres pair, a dropdown's own ffhaslistbox and ffl entries); one variable per string-destination member (a plainText's ffdeftext, the shared ffname) -- and only concatenated into this writer's own stated sequence in the `return` below, once every fragment is known. This is a correction, not merely a description: an earlier version of this function built every formparams-shaped member into one running string via sequential `+=` calls in whatever order its own controlType branch happened to run, which put ffprot/ffownhelp (mutated only after that branch returned) after ffhaslistbox/ffdefres/ffres despite this same comment already claiming a fixed order -- the fragment split here is what actually makes that claim true, verified against the raw emitted bytes in write.test.ts's own formfield-payload-order assertions.
