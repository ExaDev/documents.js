...
//
// RTF code page 932: lead byte -> a dense 256-character string indexed by the trail byte (0x00-0xFF), the decoded character for that lead+trail pair, or U+FFFD where the page leaves that pair undefined.
...
// Bytes in 0x80-0xFF that stand alone rather than leading a pair for code page 932 (932's halfwidth katakana range and a handful of others), a dense 128-character string indexed by byte-0x80, U+FFFD where that byte is instead a lead byte or genuinely undefined.
export const SINGLE_BYTE_EXTRAS =
