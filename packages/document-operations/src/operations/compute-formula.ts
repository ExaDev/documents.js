...
// ExaDev/documents.js#928: document-compute.js's evaluate() had no consumer anywhere in the family. This operation is the first one -- reading a document's own embedded formulas (ContentFormula.content, the MathExpression semantic layer beside its LaTeX/MathML presentation) and evaluating each through document-compute.js, so a caller can ask "does this document's arithmetic actually check out" without hand-walking the schema itself. The walk over every ContentDocument arm a formula can travel through -- wordprocessing/presentation/drawing block flow (table cells included), the spreadsheet embeddedObjects array, the standalone 'formula' kind, and, at any depth beneath any of those, a non-formula embedded object's own nested document -- is documents.js's own collectDocumentFormulas (src/model/formula.ts): the same shared function src/latex/lint.ts's coherence lint consumes, so this operation and that lint can never silently diverge on which formulas a document actually carries.
...
// Derived from ContentDocumentSchema's own discriminated-union members rather than hand-listed, so deleting or renaming a ContentDocument kind is caught here at the type level (documentKind's own z.enum stops accepting the removed literal, and every caller of it fails to typecheck) instead of surfacing only at runtime -- the same drift risk collectDocumentFormulas's own exhaustiveness guard (documents.js's src/model/formula.ts) already closes for its switch.
...
const ComputeFormulaInputSchema = z.object({
...
export const computeFormulaOperation = defineOperation({
  ...
  inputSchema: ComputeFormulaInputSchema,
  ...
  async run({ source, bindings }, context) {
    ...
    const formulas = entries.map((entry, index) => ({
      ...
      // entry.symbolTable is the GOVERNING table for this specific formula (documents.js's src/model/formula.ts, resolved nested-first) -- never the outermost document's own symbolTable, which for a formula nested inside another embedded object's document would resolve its symbol/unit ids against the wrong curation entirely.
