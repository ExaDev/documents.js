...
Rendering a page, or one region of one, to pixels — for OCR, a vision model, or a thumbnail — through a rasteriser the caller supplies ([#1198](https://github.com/ExaDev/documents.js/issues/1198)):

```ts
import { renderPdfPage } from "pdf-codec";
import { createCpuRasteriser } from "pdf-raster-cpu"; // the pure-software reference backend
...
`renderPdfPage(pdfBytes, pageIndex, options, rasteriser)` walks the page's content through the same read machinery `readPdf` runs (the same interpreter, the same image recovery), transforms everything into a top-left-origin device space at the requested `scale` (or `dpi` — a PDF point is 1/72 inch, so `dpi: 144` is `scale: 2`), and drives the rasteriser with positioned draw operations: rectangle fills, filled/stroked vector paths (ellipses, arbitrary curves, and glyph outlines), and affine-placed images. It returns whatever the rasteriser's `finish()` produces — PNG bytes with the reference backend. The deliberate design point, from the issue's own clarification: **this package defines the `PageRasteriser` interface and takes an implementation; it does not pick a rasteriser or depend on any canvas API.** A runtime with no canvas at all (Cloudflare Workers has neither `canvas` nor `OffscreenCanvas`) and a runtime with a free hardware-accelerated one consume the same port, and the weight of a real raster backend lands only in the backend package a consumer installs — [`pdf-raster-cpu`](../pdf-raster-cpu/README.md) is this family's pure-software reference implementation, a browser consumer can wrap its own 2D canvas, and a Cloudflare consumer can drive Browser Rendering. No OCR, no vision, no network calls anywhere: the port hands a caller pixels and stops.

`options.clipPt` names the region to render, in the same point coordinates `LayoutFrame` (and `LayoutPage` items, and `document-outline.js`'s `segmentPdfRegions` region bounds) use — PDF user space relative to the visible region's lower-left corner, y increasing upward. A caller that located a figure via `segmentPdfRegions` passes `region.bounds` straight through as `clipPt` and gets exactly that figure's pixels, re-origined at the image's own top-left and scaled; `src/raster.test.ts` pins the coordinate agreement against `readPdf` itself (including a rotated, cropped page), so the clip contract is a tested fact rather than a hope. Text render order, fill rules, and optional-content visibility follow what a viewer displays: items in a layer the default configuration leaves `OFF` are not drawn.

**Text renders through the page's own embedded sfnt outlines, or not at all.** Each text run's embedded TrueType program (`/Type0` + `/Identity-H` + `/CIDFontType2`, the dominant mainstream shape, or a simple `/TrueType` font with a usable cmap) is opened with this package's own `glyf`/`cmap` readers and handed to the rasteriser as filled outline paths — a backend needs no font machinery of any kind. Faces that state no sfnt outlines are refused through named diagnostics (`raster/text-cff-outlines` for CFF programs, `raster/text-outlines-unavailable` for standard-14/unembedded fonts, Type 3 glyph procedures, and unreadable programs) rather than approximated with a substitute shape; the rest of the page still renders. Per-glyph placement follows the run's own text matrices, with one documented approximation: character-, word-, and horizontal-spacing state (`Tc`/`Tw`/`Tz`) is not modelled per glyph, and the residual is distributed uniformly across the run so its total extent always matches what the page draws.

The generic byte- and image-container primitives (`crc32`, `deflate`/`inflate`/`inflateTolerant`, `ByteWriter`/`concatBytes`, `readJpegInfo`, `decodePng`/`encodePng`, `unfilterScanlines`/`filterScanlines`) are re-exported from [byte-codec](../byte-codec/README.md). The PDF-specific image codecs (`decodeCcittFax`, `decodeJbig2Embedded`, `decodeJpeg2000`/`readJpeg2000Metadata`/`parseJp2Container`) stay here.
...
- **The codec itself, importing only `layout`/`bytes`/`image`/`crypto`/`util` plus `document-schema.js`'s port types (no OOXML or ODF knowledge):**
  ...
  - **sfnt font tables**: `sfnt.ts` (bounds-checked table-directory reader), `cmap-table.ts` (character code → glyph ID, formats 0/4/6/12, exposed both as the best Unicode lookup and as every subtable individually, since a (3, 0) or (1, 0) subtable is keyed by a font's own codes rather than by code points), `hmtx-table.ts` (per-glyph advance widths), `font-tables.ts` (`head`/`maxp`/`OS/2`/`post`/`name`, including `post`'s own glyph names where a font still carries them), `glyf.ts` (`loca` offset index, per-glyph headers, composite component records, `glyphInkBounds`), `glyf-contours.ts` (the point arrays glyf.ts deliberately leaves undecoded: a glyph's on/off-curve contours, composites resolved, consumed by the raster port's outline rendering and differentially tested against the header-declared ink boxes), `math-table.ts` (OpenType `MATH` constants/glyph-info/variants subtables). `ot-layout-common.ts` (Coverage/ClassDef, stored as sorted glyph ranges searched by bisection). `gpos-table.ts` reads `GPOS` for exactly one thing: pair-advance kerning through the `kern` feature, both PairPos formats and LookupType 9 Extension indirection — mark attachment, cursive joining, and contextual positioning have no consumer here. `gsub-table.ts` reads `GSUB` for the default-on feature set (`liga`/`rlig`/`calt`/`clig`) through Single, Ligature, Contextual, and Chaining Contextual substitutions (LookupTypes 1, 4, 5, 6, and the 7 Extension wrapper), with each lookup's `lookupFlag` skip semantics — ignore-base/ligature/marks, mark attachment classes, mark filtering sets — evaluated against `gdef-table.ts`'s reading of the font's `GDEF`; `smcp`/`dlig` stay opt-in through `GsubShaperOptions`, the switch a real shaper's caller flips. Every parser degrades to `undefined` on a missing/truncated table rather than throwing.
  ...
  - **Raster**: `raster.ts` — the `PageRasteriser` port and `renderPdfPage` (#1198): re-walks one page through `interpretContentStream` (the same interpreter `readPdf` runs), recomputes the page's crop/rotation geometry (test-pinned to agree with `readPdf` bit for bit, since the `clipPt` contract rides on it), and drives a caller-supplied rasteriser with device-space draw ops — rect fills, filled/stroked vector paths, affine-placed images, and text as filled glyph-outline paths resolved from the page's own embedded sfnt programs (`glyf-contours.ts`'s decoded contours through the run's text matrices, per-glyph advances corrected against the run's exact end matrix). Optional-content layers the default configuration leaves OFF are not drawn; faces with no sfnt outlines are refused by name. No canvas dependency, no backend, no OCR/vision/network — the port is pure data plus callbacks, so `pdf-raster-cpu` (or a browser canvas wrapper) implements it elsewhere.
...
- **The write side re-encodes bilevel images as CCITT Group 4** (`src/image/ccitt-encode.ts`, a hand-written ITU-T T.6 encoder sharing the decoder's own code-table transcription): a grayscale image whose samples are all 0 or 255, with no soft mask, is written as `/CCITTFaxDecode` with `/K -1` whenever the G4 encoding comes out smaller than Flate over the same pixels — which for vertically coherent bilevel content (a scan's edges and text baselines) it does by an order of magnitude. Whichever encoding is smaller wins, deterministically, so decorrelated or repetitive bilevel content where Flate happens to win keeps Flate. **JBIG2 and JPEG 2000 re-encoding remain permanent boundaries, not gaps**: a hand-written JBIG2 encoder is research-grade symbol-dictionary design and a JPEG 2000 encoder is the full EBCOT/wavelet stack, and re-encoding lossy JPX back is not fidelity-preserving anyway. **What no longer follows from that boundary is generation loss on a pdf-to-pdf round trip**: `LayoutImageAsset.original` (the layout twin of `document-schema.js`'s `ContentImageOriginal`, landed through the #975-scorecard schema change it named) carries a source's own JBIG2/JPX compressed stream beside the decoded canonical, captured at read time together with the decoded `/JBIG2Globals` segments a symbol-dictionary stream needs, and the writer re-embeds those bytes verbatim under their own filter — the decoded canonical never reaches the file at all for such an image, a source soft mask still re-emits as a generated `/SMask`, and the round trip pays zero generation loss for exactly the two filters this package cannot re-encode.
...
- **`renderPdfPage` renders vectors, images, and embedded-sfnt text; everything else it refuses by name.** Text whose font states no TrueType/glyf outlines (CFF programs, standard-14 faces with nothing embedded, Type 3 glyph procedures, Type0 fonts under a non-Identity-H encoding) draws nothing and reports `raster/text-cff-outlines` or `raster/text-outlines-unavailable` through the sink — a substitute shape would be a silent misrender, so the refusal is the honest behaviour and the rest of the page still renders. Image pixels are the rasteriser's own concern: the port hands through exactly what `images-read.ts` recovered (PNG for every decoded filter, JPEG bytes verbatim), so a canvas-owning backend draws DCTDecode scans natively while `pdf-raster-cpu` declines them by name (it has no JPEG decoder, and a scanned page's XObject remains extractable through `readPdf` for consumers that want the scan's own pixels). Per-glyph text placement distributes the interpreter's spacing state (`Tc`/`Tw`/`Tz`) uniformly across each run rather than modelling it per glyph, so a run's extent is exact while its internal glyph spacing under active spacing state is a bounded approximation.
...
- **`GPOS` pair kerning and `GSUB`'s default-on feature set (`liga`/`rlig`/`calt`/`clig`) are read and applied for an embedded face; the opt-in layout features are not — until a caller names them.** An `fi` in a face whose `liga` feature declares the ligature draws as that real ligature glyph (advances, kerning, subsetting, and the ToUnicode CMap all describing the one substituted sequence — a ligature's ToUnicode entry maps back to its whole character run, so copy/paste recovers `fi`, not one glyph's worth of it). The contextual features apply for real: Carlito's `calt` is a pair of Chaining Contextual format 3 lookups carrying the Greek uppercase accent discipline, and they shape `ΆΙ` to Alpha + Iotadieresis exactly as HarfBuzz does. Every lookup's `lookupFlag` is honoured against the font's `GDEF` (which `gdef-table.ts` reads for glyph classes, mark attachment classes, and mark glyph sets): Caladea Italic's one `liga` lookup carries the ignore-marks flag, so its `fi`/`fl` ligatures form across combining marks, the mark drawing after the ligature glyph and its text joining the ligature's ToUnicode destination — the cluster merge a real shaper performs. Still refused by default: the opt-in features (`smcp`, `dlig` and friends), which no real shaper turns on until a caller asks — `buildGsubShaper`'s `optInFeatures` option is that ask. The legacy `kern` table is not read either (neither Carlito nor Caladea ships one). A kerned run is shown with `TJ`, and the sign of a `TJ` number is the opposite of the adjustment it expresses (ISO 32000-1 9.4.3: a positive number moves the next glyph closer). A run with no kerning pairs stays as one unsplit hex string with `Tj`.
...
**The acceptance bar for embedded-font fidelity is "no page-count drift on a real corpus", not "line-identical".** An embedded run is placed at real `hmtx` advances adjusted by real `GPOS` pair kerning, shaped through real `GSUB` ligatures and contextual alternates — measured and drawn from one shared computation. What separates this from line-identical: the opt-in layout features are never applied unless a caller names them, and kerning is within each shown string rather than across whitespace boundaries a wrap decision measures separately.
