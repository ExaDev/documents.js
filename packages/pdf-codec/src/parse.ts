...
export function parseValue(
  ...
  switch (token.kind) {
    ...
    case "dictEnd":
      sink({
        code: "pdf/unexpected-delimiter",
        severity: "warning",
        message: `unexpected "${token.kind === "arrayEnd" ? "]" : ">>"}" where a value was expected`,
      });
      return pdfNull();
  }
}
...
function parseKeywordValue(
  ...
  sink({
    code: "pdf/unexpected-keyword",
    severity: "warning",
    message: `unexpected keyword "${keyword}" where a value was expected`,
  });
  return pdfNull();
}
...
function parseArrayBody(
  ...
  for (;;) {
    ...
    if (token === undefined) {
      sink({
        code: "pdf/unterminated-array",
        severity: "warning",
        message: "array not closed before end of input",
      });
      break;
    }
...
function parseDictOrStreamBody(
  reader: ByteReader,
  sink: PdfDiagnosticSink,
): PdfObject {
  ...
  for (;;) {
    ...
    if (token === undefined) {
      sink({
        code: "pdf/unterminated-dict",
        severity: "warning",
        message: "dictionary not closed before end of input",
      });
      break;
    }
    ...
    if (token.kind !== "name") {
      sink({
        code: "pdf/dict-key-not-name",
        severity: "warning",
        message: `expected a /Name dictionary key, found "${token.kind}"`,
      });
      continue;
    }
    const value = parseValue(reader, sink);
    if (value === undefined) {
      sink({
        code: "pdf/dict-missing-value",
        severity: "warning",
        message: `dictionary key "/${token.value}" has no value before end of input`,
      });
      break;
    }
...
function skipStreamDataStart(
  ...
  if (reader.peek() === 0x0d) {
    ...
    if (reader.peek() === 0x0a) {
      ...
      sink({
        code: "pdf/stream-bad-eol",
        severity: "warning",
        message:
          "stream keyword followed by a bare CR, not CRLF or LF as required",
      });
    }
    return;
  }
  ...
  sink({
    code: "pdf/stream-bad-eol",
    severity: "warning",
    message: "stream keyword not followed by an end-of-line marker",
  });
}
...
function parseStreamBody(
  ...
  if (dataEnd === undefined) {
    if (lengthObj !== undefined) {
      sink({
        code: "pdf/stream-length-invalid",
        severity: "warning",
        message:
          'stream /Length was missing, indirect, or did not land on "endstream"; falling back to a scan for "endstream"',
      });
    }
    const found = findEndstreamOffset(reader, dataStart);
    if (found === undefined) {
      sink({
        code: "pdf/stream-unterminated",
        severity: "warning",
        message:
          'no "endstream" found before end of input; treating the remainder of the file as stream data',
      });
  ...
  if (endTok?.kind !== "keyword" || endTok.value !== "endstream") {
    sink({
      code: "pdf/missing-endstream",
      severity: "warning",
      message: 'stream data was not followed by "endstream"',
    });
  }
...
function resolveDirectLength(
  ...
  if (
    lengthObj?.kind !== "number" ||
    !Number.isInteger(lengthObj.value) ||
    lengthObj.value < 0
  ) {
    return undefined;
  }
  ...
  if (
    candidateEnd > reader.length ||
    !looksLikeEndstreamAt(reader, candidateEnd)
  ) {
    return undefined;
  }
  return candidateEnd;
...
function trimTrailingStreamEol(
  reader: ByteReader,
  dataStart: number,
  endstreamOffset: number,
): number {
  const precedingTwo = reader.slice(
    Math.max(dataStart, endstreamOffset - 2),
    endstreamOffset,
  );
  if (
...
export function parseIndirectObject(
  ...
  if (value === undefined) {
    sink({
      ...
      severity: "warning",
      message: `object ${String(numTok.value)} ${String(genTok.value)} has no value before end of input`,
    });
  ...
  if (endTok?.kind !== "keyword" || endTok.value !== "endobj") {
    sink({
      code: "pdf/missing-endobj",
      severity: "warning",
      message: `object ${String(numTok.value)} ${String(genTok.value)} is missing its "endobj" keyword`,
    });
