...
describe("readFontFace on real vendored fonts", () => {
  it("reads Carlito Regular as an upright, non-bold face", () => {
    expect(readFontFace(carlitoRegularBytes(), "Carlito-Regular.ttf")).toEqual({
      family: "Carlito",
      bold: false,
      italic: false,
    });
  ...
  it("reads Carlito Bold", () => {
    expect(readFontFace(carlitoBoldBytes(), "Carlito-Bold.ttf")).toEqual({
      family: "Carlito",
      bold: true,
      italic: false,
    });
  ...
  it("reads Carlito Italic", () => {
    expect(readFontFace(carlitoItalicBytes(), "Carlito-Italic.ttf")).toEqual({
      family: "Carlito",
      bold: false,
      italic: true,
    });
  ...
  it("reads Caladea Regular, a different vendored family with its own name table", () => {
    expect(readFontFace(caladeaRegularBytes(), "Caladea-Regular.ttf")).toEqual({
      family: "Caladea",
      bold: false,
      italic: false,
    });
  ...
  it("reads Caladea Italic", () => {
    expect(readFontFace(caladeaItalicBytes(), "Caladea-Italic.ttf")).toEqual({
      family: "Caladea",
      bold: false,
      italic: true,
    });
...
describe("readFontFace style-bit precedence", () => {
  it("falls back to 'head' macStyle when the font declares no 'OS/2' table at all", () => {
    const font = buildSfnt(
      new Map([
        ...
        [
          "head",
          buildHeadTableBytes(HEAD_MAC_STYLE_BOLD | HEAD_MAC_STYLE_ITALIC),
        ],
      ]),
    );
    expect(readFontFace(font, "mac-only.ttf")).toEqual({
      family: "MacOnly",
      bold: true,
      italic: true,
    });
  ...
  it("reads a regular face's macStyle as neither bold nor italic", () => {
    ...
    expect(readFontFace(font, "mac-only-regular.ttf")).toEqual({
      family: "MacOnlyRegular",
      bold: false,
      italic: false,
    });
  });
  ...
  it("reads 'OS/2' fsSelection directly when the font also carries a 'head' table", () => {
    ...
    expect(readFontFace(font, "bold-from-os2.ttf")).toEqual({
      family: "BoldFromOs2",
      bold: true,
      italic: false,
    });
...
describe("readFontFace error handling", () => {
  ...
  it("gives a TrueType Collection its own actionable message rather than a generic parse failure", () => {
    ...
    expect(() => readFontFace(ttc, "bundle.ttc")).toThrow(
      /TrueType Collection/,
    );
  });
  ...
  it("throws when the font declares no usable family name in its own name table", () => {
    ...
    expect(() => readFontFace(font, "nameless.ttf")).toThrow(
      FontFaceParseError,
    );
  ...
  it("throws when the font has a family name but neither a readable 'OS/2' nor a readable 'head' table", () => {
    const font = buildSfnt(
      new Map([["name", buildFamilyNameTableBytes("NoStyleTables")]]),
    );
    expect(() => readFontFace(font, "no-style-tables.ttf")).toThrow(
      FontFaceParseError,
    );
    expect(() => readFontFace(font, "no-style-tables.ttf")).toThrow(
      /weight and slope/,
