...
export function parseDestination(
  ...
  const resolvedPageIndex =
    pageEl?.kind === "number"
      ? Number.isInteger(pageEl.value) && pageEl.value >= 0
        ? pageEl.value
        : undefined
      : pageIndex(pageEl);
  if (resolvedPageIndex === undefined) {
    sink({
      ...
      severity: "warning",
      message:
        "a destination names a page that is not in the document's page tree; skipping it",
    });
    return undefined;
  }
  ...
  if (type === "FitH") {
    return {
      pageIndex: resolvedPageIndex,
      target: { kind: "fitH", ...optionalCoords(arr, [["topPt", 2]]) },
    };
  }
  if (type === "FitV") {
    return {
      pageIndex: resolvedPageIndex,
      target: { kind: "fitV", ...optionalCoords(arr, [["leftPt", 2]]) },
    };
  }
  if (type === "FitR") {
    return {
  ...
  if (type === "FitBH") {
    return {
      pageIndex: resolvedPageIndex,
      target: { kind: "fitBH", ...optionalCoords(arr, [["topPt", 2]]) },
    };
  }
  if (type === "FitBV") {
    return {
      pageIndex: resolvedPageIndex,
      target: { kind: "fitBV", ...optionalCoords(arr, [["leftPt", 2]]) },
    };
  }
  sink({
    code: "pdf/destination-invalid",
...
type DestinationCoordinateField = Exclude<
  keyof LayoutDestinationTarget,
  "kind"
>;

function optionalCoords(
  arr: readonly PdfObject[],
  fields: readonly (readonly [DestinationCoordinateField, number])[],
): Partial<LayoutDestinationTarget> {
...
export function createDestinationRegistry(
  ...
  const add = (
    name: string,
    parsed: { pageIndex: number; target: LayoutDestinationTarget },
  ): void => {
  ...
  for (const entry of walkNameTree(
    namesRoot === undefined ? undefined : dictGet(namesRoot, "Dests"),
    resolver,
    sink,
  )) {
...
export function readOutline(
  ...
  const first =
    root === undefined
      ? undefined
      : resolver.resolveDict(dictGet(root, "First"));
  if (first === undefined) {
    return [];
  }
