...
const HELVETICA_REGULAR = {
  family: "Arial",
  weight: "normal",
  style: "normal",
} as const;
const CALIBRI_REGULAR = {
  family: "Calibri",
  weight: "normal",
  style: "normal",
} as const;
...
describe("createStandardFontMeasurer", () => {
  ...
  it('matches the known Helvetica "A" width at size 1000 (667 units)', () => {
    const measurer = createStandardFontMeasurer();
    expect(
      measurer.widthOfTextAtSize("A", HELVETICA_REGULAR, 1000),
    ).toBeCloseTo(667, 6);
  });

  it("applies the default Calibri width correction, making it measurably narrower than Helvetica", () => {
    const measurer = createStandardFontMeasurer();
    const helveticaWidth = measurer.widthOfTextAtSize(
      "Hello World",
      HELVETICA_REGULAR,
      12,
    );
    const calibriWidth = measurer.widthOfTextAtSize(
      "Hello World",
      CALIBRI_REGULAR,
      12,
    );
  ...
  it("a custom correction table overrides the default", () => {
    const measurer = createStandardFontMeasurer({
      widthCorrectionByFamily: { calibri: 1 },
    });
    const helveticaWidth = measurer.widthOfTextAtSize(
      "Hello",
      HELVETICA_REGULAR,
  ...
  it("lineHeightAtSize/ascenderAtSize/descenderAtSize scale with size and match known Helvetica ratios", () => {
    const measurer = createStandardFontMeasurer();
    expect(measurer.lineHeightAtSize(HELVETICA_REGULAR, 100)).toBeCloseTo(
      115,
      6,
    ); // 1.150 em
    expect(measurer.ascenderAtSize(HELVETICA_REGULAR, 1000)).toBeCloseTo(
      718,
      6,
    );
    expect(measurer.descenderAtSize(HELVETICA_REGULAR, 1000)).toBeCloseTo(
      -207,
      6,
    );
  });
  ...
  it("an unknown family defaults to no correction (scale 1.0)", () => {
    const measurer = createStandardFontMeasurer();
    expect(
      measurer.horizontalScaleFor({
        family: "SomeUnknownFont",
        weight: "normal",
        style: "normal",
      }),
    ).toBe(1);
...
function houseRegistry(
  options: { readonly verticalMetrics?: VerticalMetricPolicy } = {},
): TextMeasurer {
...
describe("createFontMeasurer: VerticalMetricPolicy on an embedded face", () => {
  it("defaults to hhea, matching how STANDARD_METRICS.lineHeightEm was itself derived", () => {
    ...
    expect(measurer.ascenderAtSize(HOUSE_SANS, 1000)).toBeCloseTo(
      glyphSpace(1950, CARLITO_UNITS_PER_EM),
      10,
    );
    expect(measurer.descenderAtSize(HOUSE_SANS, 1000)).toBeCloseTo(
      glyphSpace(-550, CARLITO_UNITS_PER_EM),
      10,
    );
    expect(measurer.lineHeightAtSize(HOUSE_SANS, 1000)).toBeCloseTo(
      glyphSpace(1950 + 550, CARLITO_UNITS_PER_EM),
      10,
    );
  });
  ...
  it("'os2Typo' reads OS/2's own sTypoAscender/sTypoDescender/sTypoLineGap instead", () => {
    ...
    expect(measurer.ascenderAtSize(HOUSE_SANS, 1000)).toBeCloseTo(
      glyphSpace(1536, CARLITO_UNITS_PER_EM),
      10,
    );
    expect(measurer.descenderAtSize(HOUSE_SANS, 1000)).toBeCloseTo(
      glyphSpace(-512, CARLITO_UNITS_PER_EM),
      10,
    );
    expect(measurer.lineHeightAtSize(HOUSE_SANS, 1000)).toBeCloseTo(
      glyphSpace(1536 + 512 + 452, CARLITO_UNITS_PER_EM),
      10,
    );
    // Genuinely different from hhea's split, not a coincidentally equal reading of the same fields.
    expect(measurer.ascenderAtSize(HOUSE_SANS, 1000)).not.toBeCloseTo(
      houseRegistry().ascenderAtSize(HOUSE_SANS, 1000),
      3,
    );
  });
  ...
  it("'os2Win' reads usWinAscent/usWinDescent, negating the descent and adding no line gap", () => {
    ...
    expect(measurer.ascenderAtSize(HOUSE_SERIF, 1000)).toBeCloseTo(
      glyphSpace(1050, CALADEA_UNITS_PER_EM),
      10,
    );
    // usWinDescent is declared as a positive 250; every descent this interface reports is negative.
    expect(measurer.descenderAtSize(HOUSE_SERIF, 1000)).toBeCloseTo(
      glyphSpace(-250, CALADEA_UNITS_PER_EM),
      10,
    );
    expect(measurer.lineHeightAtSize(HOUSE_SERIF, 1000)).toBeCloseTo(
      glyphSpace(1050 + 250, CALADEA_UNITS_PER_EM),
      10,
    );
    // Caladea's hhea set is 900/-250/0, so this is a genuinely looser line than the default policy gives.
    expect(measurer.lineHeightAtSize(HOUSE_SERIF, 1000)).toBeGreaterThan(
      houseRegistry().lineHeightAtSize(HOUSE_SERIF, 1000),
    );
  });
  ...
  it("the policy never touches a standard-14 resolution, which has only one metric set to read", () => {
    for (const policy of ["hhea", "os2Typo", "os2Win"] as const) {
      const measurer = createFontMeasurer(undefined, {
        verticalMetrics: policy,
      });
      expect(measurer.lineHeightAtSize(HELVETICA_REGULAR, 100)).toBeCloseTo(
        115,
...
function withoutOs2Table(
  ...
  stripped.set(bytes);
  const removedAt =
    TABLE_DIRECTORY_HEADER_SIZE + recordIndex * TABLE_RECORD_SIZE;
  const directoryEnd =
