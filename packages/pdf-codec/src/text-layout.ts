...
function fragmentsWidth(
  fragments: readonly StyledFragment[],
  measurer: TextMeasurer,
): number {
  let total = 0;
...
function splitTextToWidth(
  ...
  measurer: TextMeasurer,
  maxWidthPt: number,
): { fitText: string; restText: string } {
...
function splitBoxToWidth(
  ...
  for (let idx = 0; idx < atom.fragments.length; idx++) {
    ...
    const { fitText, restText } = splitTextToWidth(
      fragment.text,
      fragment.font,
      fragment.sizePt,
      measurer,
  ...
  for (const atom of atoms) {
    if (atom.kind === "box") {
      for (const fragment of atom.fragments) {
        ...
        xOffsetPt += measurer.widthOfTextAtSize(
          fragment.text,
          fragment.font,
          fragment.sizePt,
        );
        ...
        ascentPt = Math.max(
          ascentPt,
          measurer.ascenderAtSize(fragment.font, fragment.sizePt),
        );
        descentPt = Math.min(
          descentPt,
          measurer.descenderAtSize(fragment.font, fragment.sizePt),
        );
      }
    } else {
...
export function wrapRunsToWidth(
  ...
  if (maxWidthPt <= 0) {
    ...
    return atoms.length === 0
      ? [buildEmptyLine(runs, measurer)]
      : [buildLine(atoms, measurer)];
  }
