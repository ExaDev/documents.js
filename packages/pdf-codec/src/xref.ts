...
export function readXref(
  ...
  sink({
    ...
    severity: "warning",
    message:
      'the document\'s own cross-reference data was missing, unreadable, or did not check out against the bytes it points at; rebuilt the table by scanning the file for "N G obj" headers',
  });
...
function looksLikeObjectHeaderAt(
  bytes: Uint8Array<ArrayBuffer>,
  offset: number,
  expectedNum: number,
): boolean {
...
function readClassicXrefSection(
  reader: ByteReader,
  sink: PdfDiagnosticSink,
): XrefSection {
  ...
  for (;;) {
    ...
    for (let i = 0; i < countTok.value; i++) {
      ...
      if (
        offsetTok?.kind !== "number" ||
        genTok?.kind !== "number" ||
        typeTok?.kind !== "keyword"
      ) {
        sink({
          code: "pdf/xref-entry-invalid",
          severity: "warning",
...
function isBoundaryByte(byte: number | undefined): boolean {
  return (
    byte === undefined || isAsciiWhitespace(byte) || DELIMITER_BYTES.has(byte)
  );
}

function findAllKeywordPositions(
  bytes: Uint8Array<ArrayBuffer>,
  needle: Uint8Array<ArrayBuffer>,
): number[] {
...
function scanBackWhile(
  ...
  predicate: (byte: number | undefined) => boolean,
): number {
  let i = end;
...
): { num: number; gen: number; headerStart: number } | undefined {
...
function recoverXrefByLinearScan(
  bytes: Uint8Array<ArrayBuffer>,
  sink: PdfDiagnosticSink,
): XrefTable {
  const entries = new Map<number, XrefEntry>();
  for (const objPos of findAllKeywordPositions(bytes, OBJ_BYTES)) {
    ...
    entries.set(header.num, {
      ...
      gen: header.gen,
    }); // later (later-in-file) headers naturally override earlier ones for the same object number, mirroring newest-revision-wins without needing an explicit /Prev chain
  }
  ...
  for (const [num, entry] of entries) {
    ...
    if (
      indirect.value.kind === "stream" &&
      isName(dictGet(dict, "Type"), "ObjStm")
    ) {
      registerObjStmContents(
        num,
        indirect.value.dict,
        indirect.value.raw,
        entries,
  ...
  const trailer =
    findRecoveredTrailer(bytes, sink) ??
    lastXrefTrailerDict ??
    (lastCatalogRef !== undefined
      ? pdfDict({ Root: lastCatalogRef })
      : pdfDict({}));
  return { entries, trailer };
}

function registerObjStmContents(
  streamObjNum: number,
  dict: PdfDict,
  raw: Uint8Array<ArrayBuffer>,
