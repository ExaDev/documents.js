...
describe("parseValue: scalars", () => {
  it("reads null, true, and false", () => {
    expect(parseAllValues("null true false").values).toEqual([
      { kind: "null" },
  ...
  it("reads a plain integer or real number", () => {
    expect(parseAllValues("42").values).toEqual([
      { kind: "number", value: 42 },
    ]);
    expect(parseAllValues("-3.14").values).toEqual([
      { kind: "number", value: -3.14 },
    ]);
  });

  it("reads a name", () => {
    expect(parseAllValues("/Catalog").values).toEqual([
      { kind: "name", name: "Catalog" },
    ]);
  });

  it("reads a literal string and a hex string", () => {
    const { values } = parseAllValues("(Hello) <48656c6c6f>");
    expect(stringBytes(values[0])).toEqual(
      Array.from(new TextEncoder().encode("Hello")),
    );
    expect(stringBytes(values[1])).toEqual(
      Array.from(new TextEncoder().encode("Hello")),
    );
  });
...
describe('parseValue: "N G R" reference disambiguation', () => {
  it('reads "5 0 R" as a reference, not three separate numbers', () => {
    expect(parseAllValues("5 0 R").values).toEqual([
      { kind: "ref", num: 5, gen: 0 },
    ]);
  });
  ...
  it("never attempts the reference lookahead for a non-integer or negative number, even when a valid two-number-plus-R pattern would otherwise follow", () => {
    expect(parseAllValues("3.5 4 R").values).toEqual([
      { kind: "number", value: 3.5 },
      { kind: "number", value: 4 },
      { kind: "null" },
    ]);
    expect(parseAllValues("-1 2 R").values).toEqual([
      { kind: "number", value: -1 },
      { kind: "number", value: 2 },
      { kind: "null" },
    ]);
...
describe("parseValue: arrays", () => {
  ...
  it("recovers from a missing closing bracket with a diagnostic", () => {
    ...
    expect(diagnostics.some((d) => d.code === "pdf/unterminated-array")).toBe(
      true,
    );
  });
});
...
describe("parseValue: dictionaries", () => {
  ...
  it("recovers from a non-name key with a diagnostic and keeps parsing subsequent, well-formed key-value pairs", () => {
    ...
    expect(diagnostics.some((d) => d.code === "pdf/dict-key-not-name")).toBe(
      true,
    );
  });
  ...
  it('recovers from a missing closing ">>" with a diagnostic', () => {
    ...
    expect(diagnostics.some((d) => d.code === "pdf/unterminated-dict")).toBe(
      true,
    );
  });
});
...
describe("parseValue: streams", () => {
  it('reads a stream whose direct numeric /Length lands exactly on "endstream"', () => {
    ...
    expect(Array.from(value.raw)).toEqual(
      Array.from(new TextEncoder().encode("Hello")),
    );
  ...
  it('falls back to scanning for "endstream" when /Length is wrong', () => {
    ...
    expect(
      diagnostics.some((d) => d.code === "pdf/stream-length-invalid"),
    ).toBe(true);
  });
  ...
  it('treats the rest of the input as stream data, with a diagnostic, when "endstream" is never found', () => {
    ...
    expect(Array.from(value.raw)).toEqual(
      Array.from(new TextEncoder().encode("Hello, unterminated")),
    );
    expect(diagnostics.some((d) => d.code === "pdf/stream-unterminated")).toBe(
      true,
    );
  });
});
...
describe("parseIndirectObject", () => {
  it('reads "N G obj <value> endobj"', () => {
    ...
    expect(stringBytes(result?.value)).toEqual(
      Array.from(new TextEncoder().encode("Hello")),
    );
  });
  ...
  it("reads an object whose value is a dict-introduced stream", () => {
    ...
    expect(dictGet(result.value.dict, "Length")).toEqual({
      kind: "number",
      value: 5,
    });
    expect(Array.from(result.value.raw)).toEqual(
      Array.from(new TextEncoder().encode("Hello")),
    );
  });
