...
describe('walkNameTree', () => {
  it('returns the name/value pairs of a leaf node in order', () => {
    const root = pdfDict({
      Names: pdfArray([stringObj('alpha'), pdfNum(1), stringObj('beta'), pdfNum(2)]),
    });
    const entries = walkNameTree(root, resolverOf(new Map()), NOOP_DIAGNOSTIC_SINK);
  ...
  it('flattens a Kids chain of intermediate nodes depth-first, preserving document order', () => {
    ...
    const kidB = pdfDict({ Names: pdfArray([stringObj('b1'), pdfNum(2), stringObj('b2'), pdfNum(3)]) });
  ...
  it('resolves indirect references to intermediate nodes', () => {
    const kid = pdfDict({ Names: pdfArray([stringObj('nested'), pdfName('value')]) });
    const root = pdfDict({ Kids: pdfArray([pdfRef(7, 0)]) });
    const entries = walkNameTree(root, resolverOf(new Map([[7, kid]])), NOOP_DIAGNOSTIC_SINK);
    expect(entries).toEqual([{ name: 'nested', value: { kind: 'name', name: 'value' } }]);
  });
  ...
  it('mixes a node\'s own /Names with its /Kids after them, matching the spec\'s ordering rule', () => {
    ...
    const root = pdfDict({ Names: pdfArray([stringObj('own'), pdfNum(1)]), Kids: pdfArray([kid]) });
    const entries = walkNameTree(root, resolverOf(new Map()), NOOP_DIAGNOSTIC_SINK);
  ...
  it('ignores a trailing name with no value pair rather than reading past the array', () => {
    const root = pdfDict({ Names: pdfArray([stringObj('paired'), pdfNum(1), stringObj('dangling')]) });
    const entries = walkNameTree(root, resolverOf(new Map()), NOOP_DIAGNOSTIC_SINK);
    expect(entries).toEqual([{ name: 'paired', value: { kind: 'number', value: 1 } }]);
  });

  it('skips a pair whose key is not a PDF string', () => {
    const root = pdfDict({ Names: pdfArray([pdfName('not-a-string'), pdfNum(1), stringObj('real'), pdfNum(2)]) });
    const entries = walkNameTree(root, resolverOf(new Map()), NOOP_DIAGNOSTIC_SINK);
    expect(entries).toEqual([{ name: 'real', value: { kind: 'number', value: 2 } }]);
  });

  it('returns no entries for an absent root', () => {
    expect(walkNameTree(undefined, resolverOf(new Map()), NOOP_DIAGNOSTIC_SINK)).toEqual([]);
  });
});
