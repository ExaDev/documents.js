...
describe("writePdf -> readPdf: structural round trip", () => {
  ...
  it("recovers an embedded PNG image at its written placement", () => {
    ...
    expect(result.images[image.imageId]).toMatchObject({
      format: "png",
  ...
  it("round-trips through a Flate-compressed content stream too, not just the human-readable form", () => {
    ...
    expect(result.pages[0]!.items).toMatchObject([
      { kind: "text", text: "Compressed" },
    ]);
  });
    ...
    const path: LayoutPath = {
      ...
      subpaths: [
        ...
        },
        {
          ...
          closed: true,
          segments: [
            { kind: "line", xPt: 15, yPt: 5 },
            { kind: "line", xPt: 15, yPt: 15 },
            { kind: "line", xPt: 5, yPt: 15 },
          ],
        },
      ],
    };
  ...
  it("recovers page.notes from the hidden notes annotation, and omits it entirely when absent", () => {
    const withNotes = docWithPages([
      {
        ...
        heightPt: 200,
        items: [],
        notes: "Speaker notes for this page",
      },
    ]);
    ...
    const resultWithoutNotes = readPdf(
      writePdf(withoutNotes, { compress: false }),
    );
    ...
    doc.destinations = [
      {
        name: "page-two",
        pageIndex: 1,
        target: { kind: "xyz", leftPt: 20, topPt: 100 },
      },
    ];
