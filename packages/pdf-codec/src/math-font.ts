...
function measureConstruction(
  ...
  return inkAscent === Number.NEGATIVE_INFINITY
    ? undefined
    : { inkAscent, inkDescent, advanceWidth };
}
...
function stretchAtSize(
  ...
  const construction = (
    axis === "vertical" ? math.variants.vertical : math.variants.horizontal
  ).get(glyphId);
  ...
  const pt = (designUnits: number): number =>
    toPt(designUnits, unitsPerEm, sizePt);
  return {
    kind: assembled.kind,
...
function metricsAtSize(
  ...
  inkBounds: CffGlyphBounds | undefined,
  unitsPerEm: number,
  ascentDesignUnits: number,
  descentDesignUnits: number,
  sizePt: number,
): MathFontMetrics {
  ...
  return {
    ...
    fractionNumeratorDisplayShiftUpPt: pt(
      c.fractionNumeratorDisplayStyleShiftUp,
    ),
    fractionDenominatorShiftDownPt: pt(c.fractionDenominatorShiftDown),
    fractionDenominatorDisplayShiftDownPt: pt(
      c.fractionDenominatorDisplayStyleShiftDown,
    ),
    ...
    glyph(
      ...
      const advanceWidthPt = toPt(
        hmtx.advanceWidth(glyphId),
        unitsPerEm,
        glyphSizePt,
      );
      const italicCorrectionDesignUnits =
        math.glyphInfo.italicsCorrection.get(glyphId) ?? 0;
      const topAccentDesignUnits =
        math.glyphInfo.topAccentAttachment.get(glyphId);
      ...
      return {
        advanceWidthPt,
        italicCorrectionPt: toPt(
          italicCorrectionDesignUnits,
          unitsPerEm,
          glyphSizePt,
        ),
        topAccentXPt:
          topAccentDesignUnits === undefined
            ? undefined
            : toPt(topAccentDesignUnits, unitsPerEm, glyphSizePt),
        inkAscentPt:
          ink === undefined
            ? undefined
            : toPt(ink.yMax, unitsPerEm, glyphSizePt),
        inkDescentPt:
          ink === undefined
            ? undefined
            : toPt(-ink.yMin, unitsPerEm, glyphSizePt),
      };
    },
    stretch(
      codePoint: number,
...
export interface LoadedMathFont {
  ...
  readonly stretchGlyph: (
    codePoint: number,
    axis: MathStretchAxis,
    targetSizePt: number,
    sizePt: number,
  ) => MathStretchConstruction | undefined;
}
...
export function loadMathFont(): LoadedMathFont {
  ...
  const font: MathFont = {
    metrics: metricsAtSize(
      ...
      descentDesignUnits,
      unitsPerEm,
    ), // a 1-unit-per-em-sized metrics object -- exists only to satisfy the MathFont.metrics field's own type; every real caller goes through metricsAt(sizePt) instead
    cffBytes,
    descriptor: {
      unitsPerEm,
  ...
  cached = {
    ...
    stretchGlyph: (
      ...
      return assembled === undefined
        ? undefined
        : scaleMathStretchConstruction(assembled, 1 / designUnitsPerPt);
    },
  };
  return cached;
}
