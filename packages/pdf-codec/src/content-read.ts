...
export type ContentToken =
  | { readonly kind: "operation"; readonly operation: ContentOperation }
  | { readonly kind: "inlineImage"; readonly image: InlineImage };

export function readContentStream(
  ...
  for (;;) {
    ...
    if (
      ...
      tokens.push({
        kind: "operation",
        operation: { operands, operator: peeked.value },
      });
      operands = [];
      continue;
    }
...
function readInlineImage(
  ...
  for (;;) {
    ...
    if (token === undefined) {
      sink({
        code: "pdf/inline-image-truncated",
        severity: "warning",
        message: 'inline image dictionary was truncated before "ID"',
      });
      return undefined;
    }
    ...
    if (token.kind !== "name") {
      sink({
        ...
        severity: "warning",
        message: "inline image dictionary key was not a /Name",
      });
      continue;
    }
    const value = parseValue(reader, sink);
  ...
  const explicitLength = asNumber(
    dictGet(dict, "L") ?? dictGet(dict, "Length"),
  );
  let dataEnd: number;
  if (
    explicitLength !== undefined &&
    looksLikeEiAt(reader, dataStart + explicitLength)
  ) {
    ...
    if (found === undefined) {
      sink({
        ...
        severity: "warning",
        message:
          'no "EI" boundary found for an inline image; treating the remainder of the content stream as its data',
      });
  ...
  if (eiTok?.kind !== "keyword" || eiTok.value !== "EI") {
    sink({
      code: "pdf/inline-image-truncated",
      severity: "warning",
      message: 'inline image data was not followed by "EI"',
    });
  }
  return { dict, data };
}
