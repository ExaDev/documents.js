...
import {
  assembleStretchyGlyph,
  scaleMathStretchConstruction,
} from "./math-stretch";
...
function partsByGlyphId(
  construction: MathGlyphConstruction,
): ReadonlyMap<number, MathGlyphPart> {
...
function expectSoundAssembly(
  result: MathStretchConstruction,
  construction: MathGlyphConstruction,
...
function verticalConstruction(codePoint: number): MathGlyphConstruction {
  const construction = loadMathFont().font.stretchyConstruction(
    codePoint,
    "vertical",
  );
...
function stretch(
  construction: MathGlyphConstruction,
  targetSize: number,
  axis: MathStretchAxis = "vertical",
): MathStretchConstruction {
  const result = assembleStretchyGlyph(construction, {
    axis,
    targetSize,
    minConnectorOverlap: loadMathFont().font.minConnectorOverlap,
  });
...
describe("MathVariants parsing against the real STIX Two Math font", () => {
  ...
  it("reads the left parenthesis's own vertical variant sequence, at strictly increasing sizes starting from the base glyph", () => {
    ...
    expect(construction.variants[0]).toEqual({
      glyphId: loadMathFont().font.glyphId(PAREN),
      advanceMeasurement: 933,
    });
    expect(construction.variants[1]).toEqual({
      glyphId: 1301,
      advanceMeasurement: 1187,
    });
    expect(construction.variants[12]).toEqual({
      glyphId: 1312,
      advanceMeasurement: 3821,
    });
    for (let i = 1; i < construction.variants.length; i++) {
      expect(construction.variants[i]!.advanceMeasurement).toBeGreaterThan(
        construction.variants[i - 1]!.advanceMeasurement,
      );
    }
  });
    ...
    expect(assembly!.parts).toEqual([
      {
        ...
        endConnectorLength: 250,
        fullAdvance: 1273,
        isExtender: false,
      },
      {
        glyphId: 4861,
        startConnectorLength: 1000,
        endConnectorLength: 1000,
        fullAdvance: 1252,
        isExtender: true,
      },
      {
        glyphId: 4860,
        startConnectorLength: 250,
  ...
  it("names glyphs that no Unicode code point reaches, which is why drawing a construction needs glyph IDs rather than text", () => {
    ...
    for (const codePoint of [PAREN, RADICAL]) {
      for (const variant of font.font
        .stretchyConstruction(codePoint, "vertical")!
        .variants.slice(1)) {
        expect(encoded.has(variant.glyphId)).toBe(false);
      }
    }
    for (const variant of font.font
      .stretchyConstruction(OVER_BRACE, "horizontal")!
      .variants.slice(1)) {
      expect(encoded.has(variant.glyphId)).toBe(false);
    }
    ...
    for (const part of font.font.stretchyConstruction(OVER_BRACE, "horizontal")!
      .assembly!.parts) {
      expect(encoded.has(part.glyphId)).toBe(false);
    }
  }, 90_000);
  ...
  it("reads the radical sign's own vertical construction", () => {
    ...
    ).toEqual([1188, 1855, 2371, 2892]);
    expect(construction.assembly!.parts).toEqual([
      {
        glyphId: 1661,
        startConnectorLength: 200,
        endConnectorLength: 200,
        fullAdvance: 1905,
        isExtender: false,
      },
      {
        glyphId: 1664,
        startConnectorLength: 650,
        endConnectorLength: 650,
        fullAdvance: 651,
        isExtender: true,
      },
      {
        glyphId: 1662,
        startConnectorLength: 550,
        endConnectorLength: 0,
        fullAdvance: 642,
        isExtender: false,
      },
  ...
  it("reads the over-brace's own HORIZONTAL construction, and finds nothing for it on the vertical axis", () => {
    ...
    expect(
      construction!.assembly!.parts.map((part) => part.isExtender),
...
describe("assembleStretchyGlyph variant selection", () => {
  it("keeps the base glyph when it is already big enough", () => {
    ...
    expect(result.placements).toEqual([
      { glyphId: loadMathFont().font.glyphId(PAREN), offset: 0, advance: 933 },
    ]);
  });
  ...
  it("picks the SMALLEST pre-built variant that reaches the target, not the largest", () => {
    ...
    expect(result.placements).toEqual([
      { glyphId: 1303, offset: 0, advance: 1667 },
    ]);
  });
...
describe("assembleStretchyGlyph part assembly against the real font", () => {
  it("repeats the parenthesis extender to reach a target no variant covers, with sound seams", () => {
    ...
    expect(result.placements.map((placement) => placement.offset)).toEqual([
      0, 1023, 2025, 3027, 4029, 5031,
    ]);
  ...
  it("lands exactly on a radical target the parts can hit, by widening the seams rather than overshooting", () => {
    ...
    expect(
      result.placements.filter((placement) => placement.glyphId === 1664),
    ).toHaveLength(5);
  ...
  it("assembles a HORIZONTAL over-brace from its two extenders around a fixed middle", () => {
    ...
    expect(result.placements.map((placement) => placement.glyphId)).toEqual([
      2106, 2073, 2073, 2107, 2073, 2073, 2108,
    ]);
    expect(result.size).toBeCloseTo(5000, 9);
  });
...
describe("point-scaled stretching through the loaded font", () => {
  it("stretches to a target expressed in points at a given font size", () => {
    ...
    expect(result!.size).toBeCloseTo(
      (1667 / font.descriptor.unitsPerEm) * 12,
      9,
    );
  ...
  it("returns undefined for a glyph this font does not stretch", () => {
    expect(
      loadMathFont().stretchGlyph(LATIN_X, "vertical", 40, 12),
    ).toBeUndefined();
  });
  ...
  it("scales a design-unit construction into points identically to stretchGlyph", () => {
    const { font, stretchGlyph } = loadMathFont();
    const construction = verticalConstruction(PAREN);
    const inDesignUnits = stretch(
      construction,
      (80 * font.descriptor.unitsPerEm) / 12,
    );
    expect(
      scaleMathStretchConstruction(
        inDesignUnits,
        12 / font.descriptor.unitsPerEm,
      ),
    ).toEqual(stretchGlyph(PAREN, "vertical", 80, 12));
  });
});
...
describe("assembleStretchyGlyph on constructions the real font does not contain", () => {
  ...
  it("falls back to the largest variant when the target is unreachable and there is no assembly", () => {
    const construction: MathGlyphConstruction = {
      variants: [
        { glyphId: 1, advanceMeasurement: 500 },
        { glyphId: 2, advanceMeasurement: 900 },
      ],
    };
    const result = assembleStretchyGlyph(construction, {
      axis: "vertical",
    ...
    expect(result).toEqual({
      ...
      size: 900,
      italicsCorrection: 0,
      placements: [{ glyphId: 2, offset: 0, advance: 900 }],
    });
  });
  ...
  it("places an all-extender recipe at least once, since it has no fixed part to stand alone", () => {
    const construction: MathGlyphConstruction = {
      ...
      assembly: {
        ...
        parts: [
          {
            glyphId: 7,
            startConnectorLength: 400,
            endConnectorLength: 400,
            fullAdvance: 1000,
            isExtender: true,
          },
        ],
    ...
    expect(result).toEqual({
      ...
      size: 1000,
      italicsCorrection: 0,
      placements: [{ glyphId: 7, offset: 0, advance: 1000 }],
    });
  });
        ...
        parts: [
          {
            glyphId: 1,
            startConnectorLength: 0,
            endConnectorLength: 300,
            fullAdvance: 800,
            isExtender: false,
          },
          {
            glyphId: 2,
            startConnectorLength: 300,
            endConnectorLength: 300,
            fullAdvance: 90,
            isExtender: true,
          },
