...
describe("buildGposKernLookup reports what a font does not kern", () => {
  ...
  it("returns undefined for a font with no GPOS table at all", () => {
    ...
    const font = parse(bytes);
    const stripped: SfntFont = {
      bytes: font.bytes,
      tables: new Map([...font.tables].filter(([tag]) => tag !== "GPOS")),
    };
...
function pairPosFormat1(options: {
  ...
  valueFormat1: number;
  valueFormat2: number;
  value1: readonly number[];
  value2: readonly number[];
}): Uint8Array<ArrayBuffer> {
...
describe("buildGposKernLookup reads XAdvance out of a ValueRecord of any shape", () => {
  ...
  it("skips past the placement fields that precede it", () => {
    ...
    const valueFormat1 =
      VALUE_FORMAT_X_PLACEMENT |
      VALUE_FORMAT_Y_PLACEMENT |
      VALUE_FORMAT_X_ADVANCE;
    const font = fontWithGpos(
      buildGpos(
        2,
        pairPosFormat1({
    ...
    const font = fontWithGpos(
      buildGpos(
        ...
        pairPosFormat1({
          firstGlyphId: 10,
          secondGlyphId: 20,
          valueFormat1: VALUE_FORMAT_Y_ADVANCE,
          valueFormat2: 0,
          value1: [99],
          value2: [],
        }),
...
describe("buildGposKernLookup resolves Extension lookups", () => {
  ...
  it("refuses an Extension that wraps another Extension", () => {
    ...
    const font = fontWithGpos(
      buildGpos(9, extensionWrapping(extensionWrapping(pairPos, 2), 9)),
    );
  ...
  it("ignores a lookup whose type positions something other than glyph pairs", () => {
    ...
    expect(
      buildGposKernLookup(fontWithGpos(buildGpos(1, pairPos))),
    ).toBeUndefined();
    expect(
      buildGposKernLookup(
        fontWithGpos(buildGpos(9, extensionWrapping(pairPos, 1))),
      ),
    ).toBeUndefined();
  });
});
...
describe("buildGposKernLookup degrades rather than throwing on a malformed GPOS", () => {
  ...
  it("returns undefined for a GPOS truncated to less than its own header", () => {
    expect(
      buildGposKernLookup(fontWithGpos(buildGpos(2, pairPos).subarray(0, 6))),
    ).toBeUndefined();
  });
  ...
  it("returns undefined when the subtable is truncated mid-PairSet", () => {
    ...
    expect(
      buildGposKernLookup(
        fontWithGpos(truncated.subarray(0, truncated.length - 8)),
      ),
    ).toBeUndefined();
  });
});
