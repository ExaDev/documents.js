...
const SUBSTITUTE_FAMILY_BY_NORMALIZED_NAME: ReadonlyMap<
  string,
  VendoredFamily
> = new Map([
  ["calibri", "carlito"],
...
export function resolveVendoredSubstituteFamily(
  rawFamily: string,
): VendoredFamily | undefined {
  return SUBSTITUTE_FAMILY_BY_NORMALIZED_NAME.get(
    normalizeFamilyName(rawFamily),
  );
}
