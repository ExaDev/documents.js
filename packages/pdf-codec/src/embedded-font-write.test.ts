...
import {
  collectEmbeddedGlyphs,
  encodeForShowEmbedded,
  loadEmbeddedFace,
} from "./embedded-font";
...
function buildDocument(text: string = TEXT, compress = true): BuiltDocument {
  ...
  if (sfnt === undefined) {
    throw new Error(
      "the vendored Carlito Regular failed to parse as an sfnt container",
    );
  }
  const face = loadEmbeddedFace(sfnt);
  if (face === undefined) {
    throw new Error(
      "the vendored Carlito Regular failed to load as an embeddable face",
    );
  }
  const codePoints = [
  ...
  const built = buildEmbeddedFontObjects(
    ...
    usedGlyphs,
    {
      cidFontRef: pdfRef(CID_FONT_NUM, 0),
      descriptorRef: pdfRef(DESCRIPTOR_NUM, 0),
      fontFileRef: pdfRef(FONT_FILE_NUM, 0),
      toUnicodeRef: pdfRef(TO_UNICODE_NUM, 0),
    },
    compress,
  );

  const contentBytes = buildContentStream(
    encodeForShowEmbedded(text, face).codes,
  );
  const objects: AllocatedObject[] = [
    {
      num: CATALOG_NUM,
      value: pdfDict({ Type: pdfName("Catalog"), Pages: pdfRef(PAGES_NUM, 0) }),
    },
    {
      num: PAGES_NUM,
      value: pdfDict({
        Type: pdfName("Pages"),
        Kids: pdfArray([pdfRef(PAGE_NUM, 0)]),
        Count: pdfNum(1),
      }),
    },
    {
      ...
      value: pdfDict({
        ...
        MediaBox: pdfArray(
          [0, 0, PAGE_WIDTH_PT, PAGE_HEIGHT_PT].map((n) => pdfNum(n)),
        ),
        Resources: pdfDict({
          Font: pdfDict({ [FONT_RESOURCE_NAME]: pdfRef(TYPE0_NUM, 0) }),
        }),
  ...
  return {
    pdfBytes: assemblePdf(objects, CATALOG_NUM),
    face,
    subset,
    usedGlyphs,
    baseFont: built.baseFont,
  };
}
...
function fontDictOf(pdfBytes: Uint8Array<ArrayBuffer>): PdfDict {
  ...
  const fonts = document.resolveDict(
    resources === undefined ? undefined : dictGet(resources, "Font"),
  );
  const font = document.resolveDict(
    fonts === undefined ? undefined : dictGet(fonts, FONT_RESOURCE_NAME),
  );
  if (font === undefined) {
...
describe("a real PDF carrying an embedded, subsetted Carlito, read back by this package own readPdf", () => {
  ...
  it("advances the drawn text by the widths the /W array declares, not by a default", () => {
    ...
    const expectedWidthPt =
      ((shown.width1000 - kerning1000) / 1000) * FONT_SIZE_PT;
    expect(item.widthPt).toBeCloseTo(expectedWidthPt, 4);
    expect(item.widthPt).not.toBeCloseTo(
      (Array.from(TEXT).length * 1000 * FONT_SIZE_PT) / 1000,
      0,
    );
  });
  ...
  it("is a genuine Type0 / Identity-H / CIDFontType2 / FontFile2 font resource, not merely something readPdf tolerated", () => {
    ...
    const descriptor = document.resolveDict(
      dictGet(cidFont!, "FontDescriptor"),
    );
  ...
  it("embeds the subset font program itself, with /Length1 stating its UNCOMPRESSED length", () => {
    ...
    const cidFont = document.resolveDict(
      asArray(dictGet(font, "DescendantFonts"))?.[0],
    );
    const descriptor = document.resolveDict(
      dictGet(cidFont!, "FontDescriptor"),
    );
  ...
  it("states /Length1 as the plain stream length when nothing is compressed", () => {
    ...
    expect(asNumber(dictGet(fontFile.dict, "Length1"))).toBe(
      subset.bytes.length,
    );
  ...
  it("writes one /W entry per glyph the subset carries, ascending, at the face own glyph-space widths", () => {
    ...
    subset.glyphIds.forEach((glyphId, index) => {
      ...
      expect(asNumber(asArray(w?.[index * 2 + 1])?.[0])).toBeCloseTo(
        face.glyphSpaceWidth(glyphId),
        4,
      );
    });
  ...
  it("describes the face metrics in glyph space, with the flags a Latin sans text face warrants", () => {
    ...
    expect(asNumber(dictGet(descriptor!, "Ascent"))).toBeCloseTo(
      face.metrics.ascentGlyphSpace,
      4,
    );
    expect(asNumber(dictGet(descriptor!, "Descent"))).toBeCloseTo(
      face.metrics.descentGlyphSpace,
      4,
    );
    expect(asNumber(dictGet(descriptor!, "CapHeight"))).toBeCloseTo(
      face.metrics.capHeightGlyphSpace,
      4,
    );
    expect(asNumber(dictGet(descriptor!, "XHeight"))).toBeCloseTo(
      face.metrics.xHeightGlyphSpace!,
      4,
    );
...
describe("the ToUnicode CMap of an embedded subset", () => {
  it("maps exactly the glyphs that stand for a character, and nothing else", () => {
    ...
    const document = openPdfDocument(pdfBytes, NOOP_DIAGNOSTIC_SINK);
    const toUnicode = document.resolve(
      dictGet(fontDictOf(pdfBytes), "ToUnicode"),
    );
    if (toUnicode?.kind !== "stream") {
      throw new Error("the assembled PDF has no ToUnicode stream");
    }
    const text = new TextDecoder().decode(
      decodeStream(toUnicode.raw, toUnicode.dict, NOOP_DIAGNOSTIC_SINK).bytes,
    );
    ...
    expect(
      text
        .split("\n")
        .filter((line) => /^<[0-9a-f]{4}> <[0-9a-f]+>$/.test(line)).length,
    ).toBe(usedGlyphs.size);
...
describe("the subset tag", () => {
  it("is six uppercase letters, and identical for identical input", () => {
    expect(embeddedSubsetTag("Carlito-Regular", [0, 15, 59])).toMatch(
      /^[A-Z]{6}$/,
    );
    expect(embeddedSubsetTag("Carlito-Regular", [0, 15, 59])).toBe(
      embeddedSubsetTag("Carlito-Regular", [0, 15, 59]),
    );
  ...
  it("differs when the glyph set differs, which is the whole reason it exists", () => {
    ...
    expect(buildDocument("Hello").baseFont).not.toBe(
      buildDocument("Goodbye").baseFont,
    );
  });
