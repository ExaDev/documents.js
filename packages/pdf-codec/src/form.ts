...
export function readAcroForm(catalog: PdfDict, resolver: PdfObjectResolver, pageIndex: PageIndexLookup, sink: PdfDiagnosticSink): LayoutFormField[] {
...
function readField(node: PdfDict, parentName: string, resolver: PdfObjectResolver, pageIndex: PageIndexLookup, sink: PdfDiagnosticSink, out: LayoutFormField[], visited: Set<PdfDict>): void {
  if (visited.has(node)) {
    sink({ code: 'pdf/form-field-cycle', severity: 'warning', message: 'the AcroForm field tree contains a cycle; stopping descent at the repeated field' });
    return;
  }
  visited.add(node);
  const ownName = annotText(node, 'T');
  const name = ownName === undefined ? parentName : parentName.length > 0 ? `${parentName}.${ownName}` : ownName;
...
function mapFieldType(fieldType: string, flags: number): LayoutFormField['fieldType'] {
  if (fieldType === 'Tx') {
    return 'text';
  }
  if (fieldType === 'Btn') {
...
function valueFields(fieldType: string, value: string | undefined): { value?: string; checked?: boolean } {
  if (value === undefined) {
    return {};
  }
  if (fieldType === 'Btn') {
...
function fieldValue(node: PdfDict, resolver: PdfObjectResolver): string | undefined {
...
function fieldOptions(node: PdfDict, resolver: PdfObjectResolver): string[] | undefined {
  ...
  for (const opt of opts) {
    ...
    if (resolved?.kind === 'string') {
      ...
      const exportValue = first?.kind === 'string' ? decodePdfString(first.bytes) : first?.kind === 'name' ? first.name : undefined;
...
function widgetPlacement(widget: PdfDict, pageIndex: PageIndexLookup): LayoutFormWidget | undefined {
...
function rectWidget(rect: PdfObject[] | undefined, pageNo: number): LayoutFormWidget | undefined {
  if (rect === undefined) {
    return undefined;
  }
