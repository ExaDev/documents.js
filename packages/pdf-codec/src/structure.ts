  ...
  const readElement = (dict: PdfDict): LayoutStructureElement | undefined => {
    if (visited.has(dict)) {
      sink({
        ...
        severity: "warning",
        message:
          "the structure tree contains a cycle; stopping descent at the repeated element",
      });
      return undefined;
    }
    visited.add(dict);
...
function mergedAttributes(
  ...
  for (const classObj of asArray(dictGet(dict, "C")) ?? []) {
    ...
    const attributes =
      name !== undefined
        ? classAttributes.get(name)
        : elementAttributes(resolver.resolveDict(classObj));
    if (attributes !== undefined) {
  ...
  if (documentTree === undefined) {
    if (dictGet(root, "ParentTree") !== undefined) {
      sink({
        ...
        severity: "warning",
        message:
          "the /ParentTree did not resolve to a dictionary; no marked-content ownership can be associated",
      });
    }
    return () => undefined;
  }
