...
describe("readContentStream: operations", () => {
  it("reads an operator with no operands", () => {
    ...
    expect(readContentStream(textBytes("Q"), sink)).toEqual([
      { kind: "operation", operation: { operands: [], operator: "Q" } },
    ]);
  });
  ...
  it("reads a matrix operator with six numeric operands", () => {
    ...
    expect(token).toEqual({
      kind: "operation",
      operation: {
        operands: [1, 0, 0, 1, 10, 20].map((value) => ({
          kind: "number",
          value,
        })),
        operator: "cm",
  ...
  it("reads a sequence of operations in order", () => {
    ...
    expect(
      tokens.map((t) =>
        t.kind === "operation" ? t.operation.operator : t.kind,
      ),
    ).toEqual(["q", "rg", "Q"]);
  });
  ...
  it("reads an array operand", () => {
    ...
    expect(token.operation.operands[0]).toEqual({
      kind: "array",
      items: [
        { kind: "number", value: 2 },
        { kind: "number", value: 1 },
...
describe("readContentStream: inline images", () => {
  ...
  it('reads an inline image bounded by a scanned "EI" (no /L present)', () => {
    ...
    const content = concatBytes(
      textBytes("q 100 0 0 100 10 0 cm BI /W 2 /H 2 /CS /RGB /BPC 8 ID "),
      pixelData,
      textBytes(" EI Q"),
    );
    ...
    expect(dictGet(inlineToken.image.dict, "W")).toEqual({
      kind: "number",
      value: 2,
    });
  ...
  it("reads an inline image bounded by an explicit /L", () => {
    ...
    const content = concatBytes(
      textBytes(
        `BI /W 2 /H 2 /CS /RGB /BPC 8 /L ${String(pixelData.length)} ID `,
      ),
      pixelData,
      textBytes(" EI"),
    );
  ...
  it("continues reading operations after an inline image", () => {
    const { sink } = collectDiagnostics();
    const content = concatBytes(
      textBytes("BI /W 2 /H 2 /CS /RGB /BPC 8 ID "),
      pixelData,
      textBytes(" EI Q"),
    );
    const tokens = readContentStream(content, sink);
  ...
  it('reports a diagnostic and stops at end of input when "EI" is never found', () => {
    ...
    expect(
      diagnostics.some((d) => d.code === "pdf/inline-image-truncated"),
    ).toBe(true);
  });
});
