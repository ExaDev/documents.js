...
function verticalMetricsFor(
  face: EmbeddedFace,
  policy: VerticalMetricPolicy,
): VerticalMetricSet {
  const m = face.metrics;
  const hhea: VerticalMetricSet = {
  ...
  if (policy === "os2Typo") {
    if (
      m.typoAscentGlyphSpace === undefined ||
      m.typoDescentGlyphSpace === undefined ||
      m.typoLineGapGlyphSpace === undefined
    ) {
      return hhea;
    }
    return {
  ...
  if (policy === "os2Win") {
    if (
      m.winAscentGlyphSpace === undefined ||
      m.winDescentGlyphSpace === undefined
    ) {
      return hhea;
    }
...
export function createFontMeasurer(
  registry?: FontRegistry,
  options: FontMeasurerOptions = {},
): TextMeasurer {
  const corrections: Readonly<Record<string, number>> = {
    ...DEFAULT_WIDTH_CORRECTIONS,
    ...options.widthCorrectionByFamily,
  };
  const verticalPolicy =
    options.verticalMetrics ?? DEFAULT_VERTICAL_METRIC_POLICY;
  ...
  return {
    widthOfTextAtSize(text, font, sizePt) {
      ...
      if (face.kind === "embedded") {
        ...
        return (
          (encodeForShowEmbedded(text, face.face).width1000 /
      ...
      return (
        (width1000 / GLYPH_SPACE_UNITS_PER_EM) *
        sizePt *
        standardWidthCorrectionFor(font.family)
      );
    },
    lineHeightAtSize(font, sizePt) {
      ...
      if (face.kind === "embedded") {
        ...
        return (
          ((vertical.ascentGlyphSpace -
            vertical.descentGlyphSpace +
            vertical.lineGapGlyphSpace) /
            GLYPH_SPACE_UNITS_PER_EM) *
          sizePt
        );
      }
    ...
    ascenderAtSize(font, sizePt) {
      ...
      if (face.kind === "embedded") {
        return (
          (verticalMetricsFor(face.face, verticalPolicy).ascentGlyphSpace /
            GLYPH_SPACE_UNITS_PER_EM) *
          sizePt
        );
      }
      return (
        (STANDARD_METRICS[face.standardName].ascender /
          GLYPH_SPACE_UNITS_PER_EM) *
        sizePt
      );
    },
    descenderAtSize(font, sizePt) {
      ...
      if (face.kind === "embedded") {
        return (
          (verticalMetricsFor(face.face, verticalPolicy).descentGlyphSpace /
            GLYPH_SPACE_UNITS_PER_EM) *
          sizePt
        );
      }
      return (
        (STANDARD_METRICS[face.standardName].descender /
          GLYPH_SPACE_UNITS_PER_EM) *
        sizePt
      );
    },
    underlineAtSize(font, sizePt) {
      ...
      if (face.kind === "embedded") {
        ...
        return {
          offsetPt:
            (face.face.metrics.underlinePositionGlyphSpace /
              GLYPH_SPACE_UNITS_PER_EM) *
            sizePt,
          thicknessPt:
            (face.face.metrics.underlineThicknessGlyphSpace /
              GLYPH_SPACE_UNITS_PER_EM) *
            sizePt,
        };
      }
      ...
      return {
        offsetPt:
          (metrics.underlinePosition / GLYPH_SPACE_UNITS_PER_EM) * sizePt,
        thicknessPt:
          (metrics.underlineThickness / GLYPH_SPACE_UNITS_PER_EM) * sizePt,
      };
    },
    horizontalScaleFor(font) {
      ...
      return face.kind === "embedded"
        ? EMBEDDED_HORIZONTAL_SCALE
        : standardWidthCorrectionFor(font.family);
    },
  };
}
...
export function createStandardFontMeasurer(
  options: StandardFontMeasurerOptions = {},
): TextMeasurer {
