...
function expectEveryRecordChecksumToVerify(
  bytes: Uint8Array<ArrayBuffer>,
): void {
  ...
  for (const record of directory.records) {
    expect({ tag: record.tag, checkSum: record.checkSum }).toEqual({
      tag: record.tag,
      checkSum: sumRegion(
        view,
        record.offset,
        Math.ceil(record.length / 4) * 4,
      ),
    });
  }
}
...
function expectGlyphOutlinePreserved(
  ...
  expect([...copied!.subarray(original!.length)]).toEqual(
    Array<number>(copied!.length - original!.length).fill(0),
  );
}

function subsetCarlito(text: string = TEXT): {
  source: LoadedFont;
  subset: LoadedFont;
  glyphIds: readonly number[];
  unmappedCodePoints: readonly number[];
} {
  ...
  return {
    source,
    subset: load(result.bytes),
    glyphIds: result.glyphIds,
    unmappedCodePoints: result.unmappedCodePoints,
  };
}
...
const CARLITO_GLYPH_IDS = [
  0, 2, 8, 15, 45, 55, 59, 111, 117, 136, 137, 308, 309, 311, 393, 395, 401,
  403, 485, 1140, 2142, 2781,
];
...
describe("subsetSfnt against real Carlito Regular", () => {
  ...
  it("follows composite components transitively, keeping a glyph no code point maps to", () => {
    ...
    const directlyMapped = new Set(
      codePointsOf(TEXT).map((codePoint) => cmap!(codePoint)),
    );
  ...
  it("reproduces every used glyph's outline bytes byte-identically", () => {
    ...
    expect(
      subset.glyf
        .compositeComponents(2142)
        ?.map((component) => component.glyphIndex),
    ).toEqual(
      source.glyf
        .compositeComponents(2142)
        ?.map((component) => component.glyphIndex),
  ...
  it("keeps every glyph advance width the source font declares", () => {
    ...
    for (const glyphId of glyphIds) {
      expect(subsetHmtx.advanceWidth(glyphId)).toBe(
        sourceHmtx.advanceWidth(glyphId),
      );
    }
  ...
  it("rebuilds head, hhea, and maxp consistently with the subset it actually wrote", () => {
    ...
    const view = new DataView(
      subset.bytes.buffer,
      subset.bytes.byteOffset,
      subset.bytes.byteLength,
    );
    ...
    expect([head?.xMin, head?.yMin, head?.xMax, head?.yMax]).toEqual([
      sourceHead?.xMin,
      sourceHead?.yMin,
      sourceHead?.xMax,
      sourceHead?.yMax,
    ...
    const loca = parseLoca(subset.sfnt, {
      numGlyphs: subset.numGlyphs,
      indexToLocFormat: 1,
    });
    expect(loca?.length).toBe(subset.numGlyphs + 1);
    expect(loca?.[subset.numGlyphs]).toBe(
      subset.sfnt.tables.get("glyf")?.length,
    );
    expect(subset.sfnt.tables.get("loca")?.length).toBe(
      (subset.numGlyphs + 1) * 4,
    );
  });
  ...
  it("keeps the hinting programs and drops the tables an embedded CIDFontType2 program never needs", () => {
    ...
    for (const tag of ["cvt ", "fpgm", "prep"]) {
      ...
      expect([
        ...subset.bytes.subarray(
          copied!.offset,
          copied!.offset + copied!.length,
        ),
      ]).toEqual([
  ...
  it("emits a well-formed sfnt container: sorted, aligned table records whose checksums all verify", () => {
    ...
    expect(directory.records.map((record) => record.tag)).toEqual([
      "cvt ",
      "fpgm",
      "glyf",
      "head",
    ...
    const headRecord = directory.records.find(
      (record) => record.tag === "head",
    );
    expect(headRecord).toBeDefined();
    const storedAdjustment = view.getUint32(headRecord!.offset + 8);
    const zeroedSum = sumRegion(
      withZeroedCheckSumAdjustment(subset.bytes, headRecord!.offset),
      0,
      subset.bytes.length,
    );
    expect(storedAdjustment).toBe(
      (CHECKSUM_ADJUSTMENT_MAGIC - zeroedSum) >>> 0,
    );
    expect((zeroedSum + storedAdjustment) >>> 0).toBe(
  ...
  it("cuts a 613 KiB face down to under a twentieth of its size for a short string", () => {
    ...
    expect(subset.sfnt.tables.get("loca")!.length).toBe(
      (CARLITO_HIGHEST_USED_GLYPH_ID + 2) * 4,
    );
  });
...
describe("subsetSfnt against real Caladea Regular, a short-loca source", () => {
  it("rewrites a short loca index as a long one without disturbing a single outline byte", () => {
    ...
    expect(
      source.glyf
        .compositeComponents(222)
        ?.map((component) => component.glyphIndex),
    ).toEqual([45, 295]);
    expect(
      source.glyf
        .compositeComponents(295)
        ?.map((component) => component.glyphIndex),
    ).toEqual([280]);
...
describe("fonts subsetSfnt declines to subset", () => {
  ...
  it("returns undefined rather than inventing an advance width for a truncated hmtx", () => {
    ...
    const hmtxIndex = directory.records.findIndex(
  ...
  it("returns undefined for a font whose glyf table is missing entirely", () => {
    ...
    const glyfIndex = directory.records.findIndex(
      (record) => record.tag === "glyf",
    );
