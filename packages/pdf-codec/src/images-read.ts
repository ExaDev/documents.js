...
// Turns an Image XObject (or an inline image's dict+data) into bytes ready to store as a LayoutImageAsset. A DCTDecode (JPEG) image passes through completely undecoded -- its compressed bytes ARE the deliverable, exactly mirroring the write path's own lossless JPEG passthrough. Everything else is decoded to raw samples and re-encoded as PNG via byte-codec's own encodePng, since LayoutImageAsset only ever stores 'png' or 'jpeg' -- deliberately byte-codec's encodePng rather than this package's internal, independently-copied src/image/png-encode.ts: a caller's own imageId is a crc32 of the PNG bytes it originally supplied (see documents.js's registerImageBytes), so this module's re-encode of the same raster data must run through the identical function a caller would have used, not a second copy that can silently diverge from it.
...
type ResolvedColorSpace =
  ...
  | {
      ...
      readonly base: ResolvedColorSpace;
      readonly lookup: Uint8Array<ArrayBuffer>;
    }
...
function resolveColorSpace(
  ...
  if (family === "ICCBased") {
    ...
    const n =
      streamObj?.kind === "stream"
        ? asNumber(dictGet(streamObj.dict, "N"))
        : undefined;
    if (n === 1) {
      return { kind: "gray" };
    }
...
function buildRawImage(
  ...
  if (colorSpace.kind === "indexed") {
    ...
    for (const index of indices) {
      const rgb = sampleFromPalette(
        colorSpace.base,
        colorSpace.lookup,
        index * baseComponents,
      );
...
function jpeg2000ChannelKind(
  ...
  if (
    ...
    image.colourSpace === "sycc" ||
    image.colourSpace === "e-srgb" ||
    image.colourSpace === "rommrgb"
  ) {
    return "rgb";
  }
...
function readJpeg2000Image(
  ...
  if (image.components.length < required) {
    sink({
      code: "image/jpx-undecodable",
      severity: "warning",
      message: `JPXDecode image resolves to a ${kind} colour space but carries only ${String(image.components.length)} component(s); skipping this image`,
    });
    return undefined;
  }
  if (image.components.length > required) {
    ...
    sink({
      code: "image/jpx-extra-channels",
      severity: "info",
      message: `JPXDecode image carries ${String(image.components.length)} components where its colour space needs ${String(required)}; the extra channel(s) are ignored`,
    });
  }
  ...
  const withAlpha: RawImage =
    alpha !== undefined ? { ...rawImage, alpha } : rawImage;
  return {
    format: "png",
...
export function readImageXObject(
  ...
  if (asBool(dictGet(dict, "ImageMask") ?? dictGet(dict, "IM")) === true) {
    sink({
      ...
      severity: "info",
      message:
        "an /ImageMask stencil paints with the current fill colour rather than standing alone as an image; skipping",
    });
    return undefined;
  }
  ...
  if (decoded.remainingFilter === "DCTDecode") {
    ...
    try {
      ...
      sink({
        ...
        severity: "warning",
        message:
          "DCTDecode image bytes did not look like a valid JPEG (no SOF marker found); skipping this image",
      });
      return undefined;
    }
    return {
  ...
  const colorSpace = resolveColorSpace(
    dictGet(dict, "ColorSpace") ?? dictGet(dict, "CS"),
    resolver,
    sink,
  );
  ...
  const inverted =
    decodeArr !== undefined &&
    asNumber(decodeArr[0]) === 1 &&
    asNumber(decodeArr[1]) === 0;

  const rawImage = buildRawImage(
    decoded.bytes,
    width,
    height,
    bitsPerComponent,
