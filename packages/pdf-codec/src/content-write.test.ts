...
describe("writeContentStream: text in an embedded face", () => {
  ...
  function embeddedContext(
    ...
    return {
      ...fakeContext(scale),
      resolveFont: () => ({ kind: "embedded", resourceName: "E1", face }),
    };
  }
  ...
  it("emits BT/Tf/Tz/rg/Tm/Tj/ET with the embedded resource name and 2-byte Identity-H CIDs", () => {
    ...
    const { bytes, substitutions, missingGlyphs } = writeContentStream(
      [item],
      embeddedContext(face),
    );
    const hex = [...encodeForShowEmbedded("Hi", face).codes]
  ...
  it("shows a kerned run as a real TJ array, splitting the CIDs at each of the face's own adjustments", () => {
    ...
    const text = decode(
      writeContentStream([item], embeddedContext(embeddedFace())).bytes,
    );
    ...
    const hex = [...encodeForShowEmbedded("AVATAR", embeddedFace()).codes]
      .map((b) => b.toString(16).padStart(2, "0"))
      .join("");
  ...
  it("measures a kerned run's underline at the kerned width, not the bare advance sum", () => {
    ...
    expect(text).not.toContain(
      ` ${formatNumber((3333.49609375 / 1000) * 12)} `,
    );
  });
  ...
  it("always writes 100 Tz, never the measurer's standard-14 width correction", () => {
    ...
    const text = decode(
      writeContentStream([item], embeddedContext(embeddedFace(), 0.92)).bytes,
    );
    expect(text).toContain("100 Tz\n");
  ...
  it("draws the underline from the face's own post metrics, not the measurer's", () => {
    ...
    expect(text).toContain(
      `0 ${formatNumber((-103 * 10) / 2048)} 5 ${formatNumber((194 * 10) / 2048)} re\n`,
    );
  ...
  it("falls back to the face's own measured width for an underline with no widthPt, with no scale applied", () => {
    ...
    expect(text).toContain(
      ` ${formatNumber((encodeForShowEmbedded("Hi", face).width1000 / 1000) * 10)} `,
    );
  });
  ...
  it("reports a character the face has no glyph for as a missing glyph, not a WinAnsi substitution", () => {
    ...
    const { substitutions, missingGlyphs } = writeContentStream(
      [item],
      embeddedContext(embeddedFace()),
    );
...
describe("writeContentStream: line -- stroke style", () => {
  ...
  it("emits nothing extra for style: 'solid', which means exactly what an absent style means", () => {
    const text = decode(
      writeContentStream([{ ...rule, style: "solid" }], fakeContext()).bytes,
    );
    expect(text).toBe("0 0 0 RG\n2 w\n10 10 m 100 10 l\nS\n");
  });
  ...
  it("emits a [6 6] 0 d dash array (3x the 2pt stroke width, on and off) before the stroke and resets it to [] 0 d after", () => {
    const text = decode(
      writeContentStream([{ ...rule, style: "dashed" }], fakeContext()).bytes,
    );
    expect(text).toBe(
      "0 0 0 RG\n2 w\n[6 6] 0 d\n10 10 m 100 10 l\nS\n[] 0 d\n",
    );
  ...
  it("emits a [0 4] 0 d dash array and a 1 J round cap for dotted, resetting both afterwards", () => {
    const text = decode(
      writeContentStream([{ ...rule, style: "dotted" }], fakeContext()).bytes,
    );
    expect(text).toBe(
      "0 0 0 RG\n2 w\n[0 4] 0 d\n1 J\n10 10 m 100 10 l\nS\n[] 0 d\n0 J\n",
    );
  ...
  it("leaves the dash pattern reset before a later, unrelated item in the same stream", () => {
    ...
    const text = decode(
      writeContentStream([{ ...rule, style: "dashed" }, rect], fakeContext())
        .bytes,
    );
    expect(text).toBe(
      "0 0 0 RG\n2 w\n[6 6] 0 d\n10 10 m 100 10 l\nS\n[] 0 d\n1 0 0 RG\n1 w\n0 0 5 5 re\nS\n",
  ...
  it("leaves both the dash pattern and the line cap reset before a later item after a dotted line", () => {
    ...
    const text = decode(
      writeContentStream([{ ...rule, style: "dotted" }, plain], fakeContext())
        .bytes,
    );
