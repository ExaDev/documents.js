...
function load(bytes: Uint8Array<ArrayBuffer>): {
...
describe("loadEmbeddedFace metrics on Carlito Regular, a 2048-unit-per-em face", () => {
  it("converts every geometry field into 1000-unit glyph space, and none of them is the raw design value", () => {
    ...
    expect(m.bboxGlyphSpace).toEqual(
      [-1002, -529, 2351, 2078].map((n) => scaled(n, CARLITO_UNITS_PER_EM)),
    );
    expect(m.underlinePositionGlyphSpace).toBe(
      scaled(-103, CARLITO_UNITS_PER_EM),
    );
    expect(m.underlineThicknessGlyphSpace).toBe(
      scaled(194, CARLITO_UNITS_PER_EM),
    );
...
describe("loadEmbeddedFace caching and refusal", () => {
  ...
  it("measures a cap height off the H glyph when OS/2 does not declare one", () => {
    ...
    expect(face!.metrics.capHeightGlyphSpace).toBe(
      load(carlitoRegularBytes()).face.metrics.capHeightGlyphSpace,
    );
  });
});
...
describe("encodeForShowEmbedded", () => {
  it("emits one big-endian 2-byte CID per character and measures exactly those glyphs", () => {
    ...
    expect(shown.width1000).toBe(
      face.glyphSpaceWidth(hGlyph) + face.glyphSpaceWidth(iGlyph),
    );
  ...
  it("shows .notdef for a character the face cannot map, and measures .notdef own advance for it", () => {
    ...
    expect(shown.width1000).toBe(
      face.glyphSpaceWidth(aGlyph) * 2 + face.glyphSpaceWidth(0),
    );
  });
...
describe("pair kerning", () => {
  ...
  it("reports Carlito's own real adjustments, converted off its 2048-unit em into glyph space", () => {
    const { face } = load(carlitoRegularBytes());
    expect(face.kernGlyphSpace(glyphOf(face, "A"), glyphOf(face, "V"))).toBe(
      scaled(-89, CARLITO_UNITS_PER_EM),
    ); // -43.45703125
    expect(face.kernGlyphSpace(glyphOf(face, "V"), glyphOf(face, "A"))).toBe(
      scaled(-96, CARLITO_UNITS_PER_EM),
    ); // -46.875
    expect(face.kernGlyphSpace(glyphOf(face, "A"), glyphOf(face, "T"))).toBe(
      scaled(-160, CARLITO_UNITS_PER_EM),
    ); // -78.125
    ...
    expect(
      face.kernGlyphSpace(glyphOf(face, "A"), glyphOf(face, "V")),
    ).not.toBe(-89);
    expect(
      face.kernGlyphSpace(glyphOf(face, "A"), glyphOf(face, "V")),
    ).toBeCloseTo(-43.457, 3);
  ...
  it("measures a Carlito run at its kerned width, genuinely narrower than the same run's bare advances", () => {
    ...
    const naiveWidth1000 = scaled(
      1185 + 1162 + 1185 + 998 + 1185 + 1112,
      CARLITO_UNITS_PER_EM,
    );
    ...
    expect(shown.width1000).toBe(
      naiveWidth1000 + scaled(-505, CARLITO_UNITS_PER_EM),
    );
  ...
  it("records one adjustment per kerned pair, at the byte offset of the pair's own right-hand glyph", () => {
    ...
    expect(shown.width1000 - kerning1000).toBe(
      scaled(1185 + 1162 + 1185 + 998 + 1185 + 1112, CARLITO_UNITS_PER_EM),
    );
  });
  ...
  it("applies Caladea's own different adjustments, where a 1000-unit em makes glyph space and design units the same numbers", () => {
    ...
    const naiveWidth1000 = scaled(
      599 + 598 + 599 + 557 + 599 + 613,
      CALADEA_UNITS_PER_EM,
    );
    expect(naiveWidth1000).toBe(3565);
    expect(shown.width1000).toBe(3171);
    expect(shown.kerns.map((kern) => kern.adjustment1000)).toEqual([
      -117, -119, -79, -79,
    ]);
  ...
  it("leaves a run with no kerned pair completely alone", () => {
    ...
    expect(shown.width1000).toBe(
      face.glyphSpaceWidth(glyphOf(face, "H")) +
        face.glyphSpaceWidth(glyphOf(face, "i")),
    );
  });
});
