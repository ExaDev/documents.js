...
describe('readPdf: named destinations', () => {
  ...
  it('reads the /Names /Dests name tree through its /Kids split', () => {
    const doc = readPdf(navigationClusterPdf());
    expect(destinationNamed(doc, 'second')).toEqual({ name: 'second', pageIndex: 1, target: { kind: 'fit' } });
  });
  ...
  it('mints a destination for a direct destination array', () => {
    ...
    expect(minted).toEqual({ name: 'dest2', pageIndex: 1, target: { kind: 'xyz', leftPt: 20, topPt: 60 } });
  ...
  it('reads the /Outlines tree as nested items with resolved destination names', () => {
    ...
    expect(doc.outline).toEqual([
      { title: 'First heading', destination: 'firstpage', children: [] },
      {
        title: 'Second page',
        destination: 'dest1',
        children: [{ title: 'Nested child', destination: 'second', children: [] }],
      },
    ]);
    expect(destinationNamed(doc, 'dest1')).toEqual({ name: 'dest1', pageIndex: 1, target: { kind: 'xyz' } });
  });
});
...
describe('readPdf: internal link annotations', () => {
  it('reads /Dest (named), /Dest (direct array), and /A /GoTo links as internalLink items targeting destination names', () => {
    const doc = readPdf(navigationClusterPdf());
    const links = doc.pages[0]!.items.filter((item) => item.kind === 'internalLink');
    expect(links).toEqual([
      { kind: 'internalLink', destination: 'second', xPt: 10, yPt: 10, widthPt: 50, heightPt: 14 },
      { kind: 'internalLink', destination: 'dest2', xPt: 70, yPt: 10, widthPt: 50, heightPt: 14 },
      { kind: 'internalLink', destination: 'firstpage', xPt: 10, yPt: 30, widthPt: 50, heightPt: 14, title: 'Internal note' },
    ]);
  });

  it('does not misread an internal link as a URI link item', () => {
    const doc = readPdf(navigationClusterPdf());
    expect(doc.pages[0]!.items.some((item) => item.kind === 'link')).toBe(false);
  });
});
