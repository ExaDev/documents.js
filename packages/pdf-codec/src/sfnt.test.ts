...
describe("parseSfnt rejects a container it cannot read", () => {
  ...
  it("returns undefined for a version tag that is not a single-font sfnt", () => {
    const collection = buildDirectory({
      ...
      tableLength: 54,
      totalLength: 128,
    }); // 'ttcf', a TrueType Collection: several directories behind a header this reader does not walk
    expect(parseSfnt(collection)).toBeUndefined();
    const notAFont = buildDirectory({
      sfntVersion: 0x25504446,
      numTables: 1,
      tag: "head",
      tableOffset: 28,
      tableLength: 54,
  ...
  it("returns undefined when numTables claims more records than the file holds", () => {
    const overclaimed = buildDirectory({
...
describe("big-endian primitive readers", () => {
  const bytes = Uint8Array.from([
    0x00, 0x80, 0xff, 0x7f, 0x12, 0x34, 0x56, 0x78, 0x40, 0x00,
  ]);
