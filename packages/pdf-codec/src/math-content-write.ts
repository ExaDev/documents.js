...
function writeAssembledGlyphs(
  writer: ByteWriter,
  positioned: PositionedFormula,
  item: MathAssembledGlyphs,
  ctx: MathContentWriteContext,
): void {
  ...
  writeObject(
    writer,
    pdfDict({ ActualText: pdfHexString(utf16BeWithBom(item.text)) }),
  );
  ...
  for (const placement of item.placements) {
    ...
    writeRgbOperator(writer, item.color, "rg");
    writer.writeAscii(
      `1 0 0 1 ${formatNumber(toPdfX(positioned, placement.xPt))} ${formatNumber(toPdfY(positioned, placement.yPt))} Tm\n`,
    );
...
function writeRule(
  ...
  writeRgbOperator(writer, item.color, "rg");
  writer.writeAscii(
    `${formatNumber(x)} ${formatNumber(topY - item.heightPt)} ${formatNumber(item.widthPt)} ${formatNumber(item.heightPt)} re\n`,
  );
  writer.writeAscii("f\n");
}
...
function writeStroke(
  ...
  item.points.forEach((point, index) => {
    ...
    writer.writeAscii(
      `${formatNumber(x)} ${formatNumber(y)} ${index === 0 ? "m" : "l"}\n`,
    );
  });
  writer.writeAscii("S\n");
}
...
export function writeFormulaContentStream(
  formulas: readonly PositionedFormula[],
  ctx: MathContentWriteContext,
): Uint8Array<ArrayBuffer> {
...
export function collectUsedGlyphs(
  formulas: readonly PositionedFormula[],
  font: MathFont,
