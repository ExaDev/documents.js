...
function run(
  text: string,
  sizePt = 10,
): { text: string; font: typeof FONT; sizePt: number; color: typeof COLOR } {
...
function lineTexts(
  lines: ReturnType<typeof wrapRunsToWidth>,
...
describe("wrapTextToWidth", () => {
  it("wraps single-style text and returns plain strings per line", () => {
    const measurer = fakeMeasurer();
    expect(
      wrapTextToWidth("hello world", FONT, 10, COLOR, measurer, 8),
    ).toEqual(["hello", "world"]);
  });
});
