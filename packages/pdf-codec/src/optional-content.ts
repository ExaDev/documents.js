...
export function readOptionalContent(catalog: PdfDict, resolver: PdfObjectResolver, sink: PdfDiagnosticSink): OptionalContentContext {
  ...
  for (const ocgRef of ocgs) {
    ...
    if (ocg === undefined) {
      sink({ code: 'pdf/ocg-unresolved', severity: 'warning', message: 'an entry in /OCProperties /OCGs did not resolve to a dictionary; skipping it' });
      continue;
    }
    const nameObj = dictGet(ocg, 'Name');
    const name = nameObj?.kind === 'string' ? decodePdfString(nameObj.bytes) : mintLayerName(nameByGroup);
  ...
  const baseVisible = defaultConfig === undefined || asName(dictGet(defaultConfig, 'BaseState')) !== 'OFF';
  const groupsIn = (key: string): Set<PdfDict> => {
    const set = new Set<PdfDict>();
    const arr = defaultConfig === undefined ? undefined : asArray(resolver.resolve(dictGet(defaultConfig, key)));
    if (arr !== undefined) {
