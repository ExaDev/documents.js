...
function requiredRepeatCount(
  ...
  const shortfall =
    targetSize -
    sumBy(fixed, (part) => part.fullAdvance) +
    (fixed.length - 1) * minConnectorOverlap;
...
function expandParts(
  parts: readonly MathGlyphPart[],
  repeatCount: number,
): readonly MathGlyphPart[] {
...
function maximumOverlap(expanded: readonly MathGlyphPart[]): number {
  ...
  for (let i = 0; i + 1 < expanded.length; i++) {
    overlap = Math.min(
      overlap,
      expanded[i]!.endConnectorLength,
      expanded[i + 1]!.startConnectorLength,
    );
  }
...
function assembleFromParts(
  ...
  const exactOverlap =
    joins === 0 ? minOverlap : (totalAdvance - options.targetSize) / joins;
...
export function assembleStretchyGlyph(
  construction: MathGlyphConstruction,
  options: MathStretchOptions,
): MathStretchConstruction | undefined {
  const variants = construction.variants;
  const reaching = variants.findIndex(
    (variant) => variant.advanceMeasurement >= options.targetSize,
  );
  if (reaching !== -1) {
  ...
  return largest === undefined
    ? undefined
    : fromVariant(largest, variants.length === 1, options.axis);
}
...
export function scaleMathStretchConstruction(
  construction: MathStretchConstruction,
  factor: number,
): MathStretchConstruction {
  return {
