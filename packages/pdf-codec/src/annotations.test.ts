...
describe('readPdf: annotations', () => {
  ...
  it('reads a FreeText annotation\'s typed contents as its body', () => {
    const doc = readPdf(annotationsPdf());
    const freeText = doc.pages[0]!.annotations?.find((a) => a.subtype === 'FreeText');
    expect(freeText).toMatchObject({ subtype: 'FreeText', contents: 'Typed remark', author: 'Reviewer' });
  });

  it('reads a markup annotation\'s /QuadPoints transformed into page space', () => {
    const doc = readPdf(annotationsPdf());
    const highlight = doc.pages[0]!.annotations?.find((a) => a.subtype === 'Highlight');
    expect(highlight).toMatchObject({ subtype: 'Highlight', contents: 'Marked passage', author: 'Second reviewer' });
  ...
  it('carries an opaque annotation kind as quarantined PDF-syntax residue', () => {
    ...
    expect(stamp).toMatchObject({ subtype: 'Stamp', xPt: 100, yPt: 20, widthPt: 40, heightPt: 20, contents: 'Approved' });
