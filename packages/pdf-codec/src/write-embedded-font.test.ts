...
describe("writePdf: a FontRegistry resolving Calibri to the vendored Carlito substitute", () => {
  it("writes a real Type0/CIDFontType2/FontFile2 font group, not a standard-14 Helvetica fallback", () => {
    const text = decode(
      writePdf(textDoc("Hello World", CALIBRI), {
        compress: false,
  ...
  it("embeds a genuine TrueType font program, not merely a dictionary claiming one", () => {
    const bytes = writePdf(textDoc("Hello World", CALIBRI), {
      compress: false,
      fonts: createFontRegistry(),
    });
  ...
  it("shows text as Identity-H 2-byte CIDs, matching the glyph IDs the face itself resolves", () => {
    const text = decode(
      writePdf(textDoc("Hi", CALIBRI), {
        compress: false,
        fonts: createFontRegistry(),
      }),
    );
    const face = carlitoRegularFace();
    const expectedHex = [...encodeForShowEmbedded("Hi", face).codes]
  ...
  it("still writes a standard-14 dictionary for a family the registry resolves to one", () => {
    ...
    const text = decode(
      writePdf(textDoc("Hello", HELVETICA), {
        compress: false,
        fonts: createFontRegistry(),
      }),
    );
...
describe("createFontMeasurer: measurement against a real embedded face", () => {
  it("measures Calibri text at Carlito's own advances, measurably different from the standard-14 substitute", () => {
    ...
    const embeddedWidth = embedded.widthOfTextAtSize(
      "Hello World",
      CALIBRI,
      12,
    );
    const standardWidth = standard.widthOfTextAtSize(
      "Hello World",
      CALIBRI,
      12,
    );
    const helveticaWidth = standard.widthOfTextAtSize(
      "Hello World",
      HELVETICA,
      12,
    );

    expect(embeddedWidth).toBeCloseTo(
      (encodeForShowEmbedded("Hello World", face).width1000 / 1000) * 12,
      10,
    );
  ...
  it("matches a hand-computed sum of the face's own real design-unit advances", () => {
    ...
    expect(
      createFontMeasurer(createFontRegistry()).widthOfTextAtSize(
        "Hll",
        CALIBRI,
        12,
      ),
  ...
  it("returns a horizontal scale of exactly 1 for an embedded face, and the family correction for a standard one", () => {
    ...
    expect(
      createFontMeasurer(createFontRegistry()).horizontalScaleFor(CALIBRI),
    ).toBe(1);
    expect(
      createFontMeasurer(createFontRegistry()).horizontalScaleFor(CAMBRIA_BOLD),
    ).toBe(1);
    expect(
      createFontMeasurer(createFontRegistry()).horizontalScaleFor(HELVETICA),
    ).toBe(1);
    expect(createStandardFontMeasurer().horizontalScaleFor(CALIBRI)).toBe(0.92);
    expect(createStandardFontMeasurer().horizontalScaleFor(CAMBRIA_BOLD)).toBe(
      0.96,
    );
  });
  ...
  it("feeds line wrapping the embedded advances, changing where a real line actually breaks", () => {
    ...
    const embeddedWidthPt = embeddedMeasurer.widthOfTextAtSize(
      text,
      CALIBRI,
      12,
    );
  ...
  it("takes underline geometry from the embedded face's own post table", () => {
    ...
    const underline = createFontMeasurer(createFontRegistry()).underlineAtSize(
      CALIBRI,
      1000,
    );
