...
function findExactFace(
  list: readonly ProvidedFont[],
  family: string,
  bold: boolean,
  italic: boolean,
): ProvidedFont | undefined {
  const normalized = normalizeFamilyName(family);
  return list.find(
    (candidate) =>
      normalizeFamilyName(candidate.family) === normalized &&
      candidate.bold === bold &&
...
function findFamilyRegular(
  ...
  const normalized = normalizeFamilyName(family);
  const familyFaces = list.filter(
    (candidate) => normalizeFamilyName(candidate.family) === normalized,
  );
  if (familyFaces.length === 0) {
    return undefined;
  }
  return (
    familyFaces.find((candidate) => !candidate.bold && !candidate.italic) ??
    familyFaces[0]
  );
}
  ...
  function resolveUncached(
    ...
    const callerFamilyRegular =
      sourceFamilyRegular === undefined
        ? findFamilyRegular(fonts, family)
        : undefined;
    ...
    if (substitutesEnabled) {
      ...
      if (vendoredFamily !== undefined) {
        const face = embedFace(
          loadVendoredFaceBytes(vendoredFamily, bold, italic),
        );
        if (face !== undefined) {
          onSubstitution?.({
            requestedFamily: family,
...
export function resolveFaceWithRegistry(
  ...
  const standard = resolveStandardFont(
    font.family,
    font.weight === "bold",
    font.style === "italic",
  );
