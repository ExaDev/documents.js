...
function tokens(text: string): TestToken[] {
  ...
  for (;;) {
    ...
    out.push(
      t.kind === "literalString" || t.kind === "hexString"
        ? { kind: t.kind, value: Array.from(t.value) }
        : t,
    );
  }
  return out;
...
describe("nextToken: literal strings", () => {
  it("reads a plain string", () => {
    expect(tokens("(Hello)")).toEqual([
      {
        kind: "literalString",
        value: Array.from(new TextEncoder().encode("Hello")),
      },
    ]);
  });
  ...
  it("keeps balanced nested parentheses as literal content", () => {
    expect(tokens("(a(b)c)")).toEqual([
      {
        kind: "literalString",
        value: Array.from(new TextEncoder().encode("a(b)c")),
      },
    ]);
  });
  ...
  it("decodes named escapes", () => {
    ...
    expect(byteValue(token)).toEqual([
      0x0a, 0x0d, 0x09, 0x08, 0x0c, 0x28, 0x29, 0x5c,
    ]);
  });
  ...
  it("treats a backslash-newline as a line-continuation escape producing no byte", () => {
    ...
    expect(byteValue(token)).toEqual(
      Array.from(new TextEncoder().encode("ab")),
    );
  });
  ...
  it("returns whatever was read so far for a truncated (unterminated) string, rather than throwing", () => {
    ...
    expect(byteValue(token)).toEqual(
      Array.from(new TextEncoder().encode("unterminated")),
    );
  });
});
...
describe("nextToken: hex strings", () => {
  it("reads a hex string", () => {
    expect(tokens("<48656c6c6f>")).toEqual([
      {
        kind: "hexString",
        value: Array.from(new TextEncoder().encode("Hello")),
      },
  ...
  it("ignores embedded whitespace", () => {
    expect(tokens("<48 65 6c 6c 6f>")).toEqual([
      {
        kind: "hexString",
        value: Array.from(new TextEncoder().encode("Hello")),
      },
  ...
  it("zero-pads an odd trailing digit", () => {
    expect(tokens("<488>")).toEqual([
      { kind: "hexString", value: [0x48, 0x80] },
    ]);
  });
  ...
  it("reads structural keywords", () => {
    expect(
      ...
      [
        ...
        "null",
        "R",
      ].map((value) => ({ kind: "keyword", value })),
    );
  });

  it("reads content-stream operators using the same keyword rule, no special-casing", () => {
    expect(tokens("BT Tf Tj re cm Do ET")).toEqual(
      ["BT", "Tf", "Tj", "re", "cm", "Do", "ET"].map((value) => ({
        kind: "keyword",
        value,
      })),
    );
...
describe("nextToken: comments and whitespace", () => {
  ...
  it("skips a comment at the very end of input without erroring", () => {
    expect(tokens("1 % trailing comment")).toEqual([
      { kind: "number", value: 1 },
    ]);
  });
});
...
describe("nextToken: a realistic object-header sequence", () => {
  it('tokenizes "N G obj" and "N G R" as the same two-number-plus-keyword shape, leaving disambiguation to the caller', () => {
    expect(tokens("5 0 obj")).toEqual([
      { kind: "number", value: 5 },
      { kind: "number", value: 0 },
      { kind: "keyword", value: "obj" },
    ]);
    expect(tokens("5 0 R")).toEqual([
      { kind: "number", value: 5 },
      { kind: "number", value: 0 },
      { kind: "keyword", value: "R" },
    ]);
  });
});
