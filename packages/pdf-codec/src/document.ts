  ...
  return {
    ...
    entries: new Map(
      Array.from(dict.entries, ([key, entry]) => [
        key,
        decryptObject(entry, num, gen, decryptor),
      ]),
    ),
  };
}
  ...
  if (value.kind === "array") {
    return {
      ...
      items: value.items.map((item) =>
        decryptObject(item, num, gen, decryptor),
      ),
    };
  }
  if (value.kind === "dict") {
...
export function openPdfDocument(
  ...
  function fetchDirect(
    num: number,
    entry: Extract<XrefEntry, { type: "offset" }>,
  ): PdfObject {
  ...
  function decodeObjectStream(streamObjNum: number): PdfObject[] {
    ...
    const streamObj =
      streamEntry?.type === "offset"
        ? fetchDirect(streamObjNum, streamEntry)
        : undefined;
  ...
  function fetch(num: number): PdfObject {
    ...
    const value =
      entry.type === "offset"
        ? fetchDirect(num, entry)
        : (decodeObjectStream(entry.streamObjNum)[entry.indexInStream] ??
          pdfNull());
  ...
  function resolve(obj: PdfObject | undefined): PdfObject | undefined {
    ...
    if (depth >= MAX_RESOLVE_DEPTH) {
      sink({
        ...
        severity: "warning",
        message:
          "a chain of indirect references did not resolve within the depth limit; treating it as null",
      });
      return pdfNull();
    }
    return current;
  ...
  function requireCatalog(dict: PdfDict | undefined): PdfDict {
    if (dict === undefined || !isName(dictGet(dict, "Type"), "Catalog")) {
      throw new PdfParseError(
        "pdf/no-root",
        "no resolvable /Root catalog was found, even after cross-reference recovery",
      );
    }
    return dict;
  }
  ...
  if (encryptEntry !== undefined) {
    ...
    if (encryptDict === undefined) {
      throw new PdfEncryptedError(
        "the trailer names an /Encrypt dictionary that does not resolve to a dictionary at all",
      );
    }
    decryptor = createStandardDecryptor(
      encryptDict,
      firstFileId(xref.trailer),
      sink,
    );
  }
