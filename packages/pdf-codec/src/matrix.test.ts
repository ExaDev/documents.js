...
describe("applyMatrix", () => {
  it("the identity matrix leaves a point unchanged", () => {
    expect(applyMatrix(IDENTITY_MATRIX, { x: 5, y: 7 })).toEqual({
      x: 5,
      y: 7,
    });
  });
  ...
  it("a translation matrix shifts a point", () => {
    expect(applyMatrix(translationMatrix(10, 20), { x: 1, y: 1 })).toEqual({
      x: 11,
      y: 21,
    });
  });
  ...
  it("a scale matrix scales a point about the origin", () => {
    expect(applyMatrix(scaleMatrix(2, 3), { x: 4, y: 5 })).toEqual({
      x: 8,
      y: 15,
    });
  });
