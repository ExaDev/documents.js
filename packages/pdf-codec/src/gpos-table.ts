...
function xAdvanceOffset(valueFormat: number): number | undefined {
  ...
  return valueRecordSize(
    valueFormat & (VALUE_FORMAT_X_PLACEMENT | VALUE_FORMAT_Y_PLACEMENT),
  );
}
...
function parsePairPosFormat1(
  ...
  const recordSize =
    PAIR_VALUE_RECORD_GLYPH_SIZE +
    valueRecordSize(valueFormat1) +
    valueRecordSize(valueFormat2);
  ...
  return (leftGlyphId: number, rightGlyphId: number): number | undefined => {
    ...
    const pairSetOffset =
      subtableOffset + u16(bytes, pairSetOffsetsOffset + coverageIndex * 2);
    ...
    while (low <= high) {
      ...
      if (rightGlyphId < secondGlyph) {
        ...
        return advanceOffset === undefined
          ? 0
          : i16(
              bytes,
              recordOffset + PAIR_VALUE_RECORD_GLYPH_SIZE + advanceOffset,
            );
      }
    }
    return undefined;
...
function parsePairPosFormat2(
  ...
  return (leftGlyphId: number, rightGlyphId: number): number | undefined => {
    ...
    return i16(
      bytes,
      matrixOffset +
        class1 * class1RecordSize +
        class2 * class2RecordSize +
        advanceOffset,
    );
  };
}
...
function parseSubtable(
  ...
  if (lookupType === LOOKUP_TYPE_EXTENSION_POS) {
    if (
      !hasBytes(bytes, subtableOffset, EXTENSION_POS_HEADER_SIZE) ||
...
function parseLookupSubtables(
  ...
  for (let i = 0; i < subTableCount; i++) {
    const subtable = parseSubtable(
      bytes,
      lookupType,
      lookupOffset + u16(bytes, offsetsOffset + i * 2),
    );
...
export function buildGposKernLookup(
  ...
  if (
    bytes === undefined ||
    !hasBytes(bytes, 0, GPOS_HEADER_SIZE) ||
    u16(bytes, 0) !== 1
  ) {
    return undefined;
  }
