...
import {
  createArithContexts,
  createIntegerContexts,
  createSymbolIdContexts,
  decodeInteger,
  decodeSymbolId,
} from "./jbig2-arith";
...
export function referenceCornerFromCode(
  code: number,
): Jbig2ReferenceCorner | undefined {
...
export function createTextArithContexts(
...
function requireInteger(value: number | undefined, field: string): number {
  if (value === undefined) {
    throw new Jbig2ParseError(
      `JBIG2 ${field} decoded as out-of-band, which is not a value this field can take`,
    );
  }
  return value;
}
...
export function decodeTextRegion(
  ...
  let stripT =
    -requireInteger(decodeInteger(mq, contexts.iadt), "IADT") *
    params.stripSize;
  let firstS = 0;
  let instances = 0;
  ...
  while (instances < params.instanceCount) {
    ...
    for (;;) {
      const currentT =
        params.stripSize === 1
          ? 0
          : requireInteger(decodeInteger(mq, contexts.iait), "IAIT");
      const t = stripT + currentT;
      const id = decodeSymbolId(mq, contexts.iaid, symbolIdBits);
      let symbol = params.symbols[id];
      if (symbol === undefined) {
        throw new Jbig2ParseError(
          `text region referenced symbol ${String(id)}, outside the ${String(params.symbols.length)}-symbol set its segment declares`,
        );
      }
      if (
        params.refine &&
        requireInteger(decodeInteger(mq, contexts.iari), "IARI") !== 0
      ) {
      ...
      const advanceBefore = params.transposed
        ...
        : params.referenceCorner.right;
      if (advanceBefore) {
        currentS += advance;
      }
      const position = instanceTopLeft(
        currentS,
        t,
        symbol.width,
        symbol.height,
        params.referenceCorner,
        params.transposed,
      );
      combineBitmap(
        region,
      ...
      if (instances > params.instanceCount) {
        throw new Jbig2ParseError(
          `text region coded more symbol instances than the ${String(params.instanceCount)} its own SBNUMINSTANCES declares`,
        );
      }
...
export function decodeSymbolDictionary(
  ...
  while (newSymbols.length < params.newSymbolCount) {
    ...
    if (heightDelta === undefined) {
      throw new Jbig2ParseError(
        "symbol dictionary ended before all of its declared new symbols were decoded",
      );
    }
    ...
    for (;;) {
      ...
      if (newSymbols.length >= params.newSymbolCount) {
        throw new Jbig2ParseError(
          "symbol dictionary coded more symbols than its own SDNUMNEWSYMS declares",
        );
      }
      newSymbols.push(
        decodeDictionarySymbol(
          symbolWidth,
          heightClassHeight,
          params,
          newSymbols,
  ...
  return exportSymbols(
    [...params.inputSymbols, ...newSymbols],
    params.exportedSymbolCount,
    mq,
    contexts,
  );
}

function decodeDictionarySymbol(
  width: number,
  height: number,
  params: SymbolDictionaryParams,
  newSymbols: readonly Jbig2Bitmap[],
  mq: MqDecoder,
  contexts: TextArithContexts,
  symbolIdBits: number,
): Jbig2Bitmap {
  if (!params.refinementAggregate) {
    return decodeGenericRegion(
      width,
      height,
  ...
  if (aggregateInstances !== 1) {
    throw new Jbig2UnsupportedError(
      `symbol dictionary uses aggregate coding with ${String(aggregateInstances)} instances per symbol; only the single-refinement form of T.88 6.5.8.2.2 is implemented`,
    );
  }
  ...
  if (reference === undefined) {
    throw new Jbig2ParseError(
      `symbol dictionary refined against symbol ${String(id)}, which it has not decoded yet`,
    );
  }
  return decodeRefinementRegion(
    width,
...
function exportSymbols(
  ...
  for (let run = 0; index < allSymbols.length; run++) {
    if (run >= maxRuns) {
      throw new Jbig2ParseError(
        "symbol dictionary export flags never accounted for every symbol",
      );
    }
    const runLength = requireInteger(decodeInteger(mq, contexts.iaex), "IAEX");
    if (runLength < 0 || index + runLength > allSymbols.length) {
      throw new Jbig2ParseError(
        `symbol dictionary export run of ${String(runLength)} overruns its own ${String(allSymbols.length)}-symbol set`,
      );
    }
    if (currentFlag) {
  ...
  if (exported.length !== exportedCount) {
    throw new Jbig2ParseError(
      `symbol dictionary exported ${String(exported.length)} symbols but its own SDNUMEXSYMS declares ${String(exportedCount)}`,
    );
  }
  return exported;
}
