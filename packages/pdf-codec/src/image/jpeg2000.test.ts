...
describe("decodeJpeg2000: real OpenJPEG-produced codestreams", () => {
  for (const fixture of JPEG2000_FIXTURES.filter(
    (candidate) => !candidate.undecodable,
  )) {
    it(`decodes "${fixture.name}" (${fixture.description})`, () => {
      ...
      expect(differing / total).toBeLessThan(
        IRREVERSIBLE_MAX_DIFFERING_FRACTION,
      );
    });
  }
  ...
  it("recovers genuinely varied content, not a flat plane a trivially wrong decoder could also produce", () => {
    const fixture = JPEG2000_FIXTURES.find(
      (candidate) => candidate.name === "photo-many-levels",
    );
    expect(fixture).toBeDefined();
    const image = decodeJpeg2000(
...
describe("readJpeg2000Metadata", () => {
  it("reads the coding parameters of a bare codestream", () => {
    ...
    const metadata = readJpeg2000Metadata(
      jpeg2000FixtureBytes(fixture?.codestream ?? ""),
    );
    ...
    expect(metadata.components).toEqual([
      { bitDepth: 8, signed: false, dx: 1, dy: 1 },
    ]);
  ...
  it("reads a JP2 container box structure alongside the codestream it wraps", () => {
    const fixture = JPEG2000_FIXTURES.find(
      (candidate) => candidate.name === "jp2-container",
    );
    const metadata = readJpeg2000Metadata(
      jpeg2000FixtureBytes(fixture?.codestream ?? ""),
    );
  ...
  it("reports the tile grid of a multi-tile codestream", () => {
    const fixture = JPEG2000_FIXTURES.find(
      (candidate) => candidate.name === "multi-tile",
    );
    const metadata = readJpeg2000Metadata(
      jpeg2000FixtureBytes(fixture?.codestream ?? ""),
    );
    ...
    }).toEqual({ tileWidth: 16, tileHeight: 16 });
    expect(metadata.tilesWide * metadata.tilesHigh).toBe(
      metadata.tilePartCount,
    );
    expect(metadata.tilePartCount).toBeGreaterThan(1);
  });

  it("reports the quality layer count and the markers a packet is framed by", () => {
    const layered = readJpeg2000Metadata(
      jpeg2000FixtureBytes(
        JPEG2000_FIXTURES.find((candidate) => candidate.name === "multi-layer")
          ?.codestream ?? "",
      ),
    );
    expect(layered.layers).toBe(3);
    const framed = readJpeg2000Metadata(
      jpeg2000FixtureBytes(
        JPEG2000_FIXTURES.find((candidate) => candidate.name === "sop-eph")
          ?.codestream ?? "",
      ),
    );
    expect({ sop: framed.usesSopMarkers, eph: framed.usesEphMarkers }).toEqual({
      sop: true,
      eph: true,
    });
  });
  ...
  it("reports the irreversible transform and still calls it decodable", () => {
    const metadata = readJpeg2000Metadata(
      jpeg2000FixtureBytes(
        JPEG2000_FIXTURES.find(
          (candidate) => candidate.name === "irreversible-photo",
        )?.codestream ?? "",
      ),
    );
...
describe("decodeJpeg2000: scope refusals", () => {
  const fixture = JPEG2000_FIXTURES.find(
    (candidate) => candidate.name === "ramp-basic",
  );
  ...
  it("refuses the code-block styles whose segment structure it does not read, and accepts the ones it does", () => {
    ...
    expect(readJpeg2000Metadata(patched(styleOffset, 0x10)).decodable).toBe(
      true,
    );
  });
  ...
  it("refuses a position-driven progression order whose precincts genuinely subdivide a resolution level, and says so before decoding", () => {
    ...
    const entry = JPEG2000_FIXTURES.find(
      (candidate) => candidate.name === "progression-rpcl-subdivided",
    );
    ...
    expect(
      readJpeg2000Metadata(
        jpeg2000FixtureBytes(
          JPEG2000_FIXTURES.find(
            (candidate) => candidate.name === "progression-rpcl",
          )?.codestream ?? "",
        ),
      ).decodable,
    ).toBe(true);
  ...
  it("rejects data that is neither a codestream nor a JP2 file", () => {
    expect(() =>
      readJpeg2000Metadata(new Uint8Array([1, 2, 3, 4, 5, 6, 7, 8])),
