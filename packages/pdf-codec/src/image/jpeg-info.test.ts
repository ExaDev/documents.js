...
describe("readJpegInfo", () => {
  ...
  it("recovers grayscale (1-component) and CMYK (4-component) frames", () => {
    expect(
      readJpegInfo(buildJpeg({ width: 10, height: 10, components: 1 }))
        .components,
    ).toBe(1);
    expect(
      readJpegInfo(buildJpeg({ width: 10, height: 10, components: 4 }))
        .components,
    ).toBe(4);
  });
  ...
  it("throws when no SOF marker is found before EOI", () => {
    expect(() =>
      readJpegInfo(new Uint8Array([0xff, 0xd8, 0xff, 0xd9])),
    ).toThrow(/SOF/);
  });
});
