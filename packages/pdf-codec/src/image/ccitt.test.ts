...
function toBlackPixels(
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < columns; x++) {
      const bit =
        ((bytes[y * bytesPerRow + (x >> 3)] ?? 0) >> (7 - (x & 7))) & 1;
...
describe("decodeCcittFax: real libtiff-encoded streams", () => {
    ...
    for (const encoding of ENCODINGS) {
      it(`recovers the "${fixture.name}" bitmap exactly from its ${encoding.name} stream`, () => {
        ...
        const result = decodeCcittFax(
          ccittFixtureBytes(fixture.encodings[encoding.name]),
          {
            k: encoding.k,
        ...
        expect(result.bytes.length).toBe(
          Math.ceil(fixture.columns / 8) * fixture.rows,
        );
        // Compared as rendered rows rather than raw booleans so a failure shows the actual bitmap rather than an index.
        expect(
          renderRows(
            toBlackPixels(result.bytes, fixture.columns, fixture.rows),
            fixture.columns,
          ),
        ).toEqual(renderRows(ccittFixtureBitmap(fixture), fixture.columns));
      });
    }
  }
  ...
  it("stops at the end-of-block marker when the row count is not known in advance", () => {
    ...
    for (const fixture of CCITT_FAX_FIXTURES) {
      for (const encoding of ENCODINGS) {
        const result = decodeCcittFax(
          ccittFixtureBytes(fixture.encodings[encoding.name]),
          { k: encoding.k, columns: fixture.columns },
        );
        expect({
          name: fixture.name,
  ...
  it("inverts the output bit for /BlackIs1", () => {
    ...
    const normal = decodeCcittFax(
      ccittFixtureBytes(fixture!.encodings.group4),
      options,
    );
    const inverted = decodeCcittFax(
      ccittFixtureBytes(fixture!.encodings.group4),
      { ...options, blackIs1: true },
    );
    expect(Array.from(inverted.bytes)).toEqual(
      Array.from(normal.bytes, (byte) => byte ^ 0xff),
    );
    // Same pixels either way once each is read under its own convention.
    expect(
      toBlackPixels(inverted.bytes, fixture!.columns, fixture!.rows, true),
    ).toEqual(toBlackPixels(normal.bytes, fixture!.columns, fixture!.rows));
  });
});
...
describe("decodeCcittFax: /EncodedByteAlign", () => {
  it("decodes a hand-traced Group 4 stream whose rows run straight on from each other", () => {
    const result = decodeCcittFax(HAND_TRACED_UNALIGNED, {
      k: -1,
      columns: 8,
      rows: 2,
    });
  ...
  it("decodes the same image when every row is padded out to a byte boundary", () => {
    const result = decodeCcittFax(HAND_TRACED_BYTE_ALIGNED, {
      k: -1,
      columns: 8,
      rows: 2,
  ...
  it("misreads a byte-aligned stream when the parameter is not set, which is why it exists", () => {
    ...
    expect(renderRows(toBlackPixels(result.bytes, 8, 2), 8)).not.toEqual(
      HAND_TRACED_ROWS,
    );
  });
  ...
  it("reads the row count off the end of the data when /Rows is not given", () => {
    expect(
      decodeCcittFax(HAND_TRACED_UNALIGNED, { k: -1, columns: 8 }).rows,
    ).toBe(2);
    expect(
      decodeCcittFax(HAND_TRACED_BYTE_ALIGNED, {
        k: -1,
        columns: 8,
        encodedByteAlign: true,
      }).rows,
...
describe("decodeCcittFax: degradation", () => {
  it("pads a stream that runs out early with white rows and says so", () => {
    ...
    expect(warnings).toEqual([
      "CCITT fax data ended after 2 of 4 declared rows; padding the remainder white",
    ]);
  });
  ...
  it("keeps the rows it recovered when the bitstream stops making sense", () => {
    ...
    const result = decodeCcittFax(new Uint8Array([0x80, 0x02, 0x00, 0x00]), {
      k: -1,
      columns: 8,
      rows: 3,
      encodedByteAlign: true,
  ...
  it("stops on a code that is in neither run-length table, whether or not the data runs out mid-code", () => {
    for (const truncated of [
      ...
      const warnings: string[] = [];
      const result = decodeCcittFax(truncated, {
        k: 0,
        columns: 8,
        rows: 2,
        onWarning: (message) => warnings.push(message),
  ...
  it("returns nothing at all for a zero-column image", () => {
    expect(
      decodeCcittFax(HAND_TRACED_UNALIGNED, { k: -1, columns: 0, rows: 2 }),
    ).toEqual({ bytes: new Uint8Array(0), columns: 0, rows: 0 });
  });
  ...
  it("leaves the padding bits of a row that does not fill its last byte white", () => {
    ...
    const result = decodeCcittFax(
      ccittFixtureBytes(fixture!.encodings.group4),
      { k: -1, columns: 13, rows: fixture!.rows },
    );
