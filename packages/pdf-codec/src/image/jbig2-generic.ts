...
function resolveTemplate(template: number): readonly TemplatePosition[] {
  ...
  if (positions === undefined) {
    throw new Jbig2UnsupportedError(
      `generic region declares GBTEMPLATE ${String(template)}, which is outside the 0-3 range T.88 6.2.5.3 defines`,
    );
  }
  return positions;
}
...
export function decodeRefinementRegion(
  width: number,
  height: number,
  ...
  if (positions === undefined) {
    throw new Jbig2UnsupportedError(
      `refinement region declares GRTEMPLATE ${String(params.template)}, which is outside the 0-1 range T.88 6.3.5.3 defines`,
    );
  }
  if (params.tpgron) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (const position of positions) {
        if (position.source === "destination") {
          context =
            (context << 1) | getPixel(bitmap, x + position.dx, y + position.dy);
        } else if (position.source === "reference") {
          context =
            (context << 1) |
            getPixel(
              params.reference,
              x - params.dx + position.dx,
              y - params.dy + position.dy,
            );
        } else if (position.source === "destination-at") {
          const atPixel = params.at[position.index] ?? { x: 0, y: 0 };
          context =
            (context << 1) | getPixel(bitmap, x + atPixel.x, y + atPixel.y);
        } else {
          const atPixel = params.at[position.index] ?? { x: 0, y: 0 };
          context =
            (context << 1) |
            getPixel(
              params.reference,
              x - params.dx + atPixel.x,
              y - params.dy + atPixel.y,
            );
        }
      }
