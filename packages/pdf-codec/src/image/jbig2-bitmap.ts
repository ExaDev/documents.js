...
export class Jbig2BitmapTooLargeError extends Jbig2ParseError {
  constructor(width: number, height: number) {
    super(
      `JBIG2 bitmap of ${String(width)}x${String(height)} exceeds the ${String(MAX_JBIG2_BITMAP_PIXELS)}-pixel limit this decoder allocates`,
    );
...
export function createBitmap(
  ...
  if (
    ...
    width * height > MAX_JBIG2_BITMAP_PIXELS
  ) {
...
export type Jbig2CombinationOperator =
  "or" | "and" | "xor" | "xnor" | "replace";

const COMBINATION_OPERATORS: readonly Jbig2CombinationOperator[] = [
  "or",
  "and",
  "xor",
  "xnor",
...
export function combinationOperatorFromCode(
  code: number,
): Jbig2CombinationOperator | undefined {
...
function combineValue(
  destination: number,
  source: number,
  operator: Jbig2CombinationOperator,
): number {
  if (operator === "or") {
...
export function combineBitmap(
  ...
  for (let sy = 0; sy < source.height; sy++) {
    ...
    for (let sx = 0; sx < source.width; sx++) {
      ...
      destination.data[destinationIndex] = combineValue(
        destination.data[destinationIndex] ?? 0,
        source.data[sy * source.width + sx] ?? 0,
        operator,
      );
    }
  }
