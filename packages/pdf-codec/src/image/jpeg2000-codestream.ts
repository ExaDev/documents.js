...
class MarkerCursor {
  ...
  uint8(): number {
    ...
    if (value === undefined) {
      throw new Jpeg2000ParseError(
        "JPEG 2000 codestream ended in the middle of a marker segment",
      );
    }
    this.position++;
    return value;
  ...
  bytes(length: number): Uint8Array<ArrayBuffer> {
    if (length < 0 || this.position + length > this.data.length) {
      throw new Jpeg2000ParseError(
        "a JPEG 2000 marker segment declares more data than the codestream carries",
      );
    }
...
function readImageSize(
  ...
  if (xsiz <= xosiz || ysiz <= yosiz) {
    throw new Jpeg2000ParseError(
      "SIZ declares an image with no area (Xsiz/Ysiz do not exceed XOsiz/YOsiz)",
    );
  }
  if (xtsiz === 0 || ytsiz === 0) {
...
function readQuantization(
  ...
  throw new Jpeg2000ParseError(
    `QCD/QCC declares quantization style ${String(styleCode)}, which ISO/IEC 15444-1 Table A.28 does not define`,
  );
}
...
function readHeaderSegment(
  ...
  if (marker === MARKER_COD) {
    ...
    header.coc.set(
      component,
      readCodingStyleParameters(cursor, (cursor.uint8() & 0x01) !== 0),
    );
...
function validateMainHeader(header: MutableHeader): void {
  if (header.cod === undefined) {
    throw new Jpeg2000ParseError(
      "the main header carries no COD marker, so no coding style is defined for any tile",
    );
  }
  if (header.qcd === undefined) {
    throw new Jpeg2000ParseError(
      "the main header carries no QCD marker, so no quantization is defined for any tile",
    );
  }
}
...
export function parseJpeg2000Codestream(
  ...
  if (cursor.uint16() !== MARKER_SIZ) {
    throw new Jpeg2000ParseError(
      "the SOC marker is not immediately followed by SIZ, which ISO/IEC 15444-1 A.3 requires",
    );
  }
  ...
  for (;;) {
    ...
    if (marker === MARKER_PPM) {
      throw new Jpeg2000UnsupportedError(
        "the codestream carries a PPM marker (packed packet headers in the main header), an organisation this decoder does not reassemble",
      );
    }
...
function readTilePart(
  ...
  for (;;) {
    if (cursor.remaining < 2) {
      throw new Jpeg2000ParseError(
        "a tile-part header ended without an SOD marker",
      );
    }
    const marker = cursor.uint16();
  ...
  if (dataEnd < dataStart) {
    throw new Jpeg2000ParseError(
      "a tile-part declares a Psot shorter than its own header",
    );
  }
