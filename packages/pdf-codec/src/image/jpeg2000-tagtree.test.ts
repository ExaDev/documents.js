...
describe("PacketBitReader", () => {
  it("reads bits most significant first", () => {
    const reader = new PacketBitReader(
      Uint8Array.from([0b10110010, 0b01000000]),
      0,
      2,
    );
    expect([0, 1, 2, 3, 4, 5, 6, 7].map(() => reader.readBit())).toEqual([
      1, 0, 1, 1, 0, 0, 1, 0,
    ]);
  ...
  it("takes only seven bits from the byte after a 0xFF", () => {
    ...
    const reader = new PacketBitReader(
      Uint8Array.from([0xff, 0b01010101]),
      0,
      2,
    );
    expect(Array.from({ length: 8 }, () => reader.readBit())).toEqual([
      1, 1, 1, 1, 1, 1, 1, 1,
    ]);
    expect(Array.from({ length: 7 }, () => reader.readBit())).toEqual([
      1, 0, 1, 0, 1, 0, 1,
    ]);
  });
  ...
  it("discards the stuffed byte as well as the part-read one when aligning", () => {
    const plain = new PacketBitReader(
    ...
    const stuffed = new PacketBitReader(
      Uint8Array.from([0xff, 0x00, 0x56]),
      0,
      3,
    );
    for (let i = 0; i < 8; i++) {
  ...
  it("yields zero bits past the end rather than reading outside its own range", () => {
    ...
    expect(Array.from({ length: 16 }, () => reader.readBit())).toEqual(
...
describe("TagTree", () => {
  ...
  it("builds a level hierarchy that halves both axes until a single root remains", () => {
    ...
    const reader = new PacketBitReader(
      Uint8Array.from([0xff, 0x00, 0x00, 0x00, 0x00, 0x00]),
      0,
      6,
    );
