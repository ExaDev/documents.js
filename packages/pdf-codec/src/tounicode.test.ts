...
function cmapText(
  ...
  const stream: PdfObject = buildToUnicodeCMap(codeToSequence);
...
function bfcharBlockCounts(text: string): number[] {
  return [...text.matchAll(/^(\d+) beginbfchar$/gm)].map((match) =>
    Number(match[1]),
  );
}
...
describe("buildToUnicodeCMap", () => {
  ...
  it("splits into blocks of at most 100 entries, which is the limit the CMap syntax sets", () => {
    ...
    expect(
      blocks.map(
        (block) => block.split("endbfchar")[0]!.trim().split("\n").length,
      ),
    ).toEqual([100, 100, 50]);
  });
  ...
  it("round-trips through this package own CMap reader, across a block boundary", () => {
    ...
    const parsed = parseToUnicodeCMap(
      new TextEncoder().encode(cmapText(entries)),
    ...
    for (const [code, sequence] of entries) {
      expect(parsed.lookup(code)).toBe(String.fromCodePoint(...sequence));
    }
  ...
  it("emits entries sorted by code, so identical input yields byte-identical output", () => {
    ...
    expect(cmapText(forwards)).toContain(
      "<0003> <0041>\n<0005> <0042>\n<0009> <0043>",
    );
  });
