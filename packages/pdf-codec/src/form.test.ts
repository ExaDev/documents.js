...
function findField(
  fields: readonly LayoutFormField[] | undefined,
  name: string,
...
describe("readPdf: AcroForm fields", () => {
  ...
  it("reads a non-terminal field as a group whose children carry fully-qualified names", () => {
    ...
    expect(contact?.children.map((c) => c.name)).toEqual([
      "contact.country",
      "contact.subscribe",
    ]);
  });
  ...
  it("maps /FT /Ch with the combo flag to comboBox and reads /Opt choices and /V", () => {
    ...
    expect(combo?.widgets).toEqual([
      { pageIndex: 0, xPt: 10, yPt: 60, widthPt: 100, heightPt: 14 },
    ]);
  });
  ...
  it("collects the root field list in document order", () => {
    const doc = readPdf(acroFormPdf());
    expect(doc.form?.map((f) => f.name)).toEqual([
      "fullname",
      "contact",
      "sig",
    ]);
