...
describe('dimensions', () => {
  ...
  it('multiplies dimensions by adding exponents', () => {
    expect(multiplyDimensions({ length: 1 }, { length: 1 })).toEqual({ length: 2 });
    expect(multiplyDimensions({ length: 1 }, { time: -1 })).toEqual({ length: 1, time: -1 });
  ...
  it('scales every exponent for pow/sqrt', () => {
    expect(scaleDimension({ length: 2, mass: 4 }, 0.5)).toEqual({ length: 1, mass: 2 });
