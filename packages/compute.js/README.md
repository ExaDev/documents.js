...
[![GitHub](https://img.shields.io/badge/GitHub-181717?logo=github&logoColor=white)](https://github.com/ExaDev/documents.js/tree/main/packages/compute.js) [![npm](https://img.shields.io/badge/npm-CB3837?logo=npm&logoColor=white)](https://www.npmjs.com/package/compute.js) [![npm version](https://img.shields.io/npm/v/compute.js)](https://www.npmjs.com/package/compute.js) [![CI](https://img.shields.io/github/actions/workflow/status/ExaDev/documents.js/ci.yml?branch=main)](https://github.com/ExaDev/documents.js/actions)
...
| `compute/quantity` | `Quantity`, `QuantitySchema`, `quantity`, `addQuantities`, `subtractQuantities`, `multiplyQuantities`, `divideQuantities`, `negateQuantity`, `absQuantity`, `powQuantity`, `sqrtQuantity`, `sinQuantity`, `cosQuantity`, `tanQuantity` |
| `compute/interval` | `Interval`, `IntervalSchema`, `interval`, `pointInterval`, `addIntervals`, `subtractIntervals`, `multiplyIntervals`, `divideIntervals`, `negateInterval`, `absInterval` |
| `compute/bindings` | `FormulaBindings`, `FormulaBindingsSchema`, `EvaluationValue`, `EvaluationValueSchema` |
...
Every module in the table is re-exported from the package root, so its exports import from `'compute.js'` directly.

```ts
import { evaluate } from 'compute.js';
...
```ts
import { solveFor } from 'compute.js';
import type { MathExpression } from 'document-schema.js';
...
## Why `Quantity` and `FormulaBindings` live here, not in `document-schema.js`

The issue's own proposal places `Quantity` and `FormulaBindings` in `document-schema.js`, "beside the math value itself so evaluation inputs are typed contracts like everything else." That is the right long-term home: `MathExpression` already lives there, and a runtime value type sitting next to the tree it evaluates is the natural shape once the pattern is proven. It is not where they are defined in this pass, for a mechanical reason this repository's own root README states directly — `document-schema.js` is a published, versioned sibling, and adding a public schema to it is a coordinated minor/major release on that package, not a change this package's own PR can make unilaterally. Rather than skip the schemas (the issue is explicit that units-typed evaluation needs them) or vendor a workaround, they are defined here, in `compute/quantity.ts` and `compute/bindings.ts`, deliberately flagged as a **known compromise**: `Quantity` is `compute.js`'s own runtime-result schema (a resolved magnitude plus dimension vector — *not* the same shape as `document-schema.js`'s `MathQty`, which is an unresolved expression-tree leaf carrying a unit-registry id rather than a dimension vector), and `FormulaBindings` is the record of symbol ids to `Quantity | Interval` this evaluator consumes. **Follow-up**: if this evaluator's `Quantity`/`FormulaBindings` shapes prove out in practice, move them into `document-schema.js` (coordinated as a real release there, per [ExaDev/document-schema.js#15](https://github.com/ExaDev/document-schema.js/issues/15)) and have this package import them the same way it already imports `MathExpression`, `DimensionVector`, and `ExactRational` — rather than carrying a second, package-local definition indefinitely.
