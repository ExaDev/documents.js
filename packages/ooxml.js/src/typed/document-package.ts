...
//
// What a package-native reader does NOT carry is the per-format data that has no ContentDocument spelling and therefore no DocumentPackage spelling either: readDocxContent's own comments, footnotes, header/footer parts, and numbering definitions all live on DocxDocument, outside its `sections`, and none of them survives into the tree. That is not a loss this module introduces -- buildDocxPackageFromContent already writes none of those parts, so they never survived the flat pair either -- but it is the reason readDocxContent stays exported rather than being folded away: it is the only function in this package that returns them at all.
//
...
// A decoded docx Package -> the tree-form DocumentPackage, via readDocxContent's own full style-cascade and construct walk. The DocxDocument fields outside `sections` (comments, footnotes, header/footer parts, numbering) have no place in the tree and are dropped here; readDocxContent is the reader that returns them.
