...
export function readXfrm(
  ...
  return {
    ...
    rotationDeg:
      rot === undefined ? 0 : Number(rot) / ROTATION_UNITS_PER_DEGREE,
...
function readClrScheme(clrSchemeEl: XmlElement): Map<string, Color> {
  ...
  for (const slot of CLR_SCHEME_SLOTS) {
    ...
    const colorEl = wrapper.children.find(
...
function readSchemeFont(
  fontSchemeEl: XmlElement | undefined,
  ...
  const latin =
    fontEl === undefined ? undefined : childrenWithTag(fontEl, "a:latin")[0];
...
export const EMPTY_THEME: DrawingTheme = {
  colorScheme: new Map(),
...
export function readTheme(themeRoot: XmlElement): DrawingTheme {
  ...
  const colorScheme =
    clrSchemeEl === undefined
      ? new Map<string, Color>()
      : readClrScheme(clrSchemeEl);
  const fontSchemeEl = elementsWithTag([themeRoot], "a:fontScheme")[0];
  return {
    colorScheme,
    majorFont:
      readSchemeFont(fontSchemeEl, "a:majorFont") ?? DEFAULT_THEME_FONT,
    minorFont:
      readSchemeFont(fontSchemeEl, "a:minorFont") ?? DEFAULT_THEME_FONT,
  };
}
...
export function resolveThemeFontReference(
  typeface: string,
  theme: DrawingTheme,
): string {
  if (typeface === "+mj-lt") {
...
const COLOR_TRANSFORM_TAGS: ReadonlyMap<string, ColorTransform["kind"]> =
  new Map([
    ["a:shade", "shade"],
    ["a:tint", "tint"],
    ["a:lumMod", "lumMod"],
...
export function readSchemeColor(
  schemeClrEl: XmlElement,
  colorMap: ReadonlyMap<string, string>,
  theme: DrawingTheme,
): Color | undefined {
  ...
  return base === undefined
    ? undefined
    : applyColorTransforms(base, readColorTransforms(schemeClrEl));
}
...
export function readSrgbColor(srgbClrEl: XmlElement): Color | undefined {
  ...
  return val === undefined
    ? undefined
    : applyColorTransforms(rgbHexToColor(val), readColorTransforms(srgbClrEl));
}
...
export function readSolidFillColor(
  solidFillEl: XmlElement | undefined,
  colorMap: ReadonlyMap<string, string>,
  theme: DrawingTheme,
): Color | undefined {
...
function canonicalizeGroupRotation(
  rotationDeg: number,
  flipH: boolean,
  flipV: boolean,
...
): GroupChildTransform | undefined {
  if (own === undefined) {
    return undefined;
  }
  const ownRotation = canonicalizeGroupRotation(
    own.rotationDeg,
    own.flipH,
    own.flipV,
  );
...
export function applyGroupTransform(
  ...
  const scaleY =
    group.childExtHeightPt === 0
      ? 1
      : group.extHeightPt / group.childExtHeightPt;
