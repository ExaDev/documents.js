...
function readSlideSize(presentationRoot: XmlElement | undefined): PageSize {
  const sldSz =
    presentationRoot === undefined
      ? undefined
      : childrenWithTag(presentationRoot, "p:sldSz")[0];
...
function mergeRunProperties(
  base: DefaultRunProperties,
  override: DefaultRunProperties,
...
function readRun(
  ...
  const rPr = childrenWithTag(runEl, "a:rPr")[0];
  const explicit =
    rPr === undefined ? {} : readRunPropertiesFromElement(rPr, context);
...
function readLineSpacingMultiplier(
  ...
  const lnSpc =
    pPr === undefined ? undefined : childrenWithTag(pPr, "a:lnSpc")[0];
  const pct =
...
function readParagraph(
  ...
  const masterDefaults = resolveDefaultRunProperties(
    placeholderType,
    level,
    context,
  );
  const pPrDefRPr =
    pPr === undefined ? undefined : childrenWithTag(pPr, "a:defRPr")[0];
  const paragraphDefaults =
    pPrDefRPr === undefined
      ? masterDefaults
      : mergeRunProperties(
          masterDefaults,
          readRunPropertiesFromElement(pPrDefRPr, context),
        );
    ...
    if (child.tag === "a:r" || child.tag === "a:fld") {
        ...
        if (type !== undefined) {
          ...
          fieldExtents.push({
            descriptor: {
              kind: "field",
              instruction: type,
              cachedResult: cached === undefined ? "" : textContent(cached),
            },
      ...
      if (jumpTarget !== undefined) {
        linkExtents.push({
          descriptor: {
            kind: "link",
            target: { kind: "internal", anchor: jumpTarget },
          },
          startRun: runs.length - 1,
          endRun: runs.length,
        });
  ...
  return {
    kind: "paragraph",
    runs,
    ...(fieldExtents.length + linkExtents.length > 0
    ...
    spacingAfterPt: readAbsoluteSpacingPt(
      pPr === undefined ? undefined : childrenWithTag(pPr, "a:spcAft")[0],
    ),
    lineSpacing: readLineSpacingMultiplier(pPr),
    indentLeftPt: marL === undefined ? undefined : emuToPt(Number(marL)),
    indentFirstLinePt:
      indent === undefined ? undefined : emuToPt(Number(indent)),
  };
}

function textBodyParagraphs(
  txBody: XmlElement | undefined,
  placeholderType: string | undefined,
  context: SlideInheritanceContext,
  slideRels: ReadonlyMap<string, Relationship>,
): ContentParagraph[] {
  return txBody === undefined
    ? []
    : childrenWithTag(txBody, "a:p").map((p) =>
        readParagraph(p, placeholderType, context, slideRels),
      );
}
...
const NO_TEXT_BODY_EXTRAS: ShapeTextExtras = {
  insetLeftPt: 0,
  insetTopPt: 0,
  insetRightPt: 0,
  insetBottomPt: 0,
...
function readShapeTextExtras(txBody: XmlElement | undefined): ShapeTextExtras {
  ...
  const normAutofit =
    bodyPr === undefined
      ? undefined
      : childrenWithTag(bodyPr, "a:normAutofit")[0];
  const fontScale =
    normAutofit === undefined ? undefined : attr(normAutofit, "fontScale");
  const lnSpcReduction =
    normAutofit === undefined ? undefined : attr(normAutofit, "lnSpcReduction");
  return {
    insetLeftPt: emuToPt(
      lIns === undefined ? DEFAULT_INSET_LEFT_RIGHT_EMU : Number(lIns),
    ),
    insetTopPt: emuToPt(
      tIns === undefined ? DEFAULT_INSET_TOP_BOTTOM_EMU : Number(tIns),
    ),
    insetRightPt: emuToPt(
      rIns === undefined ? DEFAULT_INSET_LEFT_RIGHT_EMU : Number(rIns),
    ),
    insetBottomPt: emuToPt(
...
function resolveShapeFrame(
...
): { frame: Box; rotationDeg: number | undefined } | undefined {
  ...
  const xfrm =
    ownXfrm ??
    (key === undefined ? undefined : resolvePlaceholderXfrm(key, context));
  if (xfrm === undefined) {
    return undefined;
  }
  const localFrame: Box = {
    xPt: xfrm.xPt,
    yPt: xfrm.yPt,
  ...
  const rotationDeg = composeShapeRotationDeg(
    parentTransform,
    xfrm.rotationDeg,
  );
  return {
...
function readGraphicFrameShape(
  gf: XmlElement,
  context: SlideInheritanceContext,
  slideRels: ReadonlyMap<string, Relationship>,
  pkg: Package,
  ...
  const rotationDeg =
    composedRotationDeg === 0 ? undefined : composedRotationDeg;
...
function walkShapeTreeChildren(
  ...
  for (const node of children) {
    ...
    if (node.tag === "p:sp") {
      ...
      const shape = readGraphicFrameShape(
        node,
        context,
        slideRels,
        pkg,
      ...
      const xfrmEl =
        grpSpPr === undefined
          ? undefined
          : childrenWithTag(grpSpPr, "a:xfrm")[0];
      const ownGroupTransform = readGroupXfrm(xfrmEl);
      const composed = composeGroupTransform(
        ownGroupTransform,
        parentTransform,
      );
