...
export function resolveSlideInheritance(
  ...
  const layoutRoot =
    layoutPath === undefined ? undefined : rootElement(pkg.parts[layoutPath]);

  const masterPath =
    layoutPath === undefined
      ? undefined
      : findRelTarget(pkg, layoutPath, SLIDE_MASTER_REL_SUFFIX);
  const masterRoot =
    masterPath === undefined ? undefined : rootElement(pkg.parts[masterPath]);

  const themePath =
    masterPath === undefined
      ? undefined
      : findRelTarget(pkg, masterPath, THEME_REL_SUFFIX);
  const themeRoot =
    themePath === undefined ? undefined : rootElement(pkg.parts[themePath]);
  const theme = themeRoot === undefined ? EMPTY_THEME : readTheme(themeRoot);

  const clrMapEl =
    masterRoot === undefined
      ? undefined
      : childrenWithTag(masterRoot, "p:clrMap")[0];
...
function normalizePlaceholderType(
  type: string | undefined,
): string | undefined {
  return type === undefined
    ? undefined
    : (TYPE_NORMALIZATION.get(type) ?? type);
}
...
export function readPlaceholderKey(
  shape: XmlElement,
): PlaceholderKey | undefined {
  const ph = elementsWithTag([shape], "p:ph")[0];
  return ph === undefined
    ? undefined
    : { type: attr(ph, "type"), idx: attr(ph, "idx") };
}
...
export function findMatchingPlaceholder(
  ...
  if (key.idx !== undefined) {
    const byIdx = shapes.find(
      (shape) => readPlaceholderKey(shape)?.idx === key.idx,
    );
    if (byIdx !== undefined) {
      return byIdx;
    }
  ...
  return shapes.find(
    (shape) =>
      normalizePlaceholderType(readPlaceholderKey(shape)?.type) ===
      normalizedTarget,
  );
}
...
export function resolvePlaceholderXfrm(
  ...
  const layoutXfrm = shapeXfrm(
    findMatchingPlaceholder(context.layoutRoot, key),
  );
  return (
    layoutXfrm ?? shapeXfrm(findMatchingPlaceholder(context.masterRoot, key))
  );
}
...
export function readRunPropertiesFromElement(
  ...
  return {
    fontFamily:
      typeface === undefined
        ? undefined
        : resolveThemeFontReference(typeface, context.theme),
...
export function resolveDefaultRunProperties(
  ...
  const styleEl =
    txStyles === undefined
      ? undefined
      : childrenWithTag(txStyles, txStyleTagFor(placeholderType))[0];
  const lvlPPr =
    styleEl === undefined
      ? undefined
      : childrenWithTag(styleEl, levelTag(level))[0];
  const defRPr =
    lvlPPr === undefined ? undefined : childrenWithTag(lvlPPr, "a:defRPr")[0];
  return defRPr === undefined
    ? {}
    : readRunPropertiesFromElement(defRPr, context);
}
