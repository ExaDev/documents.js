...
export const DocxDocumentSchema = z.object({
  ...
  // The structural view of the header/footer layer, and the only one this schema carries: each referenced part as block flow, and per-section references (positional, one entry per `sections` entry) naming those parts by path.
