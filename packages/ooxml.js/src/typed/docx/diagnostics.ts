// The docx write-side diagnostic sink (ExaDev/documents.js#1398). buildDocxPackageFromContent had no diagnostic channel at all before this file existed, so a construct this writer could not carry into WordprocessingML, such as a table column's own isHeader (document-schema.js's ContentTableColumn), was simply left out of the written document with nothing telling a caller it happened. This mirrors the shape markdown-codec's, pdf-codec's, epub-codec's, rtf-codec's and ppt-codec's own write-side diagnostic channels already use in this family, and the identical PptxWriteDiagnostic pattern documents.js's own src/edit/pptx/diagnostics.ts already applies for its own write-side degrade decisions (ExaDev/documents.js#1389, #1403).

export type DocxWriteDiagnosticSeverity = "info" | "warning";

// One stable code per degrade decision buildDocxPackageFromContent's own write path can make, mirroring PptxWriteDiagnosticCodes'/PptDiagnosticCodes' identical convention: a flat namespaced string a caller can branch or filter on, never free-form message text alone.
...
// context mirrors OmmlDiagnosticSink's/PptxWriteDiagnosticSink's own shape exactly: sourcePath is the table block's own path back into the source ContentDocument, when it carries one (ContentTable.sourcePath), so a caller can name which table each diagnostic came from. ContentTableColumn itself carries no sourcePath of its own (document-schema.js), so the table's is the finest-grained location this sink can report; the message names the column index within it.
export type DocxWriteDiagnosticSink = (
  diagnostic: DocxWriteDiagnostic,
  context: { readonly sourcePath?: string },
) => void;

// Deliberately empty: the default when a caller supplies no onDiagnostic, matching NOOP_PPTX_WRITE_DIAGNOSTIC_SINK's/NOOP_PPT_DIAGNOSTIC_SINK's own precedent in this family.
