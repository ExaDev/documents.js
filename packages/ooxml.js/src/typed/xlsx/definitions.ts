...
import { attr, childrenWithTag, resolveRelationships, rootElement, textContent } from '../util';
...
function readNamedRanges(pkg: Package, out: DefinitionsTable): void {
  ...
  const container = workbook === undefined ? undefined : childrenWithTag(workbook, 'definedNames')[0];
  if (container === undefined) {
    return;
  }
  for (const definedName of childrenWithTag(container, 'definedName')) {
    const name = attr(definedName, 'name');
    if (name === undefined || name === XLNM_PRINT_AREA || name === XLNM_PRINT_TITLES) {
      continue;
    }
    const localSheetIdRaw = attr(definedName, 'localSheetId');
    const localSheetId = localSheetIdRaw === undefined ? undefined : Number.parseInt(localSheetIdRaw, 10);
    out[`namedRange:${name}`] = {
      kind: 'namedRange',
      name,
...
function readTableEntries(pkg: Package, out: DefinitionsTable): void {
  for (const entry of resolveSheetEntries(pkg)) {
    for (const rel of resolveRelationships(pkg, entry.path).values()) {
      ...
      out[`table:${name}`] = { kind: 'table', name, ref, sheet: entry.name, columns };
    }
  }
}
...
export function readWorkbookDefinitions(pkg: Package): DefinitionsTable | undefined {
