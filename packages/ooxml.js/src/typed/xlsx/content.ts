...
function readRows(worksheet: XmlElement): ContentSheetRow[] {
  ...
  for (const row of childrenWithTag(sheetData, "row")) {
    ...
    const rowNumber =
      rRaw === undefined ? undefined : Number.parseInt(rRaw, 10);
    if (
      rowNumber === undefined ||
...
): CellStyleEntry | undefined {
  const raw = attr(cell, "s");
  const index =
    raw === undefined ? DEFAULT_CELL_STYLE_INDEX : Number.parseInt(raw, 10);
...
function readCellValue(
  ...
  numericFormats: CellFormatContext,
): ResolvedCellValue | undefined {
  const type = attr(cell, "t");
  if (type === "inlineStr") {
    const is = childrenWithTag(cell, "is")[0];
    const text =
      is === undefined ? undefined : readInlineOrSharedStringText(is);
    return text === undefined
      ? undefined
      : { value: { kind: "string", value: text }, displayText: text };
  }
...
function readCell(
  ...
  const position =
    reference === undefined ? undefined : parseCellReference(reference);
  ...
  const cellEntry: ContentSheetCell = {
    row: position.row,
    column: position.column,
  ...
  for (const rule of rules) {
    ...
    if (existing === undefined) {
      const materialised: ContentSheetCell = {
        ...
        value: { kind: "empty" },
        displayText: "",
        source: { format: "xlsx", xml: buildXml([rule]) },
      };
      cells.push(materialised);
      byPosition.set(key, materialised);
      continue;
