...
//
// This module imports all three format readers while pptx/read.ts and docx/read.ts both import this module back -- module cycles that are safe under one discipline: every cross-use is call-time only, both sides export hoisted function declarations, and nothing at module-evaluation time may read a cycle partner's bindings (a top-level const whose initialiser touched a partner would TDZ, because ESM initialises a cycle's modules in an order the import graph does not pin). odf.js's equivalent (typed/draw/embedded.ts) instead moved dispatch into each calling format reader to break its analogous cycle, and that split genuinely does not transfer here: odf's embedding edges all point one way (only its spreadsheet reader resolves embedded sub-documents, dispatching to the draw/presentation/text/formula readers, and none of those dispatches back), so per-reader dispatch creates no reader-to-reader edges -- while OOXML embedding is symmetric, a docx can embed a pptx and a pptx a docx, so per-reader dispatch would trade each reader's single edge to this module for direct docx-to-pptx reader edges that re-create the cycle. Injecting the dispatch through a read context instead would change the public readDocxContent/readPptxContent signatures, which construct their own contexts and stand alone.
