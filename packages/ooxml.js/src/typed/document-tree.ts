...
//
// What a package-native reader does NOT carry is the per-format data that has no ContentDocument spelling and therefore no DocumentTree spelling either: readDocxContent's own comments, footnotes, header/footer parts, and numbering definitions all live on DocxDocument, outside its `sections`, and none of them survives into the tree -- readDocx below discards them the moment it destructures `{ metadata, sections }` out of readDocxContent's own result. This IS a loss this module introduces, unlike before ExaDev/documents.js#968: buildDocxPackageFromContent now writes every one of those parts back out when its own DocxContent input carries them, so the flat pair (readDocxContent/buildDocxPackageFromContent) round-trips a whole DocxDocument -- it is only the tree pair (readDocx/buildDocxPackage) that still cannot, because DocumentTree itself has nowhere to carry them, and buildDocxPackage below only ever receives a flattened DocumentTree. This is the reason readDocxContent (and, symmetrically, buildDocxPackageFromContent) stay exported rather than being folded away: they are the only functions in this package that carry them at all.
//
