...
// --- The DocumentTree-native surface: one reader per format producing document-schema.js's tree-form DocumentTree, one writer per format consuming it. These carry the primary names because the tree is what a caller holding a whole document wants; each wraps the flat, content-level function of the same format (readDocxContent/readPptxContent/readXlsxContent, buildDocxPackageFromContent/buildXlsxPackageFromContent, all still exported below) through assembleTree on the way out and flattenTree on the way in. See typed/document-tree.ts for why every reader mints styles rather than calling bare decompose, and for what a docx's comments/footnotes/header-footer parts/numbering do instead of riding the tree. ---
export {
  readDocx,
  buildDocxPackage,
  readPptx,
...
// word/numbering.xml's own abstractNum/num level definitions (glyph format, start-at value, restart rule) — a companion to, not a replacement for, ContentListMembership's existing per-paragraph numId/level tracking. See numbering.ts's own doc comment for why this is a separate keyed structure rather than a ContentListMembership field. buildNumberingElement is readNumberingDefinitions's inverse, exported so a consumer that only needs to synthesise a numbering part from its own NumberingDefinitions (documents.js's edit/docx/numbering.ts, building one from ContentListMembership rather than from a DocxContent) can reuse this writer instead of hand-rolling a second one.
export {
  NumberingDefinitionSchema,
  NumberingLevelSchema,
  readNumberingDefinitions,
  buildNumberingElement,
} from "./typed/docx/numbering";
export type {
  NumberingDefinition,
