...
// fflate's own DOS-date encoding range: the format's four-bit year field is an offset from 1980, and fflate itself throws for a year outside this range.
const FFLATE_DOS_DATE_YEAR_FLOOR = 1980;
const FFLATE_DOS_DATE_YEAR_CEILING = 2099;
...
describe("zipPackage: entry mtime survives every real-world timezone offset", () => {
  it.each([
    ...
    expect(year).toBeGreaterThanOrEqual(FFLATE_DOS_DATE_YEAR_FLOOR);
    expect(year).toBeLessThanOrEqual(FFLATE_DOS_DATE_YEAR_CEILING);
  });
});
