...
`readDocxContent` is the only reader in this package that returns a docx's comments, footnotes, endnotes, header/footer parts, section break kinds, and `word/numbering.xml` definitions: those sit on `DocxDocument` outside its `sections` (a section's `breakType` excepted — that rides `ContentSection` itself), which is exactly the part of it `ContentDocument` — and therefore the tree — has no place for. `buildDocxPackageFromContent` DOES write them back when its `DocxContent` input carries them (its own `comments`/`footnotes`/`endnotes`/`headerFooterParts`/`sectionHeaderFooters`/`numbering` fields, each optional and each satisfied directly by a `DocxDocument`'s own field of the same name) — a `readDocxContent` → `buildDocxPackageFromContent` round trip therefore carries the whole `DocxDocument`, not only `sections`. It is `buildDocxPackage`, the tree-form pair, that still drops them: `DocumentTree` has no place for them either, so `readDocx` discards them on the way in and `buildDocxPackage` never sees them on the way out. `endnotes`, `headerFooterParts`, and `sectionHeaderFooters` are required fields on the published `DocxDocument` shape — a reader always populates them (an empty array when the docx spells none), and code constructing a `DocxDocument` by hand rather than reading one must spell them too; `DocxContent`'s own equivalents are optional, since a caller driving `buildDocxPackageFromContent` directly need not populate parts it does not care about.
