...
- **Character sets 2 and above**, and the part of set 1 the mirrored SDK pages do not tabulate. This is a missing source rather than an unfinished feature: the mirrored SDK Help states the mechanism — "The high byte is the number of the WordPerfect character set. The low byte contains an offset value into the character set" — and tabulates the thirty-two Default Extended International Characters, but carries no character-set table of its own for any set. The full WordPerfect character sets are published elsewhere, so this closes when one of those is transcribed against a citable source, not by inference. An unmapped character renders as U+FFFD and is reported through `wpd/unmapped-character` rather than dropped.
