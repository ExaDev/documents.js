...
const config: Options = {
  ...
  plugins: [
    ...
    [
      ...
      {
        // Deliberately angular, not conventionalcommits: see ooxml.js's identical note -- conventional-changelog-writer's bundled commit partial doesn't match the conventionalcommits preset's function-based partial signature, producing a changelog with a version header and nothing under it. angular is release-notes-generator's own tested default and renders correctly; commitTypes still drives commit-analyzer's releaseRules and commitlint's type-enum regardless of which changelog preset is used.
        preset: 'angular',
      },
    ],
