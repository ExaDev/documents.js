...
export type ContentReader = (bytes: Uint8Array, options?: DocumentCodecOptions) => ContentDocument;
...
export const CONTENT_READERS: Readonly<Record<ReadContentFormat, ContentReader>> = {
  docx: (bytes, options) => {
  ...
  markdown: (bytes, options) => readMarkdownContent(decodeMarkdownText(bytes), { signal: options?.signal, images: options?.images }),
...
export function readDocumentLayout(bytes: Uint8Array, options?: DocumentCodecOptions): LayoutDocument {
