...
function firstParagraphElement(shapeElement: XmlElement): XmlElement {
  const txBody = shapeElement.children.find(
    (c): c is XmlElement => c.type === "element" && c.tag === "p:txBody",
  );
  if (txBody === undefined) {
    throw new Error("expected a p:txBody child");
  }
  const p = txBody.children.find(
    (c): c is XmlElement => c.type === "element" && c.tag === "a:p",
  );
  if (p === undefined) {
...
function bodyPrOf(shapeElement: XmlElement): XmlElement {
  ...
  const bodyPr = txBody.children.find(
    (c): c is XmlElement => c.type === "element" && c.tag === "a:bodyPr",
  );
  if (bodyPr === undefined) {
...
describe("PptxShape name", () => {
  ...
  it("writes name to p:cNvPr@name and round-trips", () => {
    ...
    const nvSpPr = shapeElement.children.find(
      (c): c is XmlElement => c.type === "element" && c.tag === "p:nvSpPr",
    );
    const cNvPr = childElement(nvSpPr, "p:cNvPr");
    expect(cNvPr?.attributes).toContainEqual({
      name: "name",
      value: "Title 1",
    });
  });
...
describe("buildDrawingParagraph spacing and indent", () => {
  it("emits a:spcBef/a:spcAft using a:spcPts in hundredths of a point", () => {
    ...
    const shape = new PptxShape([shapeElement], shapeElement);
    shape.setParagraphs([
      { runs: [{ text: "Spaced" }], spacingBeforePt: 12, spacingAfterPt: 6 },
    ]);
    ...
    expect(spcPtsBef?.attributes).toContainEqual({
      name: "val",
      value: "1200",
    });
  ...
  it("emits a:pPr@marL and @indent in EMU", () => {
    ...
    expect(pPr?.attributes).toContainEqual({
      name: "indent",
      value: "-228600",
    });
  });
...
describe("PptxTableCell background and borders", () => {
  function firstCellOf(tableElement: XmlElement): PptxTableCell {
    const tr = tableElement.children.find(
      (c): c is XmlElement => c.type === "element" && c.tag === "a:tr",
    );
    if (tr === undefined) {
      throw new Error("expected an a:tr child");
    }
    const tc = tr.children.find(
      (c): c is XmlElement => c.type === "element" && c.tag === "a:tc",
    );
    if (tc === undefined) {
  ...
  function tcPrOf(cell: PptxTableCell): XmlElement | undefined {
    ...
    return node.children.find(
      (c): c is XmlElement => c.type === "element" && c.tag === "a:tcPr",
    );
  }
...
describe("buildPptxPackage threading", () => {
  ...
  it("threads shape name through to p:cNvPr@name", () => {
    ...
    const sp = spTree.children.find(
      (c): c is XmlElement => c.type === "element" && c.tag === "p:sp",
    );
  ...
  it("threads shape insets through to a:bodyPr in EMU", () => {
    ...
    expect(bodyPr?.attributes).toContainEqual({
      name: "lIns",
      value: "114300",
    });
    expect(bodyPr?.attributes).toContainEqual({ name: "tIns", value: "57150" });
  ...
  it("threads paragraph spacing/indent through to a:pPr", () => {
    ...
    const spcBef = childElement(pPr, "a:spcBef");
    expect(childElement(spcBef, "a:spcPts")?.attributes).toContainEqual({
      name: "val",
      value: "1200",
    });
    const spcAft = childElement(pPr, "a:spcAft");
    expect(childElement(spcAft, "a:spcPts")?.attributes).toContainEqual({
      name: "val",
      value: "600",
    });
    const lnSpc = childElement(pPr, "a:lnSpc");
    expect(childElement(lnSpc, "a:spcPct")?.attributes).toContainEqual({
      name: "val",
      value: "150000",
    });
    expect(pPr?.attributes).toContainEqual({ name: "marL", value: "457200" });
  ...
  it("threads cell background and borders through to a:tcPr", () => {
    ...
    const solidFill = childElement(tcPr, "a:solidFill");
    expect(childElement(solidFill, "a:srgbClr")?.attributes).toContainEqual({
      name: "val",
      value: "FF0000",
    });
    expect(childElement(tcPr, "a:lnL")?.attributes).toContainEqual({
      name: "w",
      value: "12700",
    });
    expect(childElement(tcPr, "a:lnR")?.attributes).toContainEqual({
      name: "w",
      value: "12700",
    });
    expect(childElement(tcPr, "a:lnT")?.attributes).toContainEqual({
      name: "w",
      value: "12700",
    });
    expect(childElement(tcPr, "a:lnB")?.attributes).toContainEqual({
      name: "w",
      value: "12700",
    });
  });
