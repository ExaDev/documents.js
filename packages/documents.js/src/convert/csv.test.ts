...
describe("csv composition: same-variant bridges", () => {
  ...
  it("csvToOds accepts the TSV delimiter option, parsing the identical grid from tab-separated text", () => {
    ...
    const csvFromTsv = odsToCsv(
      csvToOds(tsvBytes, { delimiter: TSV_DELIMITER }),
    );
    expect(parseCsvRecords(decodeCsvText(csvFromTsv))).toEqual(
      parseCsvRecords(CSV_TEXT),
    );
  });

  it("odsToCsv/xlsxToCsv emit the rendered cells of real ods and xlsx fixtures", () => {
    expect(
      parseCsvRecords(decodeCsvText(odsToCsv(csvToOds(csvBytes())))),
    ).toEqual(parseCsvRecords(CSV_TEXT));
    expect(
      parseCsvRecords(decodeCsvText(xlsxToCsv(csvToXlsx(csvBytes())))),
    ).toEqual(parseCsvRecords(CSV_TEXT));
  });
  ...
  it("xlsxToCsv on a multi-sheet source throws CsvSheetNotSpecifiedError naming every sheet, and { sheet } selects one", () => {
    ...
    const twoSheets = {
      ...sheetOne,
      sheets: [...sheetOne.sheets, { ...sheetOne.sheets[0]!, name: "Second" }],
    };
    const xlsxBytes = encodeOoxmlPackage(
      buildXlsxPackageFromContent(twoSheets),
    );
    expect(() => xlsxToCsv(xlsxBytes)).toThrow(CsvSheetNotSpecifiedError);
    expect(
      parseCsvRecords(decodeCsvText(xlsxToCsv(xlsxBytes, { sheet: "Second" }))),
    ).toEqual([["A"], ["1"]]);
  });
});
...
describe("csv composition: PDF pivot", () => {
  it("csvToPdf produces valid PDF bytes (composed csv -> ods -> pdf, since csv has no layout engine of its own)", () => {
    ...
    expect(new TextDecoder("latin1").decode(pdfBytes.subarray(0, 5))).toBe(
      "%PDF-",
    );
  });
...
describe("csv composition: pdf-composed last-resort pair", () => {
  ...
  it("markdownToCsv produces well-formed RFC 4180 records from a markdown table", () => {
    ...
    expect(
      parseCsvRecords(decodeCsvText(csvFromMarkdown)).length,
    ).toBeGreaterThan(0);
  });
});
...
describe("csv through the DocumentConverter port", () => {
  it("routes csv -> xlsx and reports the target format", async () => {
    ...
    const result = await converter.convert(
      { source: { format: "csv", bytes: csvBytes() }, targetFormat: "xlsx" },
    ...
    ).toEqual({ kind: "number", value: 42.5 });
  });

  it("routes csv -> pdf and produces valid PDF bytes", async () => {
    const converter = createLocalDocumentConverter();
    const result = await converter.convert(
      { source: { format: "csv", bytes: csvBytes() }, targetFormat: "pdf" },
      { signal: new AbortController().signal },
    );
    expect(result.document.format).toBe("pdf");
    expect(
      new TextDecoder("latin1").decode(result.document.bytes.subarray(0, 5)),
    ).toBe("%PDF-");
  });
});
