...
export function stampPdfPackageTables(
  ...
  if (layout.destinations !== undefined || layout.outline !== undefined) {
    ...
    for (const destination of layout.destinations ?? []) {
      destinations[destination.name] = {
        ...
        ...(destination.target.leftPt !== undefined
          ? { leftPt: destination.target.leftPt }
          : {}),
        ...(destination.target.topPt !== undefined
          ? { topPt: destination.target.topPt }
          : {}),
        ...(destination.target.bottomPt !== undefined
          ? { bottomPt: destination.target.bottomPt }
          : {}),
        ...(destination.target.rightPt !== undefined
          ? { rightPt: destination.target.rightPt }
          : {}),
        ...(destination.target.zoom !== undefined
          ? { zoom: destination.target.zoom }
          : {}),
      };
    }
    ...
    pkg.layers = {
      ...
      ...Object.fromEntries(
        layout.layers.map((layer) => [
          layer.name,
          { kind: "layer", name: layer.name, visible: layer.visible },
        ]),
      ),
    };
  }
  ...
  layout.pages.forEach((page, pageIndex) => {
    (page.annotations ?? []).forEach(
      (annotation: LayoutAnnotation, annotIndex) => {
        definitions[`pdf-annot-${String(pageIndex)}-${String(annotIndex)}`] = {
          ...
          ...(annotation.author !== undefined
            ...
            : {}),
          ...(annotation.modifiedIso !== undefined
            ? { dateIso: annotation.modifiedIso }
            : {}),
