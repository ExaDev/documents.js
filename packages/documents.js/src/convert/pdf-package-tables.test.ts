...
function layoutOf(extra: {
  ...
  return {
    ...
    ...(extra.destinations !== undefined
      ...
      : {}),
    ...(extra.outline !== undefined
      ? { outline: cloneOutline(extra.outline) }
      : {}),
    ...(extra.attachments !== undefined
      ? { attachments: [...extra.attachments] }
      : {}),
    ...(extra.layers !== undefined ? { layers: [...extra.layers] } : {}),
    ...(extra.structure !== undefined
      ? { structure: cloneStructure(extra.structure) }
      : {}),
...
describe("stampPdfPackageTables", () => {
  it("lands named destinations and a flattened outline in the destinations table", () => {
        ...
        destinations: [
          {
            name: "intro",
            pageIndex: 0,
            target: { kind: "xyz", leftPt: 72, topPt: 700 },
          },
    ...
    expect(pkg.destinations?.intro).toEqual({
      kind: "destination",
      pageIndex: 0,
      viewKind: "xyz",
      leftPt: 72,
    ...
    expect(pkg.destinations?.dest1).toEqual({
      kind: "destination",
      pageIndex: 1,
      viewKind: "fit",
    });
    expect(pkg.destinations?.["outline-1"]).toEqual({
      kind: "outline",
      title: "Introduction",
      destination: "intro",
    });
    expect(pkg.destinations?.["outline-2"]).toEqual({
      kind: "outline",
      title: "Nested",
      parent: "outline-1",
    });
    expect(pkg.destinations?.["outline-3"]).toEqual({
      kind: "outline",
      title: "Later",
    });
  });
  ...
  it("lands attachments and layers keyed by their own names", () => {
    ...
    expect(pkg.attachments?.["notes.txt"]).toEqual({
      kind: "attachment",
      name: "notes.txt",
      description: "Notes",
      mimeType: "text/plain",
    ...
    expect(pkg.layers?.Background).toEqual({
      kind: "layer",
      name: "Background",
      visible: false,
    });
  ...
  it("carries the residue table through under the same keys", () => {
    ...
    stampPdfPackageTables(
      pkg,
      layoutOf({ source: { xmp: { format: "pdf", xml: "<?xpacket…" } } }),
    );
  ...
  it("mints comment definition bodies under the keys the anchor constructs reference", () => {
    ...
    expect(pkg.definitions?.["pdf-annot-1-0"]).toEqual({
      kind: "comment",
      body: "Second page note",
      author: "R",
      dateIso: "2026-08-19T14:03:00Z",
    });
  });
  ...
  it("lands the structure tree in the definitions table keyed by element id, parent stated as a reference (#760)", () => {
    ...
    expect(pkg.definitions?.struct1).toEqual({
      kind: "structure",
      type: "H1",
      title: "Opening",
    });
    expect(pkg.definitions?.struct2).toEqual({
      kind: "structure",
      type: "Sect",
      language: "fr",
    });
    expect(pkg.definitions?.struct3).toEqual({
      kind: "structure",
      type: "P",
      alt: "Body description",
      parent: "struct2",
