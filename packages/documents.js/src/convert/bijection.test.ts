...
import {
  oleEmbeddedDocxBytes,
  oleEmbeddedPptxBytes,
} from "../test-support/ole";
...
): { content: ContentDocument; pages: PageSize[] } {
...
const SVG_TEXT =
  '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><title>Corpus drawing</title><rect x="1" y="1" width="20" height="10" fill="black"/><ellipse cx="30" cy="6" rx="4" ry="2"/><line x1="1" y1="20" x2="10" y2="20"/><path d="M 0 30 L 10 30 C 20 30 20 40 10 40 Z"/></svg>';
...
function corpus(): readonly CorpusEntry[] {
  ...
  const entries: CorpusEntry[] = reader.map((content, index) => ({
  ...
  if (
    chapterOne.kind === "wordprocessing" &&
    chapterTwo.kind === "wordprocessing"
  ) {
    entries.push({
      name: "multi-section wordprocessing (two chapters concatenated)",
      content: {
        kind: "wordprocessing",
        metadata: chapterOne.metadata,
        sections: [...chapterOne.sections, ...chapterTwo.sections],
      },
    });
  }
  ...
  entries.push({
    name: "reader output: pptx with an OLE-embedded xlsx",
    content: readPptxContent(decodeOoxmlPackage(oleEmbeddedPptxBytes())),
  });
  // onDocument captures: real frames, real pages, real styles minting over real formatting repetition.
  entries.push({
    name: "docxToPdf capture (wrapped runs carry multiple frames)",
    ...captured((onDocument) => docxToPdf(minimalDocxBytes(), { onDocument })),
  });
  entries.push({
    name: "pptxToPdf capture",
    ...captured((onDocument) => pptxToPdf(minimalPptxBytes(), { onDocument })),
  });
  entries.push({
    name: "odtToPdf capture",
    ...captured((onDocument) => odtToPdf(minimalOdtBytes(), { onDocument })),
  });
  entries.push({
    name: "odpToPdf capture",
    ...captured((onDocument) => odpToPdf(minimalOdpBytes(), { onDocument })),
  });
  entries.push({
    name: "odsToPdf capture",
    ...captured((onDocument) => odsToPdf(gridOdsBytes(), { onDocument })),
  });
  entries.push({
    name: "odgToPdf capture",
    ...captured((onDocument) => odgToPdf(minimalOdgBytes(), { onDocument })),
  });
  entries.push({
    name: "markdownToPdf capture",
    ...captured((onDocument) =>
      markdownToPdf(new TextEncoder().encode(richMarkdownText()), {
        onDocument,
      }),
    ),
  });
  entries.push({
    name: "csvToPdf capture",
    ...captured((onDocument) =>
      csvToPdf(new TextEncoder().encode("a,b\n1,2"), { onDocument }),
    ),
  });
  entries.push({
    name: "svgToPdf capture",
    ...captured((onDocument) =>
      svgToPdf(new TextEncoder().encode(SVG_TEXT), { onDocument }),
    ),
  });
  entries.push({
    name: "odfToPdf capture (formula tree)",
    ...captured((onDocument) =>
      odfToPdf(odfFormulaBytes(FRACTION_FORMULA), { onDocument }),
    ),
  });
  entries.push({
    name: "odsToPdf capture with embedded formula",
    ...captured((onDocument) =>
      odsToPdf(sheetFormulaOdsBytes(), { onDocument }),
    ),
  });
  ...
  entries.push({
    name: "docxToPdf capture with an OLE-embedded xlsx",
    ...captured((onDocument) =>
      docxToPdf(oleEmbeddedDocxBytes(), { onDocument }),
    ),
  });
  entries.push({
    name: "pptxToPdf capture with an OLE-embedded xlsx",
    ...captured((onDocument) =>
      pptxToPdf(oleEmbeddedPptxBytes(), { onDocument }),
    ),
  });
  // Reconstruction outputs: PDF bytes read back and reconstructed per direction.
  const pdfBytes = odsToPdf(gridOdsBytes());
  entries.push({
    name: "pdfToDocx reconstruction capture",
    ...captured((onDocument) => pdfToDocx(pdfBytes, { onDocument })),
  });
  entries.push({
    name: "pdfToOds reconstruction capture",
    ...captured((onDocument) => pdfToOds(pdfBytes, { onDocument })),
  });
  entries.push({
    name: "pdfToOdg reconstruction capture",
    ...captured((onDocument) =>
      pdfToOdg(docxToPdf(minimalDocxBytes()), { onDocument }),
    ),
  });
...
describe("decompose/flatten bijection laws over the real corpus", () => {
  ...
  it("the construct corpus mints refs onto the construct groups themselves", () => {
    const withConstructRefs = constructCorpus().filter(
      (entry) =>
  ...
  it("the OLE corpus entries carry genuinely recovered embeddedObject blocks", () => {
    ...
    for (const entry of oleEntries) {
      const blocks =
        entry.content.kind === "wordprocessing"
          ...
          : entry.content.kind === "presentation"
            ? entry.content.slides.flatMap((slide) =>
      ...
      if (embedded?.kind !== "embeddedObject") {
        throw new Error(
          `${entry.name}: no embeddedObject block anywhere in the document`,
        );
      }
      expect(embedded.objectKind).toBe("spreadsheet");
      if (embedded.document.kind !== "spreadsheet") {
        throw new Error(
          `${entry.name}: the embedded block carries no spreadsheet document`,
        );
      }
      expect(embedded.document.sheets[0]?.name).toBe("Embedded");
      expect(embedded.document.sheets[0]?.cells[0]?.value).toEqual({
        kind: "string",
