...
describe("odbToXlsx", () => {
  ...
  it("writes a header row of column names, then typed data rows, on the CUSTOMERS sheet", () => {
    ...
    expect(header.map((cell) => cell.displayText)).toEqual([
      "ID",
      "NAME",
      "EMAIL",
      "ACTIVE",
    ...
    const row2 = sheet.cells
      .filter((cell) => cell.row === 2)
      .sort((a, b) => a.column - b.column);
    expect(
      row2.map((cell) => ({ column: cell.column, value: cell.value })),
    ).toEqual([
...
describe("odbToXlsx / odbToCsv onDocument", () => {
  ...
  it("neither fires anything when no onDocument is supplied", () => {
    ...
    expect(() =>
      odbToCsv(embeddedHsqldbOdbBytes(), { table: "ORDERS" }),
    ).not.toThrow();
  });
});
...
describe("odbToXlsx / odbToCsv: a real HSQLDB 1.8.0.10 CACHED-table database", () => {
  ...
  it("writes the named table as CSV, recovering TIME/TIMESTAMP/BIGINT/SMALLINT/TINYINT from TYPE_TEST", () => {
    ...
    expect(lines[1]).toBe(
      "1,14:30:00,2024-03-15 09:45:30.123456789,123456789012345,32000,120",
    );
    expect(lines[2]).toBe(
      "2,23:59:59,1999-12-31 23:59:59,-123456789012345,-32000,-120",
    );
  ...
  it("throws OdbTableNotSpecifiedError, naming all four tables, when table is omitted", () => {
    ...
    expect(() => odbToCsv(bytes)).toThrow(OdbTableNotSpecifiedError);
    expect(() => odbToCsv(bytes)).toThrow(
      /EMPLOYEES, DEPARTMENTS, EMPTY_TABLE, TYPE_TEST/,
    );
  });
});
