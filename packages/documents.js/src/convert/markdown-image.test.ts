...
const LOCAL_IMAGE_MD = new TextEncoder().encode(
  "![a local image](./local.png)",
);

function firstImageBlockKind(
  captured: DocumentTree | undefined,
): string | undefined {
...
describe("markdown image resolution through options.images", () => {
  it("markdownToPdf resolves a relative-path image into a real ContentImageBlock, not alt-text", () => {
    ...
    markdownToPdf(LOCAL_IMAGE_MD, {
      ...
      images: (destination) =>
        destination === "./local.png" ? { bytes: ONE_PIXEL_PNG } : undefined,
    });
