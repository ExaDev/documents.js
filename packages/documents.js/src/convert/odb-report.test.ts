...
describe('odbReportToDocx', () => {
  it('writes a real docx whose tables carry the report title, a detail row, and the grand total', () => {
    ...
    const detailRow = tables.find((table) => table.cell(0, 0).text === 'Acme Ltd');
  ...
  it('reports a construct-level math diagnostic through onMathDiagnostic if one ever fires (none expected for plain report text)', () => {
    let fired = false;
    odbReportToDocx(salesReportContent(), { onMathDiagnostic: () => { fired = true; } });
...
describe('odbReportToPdf', () => {
  ...
  it('reports a font substitution through onFontSubstitution when a run requests a family pdf-codec resolves via its vendored Carlito substitute', () => {
    ...
    const firstParagraph = content.sections[0]?.blocks.find((block) => block.kind === 'table')?.rows[0]?.cells[0]?.blocks[0];
    if (firstParagraph?.kind !== 'paragraph' || firstParagraph.runs[0] === undefined) {
      throw new Error('expected the report header band to carry at least one run');
    }
    firstParagraph.runs[0].fontFamily = 'Calibri';

    const substitutions: string[] = [];
    odbReportToPdf(content, { onFontSubstitution: (substitution) => substitutions.push(substitution.requestedFamily) });
