...
function odfBodyChild(pkg: OdfPackage, tag: string): OdfXmlElement {
  ...
  const child =
    body === undefined ? undefined : findChildElement(body.children, tag);
  if (child === undefined) {
...
describe("recovered vector geometry reaching real output bytes", () => {
  it("pdfToDocx writes every recovered vector as a real DrawingML shape", () => {
    expect(
      collectDrawingMlVectors(
        ooxmlPart(pdfToDocx(SOURCE_PDF), "word/document.xml"),
        "wps:spPr",
  ...
  it("pdfToPptx writes every recovered vector as a real DrawingML shape", () => {
    expect(
      collectDrawingMlVectors(
        ooxmlPart(pdfToPptx(SOURCE_PDF), "ppt/slides/slide1.xml"),
        "p:spPr",
      ).map((vector) => vector.kind),
    ).toEqual(EXPECTED_KINDS);
  });
  ...
  it("pdfToOdp writes every recovered vector as a real draw: primitive on the slide page", () => {
    ...
    const [page] = childrenWithTag(
      odfBodyChild(pkg, "office:presentation"),
      "draw:page",
    );
    if (page === undefined) {
    ...
    expect(
      readDrawPageContent(page.children, pkg).vectors.map(
        (vector) => vector.kind,
      ),
    ).toEqual(EXPECTED_KINDS);
  });
  ...
  it("keeps the curved path curved in both vocabularies, rather than approximating it with line segments", () => {
    const drawingMlPath = collectDrawingMlVectors(
      ooxmlPart(pdfToPptx(SOURCE_PDF), "ppt/slides/slide1.xml"),
      "p:spPr",
    ).find((vector) => vector.kind === "path");
    expect(
      drawingMlPath?.kind === "path" &&
        drawingMlPath.subpaths.some((subpath) =>
          subpath.segments.some((segment) => segment.kind === "cubic"),
        ),
    ).toBe(true);
    ...
    const odfPath = readDrawPageContent(page.children, pkg).vectors.find(
      (vector) => vector.kind === "path",
    );
    expect(
      odfPath?.kind === "path" &&
        odfPath.subpaths.some((subpath) =>
          subpath.segments.some((segment) => segment.kind === "cubic"),
        ),
    ).toBe(true);
  });
...
describe("readDocxContent: a hand-authored vector-only w:drawing", () => {
  ...
  it("recovers the rect at the anchor's own position, not spPr's own disagreeing a:xfrm", () => {
    ...
    expect(vector).toMatchObject({
      kind: "rect",
      frame: { xPt: 100, yPt: 50, widthPt: 40, heightPt: 30 },
      fill: { r: 1, g: 0, b: 0 },
    });
  });
});
...
describe("readPptxContent: a hand-authored slide mixing a vector shape, a connector, and a group", () => {
  ...
  it("recovers the two vector shapes as their own drawing blocks, skipping the connector and flattening the group", () => {
    ...
    if (
      rectBlock?.kind !== "embeddedObject" ||
      rectBlock.document.kind !== "drawing"
    ) {
      throw new Error(
        "expected the second shape to be a drawing-kind embeddedObject block (the connector occupies no slot)",
      );
    }
    expect(rectBlock.document.pages[0]!.vectors).toMatchObject([
      { kind: "rect", fill: { r: 0, g: 1, b: 0 } },
    ]);
    expect(shapes[2]?.blocks[0]).toMatchObject({
      kind: "paragraph",
      runs: [{ text: "Middle" }],
    });
    const ellipseBlock = shapes[3]?.blocks[0];
    if (
      ellipseBlock?.kind !== "embeddedObject" ||
      ellipseBlock.document.kind !== "drawing"
    ) {
      throw new Error(
        "expected the fourth shape (the group's own vector, flattened out) to be a drawing-kind embeddedObject block",
      );
    }
    expect(ellipseBlock.document.pages[0]!.vectors).toMatchObject([
      { kind: "ellipse", fill: { r: 0, g: 0, b: 1 } },
    ]);
  });
});
...
describe("readOdtContent: a hand-authored bare draw:rect sitting directly in office:text", () => {
  ...
  it("recovers the bare rect as a drawing block, between the two real paragraphs", () => {
    ...
    expect(drawingBlock.document.pages[0]!.vectors).toMatchObject([
      { kind: "rect", fill: { r: 0, g: 1, b: 0 } },
    ]);
  });
});
...
describe("odt <-> docx: a recovered drawing survives repeated cycles without accumulating empty paragraphs", () => {
  it("keeps exactly one paragraph-drawing-paragraph block sequence across three docx -> odt cycles", () => {
    ...
    for (let cycle = 0; cycle < 3; cycle++) {
      ...
      expect(drawingBlock.document.pages[0]!.vectors).toHaveLength(
