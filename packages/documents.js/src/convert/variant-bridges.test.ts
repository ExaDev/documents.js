...
function minimalDrawingDoc(): Extract<ContentDocument, { kind: "drawing" }> {
  const page: ContentDrawPage = {
    size: PAGE_SIZE,
    shapes: [textShape("Label")],
    vectors: [rectVector()],
  };
  return { kind: "drawing", metadata: {}, pages: [page] };
}

function minimalPresentationDoc(): Extract<
  ContentDocument,
  { kind: "presentation" }
> {
  const slide: ContentSlide = {
    size: PAGE_SIZE,
    shapes: [textShape("Slide text")],
    notes: "Speaker notes",
  };
...
describe("convertDocument: drawing <-> presentation over real fixture bytes", () => {
  ...
  it("odp -> odg produces a real odg whose pages carry the slide's text-frame shape", () => {
    ...
    if (content.kind !== "drawing") {
      throw new Error(
        `expected a drawing ContentDocument, got ${content.kind}`,
      );
    }
