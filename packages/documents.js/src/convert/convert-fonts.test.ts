...
function embeddedFaceNames(pdf: Uint8Array<ArrayBuffer>): string[] {
  return [
    ...new Set(
      [...asLatin1(pdf).matchAll(EMBEDDED_BASE_FONT_PATTERN)].map(
        (match) => match[1],
      ),
    ),
...
function expectEmbeddedTrueTypeFontResource(
...
describe("X -> PDF: caller-supplied faces", () => {
  it("odtToPdf renders a family the document did not embed through a caller-supplied face", () => {
    const pdf = odtToPdf(fontRequestOdtBytes("Bookish"), {
      fonts: [
        {
          family: "Bookish",
          bold: false,
...
function referenceDocxPdf(
  ...
  if (content.kind !== "wordprocessing") {
    throw new Error(
      "readDocxContent returned a non-wordprocessing ContentDocument",
    );
  }
  const { document: layout, formulas } = convertWordprocessingToLayout(
    content,
    { measurer: createStandardFontMeasurer(), mathMetricsAt },
  );
  return writePdf(layout, { formulas });
}

function referencePptxPdf(
  bytes: Uint8Array<ArrayBuffer>,
): Uint8Array<ArrayBuffer> {
  const content = readPptxContent(openPptx(bytes).toPackage());
  if (content.kind !== "presentation") {
    throw new Error(
      "readPptxContent returned a non-presentation ContentDocument",
    );
  }
...
function referenceOdpPdf(
  bytes: Uint8Array<ArrayBuffer>,
): Uint8Array<ArrayBuffer> {
  const content = readOdpContent(decodeOdfPackage(bytes));
  if (content.kind !== "presentation") {
    throw new Error(
      "readOdpContent returned a non-presentation ContentDocument",
    );
  }
  const { document: layout, formulas } = convertPresentationToLayout(content, {
    measurer: createStandardFontMeasurer(),
...
function referenceOdsPdf(
  bytes: Uint8Array<ArrayBuffer>,
): Uint8Array<ArrayBuffer> {
  const content = readOdsContent(decodeOdfPackage(bytes));
  if (content.kind !== "spreadsheet") {
    throw new Error(
      "readOdsContent returned a non-spreadsheet ContentDocument",
    );
  }
...
function referenceOdgPdf(
  ...
  return writePdf(
    convertDrawingToLayout(content, { measurer: createStandardFontMeasurer() })
      .document,
  );
}

const BYTE_IDENTITY_CASES: readonly {
  readonly name: string;
  readonly actual: () => Uint8Array<ArrayBuffer>;
  readonly reference: () => Uint8Array<ArrayBuffer>;
}[] = [
  {
    name: "docxToPdf",
    actual: () => docxToPdf(standardFontDocxBytes()),
    reference: () => referenceDocxPdf(standardFontDocxBytes()),
  },
  {
    name: "pptxToPdf",
    actual: () => pptxToPdf(minimalPptxBytes()),
    reference: () => referencePptxPdf(minimalPptxBytes()),
  },
  {
    name: "odtToPdf",
    actual: () => odtToPdf(minimalOdtBytes()),
    reference: () => referenceOdtPdf(minimalOdtBytes()),
  },
  {
    name: "odpToPdf",
    actual: () => odpToPdf(minimalOdpBytes()),
    reference: () => referenceOdpPdf(minimalOdpBytes()),
  },
  {
    name: "odsToPdf",
    actual: () => odsToPdf(minimalOdsBytes()),
    reference: () => referenceOdsPdf(minimalOdsBytes()),
  },
  {
    name: "odgToPdf",
    actual: () => odgToPdf(minimalOdgBytes()),
    reference: () => referenceOdgPdf(minimalOdgBytes()),
  },
];

describe("X -> PDF: backward compatibility", () => {
  it.each(BYTE_IDENTITY_CASES)(
    "$name is byte-identical to the standard-font-only pipeline when nothing is embedded and no substitute applies",
    ({ actual, reference }) => {
