...
export function buildDocumentBytes(pkg: DocumentPackage, target: DocumentFormat): Uint8Array<ArrayBuffer> {
  ...
  if (!content?.write) {
    throw new Error(`DocumentFormat '${target}' has no content.write codec in DOCUMENT_FORMAT_CODECS, and is not 'pdf'/'xlsx'/'odf' -- this is an internal invariant violation, not a caller error`);
  }
