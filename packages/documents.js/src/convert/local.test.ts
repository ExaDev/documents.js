...
describe("createLocalDocumentConverter: shape", () => {
  it("reports contractVersion and the supported conversion pairs", () => {
    ...
    // SUPPORTED_CONVERSIONS is now derived from the composition pathfinder (resolveCompositionPlan) rather than a hand-maintained DIRECT_EDGES list. The pathfinder routes every pair of non-odf formats within the 3-hop cap, plus the special-case odf -> pdf pair -- 245 pairs total, sorted by source then target for determinism. csv joins as a full spreadsheet-variant member: same-variant bridges to ods/xlsx directly, everything else composed through the identical ods pivot xlsx uses. svg joins as the drawing family's plain-text member the same way: a same-variant bridge to odg directly plus its own pdf layout pair, everything else composed through those two edges. rtf joins the wordprocessing family the same way again -- same-variant bridges to docx/odt/markdown, everything else composed through those plus each target's own pdf layout pair. wpd joins as the wordprocessing family's read-only member (see composition.ts's own ReadOnlyContentFormat): a directed edge to every other member of this table at the identical cost a read-and-write wordprocessing member would carry, but only ever as a source -- there is no reverse direction to list, since wpd-codec ships no writer at all. doc/xls/ppt join the same way rtf did, one variant each (wordprocessing/spreadsheet/presentation). epub joins the wordprocessing family the same way rtf/doc did: same-variant bridges to docx/odt/markdown/rtf/doc directly, everything else composed through those plus each target's own pdf layout pair.
    //
    // Twelve pairs are asymmetric rather than absent, since ExaDev/documents.js#1043 registered a one-way spreadsheet -> wordprocessing content transform (spreadsheetToWordprocessing, src/convert/variant-bridges.ts): csv/xlsx/xls -> rtf, csv/xlsx/xls -> doc, and csv/xlsx/xls -> epub now resolve as a single cross-variant bridge hop (the identical mechanism csv/xlsx/xls -> docx/odt/markdown already used, epub joining automatically once it registered as a wordprocessing-variant FORMAT_NODES member -- the transform is keyed by variant pair, not by individual format), and csv/xlsx/xls -> ppt resolves as two bridge hops through a wordprocessing pivot (spreadsheet -> {docx|odt|markdown|rtf|doc|epub} -> ppt), each cheaper than the four-hop PDF-composed route the 3-hop cap was blocking. The reverse direction has no registered transform -- a wordprocessing/presentation source has no cell types, formulas, or geometry of its own to recover into a spreadsheet -- so rtf<->csv, rtf<->xlsx, doc<->csv, doc<->xlsx, doc<->xls, epub<->csv, epub<->xlsx, epub<->xls, xls<->rtf (via doc/rtf being wordprocessing-sourced), xls<->ppt, ppt<->csv, and ppt<->xlsx each keep exactly one direction absent: rtf->csv, rtf->xlsx, doc->csv, doc->xlsx, doc->xls, epub->csv, epub->xlsx, epub->xls, rtf->xls, ppt->xls, ppt->csv, and ppt->xlsx are still one hop past resolveCompositionPlan's own cap (the bound stated on that function).
    expect(converter.conversions).toEqual([
      { source: "csv", target: "doc" },
      { source: "csv", target: "docx" },
      { source: "csv", target: "epub" },
      ...
      { source: "doc", target: "epub" },
      { source: "doc", target: "markdown" },
      { source: "doc", target: "odg" },
      { source: "doc", target: "odp" },
      { source: "doc", target: "ods" },
      { source: "doc", target: "odt" },
      { source: "doc", target: "pdf" },
      { source: "doc", target: "ppt" },
      { source: "doc", target: "pptx" },
      { source: "doc", target: "rtf" },
      { source: "doc", target: "svg" },
      { source: "docx", target: "csv" },
      { source: "docx", target: "doc" },
      ...
      { source: "docx", target: "ppt" },
      { source: "docx", target: "pptx" },
      { source: "docx", target: "rtf" },
      { source: "docx", target: "svg" },
      { source: "docx", target: "xls" },
      { source: "docx", target: "xlsx" },
      { source: "epub", target: "doc" },
      ...
      { source: "pdf", target: "ppt" },
      { source: "pdf", target: "pptx" },
      { source: "pdf", target: "rtf" },
      { source: "pdf", target: "svg" },
      { source: "pdf", target: "xls" },
      { source: "pdf", target: "xlsx" },
      { source: "ppt", target: "doc" },
      { source: "ppt", target: "docx" },
      { source: "ppt", target: "epub" },
      { source: "ppt", target: "markdown" },
      { source: "ppt", target: "odg" },
      { source: "ppt", target: "odp" },
      { source: "ppt", target: "ods" },
      { source: "ppt", target: "odt" },
      { source: "ppt", target: "pdf" },
      { source: "ppt", target: "pptx" },
      { source: "ppt", target: "rtf" },
      { source: "ppt", target: "svg" },
      { source: "pptx", target: "csv" },
      { source: "pptx", target: "doc" },
      ...
      { source: "pptx", target: "ppt" },
      { source: "pptx", target: "rtf" },
      { source: "pptx", target: "svg" },
      { source: "pptx", target: "xls" },
      { source: "pptx", target: "xlsx" },
      { source: "rtf", target: "doc" },
      { source: "rtf", target: "docx" },
      { source: "rtf", target: "epub" },
      { source: "rtf", target: "markdown" },
      { source: "rtf", target: "odg" },
      { source: "rtf", target: "odp" },
      { source: "rtf", target: "ods" },
      { source: "rtf", target: "odt" },
      { source: "rtf", target: "pdf" },
      { source: "rtf", target: "ppt" },
      { source: "rtf", target: "pptx" },
      { source: "rtf", target: "svg" },
      { source: "svg", target: "csv" },
      { source: "svg", target: "doc" },
...
describe("createLocalDocumentConverter: convert", () => {
  it("converts docx to pdf", async () => {
    ...
    const result = await converter.convert(
      {
        source: { format: "docx", bytes: buildSampleDocx("Hi") },
        targetFormat: "pdf",
      },
  ...
  it("converts pptx to pdf", async () => {
    ...
    const result = await converter.convert(
      {
        source: { format: "pptx", bytes: buildSamplePptx("Hi") },
        targetFormat: "pdf",
      },
      { signal: new AbortController().signal },
    );
  ...
  it("converts odp to pdf", async () => {
    ...
    const result = await converter.convert(
      {
        source: { format: "odp", bytes: minimalOdpBytes() },
        targetFormat: "pdf",
      },
      { signal: new AbortController().signal },
    );
  ...
  it("converts ods to pdf", async () => {
    ...
    const result = await converter.convert(
      {
        source: { format: "ods", bytes: minimalOdsBytes() },
        targetFormat: "pdf",
      },
      { signal: new AbortController().signal },
    );
  ...
  it("converts odg to pdf", async () => {
    ...
    const result = await converter.convert(
      {
        source: { format: "odg", bytes: minimalOdgBytes() },
        targetFormat: "pdf",
      },
      { signal: new AbortController().signal },
    );
  ...
  it("converts xlsx to pdf", async () => {
    ...
    const result = await converter.convert(
      { source: { format: "xlsx", bytes: xlsxBytes }, targetFormat: "pdf" },
      { signal: new AbortController().signal },
    );
    expect(result.document.format).toBe("pdf");
    expect(pdfHeader(result.document.bytes)).toBe("%PDF-");
  ...
  it("converts pdf to docx", async () => {
    const converter = createLocalDocumentConverter();
    const docxToPdfResult = await converter.convert(
      {
        source: { format: "docx", bytes: buildSampleDocx("Hi") },
        targetFormat: "pdf",
      },
    ...
    const result = await converter.convert(
      { source: docxToPdfResult.document, targetFormat: "docx" },
      { signal: new AbortController().signal },
    );
  ...
  it("converts pdf to pptx", async () => {
    const converter = createLocalDocumentConverter();
    const pptxToPdfResult = await converter.convert(
      {
        source: { format: "pptx", bytes: buildSamplePptx("Hi") },
        targetFormat: "pdf",
      },
    ...
    const result = await converter.convert(
      { source: pptxToPdfResult.document, targetFormat: "pptx" },
      { signal: new AbortController().signal },
    );
  ...
  it("converts pdf to odt", async () => {
    const converter = createLocalDocumentConverter();
    const odtToPdfResult = await converter.convert(
      {
        source: { format: "odt", bytes: minimalOdtBytes() },
        targetFormat: "pdf",
      },
    ...
    const result = await converter.convert(
      { source: odtToPdfResult.document, targetFormat: "odt" },
      { signal: new AbortController().signal },
    );
  ...
  it("converts pdf to odp", async () => {
    const converter = createLocalDocumentConverter();
    const odpToPdfResult = await converter.convert(
      {
        source: { format: "odp", bytes: minimalOdpBytes() },
        targetFormat: "pdf",
      },
    ...
    const result = await converter.convert(
      { source: odpToPdfResult.document, targetFormat: "odp" },
      { signal: new AbortController().signal },
    );
  ...
  it("converts pdf to ods", async () => {
    const converter = createLocalDocumentConverter();
    const odsToPdfResult = await converter.convert(
      {
        source: { format: "ods", bytes: minimalOdsBytes() },
        targetFormat: "pdf",
      },
    ...
    const result = await converter.convert(
      { source: odsToPdfResult.document, targetFormat: "ods" },
      { signal: new AbortController().signal },
    );
  ...
  it("converts pdf to odg", async () => {
    const converter = createLocalDocumentConverter();
    const odgToPdfResult = await converter.convert(
      {
        source: { format: "odg", bytes: minimalOdgBytes() },
        targetFormat: "pdf",
      },
    ...
    const result = await converter.convert(
      { source: odgToPdfResult.document, targetFormat: "odg" },
      { signal: new AbortController().signal },
    );
  ...
  it("converts pdf to xlsx", async () => {
    const converter = createLocalDocumentConverter();
    const xlsxBytes = odsToXlsx(minimalOdsBytes());
    const xlsxToPdfResult = await converter.convert(
      { source: { format: "xlsx", bytes: xlsxBytes }, targetFormat: "pdf" },
      { signal: new AbortController().signal },
    );
    const result = await converter.convert(
      { source: xlsxToPdfResult.document, targetFormat: "xlsx" },
      { signal: new AbortController().signal },
    );
  ...
  it("converts xlsx to ods directly, bypassing PDF", async () => {
    const converter = createLocalDocumentConverter();
    const odsToXlsxResult = await converter.convert(
      {
        source: { format: "ods", bytes: minimalOdsBytes() },
        targetFormat: "xlsx",
      },
    ...
    const result = await converter.convert(
      { source: odsToXlsxResult.document, targetFormat: "ods" },
      { signal: new AbortController().signal },
    );
    expect(result.document.format).toBe("ods");
    expect(result.document.bytes.length).toBeGreaterThan(0);
  ...
  it("collects a char/substituted diagnostic for a character outside WinAnsi", async () => {
    ...
    const result = await converter.convert(
      {
        source: { format: "docx", bytes: buildSampleDocx("中文") },
        targetFormat: "pdf",
      },
      { signal: new AbortController().signal },
    );
  ...
  it("collects PDF read diagnostics on the pdf->docx path", async () => {
    ...
    const result = await converter.convert(
      {
        source: { format: "pdf", bytes: brokenStartxrefPdf() },
        targetFormat: "docx",
      },
      { signal: new AbortController().signal },
    );
    expect(
      result.diagnostics.some((d) => d.code === "pdf/xref-recovered"),
    ).toBe(true);
  });
