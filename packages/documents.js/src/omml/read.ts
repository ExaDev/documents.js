...
const ARGUMENT_SLOTS: ReadonlySet<string> = new Set(['e', 'num', 'den', 'sub', 'sup', 'lim', 'deg', 'fName']);
...
function childByLocalName(element: XmlElement | undefined, name: string): XmlElement | undefined {
  return element === undefined ? undefined : elementChildren(element).find((child) => elementLocalName(child) === name);
}
...
function attrByLocalName(element: XmlElement | undefined, name: string): string | undefined {
...
function propertyValue(properties: XmlElement | undefined, name: string): string | undefined {
...
function isPropertyOn(properties: XmlElement | undefined, name: string): boolean {
...
function mathElement(tag: string, attributes: Record<string, string>, children: MathMlNode[]): MathMlElement {
  return { type: 'element', tag, attributes: Object.entries(attributes).map(([name, value]) => ({ name, value })), children };
}

function mathToken(tag: string, attributes: Record<string, string>, text: string): MathMlElement {
...
function diagnose(ctx: ReadContext, kind: OmmlDiagnosticKind, element: XmlElement): void {
...
function convertRun(run: XmlElement, ctx: ReadContext): MathMlElement[] {
  ...
  const variant: MathVariant | undefined = variantFromRunProperties(script, style);
  if (variant === undefined) {
    diagnose(ctx, 'approximated-element', run);
  }
  const tag = tokenTag(text);
  const intrinsic = tag === 'mi' ? miIntrinsicDefault(text) : 'normal';
  const attributes: Record<string, string> = variant !== undefined && variant !== intrinsic ? { mathvariant: variant } : {};
...
function slotArgument(parent: XmlElement, slot: string, ctx: ReadContext): MathMlElement {
  const nodes = slotContent(parent, slot, ctx);
  const [only] = nodes;
  return nodes.length === 1 && only !== undefined ? only : mathElement('mrow', {}, nodes);
}
...
function slotContent(parent: XmlElement, slot: string, ctx: ReadContext): MathMlElement[] {
...
function convertFraction(element: XmlElement, ctx: ReadContext): MathMlElement {
  const barless = propertyValue(childByLocalName(element, 'fPr'), 'type') === 'noBar';
  return mathElement('mfrac', barless ? { linethickness: '0' } : {}, [slotArgument(element, 'num', ctx), slotArgument(element, 'den', ctx)]);
}
...
function convertRadical(element: XmlElement, ctx: ReadContext): MathMlElement {
  ...
  const hidden = isPropertyOn(childByLocalName(element, 'radPr'), 'degHide') || degree === undefined || elementChildren(degree).length === 0;
  if (hidden) {
...
function nestedLimLow(element: XmlElement): XmlElement | undefined {
  ...
  const [only] = children;
  return children.length === 1 && only !== undefined && elementLocalName(only) === 'limLow' ? only : undefined;
}
...
function convertDelimiter(element: XmlElement, ctx: ReadContext): MathMlElement {
...
function convertNary(element: XmlElement, ctx: ReadContext): MathMlElement {
  ...
  if (hasLower && hasUpper) {
    scripted = mathElement(underOver ? 'munderover' : 'msubsup', {}, [base, slotArgument(element, 'sub', ctx), slotArgument(element, 'sup', ctx)]);
  } else if (hasLower) {
    scripted = mathElement(underOver ? 'munder' : 'msub', {}, [base, slotArgument(element, 'sub', ctx)]);
  } else if (hasUpper) {
    scripted = mathElement(underOver ? 'mover' : 'msup', {}, [base, slotArgument(element, 'sup', ctx)]);
  }
...
function convertMatrix(element: XmlElement, ctx: ReadContext): MathMlElement {
  ...
  for (const column of columns === undefined ? [] : childrenByLocalName(columns, 'mc')) {
    ...
    for (let index = 0; index < (Number.isFinite(count) && count > 0 ? count : 1); index++) {
  ...
  const rows = childrenByLocalName(element, 'mr').map((row) =>
    mathElement(
      'mtr',
      {},
      childrenByLocalName(row, 'e').map((cell) => mathElement('mtd', {}, convertNodes(cell.children, ctx))),
    ),
  );
  const attributes: Record<string, string> = alignments.some((alignment) => alignment !== 'center') ? { columnalign: alignments.join(' ') } : {};
...
function convertUnknownContainer(element: XmlElement, ctx: ReadContext): MathMlElement[] {
...
function convertElement(element: XmlElement, ctx: ReadContext): MathMlElement[] {
  ...
  switch (name) {
    ...
    case 'sSub':
      return [mathElement('msub', {}, [slotArgument(element, 'e', ctx), slotArgument(element, 'sub', ctx)])];
    case 'sSup':
      return [mathElement('msup', {}, [slotArgument(element, 'e', ctx), slotArgument(element, 'sup', ctx)])];
    case 'sSubSup':
      return [mathElement('msubsup', {}, [slotArgument(element, 'e', ctx), slotArgument(element, 'sub', ctx), slotArgument(element, 'sup', ctx)])];
    case 'sPre':
      ...
      return [mathElement('mmultiscripts', {}, [slotArgument(element, 'e', ctx), mathElement('mprescripts', {}, []), slotArgument(element, 'sub', ctx), slotArgument(element, 'sup', ctx)])];
    case 'limLow':
      return [mathElement('munder', {}, [slotArgument(element, 'e', ctx), slotArgument(element, 'lim', ctx)])];
    case 'limUpp': {
      const inner = nestedLimLow(element);
      if (inner !== undefined) {
        return [mathElement('munderover', {}, [slotArgument(inner, 'e', ctx), slotArgument(inner, 'lim', ctx), slotArgument(element, 'lim', ctx)])];
      }
    ...
    case 'func':
      return [mathElement('mrow', {}, [...slotContent(element, 'fName', ctx), ...slotContent(element, 'e', ctx)])];
    case 'e':
    case 'num':
    case 'den':
    case 'sub':
...
function convertNodes(nodes: readonly XmlNode[], ctx: ReadContext): MathMlElement[] {
...
export function collectOfficeMathElements(nodes: readonly XmlNode[]): readonly XmlElement[] {
