...
import { attrValue, elementChildren, elementLocalName, isMathMlElement, textContent } from '../mathml/nodes';
...
function diagnose(ctx: WriteContext, kind: OmmlDiagnosticKind, element: MathMlElement): void {
...
function mathRun(text: string, properties: XmlElement | undefined): XmlElement {
  const tAttrs: Record<string, string> = needsSpacePreserve(text) ? { 'xml:space': 'preserve' } : {};
...
function tokenRun(element: MathMlElement, rawText: string, intrinsicDefault: MathVariant, ctx: WriteContext, normalText: boolean): XmlElement[] {
  if (rawText.length === 0) {
    return [];
  }
...
function argSlot(tag: string, child: MathMlElement | undefined, ctx: WriteContext): XmlElement {
...
function argSlotFromChildren(tag: string, children: readonly MathMlElement[], ctx: WriteContext): XmlElement {
...
function convertFraction(element: MathMlElement, ctx: WriteContext): XmlElement {
  const children = elementChildren(element);
  const lineThickness = attrValue(element, 'linethickness');
  const barless = lineThickness !== undefined && parseMathLength(lineThickness, 1) === 0;
...
function convertRadical(element: MathMlElement, kind: 'msqrt' | 'mroot', ctx: WriteContext): XmlElement {
  const children = elementChildren(element);
  if (kind === 'msqrt') {
    return el('m:rad', {}, [el('m:radPr', {}, [el('m:degHide', { 'm:val': '1' })]), el('m:deg'), argSlotFromChildren('m:e', children, ctx)]);
  }
  return el('m:rad', {}, [argSlot('m:deg', children[1], ctx), argSlot('m:e', children[0], ctx)]);
}

function convertScripts(element: MathMlElement, kind: 'msub' | 'msup' | 'msubsup', ctx: WriteContext): XmlElement {
  const children = elementChildren(element);
  ...
  return el('m:sSubSup', {}, [base, argSlot('m:sub', children[1], ctx), argSlot('m:sup', children[2], ctx)]);
}

function isMovableLimitsOperator(element: MathMlElement | undefined): boolean {
  return element !== undefined && elementLocalName(element) === 'mo' && operatorProperties(textContent(element).trim()).movablelimits;
}
...
function convertUnderOver(element: MathMlElement, kind: 'munder' | 'mover' | 'munderover', ctx: WriteContext): XmlElement {
  const children = elementChildren(element);
  if (!ctx.displayStyle && isMovableLimitsOperator(children[0])) {
    const scriptKind = kind === 'munder' ? 'msub' : kind === 'mover' ? 'msup' : 'msubsup';
    return convertScripts(element, scriptKind, ctx);
  }
  if (kind === 'munder') {
    return el('m:limLow', {}, [argSlot('m:e', children[0], ctx), argSlot('m:lim', children[1], ctx)]);
  }
  if (kind === 'mover') {
    return el('m:limUpp', {}, [argSlot('m:e', children[0], ctx), argSlot('m:lim', children[1], ctx)]);
  }
...
function convertSpace(element: MathMlElement, ctx: WriteContext): XmlElement[] {
  ...
  const widthValue = width === undefined ? undefined : parseMathLength(width, 1);
...
function convertSemantics(element: MathMlElement, ctx: WriteContext): XmlElement[] {
...
function mstyleContext(element: MathMlElement, ctx: WriteContext): WriteContext {
  let next = ctx;
...
function convertNodes(nodes: readonly MathMlNode[], ctx: WriteContext): XmlElement[] {
...
export function buildOfficeMath(mathml: readonly MathMlNode[]): OmmlWriteResult {
  ...
  return { element: el('m:oMath', { 'xmlns:m': OMML_NS }, children), diagnostics };
}
...
export function buildOfficeMathParagraph(mathml: readonly MathMlNode[]): OmmlWriteResult {
  const diagnostics: OmmlDiagnostic[] = [];
  const children = convertNodes(mathml, rootContext(diagnostics));
  if (children.length === 0) {
    return { element: undefined, diagnostics };
  }
  return { element: el('m:oMathPara', { 'xmlns:m': OMML_NS }, [el('m:oMath', {}, children)]), diagnostics };
}
