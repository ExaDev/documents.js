  ...
  return {
    ...
    sheets: sheets.map(({ name, rows }) => ({
      ...
      columns: Array.from(
        ...
        (_unused, index) => ({ index, widthPt: 64 }),
      ),
      rows: Array.from({ length: rows.length }, (_unused, index) => ({
        index,
        heightPt: 15,
      })),
      printSettings: {
      ...
      cells: rows.flatMap((row, rowIndex) =>
        row.map((field, columnIndex) => ({
          row: rowIndex,
          column: columnIndex,
          value: { kind: "string" as const, value: field },
          displayText: field,
        })),
      ),
    })),
...
describe("readCsvContent", () => {
  ...
  it("re-types data cells through the shared cell-typing heuristic and declines exactly where it declines", () => {
    ...
    const valueAt = (column: number) =>
      sheet?.cells.find((cell) => cell.row === 1 && cell.column === column)
        ?.value;
  ...
  it("parses with the TSV delimiter when asked", () => {
    ...
    expect(
      sheet?.cells.find((cell) => cell.row === 0 && cell.column === 1)?.value,
    ).toEqual({ kind: "string", value: "42.5" });
  });
...
describe("buildCsvText", () => {
  ...
  it("throws CsvSheetNotFoundError for a named sheet that does not exist, and for a document with no sheets at all", () => {
    ...
    expect(() => buildCsvText(document, { sheet: "Missing" })).toThrow(
      CsvSheetNotFoundError,
    );
    expect(() => buildCsvText(spreadsheetDocument([]))).toThrow(
      CsvSheetNotFoundError,
    );
  });
  ...
  it("throws CsvUnsupportedDocumentKindError for a non-spreadsheet ContentDocument", () => {
    ...
    expect(() => buildCsvText(wordprocessing)).toThrow(
      CsvUnsupportedDocumentKindError,
    );
  });
  ...
  it("writes the TSV delimiter when asked, quoting on tab rather than comma", () => {
    const text = buildCsvText(
      spreadsheetDocument([{ name: "Data", rows: [["a,b", "c\td"]] }]),
      { delimiter: TSV_DELIMITER },
    );
          ...
          cells: [
            {
              row: 0,
              column: 0,
              value: { kind: "string", value: "h0" },
              displayText: "h0",
            },
            {
              row: 0,
              column: 1,
              value: { kind: "string", value: "h1" },
              displayText: "h1",
            },
            {
              row: 1,
              column: 1,
              value: { kind: "string", value: "only" },
              displayText: "only",
            },
          ],
        },
