...
export class HsqldbScriptParseError extends Error {
  ...
  constructor(message: string, statement: string) {
    super(`HSQLDB script parse error: ${message} -- in statement: ${truncateForMessage(statement)}`);
...
function truncateForMessage(statement: string): string {
  return statement.length > MESSAGE_STATEMENT_PREVIEW_LENGTH ? `${statement.slice(0, MESSAGE_STATEMENT_PREVIEW_LENGTH)}...` : statement;
}
...
export function displayTextFor(value: ContentCellValue): string {
  switch (value.kind) {
    ...
    case 'currency':
      return value.currency === undefined ? String(value.value) : `${value.value} ${value.currency}`;
    case 'boolean':
...
function isIgnorableStatement(statement: string): boolean {
  ...
  return IGNORABLE_STATEMENT_PREFIXES.some((prefix) => normalized.startsWith(prefix));
}

const CREATE_TABLE_RE = /^CREATE\s+(?:(?:MEMORY|CACHED|TEXT|TEMP|TEMPORARY|GLOBAL\s+TEMPORARY)\s+)?TABLE\s+/i;
...
function readIdentifier(s: string, start: number): { readonly name: string; readonly next: number } {
...
function readQualifiedName(s: string, start: number): { readonly last: string; readonly next: number } {
...
function readBalancedParens(s: string, openIndex: number): { readonly inner: string; readonly next: number } {
  let depth = 0;
  let inSingleQuote = false;
  let inDoubleQuote = false;
  let i = openIndex;
...
function parseLiteral(raw: string, bucket: 'date' | 'time' | 'other'): ContentCellValue {
...
function parseCreateTable(statement: string): { readonly tableName: string; readonly columns: HsqldbColumn[] } {
  const match = CREATE_TABLE_RE.exec(statement);
  if (match === null) {
    throw new HsqldbScriptParseError('expected a CREATE TABLE statement', statement);
  }
  ...
  if (statement.charAt(openIndex) !== '(') {
    throw new HsqldbScriptParseError(`CREATE TABLE "${tableName}": expected "(" to start the column list`, statement);
  }
  ...
  for (const rawPart of splitTopLevel(inner, ',')) {
    ...
    const leadingWord = /^([A-Za-z_][A-Za-z0-9_]*)/.exec(part)?.[1]?.toUpperCase();
    if (leadingWord !== undefined && TABLE_CONSTRAINT_KEYWORDS.has(leadingWord)) {
    ...
    if (name.length === 0) {
      throw new HsqldbScriptParseError(`CREATE TABLE "${tableName}": could not read a column name from "${part}"`, statement);
    }
    columns.push({ name, type: part.slice(afterName).trim() });
  }
  if (columns.length === 0) {
    throw new HsqldbScriptParseError(`CREATE TABLE "${tableName}": no columns found`, statement);
  }
  return { tableName, columns };
}

function parseInsertInto(statement: string): { readonly tableName: string; readonly explicitColumns: readonly string[] | undefined; readonly values: readonly string[] } {
  const match = INSERT_INTO_RE.exec(statement);
  if (match === null) {
    throw new HsqldbScriptParseError('expected an INSERT INTO statement', statement);
  }
  const { last: tableName, next } = readQualifiedName(statement, match[0].length);
  ...
  if (statement.charAt(i) === '(') {
    ...
    explicitColumns = splitTopLevel(inner, ',').map((part) => readIdentifier(part.trim(), 0).name);
    i = skipWs(statement, afterCols);
  }

  const valuesMatch = /^VALUES\s*/i.exec(statement.slice(i));
  if (valuesMatch === null) {
    throw new HsqldbScriptParseError(`INSERT INTO "${tableName}": expected the VALUES keyword`, statement);
  }
  i = skipWs(statement, i + valuesMatch[0].length);

  if (statement.charAt(i) !== '(') {
    throw new HsqldbScriptParseError(`INSERT INTO "${tableName}": expected "(" to start the VALUES tuple`, statement);
  }
  const { inner, next: afterTuple } = readBalancedParens(statement, i);
  const trailing = statement.slice(afterTuple).trim();
  if (trailing.length > 0) {
    throw new HsqldbScriptParseError(`INSERT INTO "${tableName}": unexpected trailing content after the VALUES tuple ("${trailing}") -- multiple tuples per INSERT are outside this bounded parser's scope`, statement);
  }

  return { tableName, explicitColumns, values: splitTopLevel(inner, ',').map((value) => value.trim()) };
}

function resolveColumnOrder(table: MutableTable, explicitColumns: readonly string[] | undefined, statement: string): readonly number[] {
  if (explicitColumns === undefined) {
    return table.columns.map((_, index) => index);
  }
  return explicitColumns.map((name) => {
    const index = table.columns.findIndex((column) => column.name.toUpperCase() === name.toUpperCase());
    if (index === -1) {
      throw new HsqldbScriptParseError(`INSERT INTO "${table.tableName}": column "${name}" is not declared on this table`, statement);
    }
    return index;
  });
}
  ...
  for (const statement of splitStatements(text)) {
    if (CREATE_TABLE_RE.test(statement)) {
      ...
      if (tablesByKey.has(key)) {
        throw new HsqldbScriptParseError(`duplicate CREATE TABLE for "${tableName}"`, statement);
      }
    ...
    if (INSERT_INTO_RE.test(statement)) {
      ...
      if (table === undefined) {
        throw new HsqldbScriptParseError(`INSERT INTO references unknown table "${tableName}" (no prior CREATE TABLE seen for it)`, statement);
      }
      const columnOrder = resolveColumnOrder(table, explicitColumns, statement);
      if (values.length !== columnOrder.length) {
        throw new HsqldbScriptParseError(`INSERT INTO "${tableName}": expected ${columnOrder.length} value(s), found ${values.length}`, statement);
      }
      const row: ContentCellValue[] = table.columns.map((): ContentCellValue => ({ kind: 'empty' }));
      columnOrder.forEach((columnIndex, position) => {
        const column = table.columns[columnIndex];
        const rawValue = values[position];
        if (column === undefined || rawValue === undefined) {
          throw new HsqldbScriptParseError(`INSERT INTO "${tableName}": internal column/value alignment failure`, statement);
        }
    ...
    throw new HsqldbScriptParseError('unrecognised statement (not CREATE TABLE, INSERT INTO, or a known ignorable statement kind)', statement);
  }

  return order.map((key) => {
    const table = tablesByKey.get(key);
    if (table === undefined) {
      throw new HsqldbScriptParseError(`internal error: table "${key}" missing from tracking map`, '');
    }
    return { tableName: table.tableName, columns: table.columns, rows: table.rows };
  });
}
