...
import { hsqldbBinaryScriptBytes, hsqldbCompressedScriptBytes } from '../test-support/odb';
import { HsqldbBinaryScriptParseError, inflateHsqldbCompressedScript, parseHsqldbBinaryScript } from './binary-script';
...
const ORACLE_TYPE_TEST = [
  [{ kind: 'number', value: 1 }, { kind: 'time', value: '14:30:00' }, { kind: 'date', value: '2024-03-15 09:45:30.123456789' }, { kind: 'number', value: 123456789012345 }, { kind: 'number', value: 32000 }, { kind: 'number', value: 120 }],
  [{ kind: 'number', value: 2 }, { kind: 'time', value: '23:59:59' }, { kind: 'date', value: '1999-12-31 23:59:59' }, { kind: 'number', value: -123456789012345 }, { kind: 'number', value: -32000 }, { kind: 'number', value: -120 }],
...
describe('parseHsqldbBinaryScript: the real hsqldb.script_format=1 fixture', () => {
  ...
  it('recovers every column of every table from that same DDL', () => {
    ...
    expect(tables.map((table) => table.tableName)).toEqual(['EMPLOYEES', 'TYPE_TEST', 'EMPTY_TABLE']);
  ...
  it('recovers every row of every table exactly as HSQLDB 1.8.0.10 itself reported via JDBC', () => {
    const byName = new Map(parseHsqldbBinaryScript(hsqldbBinaryScriptBytes()).tables.map((table) => [table.tableName, table]));
  ...
  it('honours a caller-supplied timeZone for the date/time values it decodes', () => {
    const byName = new Map(parseHsqldbBinaryScript(hsqldbBinaryScriptBytes(), { timeZone: 'America/New_York' }).tables.map((table) => [table.tableName, table]));
    // Midnight Europe/London on 2020-01-15 is 19:00 the previous day in New York.
    expect(byName.get('EMPLOYEES')?.rows[0]?.[3]).toEqual({ kind: 'date', value: '2020-01-14' });
  });
});
...
describe('inflateHsqldbCompressedScript: the real hsqldb.script_format=3 fixture', () => {
  it('inflates to a byte-identical copy of the BINARY-format script the same database wrote', () => {
    expect(inflateHsqldbCompressedScript(hsqldbCompressedScriptBytes())).toEqual(hsqldbBinaryScriptBytes());
  });

  it('decodes, once inflated, to the identical tables and rows', () => {
    const compressed = parseHsqldbBinaryScript(inflateHsqldbCompressedScript(hsqldbCompressedScriptBytes()));
    expect(compressed).toEqual(parseHsqldbBinaryScript(hsqldbBinaryScriptBytes()));
  });
});
...
describe('parseHsqldbBinaryScript: malformed input fails loudly and specifically', () => {
  it('throws for a leading record that is not a Result at all', () => {
    ...
    expect(() => parseHsqldbBinaryScript(bytes)).toThrow(HsqldbBinaryScriptParseError);
  ...
  it('throws for an implausible leading record length rather than reading past the buffer', () => {
    ...
    expect(() => parseHsqldbBinaryScript(bytes)).toThrow(/implausible length 0/);
  });

  it('throws when the stream ends mid-record', () => {
    const truncated = hsqldbBinaryScriptBytes().subarray(0, 300);
    expect(() => parseHsqldbBinaryScript(new Uint8Array(truncated))).toThrow(HsqldbBinaryScriptParseError);
  });
});
