...
export function resolveHsqldbTypeCode(declaredType: string): number {
  ...
  if (code === undefined) {
    throw new HsqldbRowFormatError(`unrecognised HSQLDB column type "${word}" -- from declared type "${declaredType}"`);
  }
  return code;
}
  ...
  while (i < bytes.length) {
    ...
    if (leadNibble === 0xc || leadNibble === 0xd) {
      ...
      if (b1 === undefined || (b1 & 0xc0) !== 0x80) {
        throw new HsqldbRowFormatError('malformed modified-UTF-8 2-byte sequence');
      }
    ...
    if (leadNibble === 0xe) {
      ...
      if (b1 === undefined || b2 === undefined || (b1 & 0xc0) !== 0x80 || (b2 & 0xc0) !== 0x80) {
        throw new HsqldbRowFormatError('malformed modified-UTF-8 3-byte sequence');
      }
      result += String.fromCharCode(((b0 & 0xf) << 12) | ((b1 & 0x3f) << 6) | (b2 & 0x3f));
      i += 3;
      continue;
    }
...
export function readHsqldbColumnValue(cursor: HsqldbDataCursor, typeCode: number, options?: HsqldbDecodeOptions): ContentCellValue {
  ...
  switch (typeCode) {
    ...
    case 100: {
      ...
      return { kind: 'string', value: readModifiedUtf8(cursor.readBytes(byteLength)) };
    }
    case 5: // SMALLINT
