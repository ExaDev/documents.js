...
// Reads a standalone .odf package's own formula content as a real 'formula'-kind ContentDocument -- a thin adapter over odf.js's own readOdfFormulaContent (that name since odf.js 5.0.0; before it it was the bare readOdfFormulaDocument, and 5.0.0 gives the bare readOdfFormula name to the tree-form DocumentPackage counterpart), exactly the pattern this package's other src/odf/{odt,odp,ods,odg}/read.ts modules already follow (readOdtContent over odf.js's readOdtContent, readOdpContent over odf.js's readOdpContent, ...), and now genuinely the same SHAPE as them too rather than a side-channel value of its own: document-schema.js 2.0.0's ContentDocument union has a 'formula' variant carrying exactly `{ mathml, starMath }`, so a standalone formula document IS a ContentDocument, and odf.js already builds that envelope itself. The source document's own metadata comes through with it, the same way every other X-to-PDF conversion in this package carries its source metadata into writePdf's own Info dict. The import is aliased only because this module's own export is itself named readOdfFormulaContent.
