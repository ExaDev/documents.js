...
function packageWithImage(): Package {
  return { parts: { 'Pictures/image1.png': { kind: 'binary', base64: tinyPngBase64() } } };
}
...
describe('collectImageFrames', () => {
  it('recovers an as-char anchored draw:frame>draw:image as a ContentImageBlock, with a zero-origin frame', () => {
    ...
    const frame = el('draw:frame', { 'text:anchor-type': 'as-char', 'svg:width': '100pt', 'svg:height': '50pt' }, [el('draw:image', { 'xlink:href': 'Pictures/image1.png' })]);

    const [detected] = collectImageFrames([frame], pkg);
    expect(detected).toBeDefined();
    expect(detected!.frameElement).toBe(frame);
    expect(detected!.image).toMatchObject({ kind: 'image', format: 'png', widthPt: 100, heightPt: 50 });
  ...
  it('recovers a frame nested inside a draw:text-box, deep inside a paragraph', () => {
    const pkg = packageWithImage();
    const imageFrame = el('draw:frame', { 'text:anchor-type': 'as-char', 'svg:width': '100pt', 'svg:height': '50pt' }, [el('draw:image', { 'xlink:href': 'Pictures/image1.png' })]);
    const textBoxFrame = el('draw:frame', { 'svg:x': '10pt', 'svg:y': '10pt', 'svg:width': '200pt', 'svg:height': '100pt' }, [
    ...
    expect(detected[0]!.image).toMatchObject({ kind: 'image', format: 'png', widthPt: 100, heightPt: 50 });
  });

  it('never treats a formula frame\'s own sibling preview draw:image as a real image', () => {
    const pkg: Package = { parts: { 'Pictures/image1.png': { kind: 'binary', base64: tinyPngBase64() } } };
    ...
    const formulaFrame = el('draw:frame', { 'svg:width': '30pt', 'svg:height': '20pt' }, [
  ...
  it('recovers a real image frame sitting alongside an unrelated formula frame, without conflating the two', () => {
    ...
    const realImageFrame = el('draw:frame', { 'text:anchor-type': 'as-char', 'svg:width': '100pt', 'svg:height': '50pt' }, [el('draw:image', { 'xlink:href': 'Pictures/image1.png' })]);
  ...
  it('returns nothing for a package whose referenced part is not a recognisable image', () => {
    const pkg: Package = { parts: { 'Pictures/image1.png': { kind: 'binary', base64: bytesToBase64(new Uint8Array([1, 2, 3, 4])) } } };
    const frame = el('draw:frame', { 'text:anchor-type': 'as-char', 'svg:width': '100pt', 'svg:height': '50pt' }, [el('draw:image', { 'xlink:href': 'Pictures/image1.png' })]);
