...
// Package -> ContentDocument (the drawing variant). A thin adapter over odf.js's own readOdg, mirroring src/odf/odp/read.ts's readOdpContent and src/odf/ods/read.ts's readOdsContent exactly: odf.js's OdgDocument is already { metadata, pages }, the identical shape document-content-model's own ContentDrawPage vocabulary already targets, so this is nothing more than the envelope wrap.
