...
const VECTOR_ELEMENT_TAGS: ReadonlySet<string> = new Set([
  "draw:rect",
  "draw:ellipse",
  "draw:circle",
  "draw:line",
...
export function collectContainerVectors(
...
export function collectSlideVectorGroups(
  ...
  for (const vector of vectors) {
    ...
    const insertBeforeShapeIndex = shapePaintOrders.filter(
      (order) => order < vectorOrder,
    ).length;
    const last = groups[groups.length - 1];
    const continuesLastGroup =
      last?.insertBeforeShapeIndex === insertBeforeShapeIndex;
