...
describe("docx: readDocxContent/buildDocxPackage read -> build -> read stability", () => {
  it("agrees on paragraph text, a styled run, and table structure across the composition", () => {
    ...
    const styledParagraph = editor.body.appendParagraph({
      alignment: "center",
    });
    ...
    const originalParagraphs = original.sections
      .flatMap((s) => s.blocks)
      .filter(isParagraph);
    const roundTrippedParagraphs = roundTripped.sections
      .flatMap((s) => s.blocks)
      .filter(isParagraph);
    expect(roundTrippedParagraphs).toHaveLength(originalParagraphs.length);
    expect(
      roundTrippedParagraphs.map((p) => p.runs.map((r) => r.text).join("")),
    ).toEqual(
      originalParagraphs.map((p) => p.runs.map((r) => r.text).join("")),
    );

    const roundTrippedStyledRun = roundTrippedParagraphs
      .flatMap((p) => p.runs)
      .find((r) => r.text === "StyledRun");
    expect(roundTrippedStyledRun).toMatchObject({
      text: "StyledRun",
      bold: true,
      sizePt: 24,
      color: { r: 1, g: 0, b: 0 },
    ...
    const originalTable = original.sections
      .flatMap((s) => s.blocks)
      .find(isTable);
    const roundTrippedTable = roundTripped.sections
      .flatMap((s) => s.blocks)
      .find(isTable);
    ...
    expect(
      roundTrippedTable?.rows[0]?.cells.map((c) =>
    ...
    expect(
      roundTrippedTable?.rows[1]?.cells.map((c) =>
        c.blocks[0]?.kind === "paragraph"
          ? c.blocks[0].runs[0]?.text
          : undefined,
      ),
    ...
    const originalImage = original.sections
      .flatMap((s) => s.blocks)
      .find(isImage);
    const roundTrippedImage = roundTripped.sections
      .flatMap((s) => s.blocks)
      .find(isImage);
...
describe("pptx: readPptxContent/buildPptxPackage read -> build -> read stability", () => {
  it("agrees on slide count, a styled shape, an image, and speaker notes across the composition", () => {
    ...
    const titleShape = firstSlide.addTextBox({
      frame: { xPt: 50, yPt: 30, widthPt: 400, heightPt: 60 },
      text: "",
    });
    ...
    firstSlide.addImage({
      frame: { xPt: 50, yPt: 120, widthPt: 80, heightPt: 60 },
      format: "png",
      bytes: PNG_BYTES,
    });
    ...
    const originalShapeTexts = original.slides.map((slide) =>
    ...
    const roundTrippedShapeTexts = roundTripped.slides.map((slide) =>
      slide.shapes.map((shape) =>
        shape.blocks
          .filter(isParagraph)
          .map((p) => p.runs.map((r) => r.text).join(""))
    ...
    const originalStyledRun = original.slides[0]?.shapes
      .flatMap((s) => s.blocks)
      .filter(isParagraph)
      .flatMap((p) => p.runs)
      .find((r) => r.text === "Styled Title");
    const roundTrippedStyledRun = roundTripped.slides[0]?.shapes
      .flatMap((s) => s.blocks)
      .filter(isParagraph)
      .flatMap((p) => p.runs)
      .find((r) => r.text === "Styled Title");
    ...
    const originalImage = original.slides[0]?.shapes
      .flatMap((s) => s.blocks)
      .find(isImage);
    const roundTrippedImage = roundTripped.slides[0]?.shapes
      .flatMap((s) => s.blocks)
      .find(isImage);
    expect(originalImage).toBeDefined();
    expect(roundTrippedImage).toEqual(originalImage);
    ...
    expect(
      roundTrippedImage !== undefined &&
        base64ToBytes(roundTrippedImage.base64),
    ).toEqual(PNG_BYTES);
