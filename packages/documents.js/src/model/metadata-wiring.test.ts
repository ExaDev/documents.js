...
describe('buildDocxPackage precedence: an already-timestamped ContentDocument is never touched by the clock', () => {
  it('never consults the supplied clock when content.metadata already carries both createdIso and modifiedIso', () => {
    ...
    const withMetadata = { ...original, metadata: { createdIso: '2020-05-01T00:00:00.000Z', modifiedIso: '2021-05-01T00:00:00.000Z' } };

    const throwingClock = {
      now: (): Date => {
        throw new Error('clock should not be consulted when both timestamps are already present');
      },
    };

    const rebuiltPackage = buildDocxPackage(withMetadata, { clock: throwingClock });
...
function docxPackageWithHandBuiltCoreProperties(createdIso: string, modifiedIso: string): ReturnType<typeof createEmptyDocxPackage> {
  ...
  const root = el(
    ...
    [
      ...
      el('dcterms:created', { 'xsi:type': 'dcterms:W3CDTF' }, [txt(createdIso)]),
      el('dcterms:modified', { 'xsi:type': 'dcterms:W3CDTF' }, [txt(modifiedIso)]),
    ],
  );
  pkg.parts['docProps/core.xml'] = {
    kind: 'xml',
    nodes: [
      { type: 'declaration', attributes: [{ name: 'version', value: '1.0' }, { name: 'encoding', value: 'UTF-8' }, { name: 'standalone', value: 'yes' }] },
      root,
    ],
  };
  return pkg;
...
describe('buildDocxPackage never clobbers a real pre-existing document\'s own creation date', () => {
  it('preserves a hand-authored old createdIso/modifiedIso even when rebuilt with a later fixedClock', () => {
    ...
    const handBuiltPkg = docxPackageWithHandBuiltCoreProperties(OLD_CREATED_ISO, OLD_MODIFIED_ISO);
...
function odtPackageWithHandBuiltMeta(createdIso: string, modifiedIso: string): OdfPackage {
  ...
  const declaration = { type: 'declaration' as const, attributes: [{ name: 'version', value: '1.0' }, { name: 'encoding', value: 'UTF-8' }, { name: 'standalone', value: 'yes' }] };
  return {
    parts: {
...
describe('buildOdtPackage never clobbers a real pre-existing document\'s own creation date', () => {
  it('preserves a hand-authored old createdIso/modifiedIso even when rebuilt with a later fixedClock', () => {
    ...
    const handBuiltPkg = odtPackageWithHandBuiltMeta(OLD_CREATED_ISO, OLD_MODIFIED_ISO);
