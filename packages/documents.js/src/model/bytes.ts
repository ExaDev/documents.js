...
function zipBytesSchema(label: string) {
  return z
    ...
    .refine((bytes) => startsWithBytes(bytes, ZIP_LOCAL_FILE_HEADER), {
...
const MIMETYPE_CONTENT_OFFSET =
  MIMETYPE_FILENAME_OFFSET + MIMETYPE_ENTRY_FILENAME.length; // 38: the filename field is exactly "mimetype".length bytes, since ODF fixes both the name and its stored (uncompressed) encoding.
...
function hasOdfMimetypeEntry(
  ...
  if (
    readAsciiSlice(
      bytes,
      MIMETYPE_FILENAME_OFFSET,
      MIMETYPE_ENTRY_FILENAME.length,
    ) !== MIMETYPE_ENTRY_FILENAME
  ) {
    return false;
  }
  return mediaTypes.some((mediaType) => {
    ...
    return (
      readAsciiSlice(bytes, MIMETYPE_CONTENT_OFFSET, mediaType.length) ===
      mediaType
    );
  });
}
...
function odfBytesSchema(label: string, mediaTypes: readonly string[]) {
  ...
  return z
    .instanceof(Uint8Array)
    .refine((bytes) => hasOdfMimetypeEntry(bytes, mediaTypes), {
...
export const OdtBytesSchema = odfBytesSchema("odt", [
  ODF_MEDIA_TYPES.odt,
  ODF_MEDIA_TYPES.ott,
]);
export const OdsBytesSchema = odfBytesSchema("ods", [
  ODF_MEDIA_TYPES.ods,
  ODF_MEDIA_TYPES.ots,
]);
export const OdpBytesSchema = odfBytesSchema("odp", [
  ODF_MEDIA_TYPES.odp,
  ODF_MEDIA_TYPES.otp,
]);
export const OdgBytesSchema = odfBytesSchema("odg", [
  ODF_MEDIA_TYPES.odg,
  ODF_MEDIA_TYPES.otg,
]);
...
export function requireArrayBufferBytes(
  bytes: Uint8Array,
): Uint8Array<ArrayBuffer> {
  if (!isArrayBufferBacked(bytes)) {
    throw new TypeError(
      "expected an ArrayBuffer-backed Uint8Array, received one backed by a SharedArrayBuffer",
    );
  }
  return bytes;
}
