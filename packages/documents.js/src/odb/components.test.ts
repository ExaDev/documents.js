...
describe('readOdbForms', () => {
  it('reads every declared form, resolving its own bound controls', () => {
    ...
    expect(definition.controls[0]).toMatchObject({ tag: 'form:text', name: 'NAME', dataField: 'NAME', label: 'Name' });
  });
});
...
describe('readOdbReports', () => {
  it('reads every declared report, resolving its own bands and bound controls', () => {
    ...
    expect(report.reportHeader?.elements[0]).toMatchObject({ tag: 'rpt:fixed-content', name: 'Title', text: 'Customer Report' });

    expect(report.detail?.elements).toHaveLength(1);
    expect(report.detail?.elements[0]).toMatchObject({ tag: 'rpt:formatted-text', name: 'NameField', dataField: 'NAME', formula: 'field:[NAME]' });
  });
});
