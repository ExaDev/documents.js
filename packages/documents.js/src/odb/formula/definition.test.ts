...
function emptyReport(overrides: Partial<OdbReport> = {}): OdbReport {
  return { name: 'R', href: 'reports/R', groups: [], functions: [], ...overrides };
}

function chainGroup(groupExpression: string | undefined, child?: OdbReportGroup): OdbReportGroup {
  return { groupExpression, functions: [], groups: child === undefined ? [] : [child] };
}
...
describe('rptDefinitionFromReport over the real fixture report', () => {
  it('flattens the nested rpt:group chain into an outermost-first list', () => {
    ...
    expect(definition.groups.map((group) => group.groupExpression)).toEqual(['rpt:HASCHANGED("REGION")', 'rpt:HASCHANGED("LEFT_QUARTER")']);
    expect(definition.functions).toEqual([{ name: 'LEFT_QUARTER', formula: 'rpt:LEFT([QUARTER];2)' }]);
  });
  ...
  it('keeps one formula slot per band element, undefined where the element carries none', () => {
    ...
    expect(definition.reportFooter?.formulas).toEqual([undefined, 'rpt:SUM([AMOUNT])']);
    expect(definition.reportFooter?.formulas.length).toBe(report.reportFooter?.elements.length);
  ...
  it('drops the page bands, which the formula evaluator has no pagination to place them against', () => {
    ...
    expect([...(report.pageHeader?.elements ?? []), ...(report.pageFooter?.elements ?? [])].every((element) => element.formula === undefined)).toBe(true);
    expect(Object.keys(rptDefinitionFromReport(report))).toEqual(['functions', 'reportHeader', 'groups', 'detail', 'reportFooter']);
  });
});

describe('rptDefinitionFromReport refusals', () => {
  it('refuses a group declaring no break test', () => {
    expect(() => rptDefinitionFromReport(emptyReport({ groups: [chainGroup(undefined)] }))).toThrow(RptReportStructureError);
    expect(() => rptDefinitionFromReport(emptyReport({ groups: [chainGroup(undefined)] }))).toThrow(/declares no rpt:group-expression/);
  });

  it('refuses sibling groups at one nesting level rather than keeping the first and dropping the rest', () => {
    const siblings = emptyReport({ groups: [chainGroup('rpt:HASCHANGED("A")'), chainGroup('rpt:HASCHANGED("B")')] });
    expect(() => rptDefinitionFromReport(siblings)).toThrow(RptReportStructureError);
    expect(() => rptDefinitionFromReport(siblings)).toThrow(/2 sibling groups are declared at one nesting level/);
  });

  it('walks a chain of any depth', () => {
    const deep = emptyReport({ groups: [chainGroup('rpt:HASCHANGED("A")', chainGroup('rpt:HASCHANGED("B")', chainGroup('rpt:HASCHANGED("C")')))] });
    expect(rptDefinitionFromReport(deep).groups.map((group) => group.groupExpression)).toEqual(['rpt:HASCHANGED("A")', 'rpt:HASCHANGED("B")', 'rpt:HASCHANGED("C")']);
  });
});
