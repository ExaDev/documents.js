...
import { OdbReportDataSourceError, odbReportCommandSql, resolveOdbReportRows } from './source';
...
function renderedBands(document: ContentDocument): readonly RenderedBand[] {
  if (document.kind !== 'wordprocessing') {
    throw new Error(`expected a wordprocessing ContentDocument, got "${document.kind}"`);
  }
  const section = document.sections[0];
  if (section === undefined || document.sections.length !== 1) {
    throw new Error(`expected exactly one section, got ${String(document.sections.length)}`);
  }
  return section.blocks.map((block) => {
    if (block.kind !== 'table') {
      throw new Error(`expected every band to render as a table, got a "${block.kind}" block`);
    }
    const row = block.rows[0];
    if (row === undefined || block.rows.length !== 1) {
      throw new Error(`expected exactly one row per band table, got ${String(block.rows.length)}`);
    }
    ...
    const bandStyles = new Set(paragraphs.map((paragraph) => paragraph.styleId));
    const band = [...bandStyles][0];
    if (band === undefined || bandStyles.size !== 1) {
      throw new Error(`expected every cell of one band to carry one band style, got ${[...bandStyles].join(', ')}`);
    }
    return { band, cells: paragraphs.map((paragraph) => paragraph.runs.map((run) => run.text).join('')) };
  });
}

function cellsOfBand(bands: readonly RenderedBand[], band: string): readonly (readonly string[])[] {
  return bands.filter((rendered) => rendered.band === band).map((rendered) => rendered.cells);
}
...
function totalsOfBand(bands: readonly RenderedBand[], band: string): readonly number[] {
...
describe('the real SalesByRegion report rendered end to end from form-and-report.odb', () => {
  it('binds to its own saved query, read straight out of the package rather than restated here', () => {
    ...
    expect(odbReportCommandSql(report, inventory.queries)).toBe(inventory.queries[0]?.command);
    expect(odbReportCommandSql(report, inventory.queries)).toContain('WHERE "SALES"."AMOUNT" >= 100');
  });

  it('renders every band in print order, with the right group structure, detail rows, and hand-computed totals', () => {
    const bands = renderedBands(readOdbReportContent(formAndReportOdbPackage()));
  ...
  it('totals exactly the hand-computed amounts at each of the three real SUM scopes', () => {
    ...
    expect(totalsOfBand(bands, 'Group Footer 2')).toEqual([1540.5, 2750.25, 1810]);
  ...
  it('carries each detail row own values through in the query own order', () => {
    ...
    expect(bands.flatMap((band) => band.cells)).not.toContain('Foxglove Design');
  });
  ...
  it('produces one A4 section whose band tables each span the full content width', () => {
    ...
    const contentWidthPt = PAGE_SIZE_A4.widthPt - (section.margins.leftPt + section.margins.rightPt);
    for (const block of section.blocks) {
      if (block.kind !== 'table') {
        throw new Error(`expected every band to render as a table, got a "${block.kind}" block`);
      }
      expect(block.columnWidthsPt.reduce((total, width) => total + width, 0)).toBeCloseTo(contentWidthPt, 10);
    }
  });
...
describe('the same real report rendered over all six real SALES rows', () => {
  it('groups every region and quarter, and totals each scope to the hand-computed amount', () => {
    const bands = renderedBands(renderOdbReportContent(salesReport(), everySalesRow()));

    expect(cellsOfBand(bands, 'Group Header 1')).toEqual([['North'], ['South'], ['West']]);
    ...
    expect(cellsOfBand(bands, 'Group Header 2')).toEqual([['Q1'], ['Q2'], ['Q1'], ['Q2'], ['Q2']]);

    expect(totalsOfBand(bands, 'Group Footer 2')).toEqual([1540.5, 2750.25, 95.75, 1810, 60]);
    expect(totalsOfBand(bands, 'Group Footer 1')).toEqual([4290.75, 1905.75, 60]);
...
describe('the three real rpt:command-type binding shapes, resolved against the real package', () => {
  ...
  it('turns a table binding into a real SELECT-all that runs against the real table', () => {
    const asTable = { ...salesReport(), command: 'SALES', commandType: 'table' };

    expect(odbReportCommandSql(asTable, queries())).toBe('SELECT * FROM "SALES"');
    // Every row of the real table, since a table binding carries no filter of its own -- two more than the saved query keeps.
    expect(resolveOdbReportRows(formAndReportOdbPackage(), asTable).rows).toHaveLength(6);
  });

  it('passes an inline command binding through as the SQL it is', () => {
    const asCommand = { ...salesReport(), command: 'SELECT CUSTOMER, AMOUNT FROM SALES WHERE AMOUNT < 100', commandType: 'command' };

    expect(odbReportCommandSql(asCommand, queries())).toBe('SELECT CUSTOMER, AMOUNT FROM SALES WHERE AMOUNT < 100');
    expect(resolveOdbReportRows(formAndReportOdbPackage(), asCommand).rows).toEqual([
      [
  ...
  it('names a binding it cannot resolve rather than falling back to another reading of it', () => {
    ...
    expect(() => odbReportCommandSql({ ...report, command: 'NoSuchQuery' }, queries())).toThrow(OdbReportDataSourceError);
    expect(() => odbReportCommandSql({ ...report, commandType: undefined }, queries())).toThrow(/no rpt:command-type/);
    expect(() => odbReportCommandSql({ ...report, command: undefined }, queries())).toThrow(/declares no rpt:command/);
    expect(() => odbReportCommandSql({ ...report, commandType: 'sql-pass-through' }, queries())).toThrow(/not one of the three values/);
  });
});
