...
function groupBandStyle(
  kind: "group-header" | "group-footer",
  level: number,
): string {
...
function printedBand(
  band: OdbReportBand | undefined,
  styleId: string,
  values: readonly (ContentCellValue | undefined)[],
): PrintedBand {
  if (band === undefined) {
    ...
    throw new Error(
      `rpt report render: a "${styleId}" band instance was emitted for a band this report does not declare`,
    );
  }
...
function bandOfInstance(
  report: OdbReport,
  groups: readonly OdbReportGroup[],
  ...
  switch (instance.kind) {
    ...
    case "group-footer": {
      ...
      if (level === undefined) {
        throw new Error(
          `rpt report render: a ${instance.kind} band instance carries no group level`,
        );
      }
      const group = groups[level];
      if (group === undefined) {
        throw new Error(
          `rpt report render: a ${instance.kind} band instance names group level ${String(level)}, which this report's own chain of ${String(groups.length)} group(s) does not reach`,
        );
      }
      return printedBand(
        instance.kind === "group-header" ? group.header : group.footer,
        groupBandStyle(instance.kind, level),
        instance.values,
      );
    }
  }
  return assertNeverInstanceKind(instance.kind);
}
...
function elementText(
  element: OdbReportElement,
  value: ContentCellValue | undefined,
): string {
  ...
  if (value === undefined) {
    throw new Error(
      `rpt report render: control "${element.formula}" carries a formula but the run produced no value for it`,
    );
  }
...
function bandTable(band: PrintedBand): ContentTable | undefined {
  ...
  if (band.values.length !== band.elements.length) {
    throw new Error(
      `rpt report render: the "${band.styleId}" band printed ${String(band.values.length)} values for ${String(band.elements.length)} control(s)`,
    );
  }
    ...
    rows: [
      {
        cells: band.elements.map((element, index) => ({
          blocks: [cellParagraph(element, band.values[index], band.styleId)],
        })),
      },
    ],
  };
  ...
  return [
    {
      ...
      values: evaluateRptBandOutsideData(
        definition,
        rptBandDefinition(band),
        resultSet,
      ),
    },
  ];
...
export function renderOdbReportContent(
  report: OdbReport,
  resultSet: SqlResultSet,
): ContentDocument {
  ...
  const printed = (
    matches: (kind: RptBandKind) => boolean,
  ): readonly PrintedBand[] =>
    instances
      .filter((instance) => matches(instance.kind))
      .map((instance) => bandOfInstance(report, groups, instance));
  ...
  const section: ContentSection = {
    ...
    blocks: [
      ...
      ...bandBlocks(
        pageBands(definition, report.pageHeader, PAGE_HEADER_STYLE, resultSet),
      ),
      ...bandBlocks(
        printed((kind) => kind !== "report-header" && kind !== "report-footer"),
      ),
      ...bandBlocks(
        pageBands(definition, report.pageFooter, PAGE_FOOTER_STYLE, resultSet),
      ),
...
// Reached only if the union behind `instance.kind` ever gains a member the switch above does not match: every current member has a case there, so `instance.kind` narrows to `never` at the call, and adding an uncovered member makes that narrowing fail and the call stop compiling. Exists so the switch's own exhaustiveness, proven by the type checker rather than by a catch-all default that would silently accept a genuinely new member, still gives consistent-return an explicit statement to see past the switch. Exported so a test can exercise the throw directly with a forced-invalid cast, since it is otherwise unreachable.
export function assertNeverInstanceKind(value: never): never {
  throw new Error(
    `documents.js: unhandled instance.kind ${JSON.stringify(value)}`,
  );
}
