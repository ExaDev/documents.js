...
const TABLE: HsqldbTable = {
  tableName: 'CUSTOMERS',
  columns: [{ name: 'ID', type: 'INTEGER' }, { name: 'NAME', type: 'VARCHAR(50)' }],
  rows: [
    [{ kind: 'number', value: 1 }, { kind: 'string', value: 'Alice' }],
...
describe('odbTablesToSpreadsheetDocument', () => {
  ...
  it('writes a header row (row 0) of string cells from the column names', () => {
    ...
    const header = content.sheets[0]?.cells.filter((cell) => cell.row === 0).sort((a, b) => a.column - b.column);
    expect(header).toEqual([
      { row: 0, column: 0, value: { kind: 'string', value: 'ID' }, displayText: 'ID' },
      { row: 0, column: 1, value: { kind: 'string', value: 'NAME' }, displayText: 'NAME' },
    ]);
  });
  ...
  it('writes data rows starting at row 1, each cell carrying the original typed value and a matching displayText', () => {
    ...
    const row1 = content.sheets[0]?.cells.filter((cell) => cell.row === 1).sort((a, b) => a.column - b.column);
    expect(row1).toEqual([
      { row: 1, column: 0, value: { kind: 'number', value: 1 }, displayText: '1' },
      { row: 1, column: 1, value: { kind: 'string', value: 'Alice' }, displayText: 'Alice' },
    ]);
    const row2 = content.sheets[0]?.cells.filter((cell) => cell.row === 2).sort((a, b) => a.column - b.column);
    expect(row2).toEqual([
      { row: 2, column: 0, value: { kind: 'number', value: 2 }, displayText: '2' },
  ...
  it('produces a real, non-empty printSettings for every sheet, so the xlsx builder has something to write', () => {
    ...
    expect(content.sheets[0]?.printSettings.pageSize.widthPt).toBeGreaterThan(0);
  });
