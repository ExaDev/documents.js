...
export type SqlToken =
  | { readonly kind: 'keyword'; readonly keyword: string; readonly text: string; readonly start: number }
  | { readonly kind: 'identifier'; readonly name: string; readonly quoted: boolean; readonly text: string; readonly start: number }
  | { readonly kind: 'string'; readonly value: string; readonly text: string; readonly start: number }
  | { readonly kind: 'number'; readonly value: number; readonly text: string; readonly start: number }
  | { readonly kind: 'operator'; readonly operator: SqlComparisonOperator; readonly text: string; readonly start: number }
  | { readonly kind: 'punctuation'; readonly punctuation: SqlPunctuation; readonly text: string; readonly start: number }
...
function readNumber(sql: string, start: number): { readonly value: number; readonly next: number } {
  let cursor = start;
  ...
  if (!Number.isFinite(value)) {
    throw new HsqldbSqlParseError(`numeric literal "${text}" is not a finite number`, sql, start);
  }
...
function readStringLiteral(sql: string, start: number): { readonly value: string; readonly next: number } {
  let cursor = start + 1;
  let value = '';
...
function readQuotedIdentifier(sql: string, start: number): { readonly name: string; readonly next: number } {
  let cursor = start + 1;
  let name = '';
  while (cursor < sql.length) {
    const character = sql.charAt(cursor);
    if (character === '"') {
      ...
      if (name === '') {
        throw new HsqldbSqlParseError('empty quoted identifier ("")', sql, start);
      }
...
export function tokenizeSql(sql: string): readonly SqlToken[] {
  ...
  while (cursor < sql.length) {
    ...
    if (character === "'") {
      ...
      cursor = literal.next;
      tokens.push({ kind: 'string', value: literal.value, text: sql.slice(start, cursor), start });
      continue;
    }
    ...
    if (character === '"') {
      ...
      tokens.push({ kind: 'identifier', name: identifier.name, quoted: true, text: sql.slice(start, cursor), start });
      continue;
    }
    ...
    if (isDigit(character) || (character === '.' && isDigit(sql.charAt(cursor + 1)))) {
      ...
      cursor = numeric.next;
      tokens.push({ kind: 'number', value: numeric.value, text: sql.slice(start, cursor), start });
      continue;
    }
    ...
    if (isIdentifierStart(character)) {
      ...
      if (KEYWORDS.has(folded)) {
        ...
        tokens.push({ kind: 'identifier', name: folded, quoted: false, text, start });
      }
      continue;
    }
    ...
    if (character === '<' || character === '>' || character === '=') {
      ...
      if (twoCharacter === '<=' || twoCharacter === '>=' || twoCharacter === '<>') {
        cursor += 2;
        tokens.push({ kind: 'operator', operator: twoCharacter, text: twoCharacter, start });
        continue;
      }
      cursor += 1;
      tokens.push({ kind: 'operator', operator: character, text: character, start });
      continue;
    }
    ...
    if (character === '(' || character === ')' || character === ',' || character === '.' || character === '*' || character === ';' || character === '-') {
      tokens.push({ kind: 'punctuation', punctuation: character, text: character, start: cursor });
      cursor += 1;
      continue;
    }
