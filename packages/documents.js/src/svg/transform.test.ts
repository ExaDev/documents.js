...
describe("matrix classification", () => {
  ...
  it("classifies by frame representability: axis-aligned maps pass however they scale or mirror, rotation-composed non-uniform maps and shears fail", () => {
    expect(isNonReflectingSimilarity(parseSvgTransform("rotate(30)")!)).toBe(
      true,
    );
    expect(
      isNonReflectingSimilarity(
        parseSvgTransform("rotate(30) scale(2) scale(0.5)")!,
      ),
    ).toBe(true);
    ...
    expect(isNonReflectingSimilarity(parseSvgTransform("scale(2,3)")!)).toBe(
      true,
    );
    expect(isNonReflectingSimilarity(parseSvgTransform("scale(-1,1)")!)).toBe(
      true,
    );
    expect(
      isNonReflectingSimilarity(parseSvgTransform("matrix(-1 1 0 1 0 0)")!),
    ).toBe(false);
    expect(
      isNonReflectingSimilarity(parseSvgTransform("rotate(30) scale(2,3)")!),
    ).toBe(false);
    expect(isNonReflectingSimilarity(parseSvgTransform("skewX(45)")!)).toBe(
      false,
    );
  });

  it("reads a similarity's rotation straight off the first column, in screen-clockwise degrees", () => {
    expect(similarityRotationDeg(parseSvgTransform("rotate(90)")!)).toBeCloseTo(
      90,
      9,
    );
    expect(
      similarityRotationDeg(parseSvgTransform("rotate(-30)")!),
    ).toBeCloseTo(-30, 9);
    expect(
      similarityRotationDeg(
        composeMatrices(
          parseSvgTransform("scale(2)")!,
          parseSvgTransform("rotate(45)")!,
        ),
      ),
    ).toBeCloseTo(45, 9);
  });
...
describe("meanScaleFactor", () => {
  it("is 1 for the identity and any rotation, the mean of the two column scales otherwise", () => {
    ...
    expect(meanScaleFactor(parseSvgTransform("rotate(90)")!)).toBeCloseTo(
      1,
      12,
    );
