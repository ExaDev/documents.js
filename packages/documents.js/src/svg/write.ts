...
export class SvgMultiPageNotSpecifiedError extends Error {
  ...
  constructor(pageCount: number) {
    super(`buildSvgText: this document has more than one page (${pageCount}) -- pass { page: <index> } to select one`);
...
export function buildSvgText(content: ContentDocument, options?: BuildSvgTextOptions): string {
  ...
  lines.push(`<svg xmlns="http://www.w3.org/2000/svg" width="${formatPathNumber(page.size.widthPt)}pt" height="${formatPathNumber(page.size.heightPt)}pt" viewBox="${buildSvgViewBox(page.size.widthPt, page.size.heightPt)}">`);
