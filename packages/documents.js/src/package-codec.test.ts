...
describe('decodeDocumentPackage: OOXML formats (docx/pptx/xlsx)', () => {
  it('docx: matches ooxml.js decodePackage', () => {
    ...
    expect(decodeDocumentPackage('docx', bytes)).toEqual(decodeOoxmlPackage(bytes));
  });

  it('pptx: matches ooxml.js decodePackage', () => {
    const bytes = minimalPptxBytes();
    expect(decodeDocumentPackage('pptx', bytes)).toEqual(decodeOoxmlPackage(bytes));
  });
...
describe('decodeDocumentPackage: ODF formats (odt/odp/ods/odg/odf)', () => {
  it('odt: matches odf.js decodePackage', () => {
    ...
    expect(decodeDocumentPackage('odt', bytes)).toEqual(decodeOdfPackage(bytes));
  });

  it('odp: matches odf.js decodePackage', () => {
    const bytes = minimalOdpBytes();
    expect(decodeDocumentPackage('odp', bytes)).toEqual(decodeOdfPackage(bytes));
  });

  it('ods: matches odf.js decodePackage', () => {
    const bytes = minimalOdsBytes();
    expect(decodeDocumentPackage('ods', bytes)).toEqual(decodeOdfPackage(bytes));
  });

  it('odg: matches odf.js decodePackage', () => {
    const bytes = minimalOdgBytes();
    expect(decodeDocumentPackage('odg', bytes)).toEqual(decodeOdfPackage(bytes));
  });
...
describe('decodeDocumentPackage / encodeDocumentPackage: unsupported formats', () => {
  it('markdown: throws UnsupportedPackageFormatError -- markdown is plain text, not a zip container', () => {
    ...
    expect(() => decodeDocumentPackage('markdown', bytes)).toThrow(UnsupportedPackageFormatError);
    expect(() => decodeDocumentPackage('markdown', bytes)).toThrow(/no raw package concept/);
    try {
  ...
  it('pdf: throws UnsupportedPackageFormatError -- pdf is not an OPC/ODF zip package', () => {
    ...
    expect(() => decodeDocumentPackage('pdf', bytes)).toThrow(UnsupportedPackageFormatError);
    expect(() => decodeDocumentPackage('pdf', bytes)).toThrow(/no raw package concept/);
  });

  it('encodeDocumentPackage: markdown throws UnsupportedPackageFormatError too', () => {
    const pkg = decodeOoxmlPackage(minimalDocxBytes());
    expect(() => encodeDocumentPackage('markdown', pkg)).toThrow(UnsupportedPackageFormatError);
  });

  it('encodeDocumentPackage: pdf throws UnsupportedPackageFormatError too', () => {
    const pkg = decodeOoxmlPackage(minimalDocxBytes());
    expect(() => encodeDocumentPackage('pdf', pkg)).toThrow(UnsupportedPackageFormatError);
  });
});
