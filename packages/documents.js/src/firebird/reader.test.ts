...
describe('FirebirdBackupReader: attribute framing', () => {
  it('reads a tag, a length-prefixed int32 attribute (little-endian), and detects end-of-stream', () => {
    ...
    const reader = new FirebirdBackupReader(new Uint8Array([5, 4, 0x2c, 0x01, 0x00, 0x00]));
  ...
  it('reads a negative int32 value correctly', () => {
    ...
    const reader = new FirebirdBackupReader(new Uint8Array([9, 4, 0xff, 0xff, 0xff, 0xff]));
    reader.readTag();
  ...
  it('reads a length-prefixed text attribute', () => {
    ...
    const reader = new FirebirdBackupReader(new Uint8Array([1, bytes.length, ...bytes]));
    reader.readTag();
  ...
  it('readCompressedPayload decodes a literal run (positive control byte)', () => {
    ...
    const reader = new FirebirdBackupReader(new Uint8Array([3, 0x41, 0x42, 0x43]));
    expect(Array.from(reader.readCompressedPayload(3))).toEqual([0x41, 0x42, 0x43]);
  });
  ...
  it('readCompressedPayload decodes a repeat run (negative control byte)', () => {
    ...
    expect(Array.from(reader.readCompressedPayload(4))).toEqual([0x5a, 0x5a, 0x5a, 0x5a]);
  });
  ...
  it('readCompressedPayload decodes a mix of literal and repeat runs across multiple control bytes', () => {
    ...
    const reader = new FirebirdBackupReader(new Uint8Array([2, 0x01, 0x02, 0xfd, 0x09]));
    expect(Array.from(reader.readCompressedPayload(5))).toEqual([0x01, 0x02, 0x09, 0x09, 0x09]);
  });
});
...
describe('XdrReader: per-type big-endian decoding', () => {
  ...
  it('readInt64 decodes a big-endian signed 64-bit value from two 32-bit words, high word first', () => {
    ...
    const xdr = new XdrReader(new Uint8Array([0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00]));
    expect(xdr.readInt64()).toBe(4294967296n);
  });

  it('readInt64 decodes a negative 64-bit value correctly', () => {
    // -1 as 64-bit: all bits set
    const xdr = new XdrReader(new Uint8Array([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]));
