...
import {
  BLOB_FIXTURE_FBK_BASE64,
  ORIGINAL_FIXTURE_FBK_BASE64,
  RICH_FIXTURE_FBK_BASE64,
} from "../test-support/firebird";
import {
...
describe("readFirebirdBackup: the rich fixture (four EMPLOYEES rows, three DEPARTMENTS rows, deliberate NULLs)", () => {
  ...
  it("decodes every EMPLOYEES row correctly, including NULLs, a negative DECIMAL, and an escaped apostrophe", () => {
    ...
    const byId = (id: number) => {
      const idIndex = employees.columns.findIndex(
      ...
      const cell = (columnName: string) =>
        row[
          employees.columns.findIndex((column) => column.name === columnName)
        ];
      return {
        salary: cell("SALARY"),
        bonus: cell("BONUS"),
        name: cell("NAME"),
        hireDate: cell("HIRE_DATE"),
        active: cell("ACTIVE"),
      };
    };
  ...
  it("decodes every DEPARTMENTS row correctly, including a NULL NUMERIC column", () => {
    ...
    const departments = tables.find(
      (table) => table.tableName === "DEPARTMENTS",
    );
    expect(departments).toBeDefined();
    if (departments === undefined) {
      return;
    }
    const deptIdIndex = departments.columns.findIndex(
      (column) => column.name === "DEPT_ID",
    );
    const nameIndex = departments.columns.findIndex(
      (column) => column.name === "DEPT_NAME",
    );
    const budgetIndex = departments.columns.findIndex(
      (column) => column.name === "BUDGET",
    );

    expect(departments.rows).toHaveLength(3);
    const byDeptId = (id: number) =>
      departments.rows.find((row) => {
        const idCell = row[deptIdIndex];
        return idCell?.kind === "number" && idCell.value === id;
      });

    const engineering = byDeptId(10);
    expect(engineering?.[nameIndex]).toEqual({
      kind: "string",
      value: "Engineering",
    });
    expect(engineering?.[budgetIndex]).toEqual({
      kind: "number",
      value: 500000,
    });
    ...
    expect(noBudget?.[nameIndex]).toEqual({
      kind: "string",
      value: "No Budget Dept",
    });
...
describe("readFirebirdBackup: format guards, never a silent wrong result", () => {
  ...
  it("throws FirebirdBackupFormatError for a backup format version this reader has not verified", () => {
    expect(() => readFirebirdBackup(minimalBurpStream(11))).toThrow(
      FirebirdBackupFormatError,
    );
    expect(() => readFirebirdBackup(minimalBurpStream(11))).toThrow(
      /format version 11 is not supported/,
    );
  });
...
describe("readFirebirdBackup: the blob fixture (a text blob, a binary blob, and NULL blobs)", () => {
  ...
  it("recovers a text blob's full content as an ordinary string", () => {
    ...
    const expected = "The quick brown fox jumps over the lazy dog. "
      .repeat(3)
      .trimEnd();
    expect(table.rows[0]?.[columnIndex("NOTES")]).toEqual({
      kind: "string",
      value: expected,
    });
    expect(table.rows[2]?.[columnIndex("NOTES")]).toEqual({
      kind: "string",
      value: "short",
    });
  });
