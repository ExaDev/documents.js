...
export class PptxShape {
  ...
  private live(): XmlElement {
    if (this.removed) {
      throw new Error(
        "this PptxShape has been removed from its slide and can no longer be used",
      );
    }
    return this.node;
  }
  ...
  private readInsetPt(
    ...
    return value === undefined
      ? undefined
      : emuToPt(Number.parseInt(value, 10));
  }
  ...
  set frame(value: Box) {
    ...
    xfrm.children = [
      el("a:off", {
        x: String(ptToEmu(value.xPt)),
        y: String(ptToEmu(value.yPt)),
      }),
      el("a:ext", {
        cx: String(ptToEmu(value.widthPt)),
  ...
  get rotationDeg(): number | undefined {
    ...
    return rot === undefined
      ? undefined
      : Number(rot) / ROTATION_UNITS_PER_DEGREE;
  }
  ...
  set text(value: string) {
    ...
    const paragraph = el("a:p", {}, [
      el("a:r", {}, [el("a:t", {}, [txt(encodeXmlText(value))])]),
    ]);
...
function buildDrawingRun(init: DrawingRunInit): XmlElement {
  ...
  if (init.color !== undefined) {
    rPrChildren.push(
      el("a:solidFill", {}, [
        el("a:srgbClr", { val: drawingMlColorHex(init.color) }),
      ]),
    );
  }
  ...
  const rPr =
    Object.keys(rPrAttrs).length > 0 || rPrChildren.length > 0
      ? [el("a:rPr", rPrAttrs, rPrChildren)]
      : [];
  const tAttrs: Record<string, string> = needsSpacePreserve(init.text)
    ? { "xml:space": "preserve" }
    : {};
  return el("a:r", {}, [
    ...rPr,
    el("a:t", tAttrs, [txt(encodeXmlText(init.text))]),
  ]);
}
  ...
  if (init.lineSpacing !== undefined) {
    pPrChildren.push(
      el("a:lnSpc", {}, [
        el("a:spcPct", {
          val: String(
            Math.round(init.lineSpacing * LINE_SPACING_PERCENT_PER_MULTIPLIER),
          ),
        }),
      ]),
    );
  ...
  if (init.spacingBeforePt !== undefined) {
    pPrChildren.push(
      el("a:spcBef", {}, [
        el("a:spcPts", {
          val: String(
            Math.round(init.spacingBeforePt * SPACING_POINTS_PER_POINT),
          ),
        }),
      ]),
    );
  ...
  if (init.spacingAfterPt !== undefined) {
    pPrChildren.push(
      el("a:spcAft", {}, [
        el("a:spcPts", {
          val: String(
            Math.round(init.spacingAfterPt * SPACING_POINTS_PER_POINT),
          ),
        }),
      ]),
    );
...
export function buildTextBoxShape(
  frame: Box,
  text: string,
  shapeId: number,
): XmlElement {
...
export function buildPictureShape(
  frame: Box,
