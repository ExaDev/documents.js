...
function presentationDoc(
  shapes: Extract<
    ContentDocument,
    { kind: "presentation" }
  >["slides"][number]["shapes"],
): ContentDocument {
  return {
    kind: "presentation",
    metadata: {},
    slides: [{ size: { widthPt: 960, heightPt: 540 }, notes: "", shapes }],
  };
}
...
describe("PptxShape.appendOfficeMath", () => {
  it("writes a real m:oMathPara/m:oMath equation into the shape's own text body", () => {
    ...
    expect(
      findDescendantElement(oMathPara!.node.children, "m:oMath"),
    ).toBeDefined();
    expect(
      findDescendantElement(oMathPara!.node.children, "m:f"),
    ).toBeDefined();
  });
...
describe("buildPptxPackage: an embedded formula block", () => {
  it("writes the block as a real m:oMathPara equation, not the plain-text stand-in", () => {
    ...
    expect(
      findDescendantElement(slideRoot.children, "m:oMathPara"),
    ).toBeDefined();
  ...
  it("reads straight back through readPptxContent as one formula-kind embedded object, on its own shape", () => {
    const recovered = readPptxContent(
      buildPptxPackage(presentationDoc(FORMULA_SHAPES)),
    );
  ...
  it("keeps a formula through repeated write/read cycles without accumulating spurious shapes", () => {
    ...
    for (let cycle = 0; cycle < 3; cycle++) {
      ...
      expect(
        next.slides[0]!.shapes[0]!.blocks.map((block) => block.kind),
      ).toEqual(["embeddedObject"]);
    }
  });
  ...
  it("reports an onMathDiagnostic for a construct src/omml/write.ts cannot translate", () => {
    ...
    const mspace: MathMlElement = {
      type: "element",
      tag: "mspace",
      attributes: [{ name: "width", value: "5pt" }],
      children: [],
    };
    const spaced: ContentFormula = {
      mathml: [mel("mrow", [mtoken("mi", "x"), mspace])],
    };
          ...
          blocks: [
            {
              kind: "embeddedObject",
              objectKind: "formula",
              document: formulaDocument(spaced),
              frame: FRAME,
            },
          ],
        },
