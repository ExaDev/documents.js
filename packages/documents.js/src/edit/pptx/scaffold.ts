...
export function buildEmptyGroupSpTree(): XmlElement {
  return el('p:spTree', {}, [el('p:nvGrpSpPr', {}, [el('p:cNvPr', { id: '1', name: '' }), el('p:cNvGrpSpPr'), el('p:nvPr')]), el('p:grpSpPr')]);
}
...
function buildFormatScheme(): XmlElement {
  const solidPhClr = (): XmlElement => el('a:solidFill', {}, [el('a:schemeClr', { val: 'phClr' })]);
  return el('a:fmtScheme', { name: 'Office' }, [
    el('a:fillStyleLst', {}, [solidPhClr(), solidPhClr(), solidPhClr()]),
    el('a:lnStyleLst', {}, [el('a:ln', {}, [solidPhClr()]), el('a:ln', {}, [solidPhClr()]), el('a:ln', {}, [solidPhClr()])]),
    el('a:effectStyleLst', {}, [el('a:effectStyle', {}, [el('a:effectLst')]), el('a:effectStyle', {}, [el('a:effectLst')]), el('a:effectStyle', {}, [el('a:effectLst')])]),
...
function buildTheme(): XmlElement {
  ...
  const fontScheme = el('a:fontScheme', { name: 'Office' }, [
    el('a:majorFont', {}, [el('a:latin', { typeface: 'Calibri Light' }), el('a:ea', { typeface: '' }), el('a:cs', { typeface: '' })]),
    el('a:minorFont', {}, [el('a:latin', { typeface: 'Calibri' }), el('a:ea', { typeface: '' }), el('a:cs', { typeface: '' })]),
  ]);
  return el('a:theme', { 'xmlns:a': DML_NS, name: 'Office Theme' }, [el('a:themeElements', {}, [clrScheme, fontScheme, buildFormatScheme()])]);
}
...
function buildSlideMaster(layoutRelationshipId: string): XmlElement {
  return el('p:sldMaster', { 'xmlns:p': PML_NS, 'xmlns:a': DML_NS, 'xmlns:r': R_NS }, [
    el('p:cSld', {}, [buildWhiteBackground(), buildEmptyGroupSpTree()]),
    buildIdentityColorMap(),
    el('p:sldLayoutIdLst', {}, [el('p:sldLayoutId', { id: String(MIN_MASTER_OR_LAYOUT_ID), 'r:id': layoutRelationshipId })]),
  ]);
}
...
function buildSlideLayout(): XmlElement {
  return el('p:sldLayout', { 'xmlns:p': PML_NS, 'xmlns:a': DML_NS, 'xmlns:r': R_NS, type: 'blank', preserve: '1' }, [
...
export function createEmptyPptxPackage(): Package {
  const contentTypes = el('Types', { xmlns: CONTENT_TYPES_NS }, [
    ...
    el('Override', { PartName: `/${PRESENTATION_PART_PATH}`, ContentType: 'application/vnd.openxmlformats-officedocument.presentationml.presentation.main+xml' }),
  ]);

  const rootRels = el('Relationships', { xmlns: RELS_NS }, [
    el('Relationship', { Id: 'rId1', Type: 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument', Target: PRESENTATION_PART_PATH }),
  ]);
  ...
  const pkg: Package = {
    parts: {
      ...
      [PRESENTATION_PART_PATH]: { kind: 'xml', nodes: [declaration(), el('p:presentation', { 'xmlns:p': PML_NS, 'xmlns:r': R_NS })] },
    },
  };

  const layoutToMasterTarget = buildRelativeTarget(SLIDE_LAYOUT_PART_PATH, SLIDE_MASTER_PART_PATH);
  addRelationship(pkg, SLIDE_LAYOUT_PART_PATH, { type: SLIDE_MASTER_REL_TYPE, target: layoutToMasterTarget });
  pkg.parts[SLIDE_LAYOUT_PART_PATH] = { kind: 'xml', nodes: [declaration(), buildSlideLayout()] };
  ...
  const masterToLayoutRelId = addRelationship(pkg, SLIDE_MASTER_PART_PATH, { type: SLIDE_LAYOUT_REL_TYPE, target: masterToLayoutTarget });
  const masterToThemeTarget = buildRelativeTarget(SLIDE_MASTER_PART_PATH, THEME_PART_PATH);
  addRelationship(pkg, SLIDE_MASTER_PART_PATH, { type: THEME_REL_TYPE, target: masterToThemeTarget });
  pkg.parts[SLIDE_MASTER_PART_PATH] = { kind: 'xml', nodes: [declaration(), buildSlideMaster(masterToLayoutRelId)] };
  ensureContentTypeOverride(pkg, SLIDE_MASTER_PART_PATH, SLIDE_MASTER_CONTENT_TYPE);

  pkg.parts[THEME_PART_PATH] = { kind: 'xml', nodes: [declaration(), buildTheme()] };
  ensureContentTypeOverride(pkg, THEME_PART_PATH, THEME_CONTENT_TYPE);

  const presentationToMasterTarget = buildRelativeTarget(PRESENTATION_PART_PATH, SLIDE_MASTER_PART_PATH);
  const presentationToMasterRelId = addRelationship(pkg, PRESENTATION_PART_PATH, { type: SLIDE_MASTER_REL_TYPE, target: presentationToMasterTarget });

  // p:sldMasterIdLst must precede p:sldIdLst in CT_Presentation's own element sequence (ECMA-376 Part 1, 13.2.4.1).
  const presentationPart = pkg.parts[PRESENTATION_PART_PATH];
  const presentationElement = presentationPart?.kind === 'xml' ? presentationPart.nodes.find((n): n is XmlElement => n.type === 'element') : undefined;
  if (presentationElement === undefined) {
    throw new Error('createEmptyPptxPackage: failed to build ppt/presentation.xml');
  }
