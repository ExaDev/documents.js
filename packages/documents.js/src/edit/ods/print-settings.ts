...
function hasManualBreak(pkg: Package, element: XmlElement, family: 'table-column' | 'table-row', propertiesTag: string): boolean {
  const styleName = attr(element, 'table:style-name');
  const styleElement = styleName === undefined ? undefined : findStyleElement(styleName, family, pkg);
  const properties = styleElement === undefined ? undefined : directChild(styleElement, propertiesTag);
  return properties !== undefined && attr(properties, 'fo:break-before') === 'page';
}
...
function scanTableStructure(pkg: Package, tableElement: XmlElement): TableStructure {
  let columnCursor = 0;
  let rowCursor = 0;
  ...
  function processColumn(columnElement: XmlElement): void {
    if (hasManualBreak(pkg, columnElement, 'table-column', 'style:table-column-properties')) {
  ...
  function processRow(rowElement: XmlElement): void {
    ...
    if (hasManualBreak(pkg, rowElement, 'table-row', 'style:table-row-properties')) {
...
export function readSheetPrintSettings(pkg: Package, tableElement: XmlElement): ContentSheetPrintSettings {
  ...
  const scaleToRaw = layoutProperties === undefined ? undefined : attr(layoutProperties, 'style:scale-to');
  const scalePercent = scaleToRaw === undefined ? undefined : parseScalePercentage(scaleToRaw);
  const fitWidth = parseNonNegativeInteger(layoutProperties === undefined ? undefined : attr(layoutProperties, 'style:scale-to-X'));
  const fitHeight = parseNonNegativeInteger(layoutProperties === undefined ? undefined : attr(layoutProperties, 'style:scale-to-Y'));
...
function collectRangeElements(children: XmlNode[], tag: string, repeatAttr: string, start: number, end: number): RangeElement[] {
  let cursor = 0;
...
export function writeSheetPrintSettings(pkg: Package, tableElement: XmlElement, settings: ContentSheetPrintSettings): void {
  ...
  stylesAutomaticStyles.children.push(
    el('style:page-layout', { 'style:name': pageLayoutName }, [
      el('style:page-layout-properties', {
        ...
        ...(settings.scalePercent !== undefined ? { 'style:scale-to': `${settings.scalePercent}%` } : {}),
        ...(settings.fitToPages !== undefined ? { 'style:scale-to-X': String(settings.fitToPages.width), 'style:scale-to-Y': String(settings.fitToPages.height) } : {}),
      }),
    ]),
  );
  ...
  if (settings.printRange !== undefined) {
    ...
    if (sheetName === undefined) {
      throw new Error('writeSheetPrintSettings: printRange requires tableElement to already carry a table:name');
    }
