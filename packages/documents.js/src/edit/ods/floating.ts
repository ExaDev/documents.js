import type { ContentEmbeddedObject, ContentSheetImage } from 'document-schema.js';
import type { Package, XmlElement } from 'odf.js';
import { base64ToBytes, findStyleElement, formatOdfLength, parseOdfLength } from 'odf.js';
...
import { COLUMN_REPEAT_ATTR, COLUMN_TAG, HEADER_COLUMNS_TAG, HEADER_ROWS_TAG, ROW_REPEAT_ATTR, ROW_TAG, collectRunMembers, isElementWithTag, readRunRepeatCount } from './address';
...
function ensureTableShapes(tableElement: XmlElement): XmlElement {
  const existing = tableElement.children.find((child): child is XmlElement => child.type === 'element' && child.tag === SHAPES_TAG);
...
function resolvedColumnWidthPt(pkg: Package, columnElement: XmlElement): number {
...
function cumulativeColumnOffsetPt(pkg: Package, tableElement: XmlElement, anchorColumn: number): number {
  let cursor = 0;
  let offsetPt = 0;
...
function cumulativeRowOffsetPt(pkg: Package, tableElement: XmlElement, anchorRow: number): number {
  let cursor = 0;
  let offsetPt = 0;
...
function buildAbsoluteFrame(xPt: number, yPt: number, widthPt: number, heightPt: number, content: XmlElement): XmlElement {
  return el(
    'draw:frame',
    { 'svg:x': formatOdfLength(xPt), 'svg:y': formatOdfLength(yPt), 'svg:width': formatOdfLength(widthPt), 'svg:height': formatOdfLength(heightPt) },
    [content],
  );
}
...
export function insertSheetImage(pkg: Package, tableElement: XmlElement, image: ContentSheetImage): void {
  const { partPath } = addImageMedia(pkg, base64ToBytes(image.base64), image.format);
...
export function insertSheetEmbeddedObject(pkg: Package, tableElement: XmlElement, object: ContentEmbeddedObject): void {
