...
export class OdsCell {
  ...
  get value(): ContentCellValue {
    ...
    switch (valueType) {
      ...
      case "date":
        return {
          kind: "date",
          value: attr(this.node, DATE_VALUE_ATTR) ?? displayText,
        };
      case "time":
        return {
          kind: "time",
          value: attr(this.node, TIME_VALUE_ATTR) ?? displayText,
        };
      case "string":
        return {
          kind: "string",
          value: attr(this.node, STRING_VALUE_ATTR) ?? displayText,
        };
      default:
        return { kind: "empty" };
    }
  ...
  get displayText(): string {
    const paragraphs = this.node.children.filter(
      (child): child is XmlElement =>
        child.type === "element" && child.tag === "text:p",
    );
    return paragraphs
      .map((paragraph) => decodeOdfText(paragraph.children))
      .join("\n");
  }
