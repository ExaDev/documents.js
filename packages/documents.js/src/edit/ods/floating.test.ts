...
// insertSheetImage/insertSheetEmbeddedObject (floating.ts) write real ODF spreadsheet floating shapes -- a table:shapes wrapper directly inside table:table, holding draw:frame elements with absolute svg:x/svg:y/svg:width/svg:height. odf.js's own readOds does not read ContentSheetImage/embeddedObjects back at all (images is hardcoded [], embeddedObjects is never set -- see src/layout/sheets.ts's own gotcha comment), so every assertion here inspects the real written package/XML structure directly (via odf.js's own query/manifest/formula-reading primitives, and a genuine zip encode/decode round trip) rather than through a ContentDocument re-read this package's own upstream dependency cannot yet perform.
...
describe('OdsSheet.addImage', () => {
  function baseImage(overrides: Partial<ContentSheetImage> = {}): ContentSheetImage {
    return { kind: 'image', format: 'png', base64: bytesToBase64(PNG_BYTES), widthPt: 40, heightPt: 30, anchorRow: 0, anchorColumn: 0, offsetXPt: 0, offsetYPt: 0, ...overrides };
  }
  ...
  it('writes a table:shapes wrapper as table:table\'s own first child, holding a draw:frame/draw:image at the anchor\'s resolved position', () => {
    ...
    expect(table.children[0]?.type === 'element' ? table.children[0].tag : undefined).toBe('table:shapes');
  ...
  it('anchors past a real, explicitly-sized column/row at the correct cumulative offset', () => {
    ...
    sheet.addImage(baseImage({ anchorRow: 1, anchorColumn: 1, offsetXPt: 5, offsetYPt: 3 }));
  ...
  it('adds the binary image part under Pictures/ with the exact source bytes, and lists it in the manifest', () => {
    ...
    expect(part?.kind === 'binary' ? part.base64 : undefined).toBe(bytesToBase64(PNG_BYTES));
    expect(readManifest(pkg).entries.some((entry) => entry.fullPath === 'Pictures/image1.png')).toBe(true);
  });
  ...
  it('a second image gets its own picture index rather than colliding with the first', () => {
    ...
    const pkg = editor.toPackage();
    expect(Object.keys(pkg.parts)).toEqual(expect.arrayContaining(['Pictures/image1.png', 'Pictures/image2.png']));
...
describe('OdsSheet.addEmbeddedObject', () => {
  const FRACTION = formulaDocument({ mathml: [mel('mfrac', [mtoken('mi', 'a'), mtoken('mi', 'b')])] });

  function formulaObject(overrides: Partial<ContentEmbeddedObject> = {}): ContentEmbeddedObject {
