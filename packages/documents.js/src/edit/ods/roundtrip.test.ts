...
describe("open -> edit -> save -> reopen: every ContentCellValueSchema variant survives a real readOdsContent cycle", () => {
  it("number/percentage/currency/boolean/date/time/string/empty all round-trip to their correct kind and value", () => {
    ...
    const rereadFromBytes = readOdsContent(
      openOds(editor.toBytes()).toPackage(),
    ); // and independently from a fresh byte round trip, to catch anything that only breaks through actual (de)serialization
    ...
    for (const document of [reread, rereadFromBytes]) {
      ...
      expect(byPosition.get("10,0")?.value).toEqual({
        kind: "number",
        value: 123.456,
      });
      expect(byPosition.get("10,1")?.value).toEqual({
        kind: "percentage",
        value: 0.75,
      });
      expect(byPosition.get("10,2")?.value).toEqual({
        kind: "currency",
        value: 5,
        currency: "EUR",
      });
      expect(byPosition.get("10,3")?.value).toEqual({
        kind: "boolean",
        value: true,
      });
      expect(byPosition.get("10,4")?.value).toEqual({
        kind: "boolean",
        value: false,
      });
      expect(byPosition.get("10,5")?.value).toEqual({
        kind: "date",
        value: "2026-12-25",
      });
      expect(byPosition.get("10,6")?.value).toEqual({
        kind: "time",
        value: "PT09H30M00S",
      });
      expect(byPosition.get("10,7")?.value).toEqual({
        kind: "string",
        value: "plain text",
      });
      expect(byPosition.get("10,8")?.value).toEqual({ kind: "empty" });
      expect(byPosition.get("10,8")?.displayText).toBe(
        "still visible even though empty",
      );

      expect(byPosition.get("11,0")?.formula).toBe("of:=SUM([.A11:.C11])");
      expect(byPosition.get("11,0")?.value).toEqual({
        kind: "number",
        value: 128.206,
      });
    }
  ...
  it("a merged range survives: the anchor carries its span, the covered position is entirely absent from cells[]", () => {
    ...
    const document = readOdsContent(openOds(editor.toBytes()).toPackage());
    const dataSheet = document.sheets.find((s) => s.name === "Data")!;
    const anchorCell = dataSheet.cells.find(
      (c) => c.row === 15 && c.column === 0,
    );
    expect(anchorCell?.value).toEqual({
      kind: "string",
      value: "Merged Header",
    });
    expect(anchorCell?.rowSpan).toBe(2);
    expect(anchorCell?.colSpan).toBe(3);
    expect(dataSheet.cells.some((c) => c.row === 15 && c.column === 1)).toBe(
      false,
    );
    expect(dataSheet.cells.some((c) => c.row === 16 && c.column === 2)).toBe(
      false,
    );
  });
});
...
describe("write-side repeat-count avoidance: a far-out cell address never materializes the skipped range", () => {
  it("setting row 500, column 50 on a fresh sheet produces a bounded, small number of XML elements -- never 500 x 50 cell objects", () => {
    ...
    expect(
      rowElements[0]?.attributes.find((a) => a.name === ROW_REPEAT_ATTR)?.value,
    ).toBe("500");
    expect(
      rowElements[1]?.attributes.some((a) => a.name === ROW_REPEAT_ATTR),
    ).toBe(false);

    const targetRow = rowElements[1]!;
    const cellElements = targetRow.children.filter(
      (c): c is XmlElement => c.type === "element",
    );
    expect(cellElements).toHaveLength(2);
    expect(
      cellElements[0]?.attributes.find((a) => a.name === COLUMN_REPEAT_ATTR)
        ?.value,
    ).toBe("50");
  ...
  it("and reading that same file back through odf.js's own readOdsContent is fast -- proof the file itself, not just this editor's in-memory view, stayed compact", () => {
    ...
    expect(sheetOut.cells[0]).toMatchObject({
      row: 50000,
      column: 500,
      value: { kind: "string", value: "far away" },
    });
  });
