...
describe('createEmptyDocxPackage', () => {
  ...
  it('has a w:body with a default section', () => {
    ...
    const body = root?.children.find((c) => c.type === 'element' && c.tag === 'w:body');
    expect(body).toBeDefined();
    const sectPr = body?.type === 'element' ? body.children.find((c) => c.type === 'element' && c.tag === 'w:sectPr') : undefined;
  ...
  it('has a styles part with a default Normal style', () => {
    ...
    const normalStyle = root?.children.find(
      (c) => c.type === 'element' && c.tag === 'w:style' && c.attributes.some((a) => a.name === 'w:default' && a.value === '1'),
    );
