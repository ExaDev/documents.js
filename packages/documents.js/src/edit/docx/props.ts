...
export function setFontFamily(rPr: XmlElement, family: string): void {
  const rFonts = getOrCreateChildElement(rPr, "w:rFonts", RPR_ORDER, () =>
    el("w:rFonts"),
  );
...
export function getSizePt(rPr: XmlElement | undefined): number | undefined {
  ...
  return val === undefined
    ? undefined
    : halfPointsToPt(Number.parseInt(val, 10));
}
...
export function setSizePt(rPr: XmlElement, sizePt: number): void {
  ...
  const szCs = getOrCreateChildElement(rPr, "w:szCs", RPR_ORDER, () =>
    el("w:szCs"),
  );
...
export function setColor(rPr: XmlElement, color: LayoutColor): void {
  const element = getOrCreateChildElement(rPr, "w:color", RPR_ORDER, () =>
    el("w:color"),
  );
...
export function setStyleId(
  ...
  const element = getOrCreateChildElement(propsElement, tag, order, () =>
    el(tag),
  );
