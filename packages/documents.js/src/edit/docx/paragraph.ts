...
export class DocxParagraph {
  ...
  constructor(
    container: XmlNode[],
    node: XmlElement,
    imageContext?: ImageMediaContext,
    pkg?: Package,
  ) {
  ...
  private live(): XmlElement {
    if (this.removed) {
      throw new Error(
        "this DocxParagraph has been removed from its body and can no longer be used",
      );
    }
    return this.node;
  }
  ...
  private pPr(create: boolean): XmlElement | undefined {
    ...
    return create
      ? ensureFirstChild(node, "w:pPr")
      : directChild(node, "w:pPr");
  }
  ...
  insertRunAt(index: number, init?: RunInit): DocxRun {
    ...
    const insertAt =
      index < runIndices.length
        ? (runIndices[index] ?? node.children.length)
  ...
  set styleId(value: string | undefined) {
    if (value === undefined) {
      ...
      const existing =
        pPr === undefined ? undefined : directChild(pPr, "w:pStyle");
  ...
  private spacingElement(create: boolean): XmlElement | undefined {
    ...
    return create
      ? getOrCreateChildElement(pPr, "w:spacing", PPR_ORDER, () =>
          el("w:spacing"),
        )
      : directChild(pPr, "w:spacing");
  }

  get spacingBeforePt(): number | undefined {
    const spacing = this.spacingElement(false);
    const before =
      spacing === undefined ? undefined : attr(spacing, "w:before");
  ...
  private indElement(create: boolean): XmlElement | undefined {
    ...
    return create
      ? getOrCreateChildElement(pPr, "w:ind", PPR_ORDER, () => el("w:ind"))
      : directChild(pPr, "w:ind");
  }
  ...
  get list(): ContentListMembership | undefined {
    ...
    const numId =
      numIdElement === undefined ? undefined : attr(numIdElement, "w:val");
    if (numId === undefined) {
      return undefined;
    }
    const ilvlElement = directChild(numPr, "w:ilvl");
    const ilvl =
      ilvlElement === undefined ? undefined : attr(ilvlElement, "w:val");
  ...
  set list(value: ContentListMembership | undefined) {
    ...
    numPr.children = [
      el("w:ilvl", { "w:val": String(value.level) }),
      el("w:numId", { "w:val": value.numId }),
    ];
  }
  ...
  get headingLevel(): number | undefined {
    const pPr = this.pPr(false);
    const outlineLvl =
      pPr === undefined ? undefined : directChild(pPr, "w:outlineLvl");
    const val =
  ...
  set headingLevel(value: number | undefined) {
    ...
    const outlineLvl = getOrCreateChildElement(
      this.pPr(true),
      "w:outlineLvl",
      PPR_ORDER,
      () => el("w:outlineLvl"),
  ...
  appendVectorAnchors(vectors: readonly ContentVector[]): void {
    ...
    if (this.imageContext === undefined) {
      throw new Error(
        "appendVectorAnchors requires a paragraph opened through a DocxEditor",
      );
    }
    ...
    vectors.forEach((vector, index) => {
      node.children.push(
        el("w:r", {}, [
          buildAnchoredVectorDrawing(
            vector,
            nextDrawingId(documentRoot),
            index,
          ),
        ]),
      );
  ...
  insertImageAfter(image: ImageInit): void {
    ...
    if (this.imageContext === undefined) {
      throw new Error(
        "insertImageAfter requires a paragraph opened through a DocxEditor",
      );
    }
    const drawing = insertImageMedia(
      this.imageContext.media,
      this.imageContext.documentRoot,
      image,
    );
  ...
  wrapLastRunInHyperlink(url: string): void {
    ...
    node.children[lastChildIndex] = el("w:hyperlink", { "r:id": rId }, [
      lastChild,
    ]);
  }
...
export function buildParagraph(init: ParagraphInit = {}): XmlElement {
  ...
  if (
    init.styleId !== undefined ||
    init.alignment !== undefined ||
    init.headingLevel !== undefined
  ) {
    const pPr = el("w:pPr");
    ...
    if (init.alignment !== undefined) {
      pPr.children.push(
        el("w:jc", {
          "w:val": init.alignment === "justify" ? "both" : init.alignment,
        }),
      );
    }
    ...
    if (init.headingLevel !== undefined) {
      pPr.children.push(
        el("w:outlineLvl", { "w:val": String(init.headingLevel - 1) }),
      );
    }
