...
describe('DocxParagraph.insertImageAfter', () => {
  it('adds the media part, content-type entry, relationship, and a w:drawing referencing it', () => {
    ...
    paragraph.insertImageAfter({ format: 'png', bytes: PNG_BYTES, widthPt: 100, heightPt: 50 });
    ...
    const imageRel = [...rels.values()].find((r) => r.target === 'word/media/image1.png');
  ...
  it('round-trips through encodePackage/decodePackage after an image is added', () => {
    const editor = createDocx();
    editor.body.appendParagraph().insertImageAfter({ format: 'png', bytes: PNG_BYTES, widthPt: 10, heightPt: 10 });
