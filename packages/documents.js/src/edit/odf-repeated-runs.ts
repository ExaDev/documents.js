...
// ODF compresses a run of adjacent, identically-formatted table:table-row or table:table-cell/table:covered-table-cell elements into ONE XML element carrying table:number-rows-repeated/table:number-columns-repeated, standing in for that many logically-identical positions -- confirmed not just a spreadsheet-scale hazard (odf.js's own typed/ods/read.ts top-of-file note) but something real LibreOffice writes routinely for a short run of identically-styled columns or empty cells in an ordinary word-processing table too, which is what ExaDev/documents.js#1374 is about. This module holds the run-individuation algorithm generic across BOTH axes (rows-within-a-table, cells-within-a-row) and BOTH ODF table users (odt's own OdtTable/OdtTableRow, in ../odt/table.ts, and ods's own OdsSheet, via ./ods/address.ts, which imports every symbol it still needs directly from here rather than re-exporting them -- this repo's own barrel-policy lint rule forbids re-exporting a value through a non-barrel file) -- the algorithm doesn't care whether it is rows-in-a-table or cells-in-a-row, only the tag/repeat-attribute names differ, so replaceRun below is written once and shared rather than duplicated per format.
//
// table:table-header-rows/table:table-header-columns (ods's own print-settings.ts) nest a contiguous run of real row/column elements one level inside a wrapper element, rather than leaving them as direct children -- collectRunMembers's own optional headerWrapperTag parameter exists for that ODS-specific concern; an odt table never uses it (there is no odt equivalent this editor reads or writes), so every odt call passes undefined and gets this module's original flat, single-level behaviour exactly.
...
export function validateIndex(value: number, label: string): void {
  if (!Number.isInteger(value) || value < 0) {
    throw new Error(`${label} must be a non-negative integer, got ${value}`);
  }
}

// The repeat count a run of `node` (and every position it stands in for) represents -- 1 when the attribute is absent or unparseable, mirroring odf.js's own private readRepeatCount in typed/ods/read.ts and typed/shared/table.ts exactly (a malformed count degrades to "just this one element", never a crash and never a silent zero-width run).
...
// Collects every `isMember` match among `children`, plus (when `headerWrapperTag` is given) every `isMember` match nested one level inside a `headerWrapperTag` element among `children` -- in overall document order, which is what every cursor-based position calculation in this module and its ODS/odt callers assumes matches logical row/column index order (exactly the assumption odf.js's own readTable/readOdfTable makes reading the same structure back).
...
// A structural clone of `node` (structuredClone -- node is plain, serializable XmlElement data, no methods or non-cloneable values) with `attrName` set to `count`, or removed entirely when count is 1 -- an un-repeated element always OMITS the repeat attribute rather than writing an explicit "1", matching every real ODF producer and this module's own readRunRepeatCount above (which treats "absent" and "1" identically, but only ever emitting "absent" for count 1 keeps our own output indistinguishable from a genuine producer's). Exported for ods/address.ts's own ensureColumnCoverage, which mints a brand-new repeated column run directly rather than individuating an existing one.
...
// 3. targetIndex falls beyond every existing run's own coverage -- append ONE placeholder run (built via `buildEmpty`, repeat count = however many positions are being skipped) covering the gap, immediately followed by the freshly individuated target (also from `buildEmpty`, no repeat attribute). A caller that has already bounds-checked targetIndex against its own total coverage (odt's own row/cell addressing, which fails loudly on an out-of-range column rather than growing the row) never reaches this branch in practice; ods's own resolveCellNode relies on it deliberately, to gap-fill a sparse spreadsheet without ever materializing one element per skipped position.
