...
function pngBytes(rgb: readonly [number, number, number]): Uint8Array<ArrayBuffer> {
  const [r, g, b] = rgb;
  return encodePng({ width: 2, height: 2, channels: 3, data: new Uint8Array([r, g, b, r, g, b, r, g, b, r, g, b]) });
}
...
describe('openPdf: editing one item on one page leaves every other item, and every other page, untouched', () => {
  it('editing item 1 on page 1 leaves item 0 on page 1 and every item on page 0 semantically unchanged', () => {
    ...
    expect(pageOne.textItems().map((item) => item.text)).toEqual(['First', 'Second']);
    ...
    expect(reparsedPageZero.items.map((item) => (item.kind === 'text' ? item.text : item.kind))).toEqual(['PageZero']);
    expect(reparsedPageOne.items.map((item) => (item.kind === 'text' ? item.text : item.kind))).toEqual(['First', 'Edited']);
...
describe('insertion at an explicit position, verified through a real toBytes()/reopen round trip', () => {
  it('a new rect inserted at index 0 appears first, and nothing after it shifted', () => {
    ...
    const page = editor.pages()[0]!;
    page.appendText({ xPt: 1, yPt: 1, text: 'A', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });
    page.appendText({ xPt: 2, yPt: 2, text: 'B', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });

    page.insertRectAt(0, { xPt: 0, yPt: 0, widthPt: 5, heightPt: 5, fill: RED });

    const reopened = openPdf(editor.toBytes());
    const reopenedPage = reopened.pages()[0]!;
    expect(reopenedPage.items().map((item) => item.kind)).toEqual(['rect', 'text', 'text']);
    expect(reopenedPage.textItems().map((item) => item.text)).toEqual(['A', 'B']);
...
describe('deletion, verified through a real toBytes()/reopen round trip', () => {
  it('a removed item is gone from items() immediately, and from the reparsed document, with survivors untouched', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    page.appendText({ xPt: 1, yPt: 1, text: 'Keep', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });
    const doomed = page.appendRect({ xPt: 0, yPt: 0, widthPt: 10, heightPt: 10, fill: RED });
    page.appendText({ xPt: 2, yPt: 2, text: 'Also keep', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });
    ...
    expect(reopenedPage.textItems().map((item) => item.text)).toEqual(['Keep', 'Also keep']);
    expect(reopenedPage.textItems()[0]?.xPt).toBe(1);
    expect(reopenedPage.textItems()[1]?.xPt).toBe(2);
  });
});
...
describe('setImage: swapped pixel bytes survive a real write/reopen, and the orphaned original is excluded from output', () => {
  it('the new image bytes decode back correctly, and the old imageId never reaches the written PDF at all', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    const image = page.appendImage({ xPt: 0, yPt: 0, widthPt: 40, heightPt: 40, bytes: pngBytes([255, 0, 0]), format: 'png' });
...
describe('resource allocation over a hand-assembled, multi-page, multi-font, multi-image document', () => {
  it('writePdf allocates distinct font/image resources correctly for a document built incrementally rather than by a layout engine in one pass', () => {
    ...
    pageOne.appendText({ xPt: 10, yPt: 10, text: 'Plain', font: DEFAULT_LAYOUT_FONT, sizePt: 12, color: BLACK });
    pageOne.appendText({ xPt: 10, yPt: 30, text: 'Bold Italic', font: { family: 'Helvetica', weight: 'bold', style: 'italic' }, sizePt: 12, color: BLUE });
    pageOne.appendImage({ xPt: 10, yPt: 50, widthPt: 20, heightPt: 20, bytes: pngBytes([255, 0, 0]), format: 'png' });

    const pageTwo = editor.appendPage();
    pageTwo.appendText({ xPt: 10, yPt: 10, text: 'Second page text', font: { family: 'Helvetica', weight: 'bold', style: 'italic' }, sizePt: 14, color: RED });
    pageTwo.appendImage({ xPt: 10, yPt: 50, widthPt: 20, heightPt: 20, bytes: pngBytes([0, 0, 255]), format: 'png' });

    const reopened = readPdf(editor.toBytes());
    expect(reopened.pages).toHaveLength(2);

    const firstPageTexts = reopened.pages[0]!.items.filter((item) => item.kind === 'text');
    expect(firstPageTexts.map((item) => item.text)).toEqual(['Plain', 'Bold Italic']);
    ...
    const secondPageText = reopened.pages[1]!.items.find((item) => item.kind === 'text');
    ...
    const firstImage = reopened.pages[0]!.items.find((item) => item.kind === 'image');
    const secondImage = reopened.pages[1]!.items.find((item) => item.kind === 'image');
...
describe('full mixed-edit round trip: open, mutate several kinds across several pages including deletions, save, reopen', () => {
  it('every mutation persists and every removed item is genuinely gone', () => {
    ...
    pageOne.appendRect({ xPt: 5, yPt: 5, widthPt: 15, heightPt: 15, fill: RED });

    // Append a brand new third page with an image and a link.
    const pageTwo = editor.appendPage({ widthPt: 250, heightPt: 150 });
    pageTwo.appendImage({ xPt: 0, yPt: 0, widthPt: 10, heightPt: 10, bytes: pngBytes([10, 20, 30]), format: 'png' });
    pageTwo.appendLink({ uri: 'https://example.com', xPt: 0, yPt: 0, widthPt: 40, heightPt: 20 });
    ...
    expect(reopenedPageZero.textItems().map((item) => item.text)).toEqual(['PageZeroEdited']);
...
describe('PdfEditor.pages / page / appendPage / insertPageAt', () => {
  ...
  it('insertPageAt places a new page at the given position without disturbing the others', () => {
    const editor = createPdf();
    editor.pages()[0]!.appendText({ xPt: 0, yPt: 0, text: 'First', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });
    editor.appendPage().appendText({ xPt: 0, yPt: 0, text: 'Third', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });

    editor.insertPageAt(1).appendText({ xPt: 0, yPt: 0, text: 'Second', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });

    expect(editor.pages().map((page) => page.textItems()[0]?.text)).toEqual(['First', 'Second', 'Third']);
  });
});
...
describe('toLayoutDocument / toBytes', () => {
  it('toLayoutDocument returns the live document itself, not a snapshot', () => {
    ...
    editor.pages()[0]!.appendText({ xPt: 0, yPt: 0, text: 'X', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });
  ...
  it('toBytes runs writePdf fresh every call, reflecting whatever has changed since the previous call', () => {
    const editor = createPdf();
    const page = editor.pages()[0]!;
    const item = page.appendText({ xPt: 0, yPt: 0, text: 'Before', font: DEFAULT_LAYOUT_FONT, sizePt: 10, color: BLACK });
