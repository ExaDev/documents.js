...
describe('openMarkdown / toMarkdownText round trip', () => {
  ...
  it('round-trips headings, bold/italic/strike, a hyperlink, a bullet list, and a table', () => {
    ...
    expect(runs.find((run) => run.hyperlink !== undefined)?.hyperlink).toBe('https://example.com/page');

    const listItems = editor.paragraphs().filter((paragraph) => paragraph.list !== undefined);
    expect(listItems.map((paragraph) => paragraph.text)).toEqual(['First item', 'Second item']);
    ...
    expect(reopenedRuns.find((run) => run.hyperlink !== undefined)?.hyperlink).toBe('https://example.com/page');
    const reopenedListItems = reopened.paragraphs().filter((paragraph) => paragraph.list !== undefined);
    expect(reopenedListItems.map((paragraph) => paragraph.text)).toEqual(['First item', 'Second item']);
    const [reopenedTable] = reopened.tables();
    expect(reopenedTable?.rows().map((row) => row.cells().map((cell) => cell.text))).toEqual([
      ['A', 'B'],
      ['A1', 'B1'],
    ]);
  });
  ...
  it('produces the documented degradation for a heading level beyond the ATX/setext six-level ceiling', () => {
    ...
    const output = editor.toMarkdownText({ sink: (diagnostic) => diagnosticCodes.push(diagnostic.code) });
    expect(output).toContain('###### Title');
    expect(output).not.toContain('####### Title');
    expect(diagnosticCodes).toContain(MarkdownDiagnosticCodes.HEADING_LEVEL_CLAMPED);
  });
});
...
describe('open -> edit -> toMarkdownText -> reopen', () => {
  it('preserves every paragraph/run/table/list-item edit made through the live editor', () => {
    ...
    const emphasised = intro.appendRun({ text: 'important', bold: true, italic: true });
    intro.appendRun({ text: ' see ' });
    const link = intro.appendRun({ text: 'docs', hyperlink: 'https://example.com/' });
    ...
    const reopenedIntro = reopenedParagraphs.find((paragraph) => paragraph.text.startsWith('Intro'));
    ...
    const reopenedListItems = reopenedParagraphs.filter((paragraph) => paragraph.list !== undefined);
    expect(reopenedListItems.map((paragraph) => paragraph.text)).toEqual(['First', 'Second']);
  });
});
