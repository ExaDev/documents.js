...
function graphicPropertiesFor(pkg: Package, styleName: string): XmlElement | undefined {
  const part = pkg.parts['content.xml'];
  const root = part?.kind === 'xml' ? part.nodes.find((n): n is XmlElement => n.type === 'element') : undefined;
  const automaticStyles = root?.children.find((c): c is XmlElement => c.type === 'element' && c.tag === 'office:automatic-styles');
  const style = automaticStyles?.children.find((c): c is XmlElement => c.type === 'element' && c.tag === 'style:style' && attr(c, 'style:name') === styleName);
...
describe('buildGraphicStyle: draw:fill is always explicit, never inferred', () => {
  it('writes draw:fill="solid" alongside draw:fill-color when a fill colour is set', () => {
    ...
    expect(props === undefined ? undefined : attr(props, 'draw:fill')).toBe('solid');
    expect(props === undefined ? undefined : attr(props, 'draw:fill-color')).toBe('#ff0000');
  });

  it('writes draw:fill="none" and no draw:fill-color at all when no fill is set', () => {
    const pkg = createEmptyOdgPackage();
    const name = buildGraphicStyle(pkg, { stroke: { color: BLUE, widthPt: 1 } });
    const props = graphicPropertiesFor(pkg, name);
    expect(props === undefined ? undefined : attr(props, 'draw:fill')).toBe('none');
    expect(props === undefined ? undefined : attr(props, 'draw:fill-color')).toBeUndefined();
  });
});
