...
import { createEmptyOdgPackage, MASTER_PAGE_NAME, PAGE_LAYOUT_NAME } from './scaffold';
...
function findOfficeDrawing(pkg: Package): XmlElement {
  ...
  const drawing = body === undefined ? undefined : directChild(body, 'office:drawing');
  if (drawing === undefined) {
    throw new Error(`${CONTENT_PART_PATH} has no office:body/office:drawing element`);
  }
  return drawing;
}
...
function findPageLayoutProperties(pkg: Package): XmlElement {
  ...
  const pageLayout = automaticStyles?.children.find(
    (c): c is XmlElement => c.type === 'element' && c.tag === 'style:page-layout' && attr(c, 'style:name') === PAGE_LAYOUT_NAME,
  );
  const properties = pageLayout === undefined ? undefined : directChild(pageLayout, 'style:page-layout-properties');
  if (properties === undefined) {
    throw new Error(`${STYLES_PART_PATH} has no style:page-layout[style:name="${PAGE_LAYOUT_NAME}"]/style:page-layout-properties element`);
  }
  return properties;
}
...
export class OdgEditor {
  ...
  removePageAt(index: number): void {
    const drawing = findOfficeDrawing(this.pkg);
    const pageElements = drawing.children.filter((c): c is XmlElement => c.type === 'element' && c.tag === 'draw:page');
  ...
  get pageSize(): PageSize {
    ...
    const heightPt = heightValue === undefined ? undefined : parseOdfLength(heightValue);
...
export function createOdg(): OdgEditor {
