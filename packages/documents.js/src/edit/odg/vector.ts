...
// Live-view classes over odg's own vector-primitive elements -- draw:rect/draw:ellipse/draw:line/draw:path, the geometry a drawing carries that a presentation typically doesn't (see odf.js's own typed/draw/shapes.ts top-of-file note, which this module's builders are the write-side counterpart to). Unlike OdpShape's draw:frame, NONE of these ever carry a draw:transform/rotation -- ContentVectorSchema's own rect/ellipse/path variants have no rotationDeg field at all (document-content-model's content.ts; see shapes.ts's own resolveVectorFrame comment for why the reader discards it), so frame get/set here is always plain svg:x/y/width/height, never draw:transform.
//
