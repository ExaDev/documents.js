...
describe('insertImageFrameMedia', () => {
  it('adds a Pictures/ media part and returns a draw:frame referencing it at the given frame', () => {
    ...
    const frameElement = insertImageFrameMedia({ pkg }, frame, { format: 'png', bytes: PNG_BYTES });
    ...
    expect(image?.type === 'element' ? image.attributes : undefined).toContainEqual({ name: 'xlink:href', value: mediaParts[0] });
  });

  it('adding two images allocates distinct Pictures/ part paths', () => {
    const pkg = createEmptyOdpPackage();
    insertImageFrameMedia({ pkg }, { xPt: 0, yPt: 0, widthPt: 10, heightPt: 10 }, { format: 'png', bytes: PNG_BYTES });
    insertImageFrameMedia({ pkg }, { xPt: 0, yPt: 0, widthPt: 10, heightPt: 10 }, { format: 'png', bytes: PNG_BYTES });
    const mediaParts = Object.keys(pkg.parts).filter((p) => p.startsWith('Pictures/'));
