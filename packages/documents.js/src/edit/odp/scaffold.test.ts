...
describe('createEmptyOdpPackage', () => {
  ...
  it('has an office:body/office:presentation element in content.xml, with no slides yet', () => {
    ...
    const presentation = body?.type === 'element' ? body.children.find((c) => c.type === 'element' && c.tag === 'office:presentation') : undefined;
    expect(presentation).toBeDefined();
    expect(presentation?.type === 'element' ? presentation.children : undefined).toEqual([]);
  });
  ...
  it('defines the Heading_20_N paragraph styles in styles.xml office:styles, so a text-box heading\'s style reference resolves', () => {
    ...
    const officeStyles = root?.children.find((c) => c.type === 'element' && c.tag === 'office:styles');
    const heading2 = officeStyles?.type === 'element'
      ? officeStyles.children.find((c) => c.type === 'element' && c.tag === 'style:style' && c.attributes.some((a) => a.name === 'style:name' && a.value === 'Heading_20_2'))
      : undefined;
    expect(heading2).toBeDefined();
    const textProperties = heading2?.type === 'element' ? heading2.children.find((c) => c.type === 'element' && c.tag === 'style:text-properties') : undefined;
    expect(textProperties?.type === 'element' ? textProperties.attributes : undefined).toContainEqual({ name: 'fo:font-size', value: '22pt' });
    expect(textProperties?.type === 'element' ? textProperties.attributes : undefined).toContainEqual({ name: 'fo:font-weight', value: 'bold' });
  });
  ...
  it('has a page-layout -> master-page chain in styles.xml, declaring the standard 16:9 widescreen size', () => {
    ...
    const properties = pageLayout?.type === 'element' ? pageLayout.children.find((c) => c.type === 'element' && c.tag === 'style:page-layout-properties') : undefined;
    expect(properties?.type === 'element' ? properties.attributes : undefined).toContainEqual({ name: 'fo:page-width', value: '960pt' });
    expect(properties?.type === 'element' ? properties.attributes : undefined).toContainEqual({ name: 'fo:page-height', value: '540pt' });
