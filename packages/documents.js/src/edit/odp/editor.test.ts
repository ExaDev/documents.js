...
describe("OdpEditor.addSlide / removeSlideAt / moveSlide", () => {
  ...
  it("slides are ordered by document position, matching odf.js's own draw:page order (no id indirection to resolve)", () => {
    ...
    expect(editor.slides().map((s) => s.shapes()[0]?.text)).toEqual([
      "First",
      "Second",
      "Third",
    ]);
...
describe("live-view fidelity for odp", () => {
  it("styling a newly appended run leaves every existing automatic style entry untouched", () => {
    ...
    if (paragraph === undefined) {
      throw new Error(
        "expected the Title shape to have at least one paragraph",
      );
    }
...
describe("full editor round trip: build a presentation from scratch, save, reread via odf.js's own readOdpContent", () => {
  it("a new slide, a styled multi-paragraph text box, an image, a rotated shape, and speaker notes all survive, as read by odf.js itself", () => {
    ...
    const textShape = slide.addTextBox({
      frame: { xPt: 20, yPt: 20, widthPt: 400, heightPt: 100 },
      text: "",
    });
    ...
    const imageShape = slide.addImage({
      frame: { xPt: 20, yPt: 200, widthPt: 60, heightPt: 60 },
      format: "png",
      bytes: PNG_BYTES,
    });
    expect(imageShape.frame).toEqual({
      xPt: 20,
      yPt: 200,
      widthPt: 60,
      heightPt: 60,
    ...
    const rotatedShape = slide.addTextBox({
      frame: { xPt: 300, yPt: 300, widthPt: 100, heightPt: 40 },
      text: "Rotated",
    });
    ...
    expect(reopenedSlide.notes).toBe(
      "These are the freshly written speaker notes.",
    );
    const reopenedShapes = reopenedSlide.shapes();
    expect(reopenedShapes.map((s) => s.text)).toContain(
      "Freshly built title\nBody paragraph",
    );
    ...
    expect(readSlide?.notes).toBe(
      "These are the freshly written speaker notes.",
    );
    const titleShape = readSlide?.shapes.find((s) =>
      s.blocks.some(
        (b) =>
          b.kind === "paragraph" &&
          b.runs.some((r) => r.text.includes("Freshly built title")),
      ),
    );
    expect(titleShape).toBeDefined();
    const titleBlock = titleShape?.blocks.find(
      (b) =>
        b.kind === "paragraph" &&
        b.runs.some((r) => r.text.includes("Freshly built title")),
    );
    ...
    const titleReadRun = titleBlock.runs.find((r) =>
      r.text.includes("Freshly built title"),
    );
    expect(titleReadRun?.bold).toBe(true);
    expect(titleReadRun?.color).toEqual({ r: 1, g: 0, b: 0 });

    const readRotated = readSlide?.shapes.find(
      (s) => s.rotationDeg !== undefined,
    );
    expect(readRotated?.rotationDeg).toBeCloseTo(45, 6);

    const readImage = readSlide?.shapes.find((s) =>
