...
function findOfficePresentation(pkg: Package): XmlElement {
  ...
  const presentation = body === undefined ? undefined : directChild(body, 'office:presentation');
  if (presentation === undefined) {
    throw new Error(`${CONTENT_PART_PATH} has no office:body/office:presentation element`);
  }
  return presentation;
}
...
export class OdpEditor {
  ...
  removeSlideAt(index: number): void {
    const presentation = findOfficePresentation(this.pkg);
    const pageElements = presentation.children.filter((c): c is XmlElement => c.type === 'element' && c.tag === 'draw:page');
