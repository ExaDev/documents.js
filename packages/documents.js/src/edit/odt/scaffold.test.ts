...
describe("createEmptyOdtPackage", () => {
  ...
  it("has an office:body/office:text element in content.xml", () => {
    ...
    const text =
      body?.type === "element"
        ? body.children.find(
            (c) => c.type === "element" && c.tag === "office:text",
          )
        : undefined;
  ...
  it("defines a Heading_20_N common style for every level of the heading convention in office:styles", () => {
    ...
    const officeStyles = root?.children.find(
      (c): c is XmlElement => c.type === "element" && c.tag === "office:styles",
    );
    const styleElements = (officeStyles?.children ?? []).filter(
    ...
    for (const [level, style] of Object.entries(HEADING_STYLES)) {
      const styleElement = styleElements.find(
        (c) => attr(c, "style:name") === `Heading_20_${level}`,
      );
      expect(styleElement, `Heading_20_${level}`).toBeDefined();
      expect(attr(styleElement!, "style:display-name")).toBe(
        `Heading ${level}`,
      );
      expect(attr(styleElement!, "style:family")).toBe("paragraph");
      const textProperties = styleElement!.children.find(
        (c): c is XmlElement =>
          c.type === "element" && c.tag === "style:text-properties",
      );
      expect(attr(textProperties!, "fo:font-size")).toBe(
        `${String(style.sizePt)}pt`,
      );
      expect(attr(textProperties!, "fo:font-weight")).toBe(
