...
export function ensureAutomaticStyles(pkg: Package): XmlElement {
  ...
  if (part?.kind !== 'xml') {
    throw new Error(`ensureAutomaticStyles: package has no ${CONTENT_PART_PATH} part`);
  }
  const root = part.nodes.find((n): n is XmlElement => n.type === 'element');
  if (root === undefined) {
    throw new Error(`ensureAutomaticStyles: ${CONTENT_PART_PATH} has no root element`);
  }
  ...
  const insertBeforeTags = new Set(['office:body', 'office:master-styles', 'office:settings']);
...
export function nextStyleName(automaticStyles: XmlElement, tag: string, prefix: string): string {
  let max = 0;
...
export function ensurePageBreakStyleName(pkg: Package): string {
  ...
  const existing = automaticStyles.children.find(
    (c) => c.type === 'element' && c.tag === 'style:style' && attr(c, 'style:name') === PAGE_BREAK_STYLE_NAME,
  );
  if (existing !== undefined) {
    return PAGE_BREAK_STYLE_NAME;
  }
  automaticStyles.children.push(
    el('style:style', { 'style:name': PAGE_BREAK_STYLE_NAME, 'style:family': 'paragraph' }, [
