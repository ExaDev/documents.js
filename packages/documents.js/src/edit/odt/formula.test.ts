...
function contentRoot(pkg: Package): XmlElement {
  ...
  if (root === undefined) {
    throw new Error(
      "expected the built odt to have a content.xml root element",
    );
  }
  return root;
}

type WordprocessingBlocks = Extract<
  ContentDocument,
  { kind: "wordprocessing" }
...
describe("OdtBody.appendFormula", () => {
  ...
  it("lists the sub-document in the manifest with the genuine ODF formula media type, leaving the manifest valid", () => {
    ...
    expect(
      entries.find((entry) => entry.fullPath === "Object 1/content.xml")
        ?.mediaType,
    ).toBe("text/xml");
    expect(
      validateManifest(pkg).filter((problem) => problem.severity === "error"),
    ).toEqual([]);
  });
  ...
  it("numbers a second formula into its own object directory rather than overwriting the first", () => {
    const editor = createOdt();
    editor.body.appendFormula(FRACTION, FRAME);
    editor.body.appendFormula(
      { mathml: [mel("msqrt", [mtoken("mi", "x")])] },
      FRAME,
    );
    const pkg = editor.toPackage();
    expect(Object.keys(pkg.parts)).toEqual(
      expect.arrayContaining(["Object 1/content.xml", "Object 2/content.xml"]),
    );
    const hrefs = elementsWithTag([contentRoot(pkg)], "draw:object").map(
    ...
    expect(
      entries
        .filter(
          (entry) =>
            entry.mediaType === "application/vnd.oasis.opendocument.formula",
        )
  ...
  it("writes a StarMath annotation odf.js reads back as ContentFormula.starMath", () => {
    ...
    const pkg = editor.toPackage();
    expect(readOdfEmbeddedFormula(pkg, "Object 1")?.starMath).toBe(
      "{a} over {b}",
    );
  });
  ...
  it("survives a real zip encode/decode round trip, not just the in-memory package", () => {
    ...
    expect(
      signature(readOdfEmbeddedFormula(reopened, "Object 1")?.mathml ?? []),
    ).toBe("mfrac(mi(a),mi(b))");
  });
});
...
describe("buildOdtPackage: an embedded formula block", () => {
  const formulaBlocks = [
    { kind: "paragraph" as const, runs: [{ text: "Before the formula" }] },
    {
      ...
      document: formulaDocument(FRACTION),
      frame: FRAME,
    },
  ];
  ...
  it("keeps a formula through repeated write/read cycles without accumulating blank paragraphs", () => {
    ...
    for (let cycle = 0; cycle < 3; cycle++) {
      ...
      content = next;
      expect(next.sections[0]!.blocks.map((block) => block.kind)).toEqual([
        "paragraph",
        "embeddedObject",
      ]);
    }
      ...
      wordDoc([
        {
          kind: "embeddedObject",
          objectKind: "formula",
          document: formulaDocument({ mathml: [], starMath: "{a} over {b}" }),
          frame: FRAME,
        },
      ]),
    );
