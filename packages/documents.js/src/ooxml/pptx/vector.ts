...
// Detects and collapses a slide's own vector-only p:sp shapes back into a real drawing block -- the pptx-side counterpart to src/ooxml/docx/vector.ts. Unlike docx (where a vector-only w:drawing leaves no trace in readDocx's output at all, needing a new block INSERTED) and odp (where a bare vector primitive produces no shape at all either), ooxml.js's own readPptx ALWAYS produces an ordinary, EMPTY ContentShape for a p:sp regardless of its content -- so recovering a vector here is attach-and-replace, collapsing a run of already-existing (empty) shape slots into one synthetic shape carrying the recovered geometry, never an insertion that would shift any other shape's index.

// A shape-tree walk mirroring ooxml.js's own walkShapeTreeChildren exactly: p:sp/p:pic/p:graphicFrame each occupy one shape slot (in that document order), p:grpSp recurses (flattening a group's own shapes into the same flat array, exactly as readPptx does), and p:cxnSp -- a connector -- never occupies a slot and is never recursed into, since readPptx does not visit one at all.
interface WalkState {
  shapeIndex: number;
}
