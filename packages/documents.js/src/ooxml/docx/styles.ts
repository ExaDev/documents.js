...
import { halfPointsToPt, lineUnitsToMultiplier, twipsToPt } from '../../model/units';
...
// w:ascii (a literal font name) takes precedence over w:asciiTheme (a theme font reference) when both are present, per ECMA-376's own precedence rule. Only the Latin (ascii/asciiTheme) slot is read -- East Asian/complex-script fonts are out of v1 scope (RTL/bidi/East Asian typography).
...
// Cascade: docDefaults -> default paragraph style -> the paragraph's own named style chain (root-first) -> the paragraph's own direct w:pPr. Numbering-level properties are deliberately not merged in here -- list indentation/marker layout is a src/layout/engine.ts concern once a paragraph's list membership (numId/level) is known, not a style-resolution one.
