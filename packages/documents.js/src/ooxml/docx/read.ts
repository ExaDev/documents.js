...
export function readDocxContent(
  ...
  // The upstream reader already threw if the body part has no w:body, so this only guards the type -- there is no reachable "read succeeded but the body is gone" state.
  const sections =
    body === undefined
      ? docxDoc.sections
