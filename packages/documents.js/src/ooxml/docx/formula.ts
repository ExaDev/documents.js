...
export function collectBodyTables(
  nodes: readonly XmlNode[],
  out: XmlElement[],
): void {
  for (const node of nodes) {
...
export const PARAGRAPH_NON_CONTENT_TAGS: ReadonlySet<string> = new Set([
  "w:pPr",
  "w:bookmarkStart",
  "w:bookmarkEnd",
  "w:proofErr",
...
function mathRunSizePt(equation: XmlElement): number | undefined {
  ...
  while (stack.length > 0) {
    ...
    if (node.tag === "w:sz") {
      const halfPoints = Number.parseFloat(
        node.attributes.find((attribute) => attribute.name === "w:val")
          ?.value ?? "",
      );
...
export function equationFrame(equation: XmlElement): Box {
  return {
    ...
    widthPt: 0,
    heightPt:
      (mathRunSizePt(equation) ?? DEFAULT_MATH_SIZE_PT) *
      FRAME_HEIGHT_PER_SIZE_PT,
  };
}
