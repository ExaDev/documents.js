...
function anchorFrame(anchor: XmlElement): Box | undefined {
  ...
  if (positionH === undefined || positionV === undefined || extent === undefined) {
    return undefined;
  }
  if (attr(positionH, 'relativeFrom') !== 'page' || attr(positionV, 'relativeFrom') !== 'page') {
    return undefined;
  }
  ...
  if (xEmu === undefined || yEmu === undefined || cx === undefined || cy === undefined) {
    return undefined;
  }
  return { xPt: emuToPt(xEmu), yPt: emuToPt(yEmu), widthPt: emuToPt(cx), heightPt: emuToPt(cy) };
}
...
export function collectParagraphVectors(paragraph: XmlElement): readonly DetectedParagraphVector[] {
        ...
        for (const graphic of childrenWithTag(anchor, 'a:graphic')) {
          for (const graphicData of childrenWithTag(graphic, 'a:graphicData')) {
            ...
            for (const wsp of childrenWithTag(graphicData, 'wps:wsp')) {
              ...
              out.push({ drawingElement: drawing, vector: { ...vector, paintOrder: out.length } });
            }
          }
        }
      }
