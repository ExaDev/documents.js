...
export function defaultContentTypeForExtension(extension: string): string {
  ...
  if (contentType === undefined) {
    throw new Error(`no known default content type for extension: ${extension}`);
  }
  return contentType;
}
...
export function ensureDefaultContentType(pkg: Package, extension: string, contentType: string): void {
  const root = ensureContentTypesRoot(pkg);
  for (const child of root.children) {
    if (child.type === 'element' && child.tag === 'Default' && attr(child, 'Extension') === extension) {
      return;
    }
  }
  root.children.push(el('Default', { Extension: extension, ContentType: contentType }));
}
...
export function ensureContentTypeOverride(pkg: Package, partPath: string, contentType: string): void {
  const root = ensureContentTypesRoot(pkg);
  const partName = `/${partPath}`;
  for (const child of root.children) {
    if (child.type === 'element' && child.tag === 'Override' && attr(child, 'PartName') === partName) {
      return;
    }
  }
  root.children.push(el('Override', { PartName: partName, ContentType: contentType }));
}
