...
export function addCoreProperties(pkg: Package, metadata: LayoutMetadata): void {
  ...
  pkg.parts[CORE_PROPERTIES_PART_PATH] = { kind: 'xml', nodes: [declaration(), root] };
  ensureContentTypeOverride(pkg, CORE_PROPERTIES_PART_PATH, CORE_PROPERTIES_CONTENT_TYPE);
  addRootRelationship(pkg, { type: CORE_PROPERTIES_REL_TYPE, target: CORE_PROPERTIES_PART_PATH });
}
