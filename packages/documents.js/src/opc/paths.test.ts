...
describe('buildRelativeTarget', () => {
  it('targets a sibling media directory with no ups', () => {
    expect(buildRelativeTarget('word/document.xml', 'word/media/image1.png')).toBe('media/image1.png');
  });

  it('targets a media directory one level up', () => {
    expect(buildRelativeTarget('ppt/slides/slide1.xml', 'ppt/media/image1.png')).toBe(
      '../media/image1.png',
    );
  });

  it('targets a part in the same directory', () => {
    expect(buildRelativeTarget('word/document.xml', 'word/styles.xml')).toBe('styles.xml');
  });

  it('targets a deeper nested directory from a shallower one', () => {
    expect(buildRelativeTarget('ppt/presentation.xml', 'ppt/slides/slide1.xml')).toBe('slides/slide1.xml');
  });
});
