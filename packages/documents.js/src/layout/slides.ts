...
function shapePlacement(
  ...
  const center: Point = {
    x: flippedFrame.xPt + flippedFrame.widthPt / 2,
    y: flippedFrame.yPt + flippedFrame.heightPt / 2,
  };
  return {
    place: (p) => rotatePointAboutCenter(p, center, ccwDeg),
...
function layoutParagraph(
  ...
  const paragraphWidthPt = Math.max(
    0,
    contentWidthPt - (paragraph.indentLeftPt ?? 0),
  );
  const lines = wrapRunsToWidth(effectiveRuns, measurer, paragraphWidthPt);

  lines.forEach((line, lineIndex) => {
    const naturalLineHeightPt = lineNaturalHeightPt(
      line,
      measurer,
      fallbackRun,
    );
    ...
    for (const cell of row.cells) {
      ...
      for (const block of cell.blocks) {
        if (block.kind === "paragraph") {
          cellCursorYDown = layoutParagraph(
            block,
            cellXDown,
            cellWidthPt,
            cellCursorYDown,
...
export function convertShape(
  ...
  const contentWidthPt = Math.max(
    0,
    shape.frame.widthPt - shape.insetLeftPt - shape.insetRightPt,
  );
...
function convertSlide(
  ...
  measurer: TextMeasurer,
  images: Record<string, LayoutImageAsset>,
  positioned: PositionedFormula[],
  mathMetricsAt: (sizePt: number) => MathFontMetrics,
): LayoutPage {
  const items: LayoutItem[] = [];
  const formulaContext: ShapeFormulaContext = {
