...
function retyped(text: string): { kind: string; [key: string]: unknown } {
  ...
  if (result?.outcome !== 'retyped') {
    throw new Error(`expected ${text} to be re-typed, got ${JSON.stringify(result)}`);
  }
  return result.value;
}
...
function declineReason(text: string): string {
  ...
  if (result?.outcome !== 'declined') {
    throw new Error(`expected ${text} to be declined, got ${JSON.stringify(result)}`);
  }
...
describe('inferCellValue: percentages and currency', () => {
  ...
  it('names the currency only where the symbol identifies exactly one', () => {
    expect(retyped('£1,234.50')).toEqual({ kind: 'currency', value: 1234.5, currency: 'GBP' });
    expect(retyped('€99.99')).toEqual({ kind: 'currency', value: 99.99, currency: 'EUR' });
    expect(retyped('$42')).toEqual({ kind: 'currency', value: 42, currency: undefined }); // '$' is USD, CAD, AUD and more -- the KIND is certain, the code is not
    expect(retyped('¥500')).toEqual({ kind: 'currency', value: 500, currency: undefined });
  });

  it('reads a negative currency in the sign-before-symbol form both Calc and Excel print', () => {
    expect(retyped('-£5.00')).toEqual({ kind: 'currency', value: -5, currency: 'GBP' });
  });
  ...
  it('tolerates no-break spaces around a value, not only ordinary ones', () => {
    expect(retyped('\u00A31234.50')).toEqual({ kind: 'currency', value: 1234.5, currency: 'GBP' });
    expect(retyped('\u00A3\u00A01234.50')).toEqual({ kind: 'currency', value: 1234.5, currency: 'GBP' });
    expect(retyped('\u00A3\u202F1234.50')).toEqual({ kind: 'currency', value: 1234.5, currency: 'GBP' });
    expect(retyped('42.5\u00A0')).toEqual({ kind: 'number', value: 42.5 });
  });
...
describe('inferCellValue: dates', () => {
  it('re-types an ISO date, whose component roles the text itself states', () => {
    expect(retyped('2024-01-15')).toEqual({ kind: 'date', value: '2024-01-15' });
  });

  it('re-types a named-month date in either order, normalising to ISO', () => {
    expect(retyped('15 Jan 2024')).toEqual({ kind: 'date', value: '2024-01-15' });
    expect(retyped('15 January 2024')).toEqual({ kind: 'date', value: '2024-01-15' });
    expect(retyped('Jan 15, 2024')).toEqual({ kind: 'date', value: '2024-01-15' });
    expect(retyped('3-Mar-2019')).toEqual({ kind: 'date', value: '2019-03-03' });
  });
  ...
  it('accepts a real leap day, so the calendar check is a genuine one rather than a fixed month-length table', () => {
    expect(retyped('2024-02-29')).toEqual({ kind: 'date', value: '2024-02-29' });
