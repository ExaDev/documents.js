...
function tinyPngBlock(
  overrides: Partial<ContentImageBlock> = {},
): ContentImageBlock {
...
describe("sourcePath propagation: docx flow (engine.ts)", () => {
  ...
  it("leaves sourcePath undefined for a run with none, rather than fabricating one", () => {
    const { document: layout } = convertWordprocessingToLayout(
      wordprocessingDoc([section([paragraph([run("Hi", { sizePt: 10 })])])]),
      { measurer: fakeMeasurer(), mathMetricsAt },
    );
    const [text] = textItems(layout.pages[0]!.items);
  ...
  it("gives each fragment of a word split across a run boundary its own run's sourcePath", () => {
      ...
      wordprocessingDoc([
        section([
          paragraph([
            ...
            run("lo", {
              sizePt: 10,
              sourcePath: "sections[0].blocks[0].runs[1]",
            }),
          ]),
        ]),
      ]),
    ...
    expect(texts.map((t) => t.sourcePath)).toEqual([
      "sections[0].blocks[0].runs[0]",
      "sections[0].blocks[0].runs[1]",
    ]);
  });
  ...
  it("gives every fragment of one run emergency-split across multiple pages the SAME sourcePath, not a fabricated per-fragment one", () => {
    ...
    expect(new Set(texts.map((t) => t.sourcePath))).toEqual(
      new Set(["sections[0].blocks[0].runs[0]"]),
    );
  });
  ...
  it("copies an image block's sourcePath onto its LayoutImage", () => {
    const { document: layout } = convertWordprocessingToLayout(
      wordprocessingDoc([
        section([tinyPngBlock({ sourcePath: "sections[0].blocks[0]" })]),
      ]),
      { measurer: fakeMeasurer(), mathMetricsAt },
    );
  ...
  it("attributes a table cell's background rect to the table's own sourcePath, since ContentTableCell has none of its own", () => {
    ...
    const { document: layout } = convertWordprocessingToLayout(
      wordprocessingDoc([section([table])]),
      { measurer: fakeMeasurer(), mathMetricsAt },
    );
              ...
              blocks: [
                paragraph([
                  run("Cell", {
                    sizePt: 10,
                    sourcePath:
                      "sections[0].blocks[0].rows[0].cells[0].blocks[0].runs[0]",
                  }),
                ]),
              ],
            },
...
describe("sourcePath propagation: pptx direct placement (slides.ts)", () => {
    ...
    const s = shape({
      blocks: [
        paragraph([
          run("Hi", {
            sizePt: 10,
            sourcePath: "slides[0].shapes[0].blocks[0].runs[0]",
          }),
        ]),
      ],
    });
    const { document: layout } = convertPresentationToLayout(
      presentationDoc([slide([s])]),
      { measurer: fakeMeasurer(), mathMetricsAt },
    );
    const [text] = textItems(layout.pages[0]!.items);
  ...
  it("copies an image block's sourcePath onto its LayoutImage", () => {
    const s = shape({
      blocks: [tinyPngBlock({ sourcePath: "slides[0].shapes[0].blocks[0]" })],
    });
    const { document: layout } = convertPresentationToLayout(
      presentationDoc([slide([s])]),
      { measurer: fakeMeasurer(), mathMetricsAt },
...
describe("sourcePath propagation: docx end-to-end (createDocx -> readDocxContent -> convertWordprocessingToLayout)", () => {
  it("carries ooxml.js-assigned sourcePath strings through the real read+layout pipeline", () => {
    ...
    const { document: layout } = convertWordprocessingToLayout(content, {
...
describe("sourcePath propagation: pptx end-to-end (createPptx -> readPptxContent -> convertPresentationToLayout)", () => {
  it("carries ooxml.js-assigned sourcePath strings through the real read+layout pipeline", () => {
    ...
    const textBox = firstSlide.addTextBox({
      frame: { xPt: 50, yPt: 50, widthPt: 400, heightPt: 100 },
      text: "",
    });
    textBox.setParagraphs([
      {
        runs: [
          { text: "Bold", bold: true },
          { text: "Red", color: { r: 1, g: 0, b: 0 } },
        ],
      },
    ]);
    const secondSlide = editor.addSlide();
    const secondTextBox = secondSlide.addTextBox({
      frame: { xPt: 50, yPt: 50, widthPt: 400, heightPt: 100 },
      text: "",
    });
    ...
    const { document: layout } = convertPresentationToLayout(content, {
      measurer: createStandardFontMeasurer(),
      mathMetricsAt,
    });
    ...
    expect(firstSlideTexts.map((t) => t.sourcePath)).toEqual([
      "slides[0].shapes[0].blocks[0].runs[0]",
      "slides[0].shapes[0].blocks[0].runs[1]",
    ]);
    expect(secondSlideTexts.map((t) => t.text)).toEqual(["Second"]);
    expect(secondSlideTexts.map((t) => t.sourcePath)).toEqual([
