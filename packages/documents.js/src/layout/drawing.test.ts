...
describe("convertDrawingToLayout: rect vector", () => {
  it("flips a rect vector's frame from top-left/y-down page space into bottom-left/y-up PDF space", () => {
    ...
    expect(layout.pages[0]?.items).toEqual([
      {
        kind: "rect",
        xPt: 10,
        yPt: 300 - 20 - 40,
  ...
  it("carries stroke and sourcePath straight through", () => {
    const vector: ContentVector = {
      ...
      sourcePath: "pages[0].vectors[0]",
    };
    const layout = convert([page({ vectors: [vector] })]);
    expect(layout.pages[0]?.items[0]).toMatchObject({
      kind: "rect",
      stroke: { color: BLACK, widthPt: 2 },
      sourcePath: "pages[0].vectors[0]",
    });
      ...
      subpaths: [
        {
          ...
          closed: true,
          segments: [
            { kind: "line", to: { xPt: 40, yPt: 0 } },
            { kind: "line", to: { xPt: 20, yPt: 40 } },
          ],
        },
      ],
      fill: RED,
      ...
      subpaths: [
        {
          ...
          segments: [
            {
              kind: "cubic",
              control1: { xPt: 10, yPt: 20 },
              control2: { xPt: 30, yPt: 40 },
              to: { xPt: 50, yPt: 60 },
            },
          ],
        },
    ...
    const vector: ContentVector = {
      ...
      subpaths: [
        ...
        },
        {
          start: { xPt: 5, yPt: 5 },
          closed: true,
          segments: [{ kind: "line", to: { xPt: 8, yPt: 5 } }],
        },
      ],
    };
    const layout = convert([page({ vectors: [vector] })]);
...
describe("convertDrawingToLayout: paint order", () => {
  it("paints every vector before every shape, per the documented vectors-first convention", () => {
    const rectVector: ContentVector = {
      kind: "rect",
      frame: { xPt: 0, yPt: 0, widthPt: 10, heightPt: 10 },
      fill: RED,
    };
    ...
    expect(layout.pages[0]?.items.map((item) => item.kind)).toEqual([
      "rect",
      "text",
    ]);
  });
  ...
  it("preserves each array's own internal document order (already paint-ordered by the reader)", () => {
    const back: ContentVector = {
      kind: "rect",
      frame: { xPt: 0, yPt: 0, widthPt: 10, heightPt: 10 },
      fill: RED,
    };
    const front: ContentVector = {
      kind: "rect",
      frame: { xPt: 5, yPt: 5, widthPt: 10, heightPt: 10 },
      fill: BLUE,
    };
    const layout = convert([page({ vectors: [back, front] })]);
    const items = layout.pages[0]?.items ?? [];
    expect(
      items.map((item) => (item.kind === "rect" ? item.fill : undefined)),
    ).toEqual([RED, BLUE]);
  });
...
describe("convertDrawingToLayout: multiple pages", () => {
  it("produces one LayoutPage per ContentDrawPage, in order", () => {
    const layout = convert([
      page({ size: { widthPt: 200, heightPt: 100 } }),
      page({ size: { widthPt: 300, heightPt: 150 } }),
    ]);
...
function expectPath(item: LayoutItem | undefined): LayoutPath {
  if (item?.kind !== "path") {
    throw new Error(
      `expected a LayoutPath item, got ${item?.kind ?? "nothing"}`,
    );
  }
  return item;
}
...
describe("convertDrawingToLayout: vector rotation", () => {
  ...
  it("turns a rotated rect into a closed four-point LayoutPath whose corners are genuinely rotated about the frame centre", () => {
    const vector: ContentVector = {
      kind: "rect",
      frame: { xPt: 100, yPt: 100, widthPt: 40, heightPt: 20 },
      rotationDeg: 90,
      fill: RED,
    };
    ...
    const corners = [
      ...
      ...subpath!.segments.map((s) => ({ x: s.xPt, y: s.yPt })),
    ];
  ...
  it("keeps the plain LayoutRect fast path for an unrotated rect, and for an explicit rotationDeg of 0", () => {
    const unrotated: ContentVector = {
      kind: "rect",
      frame: { xPt: 0, yPt: 0, widthPt: 10, heightPt: 10 },
      fill: RED,
    };
    const zero: ContentVector = {
      kind: "rect",
      frame: { xPt: 0, yPt: 0, widthPt: 10, heightPt: 10 },
      rotationDeg: 0,
      fill: RED,
    ...
    expect(
      convert([page({ vectors: [unrotated, zero] })]).pages[0]!.items.map(
        (i) => i.kind,
      ),
    ).toEqual(["rect", "rect"]);
  });
  ...
  it("turns a rotated ellipse into a LayoutPath of four cubics, still centred on its own frame centre", () => {
    ...
    expect(
      Math.hypot(subpath!.startXPt - 120, subpath!.startYPt - 190),
    ).toBeCloseTo(20, 6);
  });
  ...
  it("keeps the plain LayoutEllipse fast path for an unrotated ellipse", () => {
    ...
    expect(
      convert([page({ vectors: [vector] })]).pages[0]!.items.map((i) => i.kind),
    ).toEqual(["ellipse"]);
  });
  ...
  it("rotates a path vector's own points about its frame centre, still emitting one LayoutPath", () => {
    const subpaths = [
      {
        ...
        closed: true,
        segments: [
          { kind: "line" as const, to: { xPt: 40, yPt: 0 } },
          { kind: "line" as const, to: { xPt: 40, yPt: 20 } },
        ],
      },
    ];
    const plain: ContentVector = {
      kind: "path",
      frame: { xPt: 100, yPt: 100, widthPt: 40, heightPt: 20 },
      subpaths,
      fill: RED,
    ...
    const plainPath = expectPath(
      convert([page({ vectors: [plain] })]).pages[0]!.items[0],
    );
    const rotatedPath = expectPath(
      convert([page({ vectors: [rotated] })]).pages[0]!.items[0],
    );
    ...
    const subpaths = [
      {
        ...
        segments: [
          {
            kind: "cubic" as const,
            control1: { xPt: 10, yPt: 0 },
            control2: { xPt: 30, yPt: 20 },
            to: { xPt: 40, yPt: 20 },
          },
        ],
      },
    ];
    const rotated: ContentVector = {
      kind: "path",
      frame: { xPt: 100, yPt: 100, widthPt: 40, heightPt: 20 },
      rotationDeg: 90,
      subpaths,
...
describe("convertDrawingToLayout: shared paintOrder", () => {
  ...
  it("interleaves shapes and vectors by paintOrder rather than painting every vector first", () => {
    ...
    const layout = convert([
      page({ shapes: [middle], vectors: [back, front] }),
    ]);
    expect(layout.pages[0]?.items.map((item) => item.kind)).toEqual([
      "rect",
      "text",
...
describe("convertDrawingToLayout -> reconstructDrawing: paint order survives the round trip", () => {
  it("preserves a genuinely interleaved vector/shape/vector stack rather than collapsing it to vectors-then-shapes", () => {
    ...
    const front: ContentVector = {
      kind: "rect",
      frame: { xPt: 20, yPt: 20, widthPt: 40, heightPt: 40 },
      fill: BLUE,
      paintOrder: 2,
    };
    const layout = convert([
    ...
    const relaid = convertDrawingToLayout(recovered, {
      measurer: fakeMeasurer(),
    }).document;
    expect(relaid.pages[0]?.items.map((item) => item.kind)).toEqual([
      "rect",
      "text",
      "rect",
    ]);
