...
export function textBoxForFragment(
  item: LayoutText,
  textWidthPt: number,
  ascentPt: number,
  descentPt: number,
...
export function stampFragmentFrame(
  runs: readonly ContentRun[],
  fragment: SourcedFragment,
  pageIndex: number,
  item: LayoutText,
  measurer: TextMeasurer,
  line: { readonly ascentPt: number; readonly descentPt: number },
): void {
  ...
  stampFrame(
    ...
    textBoxForFragment(
      item,
      measurer.widthOfTextAtSize(fragment.text, fragment.font, fragment.sizePt),
      line.ascentPt,
      line.descentPt,
    ),
  );
...
export function layoutDocumentOf(
...
export function formulaSizePtForFrame(
  ...
  const widthScale =
    frame.widthPt > 0 ? frame.widthPt / box.widthPt : Number.POSITIVE_INFINITY;
  return Math.max(
    MIN_FORMULA_SIZE_PT,
    REFERENCE_FORMULA_SIZE_PT * Math.min(heightScale, widthScale),
  );
}
...
export function headingStyleFor(
  styleId: string | undefined,
): { bold: boolean; sizePt: number } | undefined {
...
export function toStyledRuns(
  runs: readonly ContentRun[],
  fontScale = 1,
  headingStyle?: { bold: boolean; sizePt: number },
): SourcedRun[] {
  return runs.map((run, runIndex) => ({
    text: run.text,
    font: runFont(run, headingStyle?.bold),
    sizePt:
      (run.sizePt ?? headingStyle?.sizePt ?? NOMINAL_TEXT_SIZE_PT) * fontScale,
...
export function effectiveStyledRuns(
  runs: readonly ContentRun[],
  fontScale = 1,
  headingStyle?: { bold: boolean; sizePt: number },
): SourcedRun[] {
    ...
    : [
        {
          ...
          sizePt: NOMINAL_TEXT_SIZE_PT * fontScale,
          color: COLOR_BLACK,
        },
      ];
}

export function lineNaturalHeightPt(
  line: WrappedLine,
  measurer: TextMeasurer,
  fallback: StyledRun,
): number {
  ...
  for (const fragment of line.fragments) {
    max = Math.max(
      max,
      measurer.lineHeightAtSize(fragment.font, fragment.sizePt),
    );
  }
  return max;
}
...
export function alignmentOffsetPt(
...
export function justifyLineGapsPt(
  line: WrappedLine,
...
export function pushCellBorderLines(
