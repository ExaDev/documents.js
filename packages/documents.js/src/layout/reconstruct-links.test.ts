...
import type { LayoutAnnotation, LayoutDocument, LayoutFormField, LayoutImageAsset, LayoutItem, LayoutPage, LayoutText } from 'pdf-codec';
...
function text(overrides: { text: string; xPt: number; yPt: number; widthPt: number; sizePt?: number; layer?: string }): LayoutText {
  return {
    kind: 'text',
    text: overrides.text,
    xPt: overrides.xPt,
...
function page(widthPt: number, heightPt: number, items: LayoutItem[], annotations?: LayoutAnnotation[]): LayoutPage {
  return { widthPt, heightPt, items, ...(annotations !== undefined ? { annotations } : {}) };
}

function docFrom(pages: LayoutPage[], extra: { layers?: { name: string; visible: boolean }[]; form?: LayoutFormField[] } = {}): LayoutDocument {
  return { formatVersion: 1, metadata: {}, pages, images: { img1: tinyPngAsset() }, ...(extra.layers !== undefined ? { layers: extra.layers } : {}), ...(extra.form !== undefined ? { form: extra.form } : {}) };
}
...
function tinyPngAsset(): LayoutImageAsset {
  ...
  return { format: 'png', base64: bytesToBase64(bytes), widthPx: 2, heightPx: 2 };
}
...
describe('reconstructWordprocessing: link reconciliation (#721)', () => {
    ...
    const doc = reconstructWordprocessing(
      docFrom([
        page(612, 792, [
          ...
          { kind: 'link', uri: 'https://example.com', xPt: 80, yPt: 688, widthPt: 70, heightPt: 14 },
        ]),
      ]),
    );
    const hyperlinkRuns = runs(doc).filter((r) => r.hyperlink !== undefined);
    expect(hyperlinkRuns).toHaveLength(1);
    expect(hyperlinkRuns[0]).toMatchObject({ text: 'example.com', hyperlink: 'https://example.com' });
  });
  ...
  it('wraps the best-matching block in a link construct pair when no run matches an external link', () => {
    const doc = reconstructWordprocessing(
      docFrom([
        page(612, 792, [
          ...
          { kind: 'image', imageId: 'img1', xPt: 40, yPt: 600, widthPt: 100, heightPt: 60 },
          { kind: 'link', uri: 'https://images.example', xPt: 40, yPt: 600, widthPt: 100, heightPt: 60 },
        ]),
      ]),
    );
    ...
    expect(list[start]).toMatchObject({ kind: 'constructStart', descriptor: { kind: 'link', target: { kind: 'external', uri: 'https://images.example' } } });
    ...
    const doc = reconstructWordprocessing(
      docFrom([
        page(612, 792, [
          ...
          { kind: 'internalLink', destination: 'section-two', xPt: 50, yPt: 688, widthPt: 90, heightPt: 14 },
        ]),
      ]),
    );
    const list = blocks(doc);
    const start = list.findIndex((b) => b.kind === 'constructStart');
    expect(list[start]).toMatchObject({ kind: 'constructStart', descriptor: { kind: 'link', target: { kind: 'internal', anchor: 'section-two' } } });
...
describe('reconstructWordprocessing: optional content visibility (#721)', () => {
      ...
      docFrom(
        [
          page(612, 792, [
            ...
            text({ text: 'Hidden', xPt: 50, yPt: 680, widthPt: 40, layer: 'Background' }),
          ]),
        ],
        { layers: [{ name: 'Background', visible: false }] },
      ),
    );
    const texts = runs(doc).map((r) => r.text).join(' ');
  ...
  it('keeps items in a layer with no visibility entry (the honest default: unstated means shown)', () => {
    const doc = reconstructWordprocessing(
      docFrom(
        [
          page(612, 792, [text({ text: 'Kept', xPt: 50, yPt: 700, widthPt: 40, layer: 'Unknown' })]),
        ],
        { layers: [{ name: 'Background', visible: false }] },
      ),
    );
    expect(runs(doc).map((r) => r.text).join(' ')).toContain('Kept');
  });
});
...
describe('reconstructWordprocessing: annotation and form constructs (#721)', () => {
    ...
    const doc = reconstructWordprocessing(
      docFrom([
        page(
          ...
          [{ subtype: 'Text', xPt: 500, yPt: 740, widthPt: 16, heightPt: 16, contents: 'A note', author: 'Reviewer' }],
        ),
      ]),
    );
    const list = blocks(doc);
    const start = list.findIndex((b) => b.kind === 'constructStart');
    expect(list[start]).toMatchObject({ kind: 'constructStart', descriptor: { kind: 'anchor', anchorType: 'comment', name: 'pdf-annot-0-0', definition: 'pdf-annot-0-0' } });
  ...
  it('emits a contentControl construct around a form field\'s best-matching block', () => {
    const doc = reconstructWordprocessing(
      docFrom(
        [
          page(612, 792, [text({ text: 'Jane Doe', xPt: 50, yPt: 700, widthPt: 60 })]),
        ],
        {
          form: [
            {
              ...
              alias: 'Full name',
              readOnly: true,
              widgets: [{ pageIndex: 0, xPt: 45, yPt: 688, widthPt: 70, heightPt: 16 }],
              children: [],
            },
          ],
        },
    ...
    expect(list[start]).toMatchObject({
      kind: 'constructStart',
      descriptor: { kind: 'contentControl', controlType: 'plainText', tag: 'fullname', value: 'Jane Doe', alias: 'Full name', lock: 'content' },
    });
    expect(list[start + 1]?.kind).toBe('paragraph');
    expect(list[start + 2]?.kind).toBe('constructEnd');
  });
  ...
  it('emits nothing for signature fields -- certification is residue, not a control', () => {
    const doc = reconstructWordprocessing(
      docFrom(
        [
          page(612, 792, [text({ text: 'Body', xPt: 50, yPt: 700, widthPt: 40 })]),
        ],
        {
          form: [{ name: 'sig', fieldType: 'signature', widgets: [{ pageIndex: 0, xPt: 40, yPt: 688, widthPt: 80, heightPt: 20 }], children: [] }],
        },
      ),
    );
