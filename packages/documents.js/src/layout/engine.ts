...
function ensureRoom(
  ...
  if (
    ...
    state.cursorYDown + neededHeightPt > contentBottomYDown
  ) {
...
function layoutParagraphFlow(
  ...
  const effectiveRuns = effectiveStyledRuns(
    paragraph.runs,
    1,
  ...
  const listIndentPt =
    paragraph.list !== undefined
      ...
      : 0;
  const paragraphLeftXDown =
    contentLeftXDown + (paragraph.indentLeftPt ?? 0) + listIndentPt;
  const paragraphWidthPt = Math.max(
    0,
    contentWidthPt - (paragraph.indentLeftPt ?? 0) - listIndentPt,
  );
  const lines = wrapRunsToWidth(effectiveRuns, measurer, paragraphWidthPt);

  lines.forEach((line, lineIndex) => {
    const lineHeightPt =
      lineNaturalHeightPt(line, measurer, fallbackRun) *
    ...
    line.fragments.forEach((fragment, fragmentIndex) => {
      ...
      if (fragment.hyperlink !== undefined) {
        const fragmentWidthPt = measurer.widthOfTextAtSize(
          fragment.text,
          fragment.font,
          fragment.sizePt,
        );
...
function layoutParagraphInCell(
  ...
  const listIndentPt =
    paragraph.list !== undefined
      ...
      : 0;
  const paragraphLeftXDown =
    cellLeftXDown + (paragraph.indentLeftPt ?? 0) + listIndentPt;
  const paragraphWidthPt = Math.max(
    0,
    cellWidthPt - (paragraph.indentLeftPt ?? 0) - listIndentPt,
  );
  const lines = wrapRunsToWidth(effectiveRuns, measurer, paragraphWidthPt);
    ...
    for (const cell of row.cells) {
      ...
      for (const block of cell.blocks) {
        if (block.kind === "paragraph") {
          cellCursorYDown = layoutParagraphInCell(
            block,
            cellXDown,
            cellWidthPt,
            cellCursorYDown,
...
function paginateSection(
  ...
  const contentWidthPt = Math.max(
    0,
    section.pageSize.widthPt - section.margins.leftPt - section.margins.rightPt,
  );
  const contentBottomYDown =
    section.pageSize.heightPt - section.margins.bottomPt;
...
export function convertWordprocessingToLayout(
