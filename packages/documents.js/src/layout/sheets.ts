...
//
// Two real gaps in document-content-model's ContentSheetCellSchema (as of 1.2.0), confirmed by reading its full definition before writing this module: it carries no per-cell background colour and no per-cell alignment override at all (unlike ContentTableCellSchema.background for docx/pptx tables), and no border information of any kind. The seven-step z-order below therefore never emits a cell-background LayoutRect or a cell-border LayoutLine -- there is no field anywhere to source either from -- and "alignment when the cell doesn't specify one" always takes the value-type default below, since a cell can never specify one. Both are documented, tracked gaps, not silent omissions.
//
