...
describe("engine frames: wordprocessing (engine.ts)", () => {
  it("stamps one frame per wrapped fragment onto the originating run, in place on the caller's own content", () => {
    ...
    const doc = wordprocessingDoc([
      section([paragraph([run("Hello from docx", { sizePt: 10 })])], {
        pageSize: { widthPt: 50, heightPt: 50 },
      }),
    ]);
    const theRun = doc.sections[0]!.blocks[0]!;
    const { pages } = convertWordprocessingToLayout(doc, {
    ...
    expect(frames[1]!.xPt).toBeGreaterThanOrEqual(
      frames[0]!.xPt + frames[0]!.widthPt - 0.01,
    );
  });
  ...
  it("stamps distinct frames per fragment even when a run splits across a page boundary", () => {
    ...
    convertWordprocessingToLayout(doc, {
      measurer: createStandardFontMeasurer(),
      mathMetricsAt,
    });
    ...
    expect(
      new Set(frames.map((frame) => frame.pageIndex)).size,
    ).toBeGreaterThan(1);
  });
  ...
  it("stamps the cell node's frame and each in-cell run's own frames for a table", () => {
    ...
    expect(cell.frames).toEqual([
      { pageIndex: 0, xPt: 0, yPt: 50 - 20, widthPt: 100, heightPt: 20 },
    ]); // PDF space: y flipped about the 50pt page
  ...
  it("stamps an image block's frame at its placed position", () => {
    ...
    expect(block.frames).toEqual([
      { pageIndex: 0, xPt: 0, yPt: 50 - 20, widthPt: 20, heightPt: 20 },
    ]);
  });
});
...
describe("engine frames: presentation (slides.ts)", () => {
  it("stamps the shape's own placement and each run's fragment frames", () => {
    const s = shape({
      frame: { xPt: 50, yPt: 40, widthPt: 300, heightPt: 100 },
      blocks: [paragraph([run("Hi", { sizePt: 10 })])],
    });
    const doc = presentationDoc([slide([s])]);
    const { pages } = convertPresentationToLayout(doc, {
      measurer: createStandardFontMeasurer(),
      mathMetricsAt,
    });
...
describe("engine frames: spreadsheet (sheets.ts)", () => {
  it("stamps each populated cell's frame at its grid position", () => {
    const doc: Extract<ContentDocument, { kind: "spreadsheet" }> = {
      kind: "spreadsheet",
      metadata: {},
      sheets: [
        sheet([
    ...
    convertSpreadsheetToLayout(doc, {
      measurer: createStandardFontMeasurer(),
      mathMetricsAt,
    });
    const cell = doc.sheets[0]!.cells[0]!;
    expect(cell.frames).toEqual([
      { pageIndex: 0, xPt: 10, yPt: 300 - 10 - 20, widthPt: 60, heightPt: 20 },
    ]);
  });
});
...
describe("engine frames: drawing (drawing.ts)", () => {
  it("stamps each vector's frame from its emitted item, flipped into PDF space", () => {
    ...
    const { pages } = convertDrawingToLayout(doc, {
      measurer: createStandardFontMeasurer(),
    });
    expect(vector.frames).toEqual([
      { pageIndex: 0, xPt: 10, yPt: 300 - 20 - 40, widthPt: 30, heightPt: 40 },
    ]);
...
describe("reconstruct frames: wordprocessing (reconstruct.ts)", () => {
  it("attaches each reconstructed paragraph and run frames from the exact items they were clustered from", () => {
    ...
    const content = reconstructWordprocessing({
      formatVersion: 1,
      metadata: {},
...
describe("from-package inverse (from-package.ts)", () => {
  it("rebuilds a LayoutDocument whose pages match the package's own and whose text comes from the runs' frames", () => {
    const doc = wordprocessingDoc([
      section([paragraph([run("Hi", { sizePt: 10 })])]),
    ]);
    const { pages } = convertWordprocessingToLayout(doc, {
      measurer: createStandardFontMeasurer(),
      mathMetricsAt,
    ...
    expect(
      layout.pages.map(({ widthPt, heightPt }) => ({ widthPt, heightPt })),
    ).toEqual(pages);
  ...
  it("renders one frame per image placement and rebuilds vectors exactly through the drawing engine's own conversion", () => {
    ...
    const doc = drawingDoc([
      drawPage({
        ...
        shapes: [
          shape({
            blocks: [image],
            frame: { xPt: 100, yPt: 100, widthPt: 20, heightPt: 20 },
          }),
        ],
      }),
    ]);
    ...
    const rebuiltRect = rebuiltItems.find(
      (item): item is Extract<LayoutItem, { kind: "rect" }> =>
        item.kind === "rect",
    );
    expect(rebuiltRect).toMatchObject({
      xPt: 10,
      yPt: 300 - 20 - 40,
      widthPt: 30,
      heightPt: 40,
    ...
    const rebuiltImage = rebuiltItems.find(
  ...
  it("renders a spreadsheet cell's displayText at its own frame (single-line by construction)", () => {
    ...
    const { pages } = convertSpreadsheetToLayout(doc, {
      measurer: createStandardFontMeasurer(),
      mathMetricsAt,
    });
    const layout = layoutDocumentFromPackage(assembleTree(doc, pages));
