...
import type { ContentDrawPage, ContentShape } from "document-schema.js";
...
describe("reconstructPresentation: block clustering", () => {
  it("merges left-aligned, similarly-sized, closely-spaced lines into one shape with separate paragraphs", () => {
    const pg = page(960, 540, [
      text({ text: "Line one", xPt: 100, yPt: 400, widthPt: 60 }),
      text({ text: "Line two", xPt: 100, yPt: 386, widthPt: 60 }),
    ]); // gap 14, well within 1.25x a 12pt line
    ...
    expect(
      shapeList[0]!.blocks.map((b) =>
        b.kind === "paragraph" ? b.runs[0]!.text : undefined,
      ),
  ...
  it("keeps misaligned lines as separate shapes", () => {
    const pg = page(960, 540, [
      text({ text: "Line one", xPt: 100, yPt: 400, widthPt: 60 }),
      text({ text: "Line two", xPt: 300, yPt: 386, widthPt: 60 }),
    ]);
    const doc = reconstructPresentation(docFrom([pg]));
  ...
  it("splits a single line into separate shapes across a large horizontal gap", () => {
    const pg = page(960, 540, [
      text({ text: "Left", xPt: 50, yPt: 400, widthPt: 20 }),
      text({ text: "Right", xPt: 100, yPt: 400, widthPt: 20 }),
    ]); // gap 30 > 2*12
    const doc = reconstructPresentation(docFrom([pg]));
    expect(shapes(doc)).toHaveLength(2);
  });
  ...
  it("zeroes every inset", () => {
    const doc = reconstructPresentation(
      docFrom([
        page(960, 540, [text({ text: "X", xPt: 100, yPt: 400, widthPt: 20 })]),
      ]),
    );
    const [shape] = shapes(doc);
...
describe("reconstructPresentation: images", () => {
  it("places an image shape before text shapes in z-order", () => {
    const pg = page(960, 540, [
      text({ text: "Text", xPt: 100, yPt: 400, widthPt: 40 }),
      image({ imageId: "img1", xPt: 200, yPt: 200, widthPt: 80, heightPt: 60 }),
    ]);
    const doc = reconstructPresentation(
      docFrom([pg], {
...
describe("reconstruct: cancellation", () => {
  it("reconstructWordprocessing throws when the signal is already aborted", () => {
    ...
    controller.abort();
    expect(() =>
      reconstructWordprocessing(docFrom([page(612, 792, [])]), {
  ...
  it("reconstructPresentation throws when the signal is already aborted", () => {
    ...
    controller.abort();
    expect(() =>
      reconstructPresentation(docFrom([page(960, 540, [])]), {
        signal: controller.signal,
      }),
    ).toThrow();
  });
...
describe("reconstructDrawing: vector mapping", () => {
  it("maps a LayoutRect to a rect ContentVector via the exact flipY inverse", () => {
    ...
    expect(pg!.vectors).toEqual([
      {
        kind: "rect",
        frame: { xPt: 20, yPt: 230, widthPt: 100, heightPt: 40 },
        fill: RED,
        stroke: undefined,
        sourcePath: undefined,
        paintOrder: 0,
        frames: [
          { pageIndex: 0, xPt: 20, yPt: 30, widthPt: 100, heightPt: 40 },
        ],
      },
    ]); // yPt = 300 - 30 - 40; the vector's own frames carry the exact PDF-space box it was recovered from
  });
  ...
  it("maps a LayoutEllipse to an ellipse ContentVector via the exact flipY inverse", () => {
    ...
    expect(pg!.vectors).toEqual([
      {
        kind: "ellipse",
        frame: { xPt: 50, yPt: 220, widthPt: 80, heightPt: 20 },
        fill: undefined,
        stroke: { color: BLACK, widthPt: 1 },
        sourcePath: undefined,
        paintOrder: 0,
        frames: [{ pageIndex: 0, xPt: 50, yPt: 60, widthPt: 80, heightPt: 20 }],
      },
    ]); // yPt = 300 - 60 - 20
  });
    ...
    expect(pg!.vectors).toEqual([
      {
        ...
        sourcePath: undefined,
        paintOrder: 0,
        frames: [{ pageIndex: 0, xPt: 10, yPt: 20, widthPt: 80, heightPt: 40 }],
      },
    ]); // yPt = 300 - y1Pt/y2Pt; the line's own frame is the bounding box of its two endpoints
  });
      ...
      subpaths: [
        {
          ...
          startYPt: 250,
          closed: false,
          segments: [{ kind: "line", xPt: 150, yPt: 250 }],
        },
      ],
      fill: RED,
    };
    ...
    expect(pg!.vectors).toEqual([
      {
        ...
        subpaths: [
          {
            start: { xPt: 0, yPt: 0 },
            closed: false,
            segments: [{ kind: "line", to: { xPt: 100, yPt: 0 } }],
          },
        ],
        fill: RED,
        fillRule: undefined,
        ...
        paintOrder: 0,
        frames: [
          { pageIndex: 0, xPt: 50, yPt: 250, widthPt: 100, heightPt: 0 },
        ],
      },
    ]);
  });
...
describe("reconstructDrawing: shape mapping", () => {
  ...
  it("maps a LayoutImage item to a ContentShape wrapping a ContentImageBlock, reusing imageToShape unmodified", () => {
    ...
    const doc = reconstructDrawing(
      docFrom([page(400, 300, [item])], {
        img1: { format: "png", base64: "AAAA", widthPx: 10, heightPx: 5 },
      }),
    );
    const [pg] = drawPages(doc);
    expect(pg!.shapes).toHaveLength(1);
    expect(pg!.shapes[0]!.blocks[0]).toMatchObject({
      kind: "image",
      format: "png",
      base64: "AAAA",
    });
...
describe("reconstructDrawing: paint order and page structure", () => {
  it("buckets items into vectors/shapes, each array keeping its own items' relative document order", () => {
    ...
    const doc = reconstructDrawing(
      docFrom([page(400, 300, [rectA, textItem, rectB, imageItem])], {
        img1: { format: "png", base64: "AAAA", widthPx: 10, heightPx: 5 },
      }),
    );
    const [pg] = drawPages(doc);
