 ...
 */
export function resolveBinDispatch(argv: readonly string[], userAgent: string | undefined): BinDispatch {
  const head = argv[0];
  const pkg: 'document-cli' | 'document-mcp' = head === 'mcp' ? 'document-mcp' : 'document-cli';
