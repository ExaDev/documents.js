...
import { HTML_PREFORMATTED_STYLE_ID, readMarkdownContent as readMarkdownFlat } from 'markdown-codec';
...
export function readMarkdownContent(text: string, options?: ReadMarkdownOptions, math?: MarkdownMathLoweringOptions): ContentDocument {
  const { document } = readMarkdownFlat(text, { frontMatter: true, ...options });
...
type WordprocessingContent = Extract<ContentDocument, { kind: 'wordprocessing' }>;

function promotePageBreakMarkers(document: WordprocessingContent): WordprocessingContent {
  return { ...document, sections: document.sections.map((section) => ({ ...section, blocks: section.blocks.map(promoteBlock) })) };
}

function promoteBlock(block: ContentBlock): ContentBlock {
  if (block.kind !== 'paragraph' || block.styleId !== HTML_PREFORMATTED_STYLE_ID) {
    return block;
  }
  return block.runs.map((run) => run.text).join('') === PAGE_BREAK_MARKER ? { kind: 'pageBreak' } : block;
}
