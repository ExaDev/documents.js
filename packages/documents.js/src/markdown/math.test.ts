...
describe("readMarkdownContent math lowering", () => {
  it("lowers a $$ display block into an embedded formula block carrying presentation, content, MathML, and provenance", () => {
    ...
    const formulaBlock = content.sections[0]?.blocks.find(
      (block) => block.kind === "embeddedObject",
    );
    if (
      formulaBlock?.kind !== "embeddedObject" ||
      formulaBlock.document.kind !== "formula"
    ) {
      throw new Error("expected an embedded formula block");
    }
    const formula = formulaBlock.document.formula;
    expect(formula.presentation).toEqual({
      latex: "\\sum_{i=1}^{n} \\frac{1}{i^2}",
    });
    ...
    expect(formula.provenance).toEqual({
      source: "markdown:math-block",
      editTrail: [],
    });
  ...
  it("seeds the document symbol table from prose, and a formula referencing the prose-defined glyph resolves to the curated entry", () => {
    ...
    expect(curated).toEqual({
      ...
      scope: "document",
      id: "symbols:R",
      definitionSource: "Where R is the resistance per unit length.",
    });
    const formulaBlock = content.sections[0]?.blocks.find(
      (block) => block.kind === "embeddedObject",
    );
    ...
    expect(formulaBlock.document.formula.content).toEqual({
      kind: "app",
      operator: "math:pow",
      args: [
        { kind: "sym", id: "symbols:R" },
        { kind: "num", numerator: "2", denominator: "1" },
      ],
    });
  });
  ...
  it("an inline \\( \\) span leaves its paragraph and follows it as a formula block", () => {
    ...
    expect(
      paragraph?.kind === "paragraph"
        ...
        : undefined,
    ).toEqual(["An inline span ", " mid-sentence."]);
    ...
    expect(formulaBlock.document.formula.presentation).toEqual({
      latex: "x^2 + 1",
    });
    expect(formulaBlock.document.formula.provenance).toEqual({
      source: "markdown:math-inline",
      editTrail: [],
    });
  });
  ...
  it("a context-starved construct degrades to unparsed inside the formula and surfaces its diagnostic through the sink", () => {
    ...
    expect(
      diagnostics.some(
        (diagnostic) => diagnostic.code === "latex/juxtaposition-unparsed",
      ),
    ).toBe(true);
  });
  ...
  it("math inside a table cell is lowered too", () => {
    ...
    const table = content.sections[0]?.blocks.find(
