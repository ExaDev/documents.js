...
// A LaTeX symbol command to its single written glyph, the form the symbol table's own `glyph` field expects ("the written form as it appears in presentation"). temml hands symbol commands through as mathord/textord nodes whose text is the raw command ('\\alpha', '\\infty'); the table keys on what a reader actually sees, so the map is to the Unicode character. Greek (both cases, plus the variant letters), the letterlike symbols, and the few common blackboard/structural glyphs engineers actually write — anything outside the map degrades to an `unparsed` node rather than entering the table under a command name no human wrote. A function rather than a module-level constant, deliberately: Stryker classifies a top-level object literal's own string values as "static" mutants, evaluated once when the module first loads, and picks exactly one arbitrary covering test out of the whole suite to check a static mutant rather than every test that actually exercises it. Confirmed directly against this file: with the table as a plain const, every one of its 49 glyph values survived under Stryker despite glyphOfSymbolText's own it.each test asserting each one individually and correctly failing when the value is hand-mutated and that one test file is run in isolation. Returning a fresh object from a function each call makes every glyph lookup a genuine per-test-covered runtime value instead of a load-once static one, and Stryker's coverage analysis then attributes each mutation to the specific test that actually exercises it.
function commandGlyphs(): Readonly<Record<string, string>> {
  return {
    "\\alpha": "α",
    "\\beta": "β",
    "\\gamma": "γ",
...
// The shape a prose-defined symbol's written form may take: one Latin or Greek letter, optionally with an underscore subscript run ("R", "m_e", "x_1", "α"). A whole word ("where the resistance is...") does not match — multi-letter runs are words, not symbols, and excluding them is most of what keeps this scanner conservative. A function rather than a module-level constant, for the identical reason commandGlyphs above is one: Stryker treats a top-level regex literal as a "static" mutant and samples one arbitrary covering test from the whole suite rather than the tests that actually assert on this pattern's own shape.
function proseSymbolPattern(): RegExp {
  return /[A-Za-zΑ-ω](?:_[A-Za-z0-9]+)?/;
}

// Sentence-level definition patterns, the two forms technical prose actually writes: "where R is the resistance per unit length" and "let x be the voltage". The verb set is deliberately small (is/are/be/denotes/denote/represents/stands for) — "where R varies..." is not a definition, and matching it would mint a wrong quantity identity, which is precisely the failure precision-over-recall is here to avoid. Matches are case-insensitive on the keyword and verb only; the symbol itself is case-sensitive because R and r are different quantities. A function for the same static-mutant reason as proseSymbolPattern above.
function proseDefinitionPatterns(): readonly RegExp[] {
  return [
...
function proseDefinitionIn(sentence: string): MathSymbolEntry | undefined {
  for (const pattern of proseDefinitionPatterns()) {
    const match = pattern.exec(sentence);
    const candidate = match?.[1];
    if (candidate === undefined) {
      continue;
    }
    if (proseSymbolPattern().exec(candidate)?.[0] !== candidate) {
      continue;
    }
    return {
      glyph: candidate,
