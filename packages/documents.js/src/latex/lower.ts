...
type TermItem =
  | { readonly kind: "node"; readonly node: TemmlNode }
  | {
...
function lowerNodeList(
  ...
  const present = nodes.filter(
    (node) => !PRESENTATION_ONLY_TYPES.has(node.type),
  );
  ...
  switch (node.type) {
    case "mathord": {
      ...
      return glyph === undefined
        ? degradeNode(node, context)
        : symbolExpression(glyph, context);
    }
    case "textord": {
      const text = nodeText(node);
      if (text === undefined) {
...
function lowerGenfrac(
  ...
  const delimited =
    (node.leftDelim !== null && node.leftDelim !== undefined) ||
    (node.rightDelim !== null && node.rightDelim !== undefined);
  if (
    !hasBar ||
    delimited ||
    !isTemmlNode(node.numer) ||
    !isTemmlNode(node.denom)
  ) {
    const detail = spanOfNodes(context, [node]);
    diagnose(context, "latex/genfrac-unparsed", detail);
    return unparsed(detail);
  }
  return app("math:divide", [
    lowerNode(node.numer, context),
    lowerNode(node.denom, context),
  ]);
}
...
function lowerArray(node: TemmlNode, context: LoweringContext): MathExpression {
  const separated =
    ...
    node.cols.some((col) => !isTemmlNode(col) || col.type === "separator");
  const body = node.body;
...
function lowerSupsub(
  ...
  if (sub !== undefined) {
    ...
    const baseGlyph =
      base !== undefined && (base.type === "mathord" || base.type === "textord")
        ? glyphOfNode(base)
        : undefined;
    ...
    return app("math:pow", [
      symbolExpression(subscriptedGlyph, context),
      lowerNodeList(sup, context),
    ]);
  }
  if (sup === undefined) {
    return degradeNode(node, context);
...
function scriptWrittenForm(
  ...
  return nodes
    .map((node) => glyphOfNode(node) ?? nodeText(node) ?? "")
    .join("");
}
...
function readBinder(node: TemmlNode, context: LoweringContext): BinderRead {
  ...
  if ((sub?.length ?? 0) >= 3) {
    ...
    const firstGlyph =
      first?.type === "mathord" ? glyphOfNode(first) : undefined;
    if (
      firstGlyph !== undefined &&
      relation?.type === "atom" &&
      nodeText(relation) === "="
    ) {
      return {
...
export function lowerLatex(
  latex: string,
  options?: LowerLatexOptions,
): LatexLoweringResult {
  return lowerParsed(latex, parseLatex(latex), options);
}

function lowerParsed(
  latex: string,
  parsed: LatexParseResult,
  options?: LowerLatexOptions,
): LatexLoweringResult {
  if (latex.trim() === "") {
...
export function latexToFormula(
  ...
  const mathml: MathMlNode[] =
    parsed.status === "parsed" ? [...parsed.mathml] : [];
