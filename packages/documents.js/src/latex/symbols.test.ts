...
describe("extractSymbolDefinitionsFromProse", () => {
    ...
    expect(entries).toEqual([
      {
        ...
        id: "symbols:R",
        definitionSource:
          "Where R is the resistance per unit length, the loss grows.",
      },
    ]);
    expect(diagnostics).toEqual([
      {
        ...
        detail:
          '"R" from: Where R is the resistance per unit length, the loss grows.',
      },
    ]);
  });
  ...
  it('seeds from "let x be the voltage" and Greek-letter subjects', () => {
    const document = wordprocessing([
      "Let x be the voltage across the load. Let α denote the attenuation constant.",
    ]);
  ...
  it("seeds an underscore-subscripted glyph whole (m_e), the table's own written-form convention", () => {
    const entries = extractSymbolDefinitionsFromProse(
      wordprocessing(["where m_e is the electron mass"]),
    );
  ...
  it("declines definitions buried mid-sentence without a where/let head", () => {
    expect(
      extractSymbolDefinitionsFromProse(
        wordprocessing(["The quantity R is the resistance per unit length."]),
      ),
    ).toEqual([]);
  });
  ...
  it("scans every section's paragraphs and ignores non-wordprocessing arms entirely", () => {
      ...
      sections: [
        {
          ...
          margins: { topPt: 20, rightPt: 20, bottomPt: 20, leftPt: 20 },
          blocks: [
            { kind: "paragraph", runs: [{ text: "where a is one thing" }] },
          ],
        },
        {
          pageSize: { widthPt: 595, heightPt: 842 },
          margins: { topPt: 20, rightPt: 20, bottomPt: 20, leftPt: 20 },
          blocks: [
            { kind: "paragraph", runs: [{ text: "unrelated prose" }] },
            { kind: "pageBreak" },
          ],
        },
      ],
    ...
    expect(
      extractSymbolDefinitionsFromProse(multiSection).map(
        (entry) => entry.glyph,
      ),
    ).toEqual(["a"]);
    const formula: ContentDocument = {
      kind: "formula",
      metadata: {},
      formula: { mathml: [] },
    };
