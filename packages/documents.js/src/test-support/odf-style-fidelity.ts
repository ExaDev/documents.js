...
export function assertAutomaticStylesOnlyAppended(before: Package, after: Package): void {
  const beforeEntries = (findAutomaticStyles(before)?.children ?? []).map((n) => buildXml([n]));
  const afterEntries = (findAutomaticStyles(after)?.children ?? []).map((n) => buildXml([n]));
