...
// The object numbers catalogPagesPageFontObjects always assigns, and the conventional "next" object every fixture built on top of it uses for its own first extra object (typically the content stream).
const CATALOG_OBJ = 1;
const PAGES_OBJ = 2;
const PAGE_OBJ = 3;
const FONT_OBJ = 4;
...
// ISO 32000-1 7.5.4 mandates a fixed 10-digit offset field in a classic cross-reference table entry.
const XREF_OFFSET_DIGIT_WIDTH = 10;
...
class FixtureBuilder {
  ...
  classicXrefAndTrailer(maxObjNum: number, trailerExtra: string): this {
    ...
    for (let n = 1; n <= maxObjNum; n++) {
      this.writer.writeAscii(
        `${this.offsetOf(n).toString().padStart(XREF_OFFSET_DIGIT_WIDTH, "0")} 00000 n \n`,
      );
    }
    this.writer.writeAscii(
...
function catalogPagesPageFontObjects(
  ...
  b.object(CATALOG_OBJ, "<< /Type /Catalog /Pages 2 0 R >>");
  b.object(PAGES_OBJ, "<< /Type /Pages /Kids [3 0 R] /Count 1 >>");
  b.object(
    PAGE_OBJ,
    `<< /Type /Page /Parent 2 0 R /MediaBox ${mediaBox} /Resources << /Font << /F1 4 0 R >> >> /Contents ${contentObjNum} 0 R ${extraPageEntries}>>`,
  );
  b.object(FONT_OBJ, "<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>");
}
...
export function minimalClassicXrefPdf(): Uint8Array<ArrayBuffer> {
  ...
  b.stream(CONTENT_OBJ, "<< >>", enc(HELLO_CONTENT));
...
export function xrefStreamWithObjectStreamPdf(): Uint8Array<ArrayBuffer> {
  ...
  const entries: readonly { readonly num: number; readonly body: string }[] = [
    { num: CATALOG_OBJ, body: catalogBody },
    { num: PAGES_OBJ, body: pagesBody },
    { num: PAGE_OBJ, body: pageBody },
  ];
  let objectData = "";
  ...
  // Type 2 (compressed) rows store the containing ObjStm's object number and the index within it; type 1 (uncompressed) rows store a plain byte offset and generation.
  const rows: number[][] = [
    [0, 0, 0, 0, 0, BYTE_MASK, BYTE_MASK], // object 0: the conventional free-list head, generation 65535
  ...
  const contentOffset = b.offsetOf(CONTENT_OBJ);
  ...
  const xrefOffset = b.length; // the xref stream object starts here, matching what stream(XREF_STREAM_OBJ, ...) is about to record
  rows[xrefOffsetPlaceholderIndex] = [1, ...be4(xrefOffset), 0, 0];
  const xrefRows = new Uint8Array(rows.length * XREF_STREAM_ROW_WIDTH);
  rows.forEach((row, i) => {
    xrefRows.set(row, i * XREF_STREAM_ROW_WIDTH);
  });
...
function be4(n: number): [number, number, number, number] {
  return [
    (n >>> BYTE_3_SHIFT) & BYTE_MASK,
    (n >>> BYTE_2_SHIFT) & BYTE_MASK,
    (n >>> BYTE_1_SHIFT) & BYTE_MASK,
    n & BYTE_MASK,
  ];
}
...
export function incrementalUpdatePdf(): Uint8Array<ArrayBuffer> {
  ...
  for (let n = 1; n <= CONTENT_OBJ; n++) {
    b.raw(
      `${b.offsetOf(n).toString().padStart(XREF_OFFSET_DIGIT_WIDTH, "0")} 00000 n \n`,
    );
  }
  b.raw("trailer\n<< /Size 6 /Root 1 0 R >>\nstartxref\n");
  ...
  b.raw("xref\n3 1\n");
  b.raw(
    `${b.offsetOf(PAGE_OBJ).toString().padStart(XREF_OFFSET_DIGIT_WIDTH, "0")} 00000 n \n`,
  );
  b.raw(
...
// The Info dict / Encrypt dict / Form XObject / second content stream object each fixture below adds on top of the standard Catalog/Pages/Page/Font/Content quintet.
const EXTRA_OBJ = 6;
...
export function encryptedPdf(): Uint8Array<ArrayBuffer> {
  ...
  b.classicXrefAndTrailer(EXTRA_OBJ, "/Root 1 0 R /Encrypt 6 0 R");
  return b.bytes();
}
...
export function formXObjectPdf(): Uint8Array<ArrayBuffer> {
  ...
  b.stream(CONTENT_OBJ, "<< >>", enc(pageContent));
  ...
  b.classicXrefAndTrailer(EXTRA_OBJ, "/Root 1 0 R");
  return b.bytes();
}
...
export function inlineImagePdf(): Uint8Array<ArrayBuffer> {
  ...
  b.stream(CONTENT_OBJ, "<< >>", writer.toBytes());
  b.classicXrefAndTrailer(CONTENT_OBJ, "/Root 1 0 R");
  return b.bytes();
}
...
// This fixture has two Page objects before its Font object, pushing every later object one slot past the standard layout: PAGE_OBJ is the first page only, and FONT_OBJ/CONTENT_OBJ/EXTRA_OBJ are reused below to number its second page, font, and content stream instead.
export function inheritedPageAttributesPdf(): Uint8Array<ArrayBuffer> {
  ...
  b.stream(EXTRA_OBJ, "<< >>", enc(HELLO_CONTENT)); // content
  b.classicXrefAndTrailer(EXTRA_OBJ, "/Root 1 0 R");
  return b.bytes();
}

// The UTF-16BE title string's hex-digit field width (each UTF-16 code unit renders as exactly 4 hex digits).
...
export function withInfoDictPdf(): Uint8Array<ArrayBuffer> {
  ...
  const titleHex = `feff${Array.from("Test Doc")
    .map((ch) =>
      ch.charCodeAt(0).toString(HEX_RADIX).padStart(UTF16_HEX_DIGIT_WIDTH, "0"),
    )
    .join("")}`;
  b.object(
    EXTRA_OBJ,
  ...
  b.classicXrefAndTrailer(EXTRA_OBJ, "/Root 1 0 R /Info 6 0 R");
  return b.bytes();
}
...
// The second content stream this fixture adds beyond EXTRA_OBJ (its first page's own content).
const SECOND_CONTENT_OBJ = 7;

// As in inheritedPageAttributesPdf, two Page objects before the Font object push everything past PAGE_OBJ one slot later than the standard layout: FONT_OBJ/CONTENT_OBJ/EXTRA_OBJ number its second page, font, and first content stream.
export function twoPageMultiTextPdf(): Uint8Array<ArrayBuffer> {
  const b = new FixtureBuilder().header("1.4");
  b.object(CATALOG_OBJ, "<< /Type /Catalog /Pages 2 0 R >>");
  b.object(PAGES_OBJ, "<< /Type /Pages /Kids [3 0 R 4 0 R] /Count 2 >>");
  b.object(
    PAGE_OBJ,
  ...
  b.stream(
    EXTRA_OBJ, // first page's content
    "<< >>",
    enc("BT /F1 12 Tf 10 50 Td (PageZero) Tj ET"),
  );
  b.stream(
    SECOND_CONTENT_OBJ,
    "<< >>",
    enc(
      "BT /F1 12 Tf 10 50 Td (First) Tj ET BT /F1 12 Tf 10 90 Td (Second) Tj ET",
    ),
  );
  b.classicXrefAndTrailer(SECOND_CONTENT_OBJ, "/Root 1 0 R");
  return b.bytes();
}
