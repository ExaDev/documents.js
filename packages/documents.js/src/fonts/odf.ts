...
function styleFromFontBytes(bytes: Uint8Array<ArrayBuffer>): { readonly bold: boolean; readonly italic: boolean } | undefined {
  ...
  return { bold: (os2.fsSelection & FS_SELECTION_BOLD) !== 0, italic: (os2.fsSelection & FS_SELECTION_ITALIC) !== 0 };
}

function fontPartBytes(pkg: Package, href: string): Uint8Array<ArrayBuffer> {
  const part = pkg.parts[href];
  if (part === undefined) {
    throw new OdfEmbeddedFontError(`svg:font-face-uri references ${JSON.stringify(href)}, which is not a part of this package`);
  }
  if (part.kind !== 'binary') {
    throw new OdfEmbeddedFontError(`embedded font part ${JSON.stringify(href)} is an XML part, not binary font data`);
  }
  return base64ToBytes(part.base64);
}

function collectFontFace(pkg: Package, fontFace: XmlElement, seenHrefs: Set<string>, out: ProvidedFont[]): void {
  ...
  for (const src of childrenWithTag(fontFace, 'svg:font-face-src')) {
    for (const uri of childrenWithTag(src, 'svg:font-face-uri')) {
      ...
      if (rawHref === undefined) {
        throw new OdfEmbeddedFontError('a <svg:font-face-uri> carries no xlink:href');
      }
      ...
      if (declaredFamily === undefined) {
        throw new OdfEmbeddedFontError(`the <style:font-face> embedding ${JSON.stringify(href)} declares neither svg:font-family nor style:name`);
      }
      const bytes = fontPartBytes(pkg, href);
      if (!looksLikeSfnt(bytes)) {
        throw new OdfEmbeddedFontError(`embedded font part ${JSON.stringify(href)} does not begin with a recognisable sfnt signature`);
      }
      ...
      out.push({
        ...
        bold: (declaredWeight === undefined ? undefined : boldFromWeightAttribute(declaredWeight)) ?? intrinsic?.bold ?? false,
        italic: (declaredStyle === undefined ? undefined : italicFromStyleAttribute(declaredStyle)) ?? intrinsic?.italic ?? false,
        bytes,
      });
    }
  }
