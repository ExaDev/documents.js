...
describe("extractSourceFonts", () => {
  it("routes a docx package to the OOXML extractor", () => {
    expect(
      extractSourceFonts({
        ...
        (font) => `${font.family}/${String(font.bold)}/${String(font.italic)}`,
      ),
    ).toEqual(["Caladea/false/false", "Caladea/true/false"]);
  });
  ...
  it("routes a pptx package to the OOXML extractor", () => {
    ...
    ).toEqual(["Caladea/false/false", "Caladea/false/true"]);
  });
...
describe("createDocumentFontRegistry", () => {
  it("resolves a family the source package embedded to the embedded face itself", () => {
    ...
    expect(
      resolved.kind === "embedded" ? resolved.face.postScriptName : undefined,
    ).toBe("Caladea-Regular");
  });
  ...
  it("resolves each embedded face separately by weight", () => {
    ...
    const bold = registry.resolve({
      family: "Caladea",
      weight: "bold",
      style: "normal",
    });
    expect(
      bold.kind === "embedded" ? bold.face.postScriptName : undefined,
    ).toBe("Caladea-Bold");
  });
  ...
  it("prefers the source package own face over a caller-supplied face for the same slot", () => {
    const registry = createDocumentFontRegistry(
      ...
      { fonts: [providedFont("Caladea", false, false, caladeaItalicBytes())] },
    );
    const resolved = registry.resolve({
      family: "Caladea",
      weight: "normal",
  ...
  it("falls back to a caller-supplied face for a family the source package did not embed", () => {
    const registry = createDocumentFontRegistry(
      { kind: "docx", package: embeddedFontDocxPackage() },
      { fonts: [providedFont("Bookish", false, true, caladeaItalicBytes())] },
    );
    const resolved = registry.resolve({
      family: "Bookish",
      weight: "normal",
  ...
  it("produces a working registry for a package that embeds no fonts at all", () => {
    ...
    ).toEqual({ kind: "standard", standardName: "Helvetica", matched: true });
  });
  ...
  it("resolves an ODF-sourced face identically to its OOXML-sourced equivalent", () => {
    const odf = createDocumentFontRegistry({
      kind: "odf",
      package: embeddedFontOdtPackage(),
    }).resolve({ family: "Caladea", weight: "bold", style: "normal" });
    const ooxml = createDocumentFontRegistry({
      kind: "docx",
      package: embeddedFontDocxPackage(),
    }).resolve({ family: "Caladea", weight: "bold", style: "normal" });
    expect(odf.kind === "embedded" ? odf.face.postScriptName : undefined).toBe(
      "Caladea-Bold",
    );
    expect(
      ooxml.kind === "embedded" ? ooxml.face.postScriptName : undefined,
    ).toBe("Caladea-Bold");
  });
});
...
describe("a cmap miss on a source-embedded face", () => {
  it("reports per character and still writes a PDF", () => {
    ...
    const bytes = writePdf(
      textPage(`before ${UNMAPPED_CHARACTER} after`, "Caladea"),
      {
        fonts: createDocumentFontRegistry({
          kind: "docx",
          package: embeddedFontDocxPackage(),
        }),
  ...
  it("does not throw when no onMissingGlyph callback is supplied at all", () => {
    expect(() =>
      writePdf(textPage(UNMAPPED_CHARACTER, "Caladea"), {
        fonts: createDocumentFontRegistry({
          kind: "odf",
          package: embeddedFontOdtPackage(),
        }),
  ...
  it("still embeds the real source face rather than falling back to a substitute for the whole run", () => {
    ...
    expect(
      resolved.kind === "embedded"
        ? resolved.face.glyphId(UNMAPPED_CHARACTER.codePointAt(0) ?? 0)
        : 0,
    ).toBeUndefined();
    expect(
      resolved.kind === "embedded"
