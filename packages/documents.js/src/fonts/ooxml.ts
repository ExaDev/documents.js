...
const DOCX_EMBED_ELEMENTS: readonly (readonly [tag: string, bold: boolean, italic: boolean])[] = [
...
const PPTX_EMBED_ELEMENTS: readonly (readonly [tag: string, bold: boolean, italic: boolean])[] = [
...
function relatedPartPath(pkg: Package, fromPartPath: string, relationshipType: string): string | undefined {
  for (const relationship of resolveRelationships(pkg, fromPartPath).values()) {
    if (relationship.type === relationshipType && relationship.targetMode === undefined) {
...
function officeDocumentPartPath(pkg: Package): string {
  ...
  if (rels !== undefined) {
    for (const relationship of childrenWithTag(rels, 'Relationship')) {
      if (attr(relationship, 'Type') !== OFFICE_DOCUMENT_RELATIONSHIP || attr(relationship, 'TargetMode') !== undefined) {
        continue;
      }
...
function fontPartBytes(pkg: Package, fromPartPath: string, relationships: ReadonlyMap<string, Relationship>, relationshipId: string): Uint8Array<ArrayBuffer> {
  const relationship = relationships.get(relationshipId);
  if (relationship === undefined) {
    throw new OoxmlEmbeddedFontError(`${fromPartPath} references relationship id ${JSON.stringify(relationshipId)}, which its .rels part does not declare`);
  }
  const part = pkg.parts[relationship.target];
  if (part === undefined) {
    throw new OoxmlEmbeddedFontError(`relationship id ${JSON.stringify(relationshipId)} targets ${JSON.stringify(relationship.target)}, which is not a part of this package`);
  }
  if (part.kind !== 'binary') {
    throw new OoxmlEmbeddedFontError(`embedded font part ${JSON.stringify(relationship.target)} is an XML part, not binary font data`);
  }
...
function collectFaces(
  ...
  fontElementChildren: readonly (readonly [tag: string, bold: boolean, italic: boolean])[],
  ...
  for (const [tag, bold, italic] of fontElementChildren) {
    for (const embed of childrenWithTag(fontElement, tag)) {
      ...
      if (relationshipId === undefined) {
        throw new OoxmlEmbeddedFontError(`<${tag}> for font ${JSON.stringify(family)} carries no r:id`);
      }
      const fontKey = fontKeyAttribute === undefined ? undefined : attr(embed, fontKeyAttribute);
      out.push({ family, bold, italic, bytes: deobfuscateEmbeddedFont(fontPartBytes(pkg, declaringPartPath, relationships, relationshipId), fontKey) });
    }
  }
}
...
function extractDocxFonts(pkg: Package): ProvidedFont[] {
  ...
  const fontTablePath = relatedPartPath(pkg, documentPartPath, FONT_TABLE_RELATIONSHIP);
  if (fontTablePath === undefined) {
    return [];
  }
  const fontTablePart = pkg.parts[fontTablePath];
  if (fontTablePart?.kind !== 'xml') {
    throw new OoxmlEmbeddedFontError(`font table part ${JSON.stringify(fontTablePath)} is missing or is not an XML part`);
  }
  ...
  for (const font of childrenWithTag(fonts, 'w:font')) {
    ...
    if (family === undefined) {
      throw new OoxmlEmbeddedFontError(`a <w:font> in ${JSON.stringify(fontTablePath)} carries no w:name`);
    }
...
function extractPptxFonts(pkg: Package): ProvidedFont[] {
  ...
  if (presentationPart?.kind !== 'xml') {
    throw new OoxmlEmbeddedFontError(`presentation part ${JSON.stringify(presentationPartPath)} is missing or is not an XML part`);
  }
  ...
  for (const list of childrenWithTag(presentation, 'p:embeddedFontLst')) {
    for (const embeddedFont of childrenWithTag(list, 'p:embeddedFont')) {
      ...
      if (fontElement === undefined) {
        throw new OoxmlEmbeddedFontError(`a <p:embeddedFont> in ${JSON.stringify(presentationPartPath)} carries no <p:font>`);
      }
      const family = attr(fontElement, 'typeface');
      if (family === undefined) {
        throw new OoxmlEmbeddedFontError(`a <p:font> in ${JSON.stringify(presentationPartPath)} carries no typeface`);
      }
      // Entities raw in the model, same as docx's w:name above.
      collectFaces(pkg, presentationPartPath, relationships, PPTX_EMBED_ELEMENTS, embeddedFont, decodeEntities(family), undefined, out);
    }
  }
  return out;
}
...
export function extractOoxmlEmbeddedFonts(pkg: Package, kind: 'docx' | 'pptx'): ProvidedFont[] {
