// documents.js's public surface: bidirectional conversion among docx/pptx/xlsx/odt/odp/ods/odg/markdown/epub/csv/svg and PDF (every content format's own PDF round trip plus the cross-format bridges), a read+write live-view editor for docx/pptx/odt/odp/ods/odg, and a hand-written PDF codec, built on ooxml.js's lossless OOXML core, odf.js's lossless ODF core, markdown-codec, and epub-codec.
...
// epub <-> ContentDocument -- like xlsx above, epub-codec's own readEpubContent/writeEpubContent already read/write a real wordprocessing ContentDocument directly (bytes in, bytes out -- no decode/encode split at all, unlike csv/svg's own text-boundary pipelines above, since epub's OCF/ZIP layer is entirely internal to epub-codec), so they're re-exported as-is here rather than behind a documents.js-local wrapper: the same read/build pair the epub<->docx/odt/markdown same-variant bridge (src/convert/composition.ts's own BytesFormatNode entry) and the epub codec entry (src/codecs/registry.ts) already use internally, now reachable without going through either.
