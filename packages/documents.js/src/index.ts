...
// Magic-byte-validated Uint8Array schemas, so a caller passing the wrong format -- to these functions directly, or as the input/output schema half of a z.codec() below -- gets a clear Zod validation error instead of a confusing failure three layers down. The Odt/Ods/Odp/Odg schemas check the package's actual declared media type (see src/model/bytes.ts), a stronger check than Docx/PptxBytesSchema's generic ZIP-signature check. MarkdownBytesSchema and CsvBytesSchema are architecturally different from every other schema here: each checks only that the bytes are text in some character encoding, since markdown and csv are plain text with no magic bytes or format-level header of their own to check (see src/model/bytes.ts's own comment). Neither is a format check, and neither ever was: which of markdown, csv, rtf or anything else a file is read as comes from its extension or an explicit format argument, never from trying these schemas in turn. SvgBytesSchema sits in between: svg is plain text too, but it has a recognisable root element, so its schema additionally requires the decoded text to contain an <svg tag; unlike markdown/csv it does not detect the encoding from the bytes, since SVG is XML and XML's own <?xml ... encoding="..."?> declaration is authoritative (src/svg/xml-encoding.ts), with UTF-8 the strict default when neither that declaration nor a byte order mark is present.
export {
  CsvBytesSchema,
  DocBytesSchema,
  DocxBytesSchema,
...
export type {
  SlideImageInit as OdpSlideImageInit,
  SlideTableInit as OdpSlideTableInit,
  TextBoxInit as OdpTextBoxInit,
} from "./edit/odp/slide";
...
export type {
  PageImageInit as OdgPageImageInit,
  TextBoxInit as OdgTextBoxInit,
} from "./edit/odg/page";
...
export type {
  BoxVectorInit as OdgBoxVectorInit,
  LineVectorInit as OdgLineVectorInit,
  OdgVector,
  OdgVectorKind,
  PathVectorInit as OdgPathVectorInit,
} from "./edit/odg/vector";
...
export {
  ...
  looksLikeSfnt,
} from "./fonts/obfuscation";
export {
  readFontFace as describeFontFace,
  FontFaceParseError,
} from "pdf-codec";
...
export {
  ...
  isMathMlElement,
  localName,
  textContent as mathMlTextContent,
} from "./mathml/nodes";
...
export {
  buildXlsxPackageFromContent as buildXlsxPackage,
  readXlsxContent,
} from "ooxml.js";
...
export {
  ...
  CsvUnsupportedDocumentKindError,
} from "./csv/write";
export { CsvParseError } from "./csv/records";
// svg <-> ContentDocument -- the fifth adapter family, sharing the drawing variant with odg: decodeSvgText/encodeSvgText are the byte<->text boundary, decodeSvgText reading a byte order mark or the XML prolog's own <?xml ... encoding="..."?> declaration (src/svg/xml-encoding.ts) rather than detecting the encoding from the bytes the way csv/markdown now do, since SVG is XML and XML's own declaration is authoritative, with UTF-8 the strict default when neither is present; readSvgContent maps the six SVG shape primitives (rect/circle/ellipse/line/polyline/polygon/path) onto a one-page drawing ContentDocument, and buildSvgText writes them back out. The errors are exported beside them so a caller composing the stages directly can branch on them by class: SvgUnsupportedEncodingError (a declared encoding outside byte-codec's own bounded decodeText set, ExaDev/documents.js#1361), SvgUndecodableTextError (bytes contradicting the encoding that settled them), SvgMissingRootElementError (text with no <svg root), SvgUnsupportedDocumentKindError (a non-drawing ContentDocument), and the multi-page selection pair SvgMultiPageNotSpecifiedError/SvgPageNotFoundError -- the identical contract buildCsvText holds for sheets, carried by page index because drawing pages are anonymous. SVG_DIAGNOSTIC_CODES is the full vocabulary of scope limits and bounded approximations both directions report through onSvgDiagnostic (text, gradients, images, CSS, and <use> degrade under a diagnostic, never silently).
export {
  decodeSvgText,
  encodeSvgText,
...
// ppt is the one legacy binary format here with a genuine documents.js-local wrapper, for the identical reason readPptxContent/readOdpContent above have one: ppt-codec's own readPptContent/writePptContent operate on the flat { metadata, slides } shape ooxml.js's and odf.js's own upstream flat readers use, not a full 'presentation'-kind ContentDocument envelope directly -- src/ppt/read.ts and src/ppt/write.ts do nothing but that envelope wrap/unwrap, mirroring src/ooxml/pptx/read.ts and src/odf/odp/read.ts exactly (minus their own formula/vector-recovery passes, which ppt-codec's reader has no upstream equivalent for -- see src/ppt/read.ts's own comment). ppt-codec's own flat readPptContent/writePptContent are deliberately NOT re-exported here, for the same "don't expose both the wrapper and the thing it wraps" reason readPptxContent/readOdpContent's own top-of-file note gives.
export { readPptContent } from "./ppt/read";
export { writePptContent } from "./ppt/write";
...
export type {
  CellTypeDeclineReason,
  CellTypeInference,
  CellTypeInferenceResult,
  CellTypeInferenceSink,
  CellTypeRule,
} from "./layout/cell-typing";
...
export {
  displayTextFor as hsqldbCellDisplayText,
...
export type {
  OdbReportToDocxOptions,
  OdbReportToOdtOptions,
} from "./convert/convert";
export {
  odbReportToDocx,
  odbReportToOdt,
...
export type {
  UnifiedConversionOptions,
  ConversionPlan,
  CompositionHop,
} from "./convert/composition";
