...
const ZIP_LOCAL_FILE_HEADER_MAGIC = Array.from("PK\u0003\u0004", (c) =>
  c.charCodeAt(0),
);
const ZIP_END_OF_CENTRAL_DIRECTORY_MAGIC = Array.from("PK\u0005\u0006", (c) =>
  c.charCodeAt(0),
);
...
export function isZipArchive(bytes: Uint8Array): boolean {
  return (
    startsWithMagic(bytes, ZIP_LOCAL_FILE_HEADER_MAGIC) ||
    startsWithMagic(bytes, ZIP_END_OF_CENTRAL_DIRECTORY_MAGIC)
  );
}
