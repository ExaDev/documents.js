...
function entryByPath(
  entries: ReturnType<typeof walkArchive>,
  path: string,
  ancestors: readonly string[],
) {
  return entries.find(
    (e) => e.path === path && e.ancestors.join("/") === ancestors.join("/"),
  );
}
...
function nestZip(wrappers: number): Uint8Array<ArrayBuffer> {
  let current: Uint8Array<ArrayBuffer> = zipSync({
    "innermost.txt": enc.encode("payload"),
  });
...
describe("walkArchive", () => {
  it("lists every entry of a flat archive with no ancestors", () => {
    ...
    expect(entryByPath(entries, "a.txt", [])?.bytes).toEqual(
      enc.encode("alpha"),
    );
  });
  ...
  it("descends into a ZIP-in-ZIP entry and reports the ancestor chain", () => {
    ...
    expect(
      entryByPath(entries, "word/embeddings/oleObject1.xlsx", []),
    ).toBeDefined();
    const inner1 = entryByPath(entries, "xl/workbook.xml", [
      "word/embeddings/oleObject1.xlsx",
    ]);
    const inner2 = entryByPath(entries, "[Content_Types].xml", [
      "word/embeddings/oleObject1.xlsx",
    ]);
  ...
  it("walks three levels of nesting, chaining ancestors outermost-first", () => {
    const innermost = zipSync({
      "META-INF/MANIFEST.MF": enc.encode("Manifest-Version: 1.0"),
    });
    const middle = zipSync({ "lib/nested.jar": innermost });
    const outer = zipSync({
      "app.war": middle,
      "readme.txt": enc.encode("see also"),
    });
    const entries = walkArchive(outer);
    const deep = entryByPath(entries, "META-INF/MANIFEST.MF", [
      "app.war",
      "lib/nested.jar",
    ]);
  ...
  it("throws for a root that is not a ZIP archive", () => {
    expect(() => walkArchive(enc.encode("plainly not an archive"))).toThrow(
      /not a ZIP archive/,
    );
  });
});
...
describe("walkArchive depth guard", () => {
  ...
  it("honours a tighter caller-supplied maxDepth", () => {
    ...
    expect(catchLimit(() => walkArchive(bytes, { maxDepth: 2 })).limit).toBe(
      "depth",
    );
  });
});
...
describe("walkArchive cumulative-size guard", () => {
  ...
  it("counts decompressed bytes cumulatively across nesting levels, not per archive", () => {
    ...
    expect(
      catchLimit(() => walkArchive(outer, { maxTotalBytes: 1024 * 1024 }))
        .limit,
    ).toBe("total-bytes");
  });
