...
export function readCompoundFile(
  ...
  if (bytes.length < HEADER_SIZE) {
    throw new CompoundFileFormatError(
      `compound file is ${bytes.length} bytes, shorter than the fixed ${HEADER_SIZE}-byte header`,
    );
  }
  ...
  if (sectorCount < 1) {
    throw new CompoundFileFormatError(
      `compound file holds no complete ${sectorSize}-byte sector after its header`,
    );
  }
  const sectorOffset = (sector: number): number => (sector + 1) * sectorSize;
  const sectorBytes = (sector: number): Uint8Array<ArrayBuffer> =>
  ...
  const acceptFatSector = (sector: number, provenance: string): void => {
    ...
    if (sector >= sectorCount) {
      throw new CompoundFileFormatError(
        `${provenance} names FAT sector ${sector}, which is outside the file's ${sectorCount} sectors`,
      );
    }
  ...
  while (difatSector !== ENDOFCHAIN) {
    if (difatSector >= sectorCount) {
      throw new CompoundFileFormatError(
        `the DIFAT chain names sector ${difatSector}, which is outside the file's ${sectorCount} sectors`,
      );
    }
  ...
  const chainSectorIds = (start: number): number[] => {
    ...
    while (current !== ENDOFCHAIN) {
      ...
      if (next === FREESECT || next === FATSECT || next === DIFSECT) {
        throw new CompoundFileFormatError(
          `a FAT chain steps to sector ${current}'s entry ${next}, which is a sector-role marker, not a chain continuation`,
        );
      }
      current = next;
    }
  ...
  const extractStream = (entry: DirectoryEntry): Uint8Array<ArrayBuffer> => {
    ...
    if (entry.size > Number.MAX_SAFE_INTEGER) {
      throw new CompoundFileFormatError(
        `stream '${entry.name}' declares a size beyond the integer range this reader addresses`,
      );
    }
    ...
    if (entry.size < miniStreamCutoff) {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        raw.set(
          miniStream.subarray(
            miniSector * miniSectorSize,
            (miniSector + 1) * miniSectorSize,
          ),
          i * miniSectorSize,
        );
      }
    ...
    if (out.length < entry.size) {
      throw new CompoundFileFormatError(
        `stream '${entry.name}' declares ${entry.size} bytes but its chain holds only ${out.length}`,
      );
    }
    totalExtractedBytes += entry.size;
    if (totalExtractedBytes > maxTotalBytes) {
      throw new CompoundFileFormatError(
        `cumulative extracted stream size exceeded the ${maxTotalBytes}-byte budget at '${entry.name}'`,
      );
    }
  ...
  for (let frame = stack.pop(); frame !== undefined; frame = stack.pop()) {
    if (frame.stage === "descend") {
      ...
      if (visited.has(id)) {
        throw new CompoundFileFormatError(
          `the directory tree reaches entry ${id} twice, so its sibling and child links cycle`,
        );
      }
      visited.add(id);
      if (
        ...
        throw new CompoundFileFormatError(
          `directory entry ${id} declares name length ${entry.nameLength}, which is not an even byte count between 2 and 64`,
        );
      }
      if (
        entry.objectType !== OBJECT_TYPE_STORAGE &&
        entry.objectType !== OBJECT_TYPE_STREAM &&
        entry.objectType !== OBJECT_TYPE_ROOT
      ) {
        throw new CompoundFileFormatError(
          `directory entry ${id} ('${entry.name}') carries object type ${entry.objectType}, which is not a storage (1), stream (2), or root (5) entry`,
        );
      }
      ...
      throw new CompoundFileFormatError(
        `the directory tree reaches entry ${frame.entry.name}, which is not a storage or stream entry`,
      );
    }
  }
  return streams;
