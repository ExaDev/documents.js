...
describe('isCompoundFile', () => {
  ...
  it('rejects a non-CFB prefix (including the ZIP signature)', () => {
    ...
    expect(isCompoundFile(new Uint8Array([0x50, 0x4b, 0x03, 0x04]))).toBe(false);
  });

  it('requires the signature at offset zero, not merely somewhere in the bytes', () => {
    const shifted = new Uint8Array([0x00, 0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1]);
...
describe('detectArchiveFormat', () => {
  it('reports cfb for compound-file bytes and unknown for anything else', () => {
    expect(detectArchiveFormat(new Uint8Array([0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1]))).toBe('cfb');
    expect(detectArchiveFormat(new TextEncoder().encode('nope'))).toBe('unknown');
  });
});
