...
// The buffer accumulator both walks below thread through their own recursion. It was previously a defaulted second parameter on each exported function, which made an internal recursion detail part of the public signature; every caller only ever passed one argument, so the recursion now lives in a private helper and the exported functions take the value alone.
interface BufferSink {
  readonly found: ArrayBufferLike[];
}
...
export function collectTransferableBuffers(value: unknown): ArrayBufferLike[] {
  const sink: BufferSink = { found: [] };
  collectInto(value, sink);
  return sink.found;
}

function collectInto(value: unknown, sink: BufferSink): void {
  if (value instanceof Uint8Array) {
    sink.found.push(value.buffer);
  } else if (Array.isArray(value)) {
    for (const item of value) collectInto(item, sink);
  } else if (isRecord(value)) {
    for (const key of Object.keys(value)) {
      collectInto(value[key], sink);
    }
  }
}
...
export function cloneAndCollectTransferableBuffers(
  value: unknown,
): ArrayBufferLike[] {
  const sink: BufferSink = { found: [] };
  cloneAndCollectInto(value, sink);
  return sink.found;
}

function cloneAndCollectInto(value: unknown, sink: BufferSink): void {
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) {
      ...
      if (item instanceof Uint8Array) {
        ...
        value[index] = clone;
        sink.found.push(clone.buffer);
      } else {
        cloneAndCollectInto(item, sink);
      }
    }
  } else if (isRecord(value)) {
    for (const key of Object.keys(value)) {
