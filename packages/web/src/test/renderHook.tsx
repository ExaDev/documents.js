...
export interface RenderedHook<T> {
  result: { current: T };
  // Exposed so a test can assert on the exact queryKey/staleTime a hook registers (via queryClient.getQueryCache()/getQueryState()) — observing only result.current's eventual data can't tell a hook's real cache key apart from a mutated one, since react-query resolves the query and populates data regardless of what key it was cached under.
