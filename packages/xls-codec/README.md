...
- **`src/metadata.ts`** — maps `archive-codec`'s `SummaryInformationProperties` to and from `LayoutMetadata` — read and write together, since both directions share one field mapping (see [Metadata](#metadata)).
