...
function fileNameFromVirtPath(virtPath: string): string | undefined {
  ...
  // [MS-XLS]'s own VirtualPath grammar does not actually forbid a bracket in an ordinary file-path segment -- `[`/`]` are legal path characters there, and a real Windows file named e.g. "My[Draft].xlsx" is grammatically valid. This segment is declined anyway because a bracket is genuinely AMBIGUOUS at this point: nothing in the byte stream distinguishes a real bracket-named file from the start of the bracketed sheet-name form (`[Book.xlsx]Sheet1`), which this reader declines rather than reconstructs (see this function's own docstring above), and guessing wrong would return a segment carrying a raw bracket as if it were a plain file name -- doubling up with resolveXti's own `[${fileName}]` bracketing into a mangled label. Declining is strictly better than that: it trades a known, accepted false negative (a legitimately bracket-named external workbook resolves to no name at all) for never emitting a corrupted one. Checked on the EXTRACTED FINAL SEGMENT rather than on the whole path up front: a bracketed sheet-name reached through a directory separator (`sub`, a separator, then `[Book.xlsx]Sheet1`) or an unbalanced bracket with no separator at all (`abc[def`) both leave the leading character untouched by a start-of-path check, and both would otherwise return a segment carrying a raw bracket as if it were a plain file name, with the caller's own `diagnostic` flag left FALSE for it since this function has already reported success by returning a defined string.
