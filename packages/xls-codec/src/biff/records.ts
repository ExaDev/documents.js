...
export function readRecords(
  ...
  while (offset < stream.length) {
    ...
    if (size > MAX_RECORD_DATA_SIZE) {
      throw new BiffFormatError(
        `record 0x${type.toString(HEX_RADIX)} at offset ${offset} declares ${size} bytes of data, above the ${MAX_RECORD_DATA_SIZE}-byte maximum`,
      );
    }
    const dataStart = offset + HEADER_SIZE;
    if (dataStart + size > stream.length) {
      throw new BiffFormatError(
        `record 0x${type.toString(HEX_RADIX)} at offset ${offset} declares ${size} bytes of data, running past the end of the ${stream.length}-byte stream`,
      );
    }
    records.push({
      type,
