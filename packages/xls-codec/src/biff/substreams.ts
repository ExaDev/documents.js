...
/** One logical record: its type, and its data blocks -- the base record's, then one per Continue that followed it. */
export interface RecordGroup {
  readonly type: number;
  readonly blocks: readonly Uint8Array<ArrayBuffer>[];
}

/** One substream: the document type its BOF declared, the records between that BOF and its EOF, and its ordinal position among the stream's substreams. */
...
export function groupRecords(
  ...
  const groups: { type: number; blocks: Uint8Array<ArrayBuffer>[] }[] = [];
  for (const record of records) {
    ...
    groups.push({ type: record.type, blocks: [record.data] });
  }
  return groups;
}
...
export function splitSubstreams(
  ...
  let current: { documentType: number; records: RecordGroup[] } | undefined;
  for (const group of groups) {
    if (group.type === RECORD_BOF) {
      ...
      current = { documentType: readBofDocumentType(group), records: [] };
      continue;
    }
