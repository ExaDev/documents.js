...
// The Font record ([MS-XLS] 2.4.122, https://learn.microsoft.com/en-us/openspecs/office_file_formats/ms-xls/291a910c-cb69-4799-875e-a201845d4fd1) -- one entry of the workbook's font table, indexed by an XF record's own ifnt field and therefore the font every cell referencing that XF renders through. Read here into exactly the fields ContentSheetCell.font can express, so what the record states that the schema has no member for (superscript/subscript script, outline/shadow/condense/extend effects, font family and character set) is read past rather than half-modelled: a cell font keeps every property the shared model carries and loses exactly the ones it has no field for, the identical scope limit the border reader already applies to diagonal borders.
