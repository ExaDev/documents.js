...
/** Asserts two byte arrays hold the same bytes, reporting the first index they differ at. Used where the arrays run to a megabyte: a deep-equality comparison of two typed arrays that size takes seconds, several times longer than encoding and decoding them both, and prints a diff nobody can read when it fails. */
...
describe("base64ToBytes", () => {
  ...
  it("returns a byte array owning exactly the decoded bytes, with no slack left over from an over-allocated buffer", () => {
    const decoded = base64ToBytes("/w==");
  ...
  it("throws with the exact 'invalid base64 input' message when only the first character of a 4-character group is unmappable", () => {
    // '=' is not a member of the base64 alphabet DECODE maps (it is stripped from TABLE's own 64 characters), so it decodes to the miss sentinel exactly like a genuinely unmappable character would.
...
describe("base64ToBytes against Node's Buffer", () => {
  it("decodes bytesToBase64's own output back to the original bytes, and agrees with Buffer, for every length from 0 to 64 bytes", () => {
    for (let length = 0; length <= 64; length += 1) {
      const bytes = seededBytes(length, length + 3);
      const encoded = bytesToBase64(bytes);
      expect(base64ToBytes(encoded)).toEqual(bytes);
      expect(base64ToBytes(encoded)).toEqual(
        new Uint8Array(Buffer.from(encoded, "base64")),
      );
    }
  });

  it("decodes all 256 byte values in order and in reverse back to the identical bytes", () => {
    const ascending = Uint8Array.from(
      { length: 256 },
      (_unused, index) => index,
    );
  ...
  it("agrees with Buffer on a large pseudo-random buffer whose length leaves a two-byte remainder", () => {
    const bytes = seededBytes(1024 * 1024 + 2, 0x0dec0de);
    const encoded = bytesToBase64(bytes);
    const decoded = base64ToBytes(encoded);
    expectSameBytes(decoded, bytes);
    expectSameBytes(decoded, new Uint8Array(Buffer.from(encoded, "base64")));
  });
});
