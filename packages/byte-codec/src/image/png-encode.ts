...
// Encodes normalised raw pixel data (8 bits per channel, optionally with a separate alpha plane) into PNG file bytes -- the exact inverse of decodePng's RawImage shape. A channels === 3 image is first checked for a lossless indexed-colour (colour type 3) representation, and encoded that way whenever its pixels reduce to 256 or fewer distinct colours -- indexed colour is substantially smaller than truecolour for the flat-colour images typical of diagrams and screenshots, and decodePng reconstructs the original RGB(A) data exactly via its own PLTE/tRNS lookup, so this never changes what decodePng(encodePng(image)) hands back. Every other image (grayscale, or a truecolour image with more than 256 distinct colours) falls back to the plain truecolour/greyscale path.
