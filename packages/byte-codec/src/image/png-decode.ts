...
function unpackRow(
  ...
  // Built by an exact-length Array.from rather than a manually bounded for loop, so there is no separate loop-bound comparison that could drift from sampleCount: every element index 0..sampleCount-1 is generated by construction, never by a comparison that could run one iteration short or long.
  if (bitDepth === 16) {
    return Array.from({ length: sampleCount }, (_, i) => rowBytes[i * 2]!); // high byte only
  }
...
function buildRawImage(
  ...
  // Iterated via an exact-length Array.from rather than a manually bounded for loop, for both the row and column indices: `data`/`alpha` are each allocated to exactly width*height*outChannels / width*height elements, so there is no separate loop-bound comparison whose own boundary could ever be observed through them.
