...
//
// Legacy multi-byte and double-byte encodings (Shift_JIS, EUC-JP, ISO-2022-JP, GBK, gb18030, Big5, EUC-KR) are deliberately excluded: they are not a bounded extension of this table shape at all, needing a variable-width, often stateful decoder over index tables with tens of thousands of entries rather than a further 256-byte lookup, so folding them into this table would not be the same kind of change. That is real, wanted functionality, not declined outright, so it is tracked as its own issue (ExaDev/documents.js#1388) rather than left unrecorded.
//
