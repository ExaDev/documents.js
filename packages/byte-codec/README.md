...
| `image/png-encode` | `encodePng` (raw RGB/RGBA pixels → PNG bytes; a truecolour source image is written as indexed colour, PNG colour type 3 with a PLTE and, where needed, a tRNS chunk, whenever it reduces to 256 or fewer distinct colours, since that is losslessly smaller for the flat-colour images typical of diagrams and screenshots) |
