...
export function buildFib(spec: FibWriteSpec): Uint8Array<ArrayBuffer> {
  ...
  // Every other FibRgLw97 field (ccpTxbx, ccpHdrTxbx) stays 0: this writer produces no textbox subdocument, so the one subdocument kind this package's own reader is aware of but this writer never emits is genuinely empty rather than merely unpopulated.
