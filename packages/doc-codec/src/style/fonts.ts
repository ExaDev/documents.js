...
// An FFN's fixed fields ([MS-DOC] 2.9.69): one byte each of family, charset and ixchSzAlt, a 2-byte weight, a 10-byte panose, and a 24-byte FontSignature.
const FFID_SIZE = 1;
const FFN_WEIGHT_SIZE = 2;
const FFN_CHARSET_SIZE = 1;
const IXCH_SZ_ALT_SIZE = 1;
const PANOSE_BYTES = 10;
const FONT_SIGNATURE_BYTES = 24;
...
const FFN_FIXED_SIZE =
  FFID_SIZE +
  FFN_WEIGHT_SIZE +
  FFN_CHARSET_SIZE +
  IXCH_SZ_ALT_SIZE +
  PANOSE_BYTES +
  FONT_SIGNATURE_BYTES;
...
export function buildFontTable(names: readonly string[]): Uint8Array {
  ...
  const push16 = (value: number): void => {
    bytes.push(value & U8_MASK, (value >> BITS_PER_BYTE) & U8_MASK);
  };
...
function buildFfnRecord(name: string): number[] {
  const record: number[] = [
    ...
    FFN_WEIGHT_NORMAL & U8_MASK,
    (FFN_WEIGHT_NORMAL >> BITS_PER_BYTE) & U8_MASK,
    FFN_CHARSET_ANSI,
    0x00, // ixchSzAlt: no xszAlt.
    ...new Array<number>(PANOSE_BYTES).fill(0), // panose.
    ...new Array<number>(FONT_SIGNATURE_BYTES).fill(0), // fs (FontSignature).
  ];
  for (const character of name) {
    const code = character.charCodeAt(0);
    record.push(code & U8_MASK, (code >> BITS_PER_BYTE) & U8_MASK);
  }
