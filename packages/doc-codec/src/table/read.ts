...
// Whether one row's own physical cell's visual bottom edge is the table's real bottom edge: true directly when this IS the table's own last row; true for a vertically-merged anchor sitting in an earlier row whenever its own vertMerge continuation chain -- walked on the table's shared grid by vertMergeChainLastRow, the identical matching buildRows' own rowSpan computation performs -- reaches a continuation cell physically sitting in the table's last row; and true, independently of any merge, for a plain cell in a ragged (non-rectangular) table whose own column is never covered by any row below it at all (columnCoveredByALaterRow) -- a later row can simply be narrower and never state a cell reaching this far, in which case nothing physically sits beneath this cell, and its own bottom edge genuinely IS what a reader sees as the table's bottom in that column even though this row is not the table's last physical row (ExaDev/documents.js#945's own follow-up fix). A horzMerge-continuation or vertMerge-continuation physical cell's own answer is never actually observed downstream (logicalCellsForRow folds a horzMerge continuation into its anchor's span without consulting this cell's own borders at all, and buildRows drops a vertMerge continuation's own borders unconditionally -- see cascadeRowBorders' own note), so grid-index resolution for either is never asked to be more than merely non-throwing.
