...
// The fixed entry timestamp every written EPUB carries. fflate's default mtime is the wall clock, which would make two serialisations of the same content differ in bytes whenever they straddle a 2-second DOS-timestamp boundary -- undermining the byte-layout determinism this module exists to pin (see zipPackage's own note). These are freshly built packages, never restorations of a producer's own archive, so no real timestamp is lost by pinning one. Deliberately NOT the DOS-date floor itself (1980-01-01T00:00:00Z, matching ooxml.js's and odf.js's own identical fix): fflate's own DOS-date encoding (wzh) reads a Date through its *local*, not UTC, calendar getters, so a value sitting exactly on that floor rolls back into 1979 -- one year below fflate's own accepted range -- under any reading process whose local zone has a negative UTC offset at that instant (confirmed for America/New_York today, and for Pacific/Kiritimati before its 1995 date-line move, when it was UTC-10:40 rather than its current UTC+14). Noon UTC in the middle of the year gives comfortable margin against every real IANA zone's offset, past or present, in either direction.
