...
- **A `<p>` containing a direct `<img>`** (some producers/editors wrap every floating image in a paragraph tag rather than a `<figure>`) is split at the image: the phrasing content before and after becomes its own paragraph (dropped entirely when empty), the image its own block, in source order. An `<img>` nested deeper than a direct child (inside a `<span>` or `<a>`, say) is still detected — `src/xhtml/inline.ts`'s own run-building recursion reaches it wherever it sits — but by then the surrounding content has already committed to being one paragraph's own run sequence, with no block list left to insert a sibling image block into; it degrades to its alt text (or nothing, when it carries none) with an `epub/image-nested-inline-unsupported` diagnostic, never silently vanishing.
