...
function describeDataSource(
  ...
  return commandType === undefined
    ? quoted(command)
    : `${commandType} ${quoted(command)}`;
}

export function countOdbFormControls(
  controls: readonly OdbFormControl[],
): number {
  return controls.reduce(
    (total, control) => total + 1 + countOdbFormControls(control.controls),
    0,
  );
}

export function countOdbFormDefinitionControls(
  definition: OdbFormDefinition,
): number {
  return (
    countOdbFormControls(definition.controls) +
    definition.subForms.reduce(
      (total, subForm) => total + countOdbFormDefinitionControls(subForm),
      0,
    )
  );
}

function countOdbFormBoundControls(
  controls: readonly OdbFormControl[],
): number {
  return controls.reduce(
    (total, control) =>
      total +
      (control.dataField === undefined ? 0 : 1) +
      countOdbFormBoundControls(control.controls),
    0,
  );
}

function countOdbFormDefinitionBoundControls(
  definition: OdbFormDefinition,
): number {
  return (
    countOdbFormBoundControls(definition.controls) +
    definition.subForms.reduce(
      (total, subForm) => total + countOdbFormDefinitionBoundControls(subForm),
      0,
    )
  );
}
...
export function describeOdbForm(form: OdbForm): string {
  const controlCount = form.forms.reduce(
    (total, definition) => total + countOdbFormDefinitionControls(definition),
    0,
  );
  const boundCount = form.forms.reduce(
    (total, definition) =>
...
function formControlLines(
  ...
  const nested = control.controls.flatMap((child) =>
    formControlLines(child, depth + 1),
  );
  return [`${indent(depth)}${parts.join(" ")}`, ...nested];
}

function formDefinitionLines(
  definition: OdbFormDefinition,
  depth: number,
  kindLabel: string,
): readonly string[] {
...
export function formatOdbFormLines(form: OdbForm): readonly string[] {
  ...
  return form.forms.flatMap((definition) =>
    formDefinitionLines(definition, 0, "form"),
  );
}
...
function countReportGroupElements(group: OdbReportGroup): number {
  return (
    countReportBandElements(group.header) +
    countReportBandElements(group.footer) +
    group.groups.reduce(
      (total, nested) => total + countReportGroupElements(nested),
      0,
    )
  );
}
...
function countReportGroups(groups: readonly OdbReportGroup[]): number {
  return groups.reduce(
    (total, group) => total + 1 + countReportGroups(group.groups),
    0,
  );
}
...
export function describeOdbReport(report: OdbReport): string {
  ...
  const elementCount =
    ...
    report.groups.reduce(
      (total, group) => total + countReportGroupElements(group),
      0,
    );
  const source =
...
function reportBandLines(
  ...
  const header =
    band.name === undefined ? band.kind : `${band.kind} ${quoted(band.name)}`;
...
function reportFunctionLines(
  functions: readonly OdbReportFunction[],
  depth: number,
): readonly string[] {
  ...
  return [
    `${indent(depth)}functions`,
    ...functions.map((fn) => `${indent(depth + 1)}${fn.name} = ${fn.formula}`),
  ];
}

function reportGroupLines(
  ...
  if (group.sortExpression !== undefined) {
    attributes.push(
      `sort ${group.sortExpression} ${group.sortAscending === false ? "descending" : "ascending"}`,
    );
  }
