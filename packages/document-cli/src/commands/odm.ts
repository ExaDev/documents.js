...
function collectChapterOverride(value: string, previous: ReadonlyMap<string, string>): ReadonlyMap<string, string> {
  const separatorIndex = value.indexOf('=');
  if (separatorIndex === -1) {
    throw new InvalidArgumentError(`--chapter must be formatted as <href>=<file>, got '${value}'`);
  }
...
function createResolveSubDocument(overrides: ReadonlyMap<string, string>, chaptersDir: string | undefined): (href: string) => Uint8Array<ArrayBuffer> | undefined {
  return (href) => {
...
async function runOdmToPdf(input: string, output: string | undefined, options: OdmCliOptions): Promise<number> {
...
export function registerOdmCommand(program: Command): void {
  const command = program
    ...
    .description("convert a .odm master document to pdf, resolving each chapter's external .odt reference via --chapters-dir and/or --chapter");
