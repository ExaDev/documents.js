...
async function runOdbToXlsx(
  input: string,
  output: string | undefined,
  options: OdbCliOptions,
): Promise<number> {
  ...
  const resolvedOutput =
    output ??
    options.out ??
    (input === "-" ? "-" : resolveDefaultOutputPath(input, "xlsx"));
...
async function runOdbToCsv(
  input: string,
  output: string | undefined,
  options: OdbToCsvCliOptions,
): Promise<number> {
  ...
  const resolvedOutput =
    output ??
    options.out ??
    (input === "-" ? "-" : resolveDefaultCsvOutputPath(input));
  const { signal, getAbortReason } = createRuntimeSignal({
    timeoutMs: options.timeout,
  });
  ...
  try {
    ...
    for (const table of tables) {
      process.stdout.write(
        `${table.tableName} (${table.rows.length} row${table.rows.length === 1 ? "" : "s"})\n`,
      );
...
function resolveQuerySql(
  pkg: Package,
  options: { readonly sql?: string; readonly query?: string },
): { readonly sql: string } | { readonly errorMessage: string } {
  ...
  const saved = inventory.queries.find(
...
async function runOdbQuery(
  ...
  if (options.sql === undefined && options.query === undefined) {
    process.stderr.write(
      `[${command}] pass --sql <text> or --query <savedName>\n`,
    );
    return EXIT_USAGE_ERROR;
  }
  const { signal, getAbortReason } = createRuntimeSignal({});
...
async function runOdbForms(
  ...
  try {
    ...
    if (options.json) {
      process.stdout.write(
        `${JSON.stringify(forms.map((form) => odbFormSummary(form)))}\n`,
      );
      return EXIT_SUCCESS;
    }
...
async function runOdbReports(
  ...
  try {
    ...
    const reports = readOdbReports(
      decodeOdbPackage(new Uint8Array(inputBytes)),
    );
...
function isOdbReportTargetFormat(
  format: DocumentFormat,
): format is "docx" | "odt" | "pdf" {
  return format in ODB_REPORT_TARGET_FORMATS;
}

async function runOdbRenderReport(
  input: string,
  output: string | undefined,
  options: OdbRenderReportCliOptions,
): Promise<number> {
  ...
  if (!isOdbReportTargetFormat(target.format)) {
    process.stderr.write(
      `[${command}] '${target.format}' is not a supported report render target; expected one of docx, odt, pdf\n`,
    );
    return EXIT_USAGE_ERROR;
  }
  ...
  const resolvedOutput =
    output ??
    options.out ??
    (input === "-" ? "-" : resolveDefaultOutputPath(input, targetFormat));
  const { signal, getAbortReason } = createRuntimeSignal({
    timeoutMs: options.timeout,
  });
  const reporter = createDiagnosticReporter({
...
function registerOdbToXlsxCommand(program: Command): void {
  const command = program
    .command("odb-to-xlsx <input> [output]")
    .description(
      "extract every table an embedded .odb database declares into one xlsx workbook, one sheet per table",
    );
...
function registerOdbToCsvCommand(program: Command): void {
  const command = program
    .command("odb-to-csv <input> [output]")
    .description(
      "extract exactly one named table from an embedded .odb database as CSV",
    );
  addOutOption(command);
  addTimeoutOption(command);
  addJsonOption(command);
...
function registerOdbTablesCommand(program: Command): void {
  program
    ...
    .description(
      "list every table an embedded .odb database declares, with column names/types and row counts",
    )
    .option(
      "--json",
      "emit the table list as a JSON array instead of a human-readable report",
      false,
    )
...
function registerOdbFormsCommand(program: Command): void {
  program
    ...
    .description(
      "list every form an .odb declares, with each form's own data source and its field-bound controls",
    )
    .option(
      "--json",
      "emit the form structure as a JSON array instead of a human-readable report",
      false,
    )
    .action(async (input: string, options: { readonly json: boolean }) => {
...
function registerOdbReportsCommand(program: Command): void {
  program
    ...
    .description(
      "list every report an .odb declares, with each report's own data-source command, band/group structure, and rpt: formula expressions",
    )
    .option(
      "--json",
      "emit the report structure as a JSON array instead of a human-readable report",
      false,
    )
    .action(async (input: string, options: { readonly json: boolean }) => {
...
function registerOdbQueryCommand(program: Command): void {
  program
    ...
    .description(
      "run a bounded SELECT over an embedded .odb database's own extracted tables, given directly or by naming one of its saved queries",
    )
    .option(
      "--sql <text>",
    ...
    .option(
      "--json",
      "emit the result set as JSON ({ columns, rows }) instead of a plain-text table",
      false,
    )
...
function registerOdbRenderReportCommand(program: Command): void {
  ...
  command.option(
    "--to <format>",
    "target format when it cannot be inferred from the output path (docx, odt, pdf)",
  );
  command.action(
    async (
      input: string,
