...
import { buildDocxWithEmbeddedFont, buildOdtWithEmbeddedFont, buildPptxWithEmbeddedFont } from '../test-support/embedded-font-fixture';
import { FIXTURE_FONT_FAMILY, fixtureCalibriFontBytes } from '../test-support/font-fixture';
...
beforeAll(async () => {
  ...
  await writeFile(join(workspace, 'embedded.docx'), buildDocxWithEmbeddedFont({ family: FIXTURE_FONT_FAMILY, fontBytes }));
  await writeFile(join(workspace, 'embedded.pptx'), buildPptxWithEmbeddedFont({ family: FIXTURE_FONT_FAMILY, fontBytes }));
  await writeFile(join(workspace, 'embedded.odt'), buildOdtWithEmbeddedFont({ family: FIXTURE_FONT_FAMILY, fontBytes }));
...
describe('fonts', () => {
  it('lists the one embedded face a docx declares, as a human-readable report', async () => {
    const { exitCode, stdout, stderr } = await runCli(['fonts', join(workspace, 'embedded.docx')]);
  ...
  it('lists the one embedded face a pptx declares, dispatched through the identical extractor', async () => {
    const { exitCode, stdout } = await runCli(['fonts', join(workspace, 'embedded.pptx')]);
  ...
  it('lists the one embedded face an odt declares, dispatched through odf.js decodePackage rather than ooxml.js', async () => {
    const { exitCode, stdout } = await runCli(['fonts', join(workspace, 'embedded.odt')]);
  ...
  it('emits {family, bold, italic, byteLength} objects under --json, never the raw font bytes', async () => {
    const { exitCode, stdout } = await runCli(['fonts', join(workspace, 'embedded.docx'), '--json']);

    expect(exitCode).toBe(EXIT_SUCCESS);
    const parsed: unknown = JSON.parse(stdout);
    expect(parsed).toStrictEqual([{ family: FIXTURE_FONT_FAMILY, bold: false, italic: false, byteLength: fontByteLength }]);
  });

  it('says so plainly for a document that embeds no source fonts', async () => {
    const { exitCode, stdout } = await runCli(['fonts', join(workspace, 'plain.docx')]);
