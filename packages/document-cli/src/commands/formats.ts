...
export function registerFormatsCommand(program: Command): void {
  program
    .command('formats')
    .description('list every source -> target conversion this CLI supports via a <source>-to-<target> command')
    .option('--json', 'emit the conversion list as a JSON array instead of a human-readable table', false)
