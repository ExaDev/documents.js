...
function isSchemaVersionMismatchError(
  error: unknown,
): error is DumpVersionMismatch {
...
async function runFromPackage(
  ...
  try {
    ...
    if (result.kind !== "DocumentTree") {
      process.stderr.write(
        `[${command}] '${input}' is a ${result.kind}, not a DocumentTree -- only a file written by --dump-package can be read back by this command\n`,
      );
      return EXIT_USAGE_ERROR;
    }
    ...
    if (error instanceof UnrecognizedDocumentSchemaError) {
      process.stderr.write(
        `[${command}] '${input}' has no recognised $schema -- only a file written by --dump-package can be read back by this command\n`,
      );
      return EXIT_INPUT_ERROR;
    }
    ...
    if (isLayoutSchemaDemotedError(error)) {
      process.stderr.write(
        `[${command}] '${input}' is a LayoutDocument dump -- LayoutDocument moved to pdf-codec in document-schema.js 4.0.0 and is no longer a schema-stamped input; re-run the source conversion with --dump-package and read that package back instead\n`,
      );
      return EXIT_INPUT_ERROR;
    }
    ...
    if (isDocumentPackageRenamedError(error)) {
      process.stderr.write(
        `[${command}] '${input}' is a document-package dump -- DocumentPackage was renamed to DocumentTree in document-schema.js 5.0.0 (ExaDev/documents.js#661); re-run the source conversion with --dump-package to write a current tree-form DocumentTree dump\n`,
      );
      return EXIT_INPUT_ERROR;
    }
...
export function registerFromPackageCommand(program: Command): void {
  const command = program
    ...
    .description(
      "read a DocumentTree previously written by --dump-package and export it to a real target format",
    );
