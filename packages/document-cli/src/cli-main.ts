...
//
// This module (and its dynamic import of ./tui/index.js) is deliberately NEVER imported, statically or dynamically, from src/sea-entry.ts or src/cli-main-sea.ts -- Ink's own dependency chain (yoga-layout, ink's own reconciler and devtools integration) has top-level await baked into its module graph and an unresolved optional peer dependency (react-devtools-core) that isn't installed, neither of which a single-file SEA bundle can paper over. A runtime flag choosing not to CALL this code would not be enough to keep Ink out of that bundle -- a bundler's own static analysis still discovers and includes a reachable dynamic import() even behind a condition it cannot prove false at build time. Keeping the reference out of the SEA entry's own module graph entirely, via cli-main-sea.ts's genuinely separate (small, intentionally duplicated) dispatch shell, is what actually excludes it. See that file's own comment for the rest of this reasoning.
