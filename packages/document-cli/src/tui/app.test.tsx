...
import {
  ...
  METADATA_FIXTURE,
} from "../test-support/metadata-fixture.js";
...
describe("App metadata screen", () => {
  it("opens the metadata screen on 'm' and shows the open document's real metadata", async () => {
    ...
    const frame = await waitForFrame(lastFrame, (candidate) =>
      candidate.includes("Document metadata"),
    );
...
describe("App Ctrl+Z undo", () => {
  it("reverts the last paragraph append when Ctrl+Z is pressed", async () => {
    ...
    stdin.write("\x1B");
    const beforeUndo = await waitForFrame(lastFrame, (frame) =>
      frame.includes("Paragraphs (1/1)"),
    );
    ...
    stdin.write("\x1a");
    const afterUndo = await waitForFrame(lastFrame, (frame) =>
      frame.includes("No paragraphs or tables yet"),
    );
