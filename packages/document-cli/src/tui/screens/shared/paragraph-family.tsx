...
} from "../../keybindings/use-navigation-input.js";
...
): run is ParagraphFamilyStyledRun {
  return "underline" in run;
}

export function paragraphFamilyDocument(
  openDocument: OpenDocument | undefined,
): ParagraphFamilyOpenDocument | undefined {
...
export function liveParagraphAt(
  doc: ParagraphFamilyOpenDocument,
  blockIndex: number,
): ParagraphFamilyLiveParagraph | undefined {
  return doc.editor.paragraphs()[blockIndex];
}

export function liveTableAt(
  doc: ParagraphFamilyOpenDocument,
  tableIndex: number,
): ParagraphFamilyLiveTable | undefined {
...
export function ParagraphFamilyBodyList(props: {
  ...
  const matches = (haystack: string): boolean =>
    query.length === 0 || haystack.toLowerCase().includes(query);
  ...
  const paragraphRows: readonly ParagraphRow[] = paragraphs
    ...
    .filter(
      (row) =>
        matches(truncatePreview(row.paragraph.text, PREVIEW_WIDTH)) ||
        matches(paragraphBadges(row.paragraph)),
    );

  const tableRows: readonly TableRow[] = tables
    .map((table, index) => ({ kind: "table" as const, index, table }))
    .filter((row) => matches(tableSummary(row.table)));

  const listRows: readonly ListRow[] = lists
    .map((list, index) => ({ kind: "list" as const, index, list }))
    .filter((row) => matches(listSummary(row.list, row.index)));
  ...
  const rows: readonly BodyRow[] = [
      ...
      ? [
          {
            ...
            label: `Paragraphs (${paragraphRows.length}/${paragraphs.length})`,
          },
          ...paragraphRows,
        ]
      : []),
    ...(tableRows.length > 0
      ? [
          {
            kind: "header" as const,
            label: `Tables (${tableRows.length}/${tables.length})`,
          },
          ...tableRows,
        ]
      : []),
    ...(listRows.length > 0
      ? [
          {
            kind: "header" as const,
            label: `Lists (${listRows.length}/${lists.length})`,
          },
          ...listRows,
        ]
      : []),
  ...
  >("closed");
  const [pendingFormulaMathml, setPendingFormulaMathml] = useState<
    readonly MathMlNode[] | undefined
  >(undefined);
  ...
  const commitAppendTable = (
    ...
    dispatch({
      type: "PUSH_SCREEN",
      screen: { kind: "tableView", blockIndex: newIndex },
    });
  };
  ...
  useInput(
    (input) => {
      if (input === "L" && adapter.lists !== undefined) {
        ...
        dispatch({
          type: "PUSH_SCREEN",
          screen: { kind: "listEditor", blockIndex: newIndex },
        });
      }
    },
    { isActive: !anyOverlayOpen(state) && !wizardOpen && !formulaFlowOpen },
  ...
  const submitWizardMergeStartColumn = (raw: string): void => {
    setWizardStartColumn(
      Math.min(
        parseNonNegativeIntField(raw, 0),
  ...
  const { selectedIndex } = usePersistedSelection(
    selectionKeyFor({ kind: "bodyList" }),
    {
      ...
      onSelect: (index) => {
        ...
        if (row.kind === "paragraph") {
          dispatch({
            type: "PUSH_SCREEN",
            screen: { kind: "paragraphDetail", blockIndex: row.index },
          });
          return;
        }
        if (row.kind === "table") {
          dispatch({
            type: "PUSH_SCREEN",
            screen: { kind: "tableView", blockIndex: row.index },
          });
          return;
        }
        if (row.kind === "list") {
          dispatch({
            type: "PUSH_SCREEN",
            screen: { kind: "listEditor", blockIndex: row.index },
          });
        }
      },
      onAppend: () => {
        ...
        dispatch({
          type: "PUSH_SCREEN",
          screen: { kind: "paragraphDetail", blockIndex: newIndex },
        });
      },
    },
  );
  ...
  return (
    <Box flexDirection="column">
      ...
      {tableWizard === "mergePrompt" ? (
        <Text color="cyan">Merge cells now? y/N</Text>
      ) : undefined}
      {tableWizard === "mergeStartRow" ? (
        <Box>
          <Text color="cyan">
            Merge start row (0-{Math.max(0, wizardRows - 1)}):{" "}
          </Text>
          <TextField
            value={wizardDraft}
            isFocused
      ...
      {tableWizard === "mergeStartColumn" ? (
        <Box>
          <Text color="cyan">
            Merge start column (0-{Math.max(0, wizardColumns - 1)}):{" "}
          </Text>
          <TextField
            value={wizardDraft}
            isFocused
      ...
      {tableWizard === "mergeRowSpan" ? (
        <Box>
          <Text color="cyan">
            Merge row span (1-{Math.max(1, wizardRows - wizardStartRow)}):{" "}
          </Text>
          <TextField
            value={wizardDraft}
            isFocused
      ...
      <Text dimColor>
        ...
        {adapter.lists !== undefined ? ", L to add a list" : ""}
        {adapter.formatLabel === "odt" ? ", m to insert a formula" : ""}, Esc
        back
      </Text>
    </Box>
  );
