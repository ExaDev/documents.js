...
export function parseColorField(raw: string): LayoutColor | undefined {
  ...
  if (
    ...
    g === undefined ||
    b === undefined ||
    ![r, g, b].every((value) => Number.isFinite(value))
  ) {
    return undefined;
  }
  return { r, g, b };
...
export function parseStrokeField(raw: string): ContentStroke | undefined {
  ...
  if (
    ...
    b === undefined ||
    widthPt === undefined ||
    ![r, g, b, widthPt].every((value) => Number.isFinite(value))
  ) {
    return undefined;
  }
...
export function defaultTriangleSubpaths(
  widthPt: number,
  heightPt: number,
): readonly ContentSubpath[] {
  return [
    {
