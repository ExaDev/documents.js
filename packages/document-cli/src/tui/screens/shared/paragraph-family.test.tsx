...
function BodyListHarness({
  ...
  readonly format: "docx" | "odt" | "markdown";
}): ReactElement | null {
  const state = useAppState();
  const dispatch = useAppDispatch();
  ...
  useEffect(() => {
    if (format === "markdown") {
      const doc: MarkdownOpenDocument = {
        format: "markdown",
...
function TableProbe({
  ...
  readonly doc: WordprocessingOpenDocument;
}): ReactElement {
  ...
  const tableBlock = content.sections
    .flatMap((section) => section.blocks)
    .find(
      (
        block,
      ): block is Extract<
        (typeof content.sections)[number]["blocks"][number],
        { readonly kind: "table" }
      > => block.kind === "table",
    );
...
function renderBodyListHarness(
  format: "docx" | "odt" | "markdown",
...
describe("ParagraphFamilyBodyList", () => {
  ...
  it("navigates the list with j and opens the selected paragraph with Enter", async () => {
    const { lastFrame, stdin } = renderHarness();
    await flush();
...
describe.each(["docx", "odt", "markdown"] as const)(
  'ParagraphFamilyBodyList "T" table-creation wizard on %s',
  (format) => {
    it(
      ...
      async () => {
        ...
        await flush();
        expect(lastFrame()).toContain(
          "probe:table=3x4 anchorColSpan=1 anchorRowSpan=1",
        );
    ...
    it(
      ...
      async () => {
        ...
        if (format === "markdown") {
          ...
          expect(lastFrame()).toContain(
            "probe:table=3x3 anchorColSpan=2 anchorRowSpan=2",
          );
        }
        expect(lastFrame()).toContain("ON tableView");
      },
