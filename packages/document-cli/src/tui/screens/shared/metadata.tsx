...
// A deliberately read-only screen, reachable for any open document ('m' from app.tsx's own shell-level global useInput): DocxEditor/OdtEditor/PptxEditor/OdpEditor/OdsEditor/OdgEditor expose no metadata setter on an already-open live editor today -- there is no `editor.metadata = {...}` the way there is `run.bold = true`. That is a genuine documents.js-level gap (see this repo's own set-metadata CLI command, which patches metadata via setDocumentMetadata against freshly-read bytes -- an in-place patch for docx (docProps/core.xml) and pdf (its own parsed LayoutDocument), a full rebuild from a freshly-read ContentDocument for every other format -- rather than mutating a live editor in place), not something this screen could work around without reimplementing that patch/rebuild inside the TUI's own edit loop. Reviewed and left out of scope deliberately: this screen shows what a document's own metadata currently is, nothing more, until documents.js grows a live setter to edit through.
