    ...
    slides: () =>
      editor.slides().map((slide) => ({
        shapes: () =>
          slide
            .shapes()
            .map((shape) => ({ text: shape.text, frame: shape.frame })),
        notes: slide.notes,
      })),
  };
}

export function buildPptxSlideFamilyAdapter(
  editor: PptxEditor,
): SlideFamilyAdapter {
  return buildSlideFamilyAdapter("pptx", editor);
}

export function buildOdpSlideFamilyAdapter(
  editor: OdpEditor,
): SlideFamilyAdapter {
...
export function describeSlideFamilyShape(
  ...
  return flattened.length === 0
    ? "[Image] (no text)"
    : `[Text] "${truncate(flattened, SHAPE_TEXT_PREVIEW_LENGTH)}"`;
}
...
export function SlideFamilySlideList(
  props: SlideFamilySlideListProps,
): ReactElement {
  const state = useAppState();
  const dispatch = useAppDispatch();
  const overlayOpen = anyOverlayOpen(state);
  const rows: readonly SlideRow[] = props.adapter
    .slides()
    .map((slide, index) => ({
      index,
      shapes: slide.shapes(),
  ...
  const { selectedIndex } = useNavigationInput({
    ...
    onSelect: (index) => {
      dispatch({
        type: "SET_SELECTION",
        key: selectionKeyFor({ kind: "slideList" }),
        index,
      });
      dispatch({
        type: "PUSH_SCREEN",
        screen: { kind: "slideDetail", slideIndex: index },
      });
    },
    onAppend: () => {
