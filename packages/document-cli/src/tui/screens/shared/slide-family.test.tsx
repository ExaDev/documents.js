...
import { PptxSlideListScreen, ShapeEditorScreen, SlideDetailScreen } from '../editors/pptx/index.js';
...
function SlideFamilyRouter({ format }: { readonly format: 'pptx' | 'odp' }): ReactElement {
  const state = useAppState();
  const screen = currentScreen(state);
  switch (screen.kind) {
    case 'slideList':
      return format === 'pptx' ? <PptxSlideListScreen /> : <OdpSlideListScreen />;
    case 'slideDetail':
      return <SlideDetailScreen screen={screen} />;
    case 'shapeEditor':
      return <ShapeEditorScreen screen={screen} />;
    default:
      throw new Error(`SlideFamilyRouter has no case for ${screen.kind}; this test only ever pushes slideList/slideDetail/shapeEditor.`);
  }
}
...
function buildPptxTestBytes(): Uint8Array<ArrayBuffer> {
  ...
  slideOne.addTextBox({ frame: { xPt: 10, yPt: 10, widthPt: 100, heightPt: 50 }, text: 'Q3 Results' });
  slideOne.addTextBox({ frame: { xPt: 10, yPt: 70, widthPt: 100, heightPt: 50 }, text: '' });
  editor.addSlide();
...
function buildOdpTestBytes(): Uint8Array<ArrayBuffer> {
  ...
  slide.addTextBox({ frame: { xPt: 5, yPt: 5, widthPt: 50, heightPt: 20 }, text: 'Agenda' });
  return editor.toBytes();
}
...
function OpenTestDocument({ format, bytes }: { readonly format: 'pptx' | 'odp'; readonly bytes: Uint8Array<ArrayBuffer> }): ReactElement | undefined {
...
function OpenedHarness({ format, bytes }: { readonly format: 'pptx' | 'odp'; readonly bytes: Uint8Array<ArrayBuffer> }): ReactElement {
  const state = useAppState();
...
function renderOpened(format: 'pptx' | 'odp', bytes: Uint8Array<ArrayBuffer>): ReturnType<typeof render> {
  return render(
    <AppStateProvider>
