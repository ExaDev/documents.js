import { readOdpContent, readPptxContent, type ContentTable, type ContentTableCell, type OdtTable, type PptxTable } from 'documents.js';
...
export function resolveSlideTable(doc: PresentationOpenDocument, slideIndex: number, tableIndex: number): ContentTable | undefined {
  const content = doc.format === 'odp' ? readOdpContent(doc.editor.toPackage()) : readPptxContent(doc.editor.toPackage());
  if (content.kind !== 'presentation') {
    throw new Error('readPptxContent/readOdpContent always resolve a presentation package to the presentation ContentDocument variant.');
  }
  ...
  return slide.shapes.flatMap((shape) => shape.blocks.filter((block): block is ContentTable => block.kind === 'table'))[tableIndex];
}
...
export function slideTableCellText(cell: ContentTableCell): string {
  return cell.blocks
    .filter((block): block is Extract<(typeof cell.blocks)[number], { readonly kind: 'paragraph' }> => block.kind === 'paragraph')
...
function summarizeGridTable(table: PptxTable | OdtTable, index: number): SlideTableSummary {
  const rows = table.rows();
  return { index, rowCount: rows.length, columnCount: rows[0]?.cells().length ?? 0 };
}
...
export function summarizeSlideTables(doc: PresentationOpenDocument, slideIndex: number): readonly SlideTableSummary[] {
  if (doc.format === 'odp') {
    return (doc.editor.slides()[slideIndex]?.tables() ?? []).map((entry, index) => summarizeGridTable(entry.table, index));
  }
