...
function defaultExportDestinationFor(
  document: OpenDocument,
  cwd: string,
): string {
  return document.path === undefined
    ? join(cwd, "untitled.pdf")
...
export function ExportOptionsScreen(): ReactElement {
  ...
  const [destination, setDestination] = useState(() =>
    document === undefined
      ? ""
      : defaultExportDestinationFor(document, state.cwd),
  );
    ...
    <Box flexDirection="column">
      ...
      <Text dimColor>
        {field === "destination"
          ...
          : "Enter to export, Esc to cancel. Each font's family is read from the file itself"}
      </Text>
    </Box>
  );
}
