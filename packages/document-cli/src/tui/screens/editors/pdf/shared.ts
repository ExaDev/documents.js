...
// Every screen in this directory is only ever reached from `pdfPageList`, the root screen `rootScreenForFormat` produces for an open PDF document or for one of the four formats opened read-only as a converted PDF preview (an xlsx workbook, a csv sheet, an svg drawing, an epub book -- see state/types.ts's own XlsxOpenDocument/CsvOpenDocument/SvgOpenDocument/EpubOpenDocument doc comments) -- so `state.openDocument` is always one of these five by the time any screen here renders. All five carry the identical `.layout: LayoutDocument` field this whole screen group reads from, and nothing else, which is exactly what lets one screen family serve all of them with no per-format branch anywhere in page-list.tsx/page-items.tsx/item-detail.tsx. This throws rather than falling back to an empty view because a mismatch would mean the app router itself is broken, not a recoverable, user-facing condition.
