...
): PdfOpenDocument | XlsxOpenDocument | CsvOpenDocument | SvgOpenDocument {
  if (
    ...
    throw new Error(
      "A PDF inspection screen rendered without an open PDF, xlsx, csv, or svg document; the app router only reaches this screen group from pdfPageList, which is only ever the root screen of one of those four formats.",
    );
  }
  return openDocument;
}
...
export function isEditablePdfDocument(
  doc: PdfOpenDocument | XlsxOpenDocument | CsvOpenDocument | SvgOpenDocument,
