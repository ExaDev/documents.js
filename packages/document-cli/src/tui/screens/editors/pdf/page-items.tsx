...
import { defaultTriangleLayoutSubpaths, formatSize, inferImageFormat, isEditablePdfDocument, parseColorField, parseFontStyle, parseFontWeight, parseNumberField, parseStrokeField, requirePdfDocument } from './shared.js';
...
const REQUIRED_COLOR_FIELD: FieldSpec = { key: 'color', label: 'Colour "r g b" (0-1 each)', defaultValue: '0 0 0' };
  ...
  switch (kind) {
    case 'text':
      return [
        ...
        { key: 'fontWeight', label: 'Font weight (normal/bold)', defaultValue: 'normal' },
        { key: 'fontStyle', label: 'Font style (normal/italic)', defaultValue: 'normal' },
    ...
    case 'image':
      return [...GEOMETRY_FIELDS, { key: 'path', label: 'Image file path (.png/.jpg/.jpeg)', defaultValue: '' }];
    case 'link':
      return [{ key: 'uri', label: 'URI', defaultValue: 'https://example.com' }, ...GEOMETRY_FIELDS];
  }
}

function readFrame(values: Readonly<Record<string, string>>): { readonly xPt: number; readonly yPt: number; readonly widthPt: number; readonly heightPt: number } {
  return {
  ...
  switch (kind) {
    case 'text': {
      const init: PdfTextInit = {
        ...
        color: parseColorField(requireFieldValue(values, 'color')) ?? { r: 0, g: 0, b: 0 },
      };
    ...
    case 'rect': {
      ...
      dispatch({ type: 'ADD_PDF_RECT', pageIndex, init: { ...frame, fill: parseColorField(requireFieldValue(values, 'fill')), stroke: parseStrokeField(requireFieldValue(values, 'stroke')) } });
      return;
    }
    case 'ellipse': {
      const frame = readFrame(values);
      dispatch({ type: 'ADD_PDF_ELLIPSE', pageIndex, init: { ...frame, fill: parseColorField(requireFieldValue(values, 'fill')), stroke: parseStrokeField(requireFieldValue(values, 'stroke')) } });
      return;
    }
    case 'line': {
      dispatch({
    ...
    case 'image': {
      ...
      try {
        ...
        dispatch({ type: 'ADD_PDF_IMAGE', pageIndex, init: { ...frame, format, bytes } });
      } catch (error) {
    ...
    case 'link': {
      const frame = readFrame(values);
      dispatch({ type: 'ADD_PDF_LINK', pageIndex, init: { uri: requireFieldValue(values, 'uri'), ...frame } });
      return;
    }
  }
}
...
export function PdfPageItemsScreen(): ReactElement {
  ...
  useInput(
    (input) => {
      ...
      dispatch({ type: 'REMOVE_PDF_ITEM', pageIndex: screen.pageIndex, itemIndex: entry.itemIndex });
    },
  ...
  return (
    <Box flexDirection="column">
      ...
      />
      {editable && <Text dimColor>a to add an item, d to delete the selected item</Text>}
    </Box>
  );
}
