...
function OpenAtListEditor({ bytes }: { readonly bytes: Uint8Array<ArrayBuffer> }): ReactElement | undefined {
  const dispatch = useAppDispatch();
  useEffect(() => {
    dispatch({ type: 'OPEN_FILE_SUCCESS', path: 'test.odt', doc: { format: 'odt', editor: openOdt(bytes), path: 'test.odt' } });
    dispatch({ type: 'PUSH_SCREEN', screen: { kind: 'listEditor', blockIndex: 0 } });
  }, [bytes, dispatch]);
  return undefined;
}

function Harness({ bytes }: { readonly bytes: Uint8Array<ArrayBuffer> }): ReactElement {
...
function renderListEditor(bytes: Uint8Array<ArrayBuffer>): ReturnType<typeof render> {
  return render(
    <AppStateProvider>
      <Harness bytes={bytes} />
    </AppStateProvider>,
