...
async function replaceField(stdin: { readonly write: (data: string) => void }, lastFrame: () => string | undefined, count: number, value: string): Promise<void> {
...
function ContentProbe({ doc }: { readonly doc: DocxOpenDocument | OdtOpenDocument }): ReactElement {
  ...
  return (
    <Text>
      probe:image={imageBlock === undefined ? 'none' : `${imageBlock.format} ${imageBlock.widthPt}x${imageBlock.heightPt} alt="${imageBlock.altText ?? ''}"`} probe:formula={formula === undefined ? 'none' : `present root=${rootTag ?? '?'}`}
    </Text>
  );
}

function Harness({ format }: { readonly format: 'docx' | 'odt' }): ReactElement | null {
  const state = useAppState();
  const dispatch = useAppDispatch();
  ...
  return (
    <Box flexDirection="column">
      ...
      {screen.kind === 'paragraphDetail' ? <ParagraphDetailScreen /> : undefined}
...
async function openFreshParagraph(stdin: { readonly write: (data: string) => void }): Promise<void> {
  stdin.write('a');
  await flush();
}
...
describe.each(['docx', 'odt'] as const)('ParagraphDetailScreen "I" image insertion on %s', (format) => {
  let workspace: string;
  let imagePath: string;

  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'document-cli-paragraph-image-'));
  ...
  it(
    ...
    async () => {
      ...
      await vi.waitFor(() => {
        expect(lastFrame()).toContain(`probe:image=png 150x75 alt="${expectedAlt}"`);
      });
    },
  ...
  it(
    ...
    async () => {
      ...
      stdin.write(ENTER);
      await vi.waitFor(() => {
        expect(lastFrame()).toContain('is not a .png or .jpg/.jpeg file -- image not inserted');
      });
      expect(lastFrame()).toContain('probe:image=none');
    },
    WIZARD_TEST_TIMEOUT_MS,
