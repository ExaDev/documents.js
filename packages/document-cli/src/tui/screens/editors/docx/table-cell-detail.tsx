...
export function TableCellDetailScreen(): ReactElement {
  ...
  const table = screen.kind === 'tableCellDetail' && doc !== undefined ? liveTableAt(doc, screen.blockIndex) : undefined;
  const row = screen.kind === 'tableCellDetail' && table !== undefined ? table.rows()[screen.row] : undefined;
  const cell = screen.kind === 'tableCellDetail' && row !== undefined ? row.cells()[screen.col] : undefined;
  ...
  if (cell === undefined) {
    return (
      <Text color="red">
        There is no cell at row {screen.row}, column {screen.col} of table {screen.blockIndex}.
      </Text>
    );
  }
  ...
  return (
    <Box flexDirection="column">
      ...
      {paragraphs.length === 0 ? (
        ...
        paragraphs.map((paragraph, index) => <ParagraphRunsView key={index} runs={paragraph.runs()} selectedRunIndex={undefined} />)
      )}
      {isEditing ? (
        <RunTextEditor
          initialText={cell.text}
          onCommit={(text) => {
            dispatch({ type: 'SET_TABLE_CELL_TEXT', tableIndex: screen.blockIndex, row: screen.row, column: screen.col, text });
            setIsEditing(false);
          }}
          onCancel={() => {
            setIsEditing(false);
