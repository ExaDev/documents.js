...
import { buildDocxWithExtras, DOCX_EXTRAS_FIXTURE } from '../../../../test-support/docx-extras-fixture.js';
...
function DocxExtrasHarness(): ReactElement {
  ...
  useEffect(() => {
    dispatch({ type: 'OPEN_FILE_SUCCESS', path: 'extras.docx', doc: { format: 'docx', editor: openDocx(buildDocxWithExtras()), path: 'extras.docx' } });
  }, [dispatch]);
...
describe('DocxExtrasScreen', () => {
  it("opens from the body list with 'x' and shows the fixture's own comments, footnotes, headers, footers, and numbering", async () => {
    ...
    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('Comments, footnotes, headers, footers, numbering'));
  ...
  it('goes back to the body list on Esc/h', async () => {
    ...
    await settle();
    stdin.write('x');
    await waitForFrame(lastFrame, (candidate) => candidate.includes('Comments, footnotes, headers, footers, numbering'));
    await settle();
