...
function AnchorSpanProbe({ format }: { readonly format: 'docx' | 'odt' }): ReactElement {
...
function TableViewHarness({ format }: { readonly format: 'docx' | 'odt' }): ReactElement {
  const state = useAppState();
  ...
  useEffect(() => {
    ...
    if (doc.format === format && screen.kind === 'bodyList') {
      ...
      dispatch({ type: 'PUSH_SCREEN', screen: { kind: 'tableView', blockIndex: 0 } });
    }
...
describe.each(['docx', 'odt'] as const)('TableViewScreen retrofit merge on %s', (format) => {
  it('merges a real rectangle of cells in an already-built table via m-to-anchor, move, m-to-commit', async () => {
    ...
    let frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('Table 0'));
