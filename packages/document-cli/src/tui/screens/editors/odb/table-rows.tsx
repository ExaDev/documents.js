...
function requireTable(
  ...
  if (table === undefined) {
    throw new Error(
      `odbTableRows was pushed for table "${tableName}", but the open .odb document has no table by that name.`,
    );
  }
  return table;
}
...
export function OdbTableRowsScreen(): ReactElement {
  ...
  if (screen.kind !== "odbTableRows") {
    throw new Error(
      `OdbTableRowsScreen rendered while the current screen is "${screen.kind}", not "odbTableRows".`,
    );
  }
  ...
  const rows =
    query === ""
      ? table.rows
      : table.rows.filter((row) => rowText(row).toLowerCase().includes(query));
  ...
  const headerLine = table.columns
      ...
      <ListView
        ...
        emptyMessage={
          query === ""
            ...
            : `No rows match "${state.searchQuery}".`
        }
