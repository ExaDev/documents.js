...
export function OdbTableListScreen(): ReactElement {
  ...
  const { selectedIndex } = useNavigationInput({
    ...
    onSelect: (index) => {
      ...
      dispatch({
        type: "PUSH_SCREEN",
        screen: { kind: "odbTableRows", tableName: table.tableName },
      });
    },
    onBack: () => {
  ...
  return (
    <Box flexDirection="column">
      ...
      <ListView
        ...
        emptyMessage={
          query === ""
            ...
            : `No tables match "${state.searchQuery}".`
        }
      ...
      />
      <Text dimColor>
        Enter to open a table, f for forms ({doc.forms.length}), r for reports (
        {doc.reports.length})
      </Text>
    </Box>
  );
