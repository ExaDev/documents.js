...
function defaultReportRenderDestination(odbPath: string, reportName: string): string {
...
export function OdbReportRenderScreen(): ReactElement {
  ...
  if (screen.kind !== 'odbReportRender') {
    throw new Error(`OdbReportRenderScreen rendered while the current screen is "${screen.kind}", not "odbReportRender".`);
  }
  ...
  const submit = (): void => {
    void (async () => {
      ...
      try {
        ...
        dispatch({ type: 'SET_STATUS', severity: 'info', text: `Rendered "${reportName}" to ${destination}` });
        ...
        dispatch({ type: 'OPEN_FILE_ERROR', message: `Could not render "${reportName}" to ${destination}`, detail: describeError(error) });
      }
    })();
  };
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text dimColor>
        {field === 'destination' ? 'Enter for fonts, Esc to cancel' : "Enter to render, Esc to cancel. The destination's own extension picks docx/odt/pdf"}
      </Text>
    </Box>
  );
}
