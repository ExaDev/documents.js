...
function requireReport(
  reports: readonly OdbReport[],
  reportName: string,
): OdbReport {
  const report = reports.find((candidate) => candidate.name === reportName);
  if (report === undefined) {
    throw new Error(
      `odbReportDetail was pushed for report "${reportName}", but the open .odb document has no report by that name.`,
    );
  }
  return report;
}
...
export function OdbReportDetailScreen(): ReactElement {
  ...
  if (screen.kind !== "odbReportDetail") {
    throw new Error(
      `OdbReportDetailScreen rendered while the current screen is "${screen.kind}", not "odbReportDetail".`,
    );
  }
  ...
  const { selectedIndex } = useNavigationInput({
    ...
    onSelect: () => {
      ...
      dispatch({
        type: "PUSH_SCREEN",
        screen: { kind: "odbReportRender", reportName: report.name },
      });
    },
    onBack: () => {
  ...
  return (
    <Box flexDirection="column">
      ...
      <ListView
        ...
        emptyMessage={
          query === ""
            ? "This report declares no structure."
      ...
      />
      <Text dimColor>
        Enter to render this report, Esc to go back to the report list
      </Text>
    </Box>
  );
}
