...
function buildBigTable(): HsqldbTable {
  ...
  for (let index = 0; index < BIG_TABLE_ROW_COUNT; index += 1) {
    rows.push([{ kind: 'string', value: `Row${String(index).padStart(3, '0')}` }]);
  }
  return { tableName: 'BIG', columns: [{ name: 'LABEL', type: 'VARCHAR' }], rows };
}
...
describe('OdbTableRowsScreen', () => {
  it('renders the table name, its own column headers with their types, and every cell via hsqldbCellDisplayText', async () => {
    ...
    await waitForFrame(lastFrame, (candidate) => candidate.includes('CUSTOMERS'));
    await settle();

    stdin.write('\r');

    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('Ada Lovelace'));
  ...
  it(
    ...
    async () => {
      ...
      const initialFrame = await waitForFrame(lastFrame, (candidate) => candidate.includes('Row000'));
      ...
      const scrolledFrame = await waitForFrame(lastFrame, (candidate) => candidate.includes(lastRowLabel));
