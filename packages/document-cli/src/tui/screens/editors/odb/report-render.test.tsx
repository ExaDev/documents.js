...
import { FORM_AND_REPORT_ODB_PATH, loadFormAndReportOdbReports } from '../../../../test-support/odb-fixture.js';
...
describe('the report-render flow (odbReportDetail -> odbReportRender)', () => {
  it('renders the report to a real pdf file at a destination path the user types', async () => {
    const { lastFrame, stdin } = render(<OdbHarness tables={SAMPLE_TABLES} reports={REPORTS} path={odbPath} />);

    await waitForFrame(lastFrame, (frame) => frame.includes('SALES'));
    await settle();
    stdin.write('r');

    await waitForFrame(lastFrame, (frame) => frame.includes('Reports (1 of 1)'));
    await settle();
    stdin.write('\r');

    await waitForFrame(lastFrame, (frame) => frame.includes('data source: query'));
    await settle();
    ...
    await waitForFrame(lastFrame, (frame) => frame.includes('Render report: SalesByRegion'));
    await settle();
    ...
    const destination = join(dirname(odbPath), `SalesByRegion.pdf${typedSuffix}`);
