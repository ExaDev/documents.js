...
describe('OdbFormListScreen and OdbFormDetailScreen', () => {
  it("reaches the form list from the table list with 'f', showing the form's own control counts", async () => {
    const { lastFrame, stdin } = render(<OdbHarness tables={SAMPLE_TABLES} forms={FORMS} />);
    ...
    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('Forms (1 of 1)'));
    expect(frame).toContain('SalesForm [forms/Obj11] -- 1 form, 6 controls (5 bound)');
  });
  ...
  it("opens a form's own control tree on Enter, with every field binding and the sub-form's own query command", async () => {
    ...
    await settle();
    stdin.write('f');
    await waitForFrame(lastFrame, (candidate) => candidate.includes('Forms (1 of 1)'));
    await settle();

    stdin.write('\r');

    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('form SalesForm'));
    ...
    expect(frame).toContain('subform HighValueSubForm on query "HighValueSales"');
  ...
  it('goes back from the form list to the table list on Esc', async () => {
    ...
    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('Tables (1 of 1)'));
  ...
  it('says so plainly when the database declares no forms at all', async () => {
    ...
    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('Forms (0 of 0)'));
