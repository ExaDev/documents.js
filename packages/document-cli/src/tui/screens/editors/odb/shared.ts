...
export function requireOdbDocument(
  openDocument: OpenDocument | undefined,
): OdbOpenDocument {
  if (openDocument?.format !== "odb") {
    throw new Error(
      "An .odb browsing screen rendered without an open .odb document; the app router only reaches this screen group from odbTableList, which is only ever the root screen of an open .odb document.",
    );
  }
  return openDocument;
}
