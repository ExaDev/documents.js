...
import {
  ...
  parseStrokeField,
  requireOdgDocument,
  requireShapeOrVectorDetailScreen,
  vectorKindLabel,
  type PageItem,
} from "./shared.js";
...
function buildVectorRows(
  ...
  if (vector.kind !== "line" && liveVector.kind !== "line") {
    ...
    rows.push({
      ...
      currentValue:
        vector.fill === undefined
          ? ""
          : `${vector.fill.r} ${vector.fill.g} ${vector.fill.b}`,
      commit: (raw) => {
        dispatch({
...
function VectorDetail(props: {
  ...
  const rows: readonly EditableRow[] =
    liveVector === undefined
      ? []
      : buildVectorRows(vector, liveVector, dispatch);
    ...
    return (
      <Box flexDirection="column">
        ...
        <Text color="yellow">
          ...
          until that mismatch is resolved outside this TUI. Open the file in a
          real ODF editor to change it.
        </Text>
...
function ShapeDetail(props: {
  ...
  readonly shapeIndexInPage: number;
    ...
    },
    {
      ...
      commit: (raw) => {
        dispatch({
          ...
          containerIndex: pageIndex,
          shapeIndex: shapeIndexInPage,
          frame: { ...frame, xPt: parseNumberField(raw, frame.xPt) },
        });
      },
    },
    {
      ...
      commit: (raw) => {
        dispatch({
          ...
          containerIndex: pageIndex,
          shapeIndex: shapeIndexInPage,
          frame: { ...frame, yPt: parseNumberField(raw, frame.yPt) },
        });
      },
    },
    {
      ...
      commit: (raw) => {
        dispatch({
          ...
          containerIndex: pageIndex,
          shapeIndex: shapeIndexInPage,
          frame: { ...frame, widthPt: parseNumberField(raw, frame.widthPt) },
        });
      },
    },
    {
      ...
      commit: (raw) => {
        dispatch({
          ...
          containerIndex: pageIndex,
          shapeIndex: shapeIndexInPage,
          frame: { ...frame, heightPt: parseNumberField(raw, frame.heightPt) },
        });
      },
    },
    {
      ...
      currentValue:
        shape.rotationDeg === undefined ? "" : String(shape.rotationDeg),
      commit: (raw) => {
        ...
        dispatch({
          ...
          shapeIndex: shapeIndexInPage,
          rotationDeg:
            trimmed.length === 0 ? undefined : parseNumberField(trimmed, 0),
        });
      },
    },
  ];
...
export function OdgShapeOrVectorDetailScreen(): ReactElement {
  ...
  return (
    <ShapeDetail
      pageIndex={pageIndex}
      shapeIndexInPage={itemIndex - vectorCount}
      shape={item.shape}
      isActive={!overlayOpen}
    />
  );
