...
export function requirePageDetailScreen(
  state: AppState,
): Extract<Screen, { kind: "pageDetail" }> {
...
export function requireShapeOrVectorDetailScreen(
  state: AppState,
): Extract<Screen, { kind: "shapeOrVectorDetail" }> {
  const screen = currentScreen(state);
...
export function vectorsParityMatch(
  ...
  return liveVectors.every(
    (live, index) => live.kind === contentVectors[index]?.kind,
  );
}
...
export function buildPageItems(
  ...
  const vectorItems: readonly PageItem[] = contentPage.vectors.map(
    (vector, index): PageItem => ({
      kind: "vector",
      vector,
      liveVector: parityOk ? liveVectors[index] : undefined,
    }),
  );
...
export function describeFillStroke(vector: ContentVector): string {
  ...
  if (vector.stroke !== undefined) {
    parts.push(
      `stroke ${formatColor(vector.stroke.color)} ${formatPt(vector.stroke.widthPt)}pt`,
    );
  }
