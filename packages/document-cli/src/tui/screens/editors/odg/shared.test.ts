import { createOdg, elementsWithTag, openOdg, rootElement, type XmlElement } from 'documents.js';
...
function openOdgDocument(bytes: Uint8Array<ArrayBuffer>, path = '/tmp/drawing.odg'): OdgOpenDocument {
  const state = appReducer(createInitialState(), { type: 'OPEN_FILE_SUCCESS', path, doc: { format: 'odg', editor: openOdg(bytes), path } });
...
describe('vectorsParityMatch', () => {
  ...
  it('matches when every kind lines up index-for-index', () => {
    expect(vectorsParityMatch([{ kind: 'rect' }, { kind: 'line' }], [{ kind: 'rect' }, { kind: 'line' }])).toBe(true);
  });

  it('fails on a synthetic length mismatch -- the shape a page with an extra reader-only element produces', () => {
    expect(vectorsParityMatch([{ kind: 'rect' }], [{ kind: 'rect' }, { kind: 'ellipse' }])).toBe(false);
  });

  it('fails on a synthetic kind mismatch at the same length', () => {
    expect(vectorsParityMatch([{ kind: 'rect' }, { kind: 'path' }], [{ kind: 'rect' }, { kind: 'ellipse' }])).toBe(false);
  });
});
...
describe('buildPageItems vector parity', () => {
  it('threads a live handle through for every vector on a page built entirely through the live-view editor', () => {
    ...
    page.addLine({ from: { xPt: 0, yPt: 100 }, to: { xPt: 100, yPt: 100 }, stroke: { color: { r: 0, g: 0, b: 0 }, widthPt: 1 } });
    ...
    expect(vectorItems.map((item) => item.vector.kind)).toEqual(['rect', 'ellipse', 'line']);
  ...
  it('does not let a mismatch on one page fall back the parity check on another', () => {
    ...
    mismatchingPage.addRect({ frame: { xPt: 10, yPt: 10, widthPt: 40, heightPt: 30 } });
    const matchingPage = editor.addPage();
    matchingPage.addEllipse({ frame: { xPt: 10, yPt: 10, widthPt: 40, heightPt: 30 } });
    ...
    const mismatchingItems = buildPageItems(doc, 0).filter((item) => item.kind === 'vector');
    expect(mismatchingItems.every((item) => item.liveVector === undefined)).toBe(true);

    const matchingItems = buildPageItems(doc, 1).filter((item) => item.kind === 'vector');
