...
function warnIfVectorAddedReadOnly(
  ...
  const vectorItems = buildPageItems(doc, pageIndex).filter(
...
function previewText(text: string): string {
  ...
  return singleLine.length > MAX_TEXT_PREVIEW_LENGTH
    ? `${singleLine.slice(0, MAX_TEXT_PREVIEW_LENGTH)}…`
    : singleLine;
}
...
export function OdgPageDetailScreen(): ReactElement {
  ...
  const rows = items
    ...
    .filter(
      (row) =>
        query.length === 0 ||
        describeItem(row.item).toLowerCase().includes(query),
    );
