...
function OpenEditAndViewSource(): ReactElement | undefined {
  ...
  useEffect(() => {
    dispatch({ type: 'OPEN_FILE_SUCCESS', path: 'notes.md', doc: { format: 'markdown', editor: openMarkdown(ORIGINAL_TEXT), originalText: ORIGINAL_TEXT, path: 'notes.md' } });
    dispatch({ type: 'APPEND_PARAGRAPH', text: 'New paragraph', styleId: undefined, alignment: undefined });
...
describe('MarkdownViewSourceScreen', () => {
  it('shows originalText unmodified alongside the edited text the live editor would save right now, proving the two are genuinely decoupled', async () => {
    ...
    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('As it will save right now'));
    ...
    const asOpenedSection = frame.slice(frame.indexOf('As opened'), frame.indexOf('As it will save right now'));
  ...
  it('pops back to the previous screen on Escape', async () => {
    ...
    await waitForFrame(lastFrame, (candidate) => candidate.includes('As it will save right now'));
    await settle();

    stdin.write(ESCAPE_KEY);
    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('popped back to'));
