...
export function MarkdownViewSourceScreen(): ReactElement {
  ...
  if (screen.kind !== 'viewSource') {
    return <Text color="red">MarkdownViewSourceScreen rendered outside a viewSource screen.</Text>;
  }
  if (doc?.format !== 'markdown') {
    return <Text color="red">view-source requires an open markdown document.</Text>;
  }
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text>{doc.originalText ?? '(this document was created fresh, with no original text to compare against)'}</Text>
