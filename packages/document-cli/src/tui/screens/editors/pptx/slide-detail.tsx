...
type AddItemMode = 'closed' | 'chooseKind' | 'textbox' | 'image' | 'tableRows' | 'tableColumns' | 'vector';
...
const VECTOR_FILL_FIELD: FieldSpec = { key: 'fill', label: 'Fill "r g b" (0-1 each), blank for none', defaultValue: '0.8 0.8 0.8' };
const VECTOR_STROKE_FIELD: FieldSpec = { key: 'stroke', label: 'Stroke "r g b widthPt" (0-1 colour, pt width), blank for none', defaultValue: '0 0 0 1' };
...
function fieldsForVectorKind(kind: VectorKind): readonly FieldSpec[] {
  switch (kind) {
    ...
    case 'ellipse':
    case 'path':
      return [...VECTOR_GEOMETRY_FIELDS, VECTOR_FILL_FIELD, VECTOR_STROKE_FIELD];
    case 'line':
      return VECTOR_LINE_FIELDS;
  }
}

function readVectorFrame(values: Readonly<Record<string, string>>): GeometryBox {
  return {
...
function buildVectorAction(kind: VectorKind, slideIndex: number, values: Readonly<Record<string, string>>): Action {
  switch (kind) {
    case 'rect':
      return { type: 'ADD_RECT', containerIndex: slideIndex, init: { frame: readVectorFrame(values), fill: parseColorField(requireFieldValue(values, 'fill')), stroke: parseStrokeField(requireFieldValue(values, 'stroke')) } };
    case 'ellipse':
      return { type: 'ADD_ELLIPSE', containerIndex: slideIndex, init: { frame: readVectorFrame(values), fill: parseColorField(requireFieldValue(values, 'fill')), stroke: parseStrokeField(requireFieldValue(values, 'stroke')) } };
    case 'line': {
      ...
      return { type: 'ADD_LINE', containerIndex: slideIndex, init: { from, to, stroke } };
    }
    case 'path': {
      ...
      return {
        ...
        init: { frame, subpaths: defaultTriangleSubpaths(frame.widthPt, frame.heightPt), fill: parseColorField(requireFieldValue(values, 'fill')), stroke: parseStrokeField(requireFieldValue(values, 'stroke')) },
      };
    }
  }
}
...
export function SlideDetailScreen(props: SlideDetailScreenProps): ReactElement {
  ...
  const [vectorKind, setVectorKind] = useState<VectorKind | undefined>(undefined);
  ...
  return (
    <Box flexDirection="column">
      ...
      {addMode === 'chooseKind' ? <Text color="cyan">Add shape: t textbox, i image, b table{doc.format === 'odp' ? ', r rect, e ellipse, n line, p path' : ''}, Esc cancel</Text> : undefined}
