...
function buildPptxTableDeckBytes(): Uint8Array<ArrayBuffer> {
  ...
  slide.addTable({ frame: { xPt: 10, yPt: 10, widthPt: 300, heightPt: 150 }, table: { rows: 3, columns: 3 } });
...
function SlideTableRouter(): ReactElement {
  ...
  switch (screen.kind) {
    ...
    default:
      throw new Error(`SlideTableRouter has no case for ${screen.kind}; this test only ever pushes slideDetail/slideTableDetail.`);
  }
}
...
function DocumentProbe({ format }: { readonly format: 'pptx' | 'odp' }): ReactElement {
  ...
  const content = format === 'pptx' ? readPptxContent(doc.editor.toPackage()) : readOdpContent(doc.editor.toPackage());
  ...
  const anchor = tableBlock?.kind === 'table' ? tableBlock.rows[0]?.cells[0] : undefined;
  return (
    <Text>
      probe:anchorColSpan={anchor?.colSpan ?? 1} anchorRowSpan={anchor?.rowSpan ?? 1}
    </Text>
  );
}

function OpenAtSlideDetail({ format, bytes }: { readonly format: 'pptx' | 'odp'; readonly bytes: Uint8Array<ArrayBuffer> }): ReactElement | undefined {
  ...
  useEffect(() => {
    ...
    dispatch({ type: 'PUSH_SCREEN', screen: { kind: 'slideDetail', slideIndex: 0 } });
...
function renderAtSlideDetail(format: 'pptx' | 'odp', bytes: Uint8Array<ArrayBuffer>): ReturnType<typeof render> {
  return render(
    <AppStateProvider>
...
describe.each(['pptx', 'odp'] as const)('SlideTableDetailScreen on %s', (format) => {
  it('is reachable from slide-detail\'s own Tables section and merges a real rectangle of cells', async () => {
    const bytes = format === 'pptx' ? buildPptxTableDeckBytes() : buildOdpTableDeckBytes();
    ...
    const merged = await waitForText(lastFrame, 'probe:anchorColSpan=2 anchorRowSpan=2');
