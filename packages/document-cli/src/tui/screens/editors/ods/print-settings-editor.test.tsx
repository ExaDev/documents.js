...
function Harness(): ReactElement {
  ...
  useEffect(() => {
    ...
    if (doc.format === "ods" && top?.kind === "sheetList") {
      dispatch({
        type: "PUSH_SCREEN",
        screen: { kind: "printSettingsEditor", sheetIndex: 0 },
      });
    }
  }, [doc, top, dispatch]);
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text>
        gridlines:{String(doc.editor.sheets()[0]?.printSettings.gridlines)}
      </Text>
    </Box>
  );
}
...
describe("OdsPrintSettingsEditorScreen", () => {
  it("renders all five of OdsSheet.printSettings own guaranteed fields", async () => {
    ...
    const frame = await waitForFrame(lastFrame, (candidate) =>
      candidate.includes("Page width (pt)"),
    );
    for (const label of [
      "Page height (pt)",
      "Margin top (pt)",
  ...
  it("toggles gridlines on Enter and commits through the reducer to the real OdsSheet.printSettings setter", async () => {
    ...
    await waitForFrame(lastFrame, (candidate) =>
      candidate.includes("gridlines:false"),
    );
    await settle();
    ...
    await waitForFrame(lastFrame, (candidate) =>
      candidate.includes("gridlines:true"),
    );
  });
});
