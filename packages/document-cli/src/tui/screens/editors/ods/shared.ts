...
export function odsDocument(state: AppState): OdsOpenDocument {
  ...
  if (doc?.format !== "ods") {
    throw new Error(
      "An ods screen was rendered while the open document was not ods; the app only ever reaches these screens from the ods root screen.",
    );
  }
  return doc;
}
...
export function resolveSheet(
  ...
  if (content.kind !== "spreadsheet") {
    throw new Error(
      "readOdsContent always resolves an ods package to the spreadsheet ContentDocument variant.",
    );
  }
...
export function sheetExtent(sheet: ContentSheet | undefined): {
  ...
  const maxOf = (values: readonly number[]): number =>
    values.reduce((max, value) => Math.max(max, value), MIN_GRID_EXTENT - 1);
  const rowCount =
    Math.max(
      maxOf(sheet.cells.map((cell) => cell.row)),
      maxOf(sheet.rows.map((row) => row.index)),
    ) + 1;
  const columnCount =
    Math.max(
      maxOf(sheet.cells.map((cell) => cell.column)),
      maxOf(sheet.columns.map((column) => column.index)),
    ) + 1;
...
export function cellLookup(
  sheet: ContentSheet | undefined,
): ReadonlyMap<string, ContentSheetCell> {
  ...
  switch (kind) {
    ...
    case "date": {
      ...
      return trimmed.length === 0
        ? undefined
        : { kind: "date", value: trimmed };
    }
    case "time": {
      const trimmed = text.trim();
      return trimmed.length === 0
        ? undefined
        : { kind: "time", value: trimmed };
    }
    case "dateTime": {
      const trimmed = text.trim();
      return trimmed.length === 0
        ? undefined
        : { kind: "dateTime", value: trimmed };
    }
    case "error": {
      const trimmed = text.trim();
      return trimmed.length === 0
        ? undefined
        : { kind: "error", value: trimmed };
    }
  }
}
