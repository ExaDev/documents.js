...
describe('OdsSheetListScreen', () => {
  it('renders the default sheet a freshly created workbook already carries', async () => {
    ...
    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('Sheet1'));
  ...
  it('adds a sheet through the "a" prompt and dispatches ADD_SHEET on submit', async () => {
    ...
    await waitForFrame(lastFrame, (candidate) => candidate.includes('sheetCount:1'));
    await settle();

    stdin.write('a');
    await waitForFrame(lastFrame, (candidate) => candidate.includes('New sheet name:'));
    ...
    const frame = await waitForFrame(lastFrame, (candidate) => candidate.includes('sheetCount:2'));
    expect(frame).toContain('Budget');
  });

  it('pushes the spreadsheetGrid screen for the highlighted sheet on Enter', async () => {
    const { lastFrame, stdin } = renderHarness();
    await waitForFrame(lastFrame, (candidate) => candidate.includes('top:sheetList'));
    await settle();

    stdin.write('\r');

    await waitForFrame(lastFrame, (candidate) => candidate.includes('top:spreadsheetGrid'));
  });
});
