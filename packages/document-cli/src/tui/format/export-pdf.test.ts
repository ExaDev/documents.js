...
function xlsxTestBytesWithPageSize(widthPt: number, heightPt: number): Uint8Array<ArrayBuffer> {
  const editor = createOds();
  const sheet = editor.addSheet('Sheet1');
  sheet.cell(0, 0).value = { kind: 'string', value: 'x' };
  sheet.printSettings = { pageSize: { widthPt, heightPt }, margins: { topPt: 10, rightPt: 10, bottomPt: 10, leftPt: 10 }, gridlines: false, headers: false, pageOrder: 'downThenOver' };
  return odsToXlsx(editor.toBytes());
}

function openXlsxDocument(bytes: Uint8Array<ArrayBuffer>, path: string): XlsxOpenDocument {
...
describe('exportToPdf for an open xlsx document', () => {
  it('writes a real PDF file, reusing the exact xlsxToPdf conversion the reducer already ran once to build the read-only preview', async () => {
    ...
    expect(new TextDecoder('latin1').decode(written.subarray(0, 5))).toBe('%PDF-');
  });
  ...
  it('re-derives the PDF from `doc.bytes` rather than reusing `doc.layout` computed at open time', async () => {
    ...
    const mismatchedDoc: XlsxOpenDocument = { format: 'xlsx', layout: readPdf(xlsxToPdf(smallBytes)), bytes: largeBytes, path: join(workspace, 'workbook.xlsx') };
    const destination = join(workspace, 'workbook.pdf');
    await exportToPdf(mismatchedDoc, destination, { onDiagnostic: () => undefined });

    const exportedPage = readPdf(new Uint8Array(await readFile(destination))).pages[0];
