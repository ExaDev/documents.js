...
const REPORT_RENDER_TARGET_FORMATS: Readonly<
  Record<"docx" | "odt" | "pdf", true>
> = { docx: true, odt: true, pdf: true };

function isReportRenderTargetFormat(
...
export async function renderOdbReportTo(
  ...
  options: RenderOdbReportOptions,
): Promise<void> {
  ...
  const bytes =
    format === "docx"
      ...
      : format === "odt"
        ...
        : odbReportToPdf(content, toPdfOptions(options, fonts));
