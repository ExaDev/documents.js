...
const COMMANDS: readonly CommandDefinition[] = [
  {
    name: "save",
    usage: ":save",
    description: "Write the open document back to its own path",
  },
  {
    name: "saveas",
    usage: ":saveas <path>",
    description: "Write the open document to a new path",
  },
  {
    name: "export",
    usage: ":export pdf [path]",
    description: "Render the open document to PDF",
  },
  {
    name: "new",
  ...
  { name: "undo", usage: ":undo", description: "Undo the last change" },
  {
    ...
    usage: ":view-source",
    description:
      "Compare a markdown document as opened vs. as it will save now",
  },
...
function resolveCommand(line: string): CommandDefinition | undefined {
  ...
  const exact = COMMANDS.find(
    (command) => command.name === token.toLowerCase(),
  );
...
async function runCommand(
  ...
  switch (command.name) {
    ...
    case "export": {
      ...
      const destination =
        explicit ??
        (doc.path === undefined ? undefined : defaultPdfPathFor(doc.path));
    ...
    case "view-source":
      if (doc?.format !== "markdown") {
        warn(
          dispatch,
          ":view-source only applies to an open markdown document",
        );
        return;
      }
