...
export function ListView<T>(props: ListViewProps<T>): ReactElement {
  ...
  const start = Math.min(Math.max(props.selectedIndex - Math.floor(viewportRows / 2), 0), maxStart);
  const visible = props.items.slice(start, start + viewportRows);

  if (props.items.length === 0) {
    return <Box flexDirection="column">{props.emptyMessage === undefined ? undefined : <Text dimColor>{props.emptyMessage}</Text>}</Box>;
  }
  ...
  return (
    <Box flexDirection="column">
      {visible.map((item, offset) => {
        ...
        return <Box key={index}>{props.renderItem(item, index === props.selectedIndex)}</Box>;
      })}
    </Box>
  );
}
