...
const METADATA_KEYS: readonly (keyof LayoutMetadata)[] = ['title', 'author', 'subject', 'keywords', 'creator', 'producer', 'createdIso', 'modifiedIso'];
...
export function presentMetadataEntries(metadata: LayoutMetadata): readonly (readonly [string, string | readonly string[]])[] {
  const entries: readonly (readonly [string, string | readonly string[] | undefined])[] = METADATA_KEYS.map((key) => [key, metadata[key]] as const);
...
export function formatMetadataLines(metadata: LayoutMetadata): readonly string[] {
