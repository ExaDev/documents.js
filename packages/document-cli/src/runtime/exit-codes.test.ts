...
describe('mapErrorToExit', () => {
  it('maps an interrupt abort reason to EXIT_INTERRUPTED regardless of the thrown error', () => {
    expect(mapErrorToExit(new Error('aborted'), 'interrupt')).toBe(EXIT_INTERRUPTED);
  });
  ...
  it('prefers the abort reason over a needs-info error class', () => {
    expect(mapErrorToExit(new OdmUnresolvedSectionError(['../chapter1.odt']), 'interrupt')).toBe(EXIT_INTERRUPTED);
  });
  ...
  it('maps OdbTableNotSpecifiedError to EXIT_NEEDS_INFO', () => {
    expect(mapErrorToExit(new OdbTableNotSpecifiedError(['CUSTOMERS', 'ORDERS']), undefined)).toBe(EXIT_NEEDS_INFO);
  });

  it('maps OdbTableNotFoundError to EXIT_NEEDS_INFO', () => {
    expect(mapErrorToExit(new OdbTableNotFoundError('MISSING', ['CUSTOMERS']), undefined)).toBe(EXIT_NEEDS_INFO);
  });

  it('maps OdbNoEmbeddedDataSourceError to EXIT_NEEDS_INFO', () => {
    expect(mapErrorToExit(new OdbNoEmbeddedDataSourceError('jdbc:mysql://example'), undefined)).toBe(EXIT_NEEDS_INFO);
  });
  ...
  it('maps PdfEncryptedError to EXIT_INPUT_ERROR', () => {
    expect(mapErrorToExit(new PdfEncryptedError('/Encrypt present'), undefined)).toBe(EXIT_INPUT_ERROR);
  });

  it('maps PdfParseError to EXIT_INPUT_ERROR', () => {
    expect(mapErrorToExit(new PdfParseError('bad-xref', 'malformed cross-reference table'), undefined)).toBe(EXIT_INPUT_ERROR);
  });

  it('falls through to EXIT_INPUT_ERROR for an ordinary error with no abort reason', () => {
    expect(mapErrorToExit(new Error('ENOENT: no such file'), undefined)).toBe(EXIT_INPUT_ERROR);
  });

  it('falls through to EXIT_INPUT_ERROR for a non-Error thrown value', () => {
    expect(mapErrorToExit('a string was thrown', undefined)).toBe(EXIT_INPUT_ERROR);
  });
});
