...
export function createFilesystemMarkdownImageResolver(
  baseDir: string,
): (destination: string) => { readonly bytes: Uint8Array } | undefined {
  return (destination: string): { readonly bytes: Uint8Array } | undefined => {
    ...
    try {
      return {
        bytes: new Uint8Array(readFileSync(resolve(baseDir, destination))),
      };
    } catch {
      return undefined;
    }
