    ...
    report(diagnostic) {
      ...
      if (json) {
        process.stderr.write(
          `${JSON.stringify({ type: "diagnostic", command, ...diagnostic })}\n`,
        );
        return;
      }
      const pageClause =
        diagnostic.pageIndex === undefined
          ? ""
          : ` (page ${diagnostic.pageIndex})`;
      process.stderr.write(
        `[${command}] ${diagnostic.severity} ${diagnostic.code}: ${diagnostic.message}${pageClause}\n`,
      );
    },
    summarize(result) {
      ...
      process.stderr.write(
        `[${command}] wrote ${result.bytes} bytes to ${result.output} (${diagnosticClause})\n`,
      );
    },
  };
}
...
export function createFontSubstitutionReporter(options: {
  ...
  readonly quiet: boolean;
  readonly command: string;
}): (substitution: FontSubstitution) => void {
  const { json, quiet, command } = options;
  ...
  return (substitution) => {
    ...
    if (json) {
      process.stderr.write(
        `${JSON.stringify({ type: "font-substitution", command, ...substitution })}\n`,
      );
      return;
    }
    const styleClause = `${substitution.requestedBold ? " bold" : ""}${substitution.requestedItalic ? " italic" : ""}`;
    process.stderr.write(
      `[${command}] font substitution: "${substitution.requestedFamily}"${styleClause} -> "${substitution.resolvedFamily}" (${substitution.reason})\n`,
    );
  };
}
...
export function substitutionToDiagnostic(
...
export function fontSubstitutionToDiagnostic(
  ...
  const detail =
    substitution.reason === "vendored-substitute"
      ? `substituted the metric-compatible "${substitution.resolvedFamily}"`
      : `substituted another face of "${substitution.resolvedFamily}"`;
  return {
    severity: "info",
    code: "font/substituted",
    message: `"${requested}" is not available; ${detail}`,
  };
}
