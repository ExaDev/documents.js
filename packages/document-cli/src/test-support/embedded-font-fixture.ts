...
const FONT_TABLE_REL_TYPE = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/fontTable';
...
function appendChild(pkg: Package, partPath: string, child: XmlElement): void {
  ...
  if (part?.kind !== 'xml') {
    throw new Error(`embedded-font-fixture: ${partPath} is missing from the scaffolded package, or is not an XML part`);
  }
...
export function buildDocxWithEmbeddedFont(options: EmbeddedFontFixtureOptions): Uint8Array<ArrayBuffer> {
  const editor = createDocx();
  editor.body.appendParagraph().appendRun({ text: 'A paragraph of ordinary body text.' });
  const pkg = editor.toPackage();

  pkg.parts['word/fonts/font1.fntdata'] = { kind: 'binary', base64: bytesToBase64(options.fontBytes) };
  pkg.parts['word/fontTable.xml'] = {
    kind: 'xml',
    nodes: [xmlDeclaration(), el('w:fonts', { 'xmlns:w': WORDML_NS, 'xmlns:r': R_NS }, [el('w:font', { 'w:name': options.family }, [el('w:embedRegular', { 'r:id': 'rId1' })])])],
  };
  pkg.parts['word/_rels/fontTable.xml.rels'] = {
    kind: 'xml',
    nodes: [xmlDeclaration(), el('Relationships', { xmlns: RELS_NS }, [el('Relationship', { Id: 'rId1', Type: FONT_REL_TYPE, Target: 'fonts/font1.fntdata' })])],
  };
  appendChild(pkg, 'word/_rels/document.xml.rels', el('Relationship', { Id: 'rIdFontTable', Type: FONT_TABLE_REL_TYPE, Target: 'fontTable.xml' }));
...
export function buildPptxWithEmbeddedFont(options: EmbeddedFontFixtureOptions): Uint8Array<ArrayBuffer> {
  const editor = createPptx();
  const pkg = editor.toPackage();

  pkg.parts['ppt/fonts/font1.fntdata'] = { kind: 'binary', base64: bytesToBase64(options.fontBytes) };
  appendChild(pkg, 'ppt/_rels/presentation.xml.rels', el('Relationship', { Id: 'rIdEmbeddedFont', Type: FONT_REL_TYPE, Target: 'fonts/font1.fntdata' }));
  appendChild(
    pkg,
    'ppt/presentation.xml',
    el('p:embeddedFontLst', {}, [el('p:embeddedFont', {}, [el('p:font', { typeface: options.family }), el('p:regular', { 'r:id': 'rIdEmbeddedFont' })])]),
  );
...
export function buildOdtWithEmbeddedFont(options: EmbeddedFontFixtureOptions): Uint8Array<ArrayBuffer> {
  const editor = createOdt();
  editor.body.appendParagraph().appendRun({ text: 'A paragraph of ordinary body text.' });
  const pkg = editor.toPackage();

  pkg.parts['Fonts/font1.ttf'] = { kind: 'binary', base64: odfBytesToBase64(options.fontBytes) };

  const contentPart = pkg.parts['content.xml'];
  if (contentPart?.kind !== 'xml') {
    throw new Error("embedded-font-fixture: createOdt() produced a package with no 'content.xml' XML part");
  }
  ...
  root.children.unshift(
    odfEl('office:font-face-decls', {}, [
      odfEl('style:font-face', { 'style:name': options.family, 'svg:font-family': options.family }, [
        odfEl('svg:font-face-src', {}, [odfEl('svg:font-face-uri', { 'xlink:href': 'Fonts/font1.ttf', 'xlink:type': 'simple' })]),
      ]),
    ]),
  );
