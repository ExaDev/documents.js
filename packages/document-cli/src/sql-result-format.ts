...
function columnWidths(columns: readonly string[], cells: readonly (readonly string[])[]): readonly number[] {
...
export function formatSqlResultSetTable(result: SqlResultSet): readonly string[] {
  const { columns, rows } = result;
  const cells = rows.map((row) => row.map((value) => hsqldbCellDisplayText(value)));
  const widths = columnWidths(columns, cells);

  const lines: string[] = [formatRow(columns, widths), formatRow(widths.map((width) => '-'.repeat(width)), widths)];
