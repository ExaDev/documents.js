...
import { loadFormAndReportOdbForms, loadFormAndReportOdbReports } from './test-support/odb-fixture';
...
function onlyForm(): OdbForm {
  ...
  if (form === undefined || forms.length !== 1) {
    throw new Error(`The fixture is expected to declare exactly one form; readOdbForms returned ${forms.length}.`);
  }
  return form;
}
...
function onlyReport(): OdbReport {
  ...
  if (report === undefined || reports.length !== 1) {
    throw new Error(`The fixture is expected to declare exactly one report; readOdbReports returned ${reports.length}.`);
  }
  return report;
}
...
describe('form rendering against the real fixture', () => {
  it('reads exactly the one form the fixture declares', () => {
    expect(loadFormAndReportOdbForms().map((form) => form.name)).toStrictEqual(['SalesForm']);
  });
  ...
  it("summarises the form with its href and its own control counts, counting a sub-form's controls too", () => {
    ...
    expect(describeOdbForm(onlyForm())).toBe('SalesForm [forms/Obj11] -- 1 form, 6 controls (5 bound)');
  });
  ...
  it("drops the form sub-document from the JSON summary while keeping the structure, so a form's whole OdtDocument never lands in a caller's output", () => {
    ...
    expect(Object.keys(summary).sort()).toStrictEqual(['forms', 'href', 'name']);
...
describe('report rendering against the real fixture', () => {
  it('reads exactly the one report the fixture declares', () => {
    expect(loadFormAndReportOdbReports().map((report) => report.name)).toStrictEqual(['SalesByRegion']);
  });
  ...
  it('summarises the report with its data-source command and its own group and element counts', () => {
    ...
    expect(describeOdbReport(onlyReport())).toBe('SalesByRegion [reports/Obj11] -- on query "HighValueSales", 2 groups, 13 elements');
  });
