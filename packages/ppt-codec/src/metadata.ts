...
// Maps between the seven fields archive-codec's oleps/summary-information module reads from and writes to a "\x05SummaryInformation" stream, and document-schema.js's own LayoutMetadata -- the shared metadata shape a PptDocument's `metadata` field always is. This is the only document-format-specific knowledge in the metadata path: archive-codec knows PID 2 means a title, this module knows LayoutMetadata's `title` is where that belongs for a presentation.
//
// The mapping is not 1:1 in either direction, and each gap is a genuine, permanent one rather than a TODO:
// - SummaryInformation's `comments` and `lastPrintedIso` have no LayoutMetadata field to land in -- LayoutMetadata was designed around what every format in the family can supply, and no other codec has a "last printed" or free-text "comments" concept, so these are read from the stream by archive-codec but simply never reach a PptDocument.
...
/** Whether a LayoutMetadata carries anything SummaryInformation can actually represent -- `creator`/`producer`/`language` alone should not force a stream carrying nothing but the CodePage property into existence, since a reader would see that back as `{}` regardless (see writePptContent's own call site). */
