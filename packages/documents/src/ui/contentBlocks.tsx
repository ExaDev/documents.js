...
export const HEADING_TAGS: Record<
  number,
  "h1" | "h2" | "h3" | "h4" | "h5" | "h6"
...
export function renderRuns(
  ...
  return runs.map((run, index) => {
    ...
    if (
      options?.treatFontFamilyAsInlineCode === true &&
...
export function renderImage(block: ContentImageBlock): ReactNode {
  return (
    <img
      src={`data:image/${block.format};base64,${block.base64}`}
      alt={block.altText ?? ""}
...
export function collectBlockGroups(
...
export function renderBlocksNeutral(
  blocks: readonly ContentBlock[],
): ReactNode {
  return collectBlockGroups(blocks).map((group, index) =>
    group.kind === "listGroup" ? (
      <div key={index}>
        {renderListNodesNeutral(buildListForest(group.items))}
      </div>
    ) : (
