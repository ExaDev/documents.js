...
import type { ContentDocument, ContentDrawPage, ContentShape, ContentSlide, ContentStroke, ContentSubpath, ContentVector } from 'documents.js';
...
export function SlidesPreview({ label, format, content, loading, error }: SlidesPreviewProps) {
  ...
  const clampedIndex = slides !== undefined && slides.length > 0 ? Math.min(activeIndex, slides.length - 1) : 0;
...
function renderSlideOrPage(slide: SlideOrPage): ReactNode {
  ...
  const items: ({ kind: 'shape'; data: ContentShape; order: number } | { kind: 'vector'; data: ContentVector; order: number })[] = [
    ...slide.shapes.map((s) => ({ kind: 'shape' as const, data: s, order: s.paintOrder ?? Number.POSITIVE_INFINITY })),
    ...vectors.map((v) => ({ kind: 'vector' as const, data: v, order: v.paintOrder ?? Number.POSITIVE_INFINITY })),
  ].sort((a, b) => a.order - b.order);

  return (
    <div className={styles.slideContainer} style={{ aspectRatio: `${widthPt} / ${heightPt}` }}>
      <svg viewBox={`0 0 ${widthPt} ${heightPt}`} className={styles.slideSvg} preserveAspectRatio="xMidYMid meet">
        {items.map((item, index) => (item.kind === 'shape' ? renderShape(item.data, index) : renderVector(item.data, index)))}
      </svg>
    </div>
  );
}

function rotationTransform(frame: { xPt: number; yPt: number; widthPt: number; heightPt: number }, rotationDeg: number | undefined): string | undefined {
...
function renderShape(shape: ContentShape, key: number): ReactNode {
  ...
  const fontSize = shape.fontScale !== undefined ? `${shape.fontScale}em` : undefined;
  const lineHeight = shape.lineSpacingReduction !== undefined ? String(1.5 - shape.lineSpacingReduction) : undefined;
  const padding = `${shape.insetTopPt}pt ${shape.insetRightPt}pt ${shape.insetBottomPt}pt ${shape.insetLeftPt}pt`;

  return (
    <foreignObject key={key} x={xPt} y={yPt} width={widthPt} height={heightPt} transform={rotationTransform(shape.frame, shape.rotationDeg)}>
      <div style={{ padding, fontSize, lineHeight, overflow: 'hidden', boxSizing: 'border-box', width: '100%', height: '100%' }}>
...
function colorToCss(color: { r: number; g: number; b: number } | undefined): string {
  return color === undefined ? 'none' : `rgb(${Math.round(color.r * 255)} ${Math.round(color.g * 255)} ${Math.round(color.b * 255)})`;
}

function strokeAttrs(stroke: ContentStroke | undefined): { stroke?: string; strokeWidth?: number; strokeDasharray?: string; strokeLinecap?: 'round' } {
  if (stroke === undefined) return {};
  const attrs: { stroke?: string; strokeWidth?: number; strokeDasharray?: string; strokeLinecap?: 'round' } = {
