...
// Renders a docx/odt-sourced ContentDocument natively as HTML instead of round-tripping it through a PDF rendition. Headings are detected via the same heading-{N} convention MarkdownPreview uses (router.ts's normalizeWordprocessingHeadings rewrites the raw Heading{N} styleIds into it). Two known, bounded gaps vs a full wordprocessor: (1) blockquotes, code blocks, and horizontal rules render as plain paragraphs -- no mapping from a real docx/odt style name to those semantic roles exists today; (2) lists render with a neutral marker (not bullet, not numbered) because ordered-vs-bullet cannot be determined from ContentDocument alone (numbering definitions aren't folded in).
