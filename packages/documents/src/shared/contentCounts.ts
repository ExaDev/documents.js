...
export function contentSummary(content: ContentDocument): readonly string[] {
  switch (content.kind) {
    case 'wordprocessing': {
      const blocks = content.sections.reduce((sum, section) => sum + countBlocksDeep(section.blocks), 0);
      return [`${content.sections.length} section${content.sections.length === 1 ? '' : 's'}`, `${blocks} block${blocks === 1 ? '' : 's'}`];
    }
    case 'presentation': {
      const shapes = content.slides.reduce((sum, slide) => sum + slide.shapes.length, 0);
      return [`${content.slides.length} slide${content.slides.length === 1 ? '' : 's'}`, `${shapes} shape${shapes === 1 ? '' : 's'}`];
    }
    case 'spreadsheet': {
      const cells = content.sheets.reduce((sum, sheet) => sum + sheet.cells.length, 0);
      return [`${content.sheets.length} sheet${content.sheets.length === 1 ? '' : 's'}`, `${cells} cell${cells === 1 ? '' : 's'}`];
    }
    case 'drawing': {
      const shapes = content.pages.reduce((sum, page) => sum + page.shapes.length, 0);
      const vectors = content.pages.reduce((sum, page) => sum + page.vectors.length, 0);
      return [`${content.pages.length} page${content.pages.length === 1 ? '' : 's'}`, `${shapes} shape${shapes === 1 ? '' : 's'}`, `${vectors} vector${vectors === 1 ? '' : 's'}`];
    }
    case 'formula':
      return ['formula'];
  }
