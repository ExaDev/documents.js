...
describe('toTreeData', () => {
  ...
  it('renders null as a leaf, not an expandable object', () => {
    ...
    expect(nodes).toEqual([{ value: 'root.producer', label: 'producer: null', children: undefined }]);
  });
  ...
  it('prefers a discriminant kind field for an array item of objects, over a bare index label', () => {
    ...
    expect(itemNode?.children).toEqual([
      { value: 'root.items[0].kind', label: 'kind: "text"', children: undefined },
  ...
  it('recurses through nested objects', () => {
    ...
    expect(nodes[0]).toMatchObject({ value: 'root.metadata', label: 'metadata' });
    expect(nodes[0]?.children).toEqual([{ value: 'root.metadata.title', label: 'title: "Report"', children: undefined }]);
  });
