...
export function minimumReleaseAgeMinutes(workspaceYamlText: string): number {
  ...
  if (!isRecord(parsed) || typeof parsed.minimumReleaseAge !== "number") {
    throw new Error(
      `${WORKSPACE_FILE} has no numeric minimumReleaseAge; refusing to reason about an age window this workspace has not configured`,
    );
  }
...
export function withOverrides(
  ...
  for (const key of Object.keys(currentOverrides(cloned))) {
    if (!Object.hasOwn(overrides, key)) cloned.deleteIn(["overrides", key]);
  }
  for (const [key, range] of Object.entries(overrides)) {
    cloned.setIn(["overrides", key], range);
  }
  return cloned;
}
...
function restoreFromGit(): void {
  ...
  // `pnpm update -r` only touches the packages it's told to; a plain `git checkout` restores the two tracked files but leaves node_modules linked against whatever the last attempt resolved, so resync it for whatever runs next (the final re-audit below, or any later step in this same job).
...
function attemptBatch(
  ...
  const updated =
    spawnSync("pnpm", ["update", "-r", ...names], { encoding: "utf8" })
      .status === 0;
  if (!updated) {
...
// Wrapped in a main guard so the helpers above stay importable from the test suite: node executes this file directly for real runs, and the unit tests import the helpers without touching pnpm-workspace.yaml or the lockfile.
function main(): void {
  ...
  if (candidates.length > 0) {
    ...
    // Read before the first write below, so the window quoted in a deferral reason is the one this run actually resolved against.
    const ageWindowMinutes = minimumReleaseAgeMinutes(
    ...
    for (const c of notFixed) {
      for (const advisory of c.advisories) {
        deferred.push({
          advisory,
          reason: `no version satisfies both the patched range and the ${String(ageWindowMinutes)}-minute minimumReleaseAge window yet (or a resolution conflict)`,
        });
      }
    }
  }
